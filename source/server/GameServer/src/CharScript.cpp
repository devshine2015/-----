...
//GetItemTradble(role,item) use getchaitem 0 is true /1 false
inline int lua_GetItemTradble(lua_State *pLS) {
	T_B
		bool success = false;
	int paramCount = lua_gettop(pLS);
	if (paramCount < 1) {
		success = false;
		goto End;
	}

	SItemGrid    *pItem = (SItemGrid *)lua_touserdata(pLS, 1);
	if (!pItem) {
		success = false;
		goto End;
	}
	CItemRecord *pRec = GetItemRecordInfo(pItem->sID);
	if (!pRec) {
		success = false;
		goto End;
	}
	...
	{
		success = (pRec->chIsTrade == 0 ? false : true);
		goto End;
	}
End:
	if (success)
		lua_pushnumber(pLS, pRec->chIsTrade);
	else
...
//GetItemThrowable(role,item) use getchaitem 0 is true /1 false
inline int lua_GetItemThrowable(lua_State *pLS) {
	T_B
		bool success = false;
	int paramCount = lua_gettop(pLS);
	if (paramCount < 1) {
	...
	{
		success = (pRec->chIsThrow == 0 ? false : true);
		//success = (pRec->chIsTrade == 0 ? false : true);
		goto End;
	}
End:
	if (success)
		lua_pushnumber(pLS, pRec->chIsThrow);
	else
		lua_pushnumber(pLS, 0);
...
//pItem->sNum
//GetItemThrowable(role,item) use getchaitem 0 is true /1 false
inline int lua_GetItemDestroyable(lua_State *pLS) {
	T_B
		bool success = false;
	int paramCount = lua_gettop(pLS);
	if (paramCount < 1) {
	...
	{
		success = (pRec->chIsDel == 0 ? false : true);
		//success = (pRec->chIsThrow == 0 ? false : true);
		//success = (pRec->chIsTrade == 0 ? false : true);
		goto End;
	}
End:
	if (success)
		lua_pushnumber(pLS, pRec->chIsDel);
	else
		lua_pushnumber(pLS, 0);
...
{T_B
	...
	//GetItemDestroyable
	REGFN(GetItemDestroyable);
	//GetItemThrowable
	REGFN(GetItemThrowable);
