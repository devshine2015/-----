...
namespace mission
{
	...
	{T_B
		...
		if (!character.IsLiveing()){
			character.SystemNotice("牺牲的海盗不能交易.");
			return FALSE;
		}
		CCharacter* pMain = &character;
	...
	{T_B
		...
		if (!pTradeData->pRequest->IsLiveing() || !pTradeData->pAccept->IsLiveing()){
			pTradeData->pAccept->SystemNotice("牺牲的海盗不能交易.");
			pTradeData->pRequest->SystemNotice("牺牲的海盗不能交易.");
			return FALSE;
		}
	...
	{T_B
			...
			pTradeData->bAcpOk == 1 && pTradeData->bReqOk == 1 )
		{
			...
			{
				pAccept->SystemNotice("角色的 [%s] IMP 在交易模式下不正确, 交易无法继续!", pRequest->GetName());
				pRequest->SystemNotice("角色的 [%s] IMP 在交易模式下不正确, 交易无法继续!", pRequest->GetName());
			...
			{
				pAccept->SystemNotice("角色的 [%s] IMP 在交易模式下不正确, 交易无法继续!", pAccept->GetName());
				pRequest->SystemNotice("角色的 [%s] IMP 在交易模式下不正确, 交易无法继续!", pAccept->GetName());
				return FALSE;
			}

			if (dwAcpIMP + pTradeData->ReqTradeData.dwIMP > 2000000000){
				pAccept->SystemNotice("角色的 [%s] IMP 如果超过20E, 交易无法继续!", pAccept->GetName());
				pRequest->SystemNotice("角色的 [%s] IMP 如果超过20E, 交易无法继续!", pAccept->GetName());
				return FALSE;
			}

			if (dwReqIMP + pTradeData->AcpTradeData.dwIMP > 2000000000){
				pAccept->SystemNotice("角色的 [%s] IMP 如果超过20E, 交易无法继续!", pRequest->GetName());
				pRequest->SystemNotice("角色的 [%s] IMP 如果超过20E, 交易无法继续!", pRequest->GetName());
				return FALSE;
			}
			...
			{
				sprintf(szNotice, "你已经收到了 [%d] IMPs 来自 (%s).", pTradeData->AcpTradeData.dwIMP, pAccept->GetName());
				pRequest->SystemNotice(szNotice);
			...
			{
				sprintf(szNotice, "你已经收到了 [%d] IMPs 来自 (%s).", pTradeData->ReqTradeData.dwIMP, pRequest->GetName());
				pAccept->SystemNotice(szNotice);
			}
