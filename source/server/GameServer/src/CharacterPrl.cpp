...
{T_B
	...
	case CMD_PM_GUILDBANK:{
		...
		if (!IsLiveing()){
			SystemNotice("牺牲的海盗无法交易.");
		}
		else if (!IsInArea(2)){
			SystemNotice("必须在安全区域才能使用银行.");
		}else{
			switch (bankType){
	...
	case CMD_CM_GUILD_PERM:{
		...
		if (guild_id == 0 || !emGldPermMgr&GetPlyMainCha()->guildPermission || game_db.GetGuildLeaderID(guild_id)==targetID){
			GetPlyMainCha()->SystemNotice("你没有这样做的权限.");
			return;
		}

		//update in DB
		if (!game_db.SetGuildPermission(targetID, permission, guild_id)){
			GetPlyMainCha()->SystemNotice("玩家没有找到");
			return;
		}
	...
	case CMD_CM_MASTER_DEL:
		{
			...
			pMainCha->SystemNotice("您的师傅删除成功");
