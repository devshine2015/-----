...
#endif
COLORREF *custColor;
static bool isdigit(char key) { return key >= '0' && key <= '9'; }
#ifdef __EDITOR__
//---------------------------------------------------------------------------
// calss CEditor
//---------------------------------------------------------------------------
...
bool CEditor::Init() {
  ...
  if (!trvEditor)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(),
                 "trvEditor");
  ...
  chkEnabledAlpha =
      dynamic_cast<CCheckBox *>(frmEditor->Find("chkEnabledAlpha"));
  if (!chkEnabledAlpha)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(),
                 "chkEnabledAlpha");
  ...
  CEdit *edtShotMiniMap =
      dynamic_cast<CEdit *>(frmEditor->Find("edtShotMiniMap"));
  if (edtShotMiniMap)
  ...
  edtSetBrushHeight =
      dynamic_cast<CEdit *>(frmEditor->Find("edtSetBrushHeight"));
  ...
  chkModifyHeight =
      dynamic_cast<CCheckBox *>(frmEditor->Find("chkModifyHeight"));
  if (!chkModifyHeight)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(),
                 "chkModifyHeight");
  ...
  chkShowLightBrush =
      dynamic_cast<CCheckBox *>(frmEditor->Find("chkShowLightBrush"));
  ...
  ckhHideOptionEx =
      dynamic_cast<CCheckBox *>(frmEditor->Find("ckhHideOptionEx"));
  if (!ckhHideOptionEx)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(),
                 "ckhHideOptionEx");
  ...
  lblSetMainChaPos =
      dynamic_cast<CLabel *>(frmEditor->Find("lblSetMainChaPos"));
  if (!lblSetMainChaPos)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(),
                 "lblSetMainChaPos");

  lblSetBrushHeight =
      dynamic_cast<CLabel *>(frmEditor->Find("lblSetBrushHeight"));
  ...
  edtSetIslandIndex =
      dynamic_cast<CEdit *>(frmEditor->Find("edtSetIslandIndex"));
  ...
  lblSetIslandIndex =
      dynamic_cast<CLabel *>(frmEditor->Find("lblSetIslandIndex"));
...
void CEditor::_TreeEvent(CTreeView *tree, bool press) {
  ...
  if (_stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_535)) == 0) {
    ...
    if (p) {
      ...
      switch (p->nTag) {
      case 0: {
        pCha->PlayAni(pCha->GetDefaultChaInfo()->nBirthBehave,
                      defCHA_DIE_EFFECT_NUM);
...
void CEditor::_ColorMouseDown(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (UIGetChooseColor(color, custColor))
...
void CEditor::_ShowEditor(CGuiData *pSender) { g_Editor.Enable(true); }

void CEditor::_HideEditor(CGuiData *pSender) { g_Editor.Enable(false); }
...
bool CEditor::_evtSetChaPosKeyChar(CGuiData *pSender, char &key) {
  ...
  if (key == ',' || isdigit(key) || key == VK_RETURN || key == VK_BACK ||
      key == '-') {
    if (key == VK_RETURN) {
      ...
      if (pos != string::npos) {
        ...
        CGameApp::GetCurScene()->SetMainCha(
            CGameApp::GetCurScene()->GetMainCha()->getID());
...
bool CEditor::_evtShotMiniMap(CGuiData *pSender, char &key) {
  ...
  if (key == ',' || isdigit(key) || key == VK_RETURN || key == VK_BACK ||
    ...
    if (key == VK_RETURN) {
      ...
      g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_544), value[0], value[1],
                         value[2], value[3]);
      CCompent::SetActive(NULL);
    }
    return true;
  ...
  if (isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
    if (key == VK_RETURN) {
      ...
      if (str.empty()) {
        ...
        CEffectObj *pEffect =
            dynamic_cast<CEffectObj *>(g_Editor.GetSelectSceneObj());
        if (!pEffect) {
