...
CEditor::CEditor()
{
	custColor = new COLORREF[16];
	memset(custColor, 0, sizeof(custColor)*16);
}
...
bool CEditor::Init()
{
    ...
    if( !trvEditor ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "trvEditor" );
    ...
    if( !imgEditor ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgEditor" );

    btnColor =  dynamic_cast<CTextButton*>(frmEditor->Find("btnColor"));
    if( !btnColor ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "btnColor" );
    ...
    if( !chkSize01 ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize01" );

    chkSize02 =  dynamic_cast<CCheckBox*>(frmEditor->Find("chkSize02"));
    if( !chkSize02 ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize02" );

    chkSize03 =  dynamic_cast<CCheckBox*>(frmEditor->Find("chkSize03"));
    if( !chkSize03 ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize03" );

    chgGroup =   dynamic_cast<CCheckGroup*>(frmEditor->Find("chgGroup"));
    if( !chgGroup ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chgGroup" );
    chgGroup->evtSelectChange = _CheckSelectChange;

    chkWireFrame =  dynamic_cast<CCheckBox*>(frmEditor->Find("chkWireFrame"));
    if( !chkWireFrame ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkWireFrame" );
    chkWireFrame->evtCheckChange = _WireCheckChange;

    chkEnabledAlpha =  dynamic_cast<CCheckBox*>(frmEditor->Find("chkEnabledAlpha"));
    if( !chkEnabledAlpha ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkEnabledAlpha" );
    ...
    if( !chkSmooth ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSmooth" );
    ...
    if( !chkHideTree ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkHideTree" );
    ...
    if( !imgTreeFrame ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgTreeFrame" );

    edtSetMainChaPos =  dynamic_cast<CEdit*>(frmEditor->Find("edtSetMainChaPos"));
    if( !edtSetMainChaPos ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "edtSetMainChaPos" );
    ...
    if( !edtSetBrushHeight ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "edtSetBrushHeight" );
    edtSetBrushHeight->evtKeyChar = _evtSetBrushHeightKeyChar;
    char buf[10] = { 0 };
	//  itoa(g_Editor.m_nBrushHeight, buf, 10);
	_itoa_s(g_Editor.m_nBrushHeight, buf, sizeof(buf),10);
    edtSetBrushHeight->SetCaption( buf );

    chkModifyHeight =  dynamic_cast<CCheckBox*>(frmEditor->Find("chkModifyHeight"));
    if( !chkModifyHeight ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkModifyHeight" );
    ...
    if( !chkColor ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkColor" );
    ...
    if( !chkShowLightBrush ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkShowLightBrush" );
    ...
    if( !ckhHideOptionEx ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "ckhHideOptionEx" );
    ...
    if( !imgEditorEx ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgEditorEx" );

    lblSetMainChaPos = dynamic_cast<CLabel*>(frmEditor->Find("lblSetMainChaPos"));
    if( !lblSetMainChaPos ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetMainChaPos" );

    lblSetBrushHeight = dynamic_cast<CLabel*>(frmEditor->Find("lblSetBrushHeight"));
    if( !lblSetBrushHeight ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetBrushHeight" );

    ckhShowCompass =  dynamic_cast<CCheckBox*>(frmEditor->Find("ckhShowCompass"));
    if( !ckhShowCompass ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "ckhShowCompass" );
    ...
    if( !lblSetIslandIndex ) return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetIslandIndex" );
...
void CEditor::_TreeEvent(CTreeView* tree, bool press )
{
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_528) )==0 )
    {
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_529) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
   ...
   //  if( stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_530) )==0 )
    if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_530) )==0 )
	{
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
        if( p ) 
        {
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_531) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
        if( p ) 
        {
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_532) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
        if( p && press ) 
        {
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_533) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_534) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
        if( p ) 
        {
	...
	if( _stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_535))==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
        if( p )
        {
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_538) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
    ...
    if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_539) )==0 )
	{
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
	...
	if( _stricmp( item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_540) )==0 )
    {
        CGraph *p = dynamic_cast<CGraph*>( tree->GetSelect()->GetItem() );
...
void CEditor::_ColorMouseDown(CGuiData *pSender, int x, int y, DWORD key)
{
    ...
    if( UIGetChooseColor( color, custColor ) ) g_stUIEditor.btnColor->GetImage()->SetColor( color );
...
bool CEditor::_evtShotMiniMap(CGuiData *pSender, char& key)
{
    ...
    if( key ==',' || isdigit( key ) || key==VK_RETURN || key==VK_BACK || key=='-' )
    {
        if( key==VK_RETURN )
        {
            ...
            if( n!=4 )
            {
                g_pGameApp->AddTipText( RES_STRING(CL_LANGUAGE_MATCH_543) );
                return true;
            }
