...
bool CJewelryUpMgr::Init() {
  ...
  for (int i = 0; i < NUM_SLOT; ++i) {
    _snprintf_s(CtrlName, _countof(CtrlName), _TRUNCATE, "txtJewelryUp%d",
                i + 1);
    ...
    _snprintf_s(CtrlName, _countof(CtrlName), _TRUNCATE, "BtnJewelryUp%d",
                i + 1);
...
void CJewelryUpMgr::ShowConfirmDialog(long OldJeyID, long OldJeyLev,
                                      long NewJeyID, long NewJeyLev) {
  char szBuf[256] = {0};

  CItemRecord *pNewStoneInfo = GetItemRecordInfo(NewJeyID);
  _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_990), NewJeyLev,
              pNewStoneInfo->szName);
  g_stUIBox.ShowSelectBox(evtConfirmEvent, szBuf, true);
}

void CJewelryUpMgr::Clear() { PopItem(); }

void CJewelryUpMgr::PushItem(CItemCommand &Item) {
  CItemCommand *pItemCommand =
      static_cast<CItemCommand *>(cmdJewelry->GetCommand());
  if (pItemCommand)
    PopItem();
  ...
  for (int i = 0; i < ItemForgeInfo.nStoneNum && i < ItemForgeInfo.nHoleNum;
       ++i) {
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
...
void CJewelryUpMgr::evtSetJewelryEvent(CGuiData *pSender, CCommandObj *pItem,
                                       bool &isAccept) {
  isAccept = false;
  ...
  if (pCmd->nTag != 999) {
    ...
    if (pCmd == cmdJewelry) {
      if (pItemRecord->sType == 1 || pItemRecord->sType == 2 ||
          pItemRecord->sType == 3 || pItemRecord->sType == 4 ||
          pItemRecord->sType == 7 || pItemRecord->sType == 9 ||
          pItemRecord->sType == 22 || pItemRecord->sType == 23 ||
          pItemRecord->sType == 24 || pItemRecord->sType == 27)
        g_stJewelryUpForm.PushItem(*pItemCommand);
      else {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_989));
...
void CJewelryUpMgr::evtJewelryUpCommit(CGuiData *pSender, int x, int y,
                                       DWORD key) {
  char buf[256] = {0};

  if (!cmdJewelry->GetCommand()) {
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_1012));
    g_pGameApp->MsgBox(buf);
    return;
  }
  ...
  SItemForge &ItemForgeInfo =
      static_cast<CItemCommand *>(cmdJewelry->GetCommand())->GetForgeInfo();
  int TotalMoney = 0;
  for (int i = 0; i < ItemForgeInfo.nStoneNum && i < ItemForgeInfo.nHoleNum;
       ++i)
    TotalMoney += ItemForgeInfo.nStoneLevel[i] * COST_PER_LEVEL;
  ...
  if (OwnedMoney < TotalMoney) {
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_459));
    g_pGameApp->MsgBox(buf);
    return;
  }
  ...
  for (short index = 0; index < NUM_SLOT; ++index) {
    if (pSender == g_stJewelryUpForm.BtnJewelryUp[index]) {
      if (index >= ItemForgeInfo.nStoneNum || index >= ItemForgeInfo.nHoleNum) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_1013));
        g_pGameApp->MsgBox(buf);
        return;
      }
      ...
      if (!(ItemForgeInfo.pStoneInfo[index]->nItemID >= 860 &&
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_1014));
        g_pGameApp->MsgBox(buf);
        return;
      }
...
void CJewelryUpMgr::evtConfirmEvent(CCompent *pSender, int nMsgType, int x,
                                    int y, DWORD dwKey) {
...
void CJewelryUpMgr::evtShowHelpInfo(CGuiData *pSender, int x, int y,
                                    DWORD key) {
