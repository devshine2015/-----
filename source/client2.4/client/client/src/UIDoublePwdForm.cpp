...
void CDoublePwdMgr::SendItemUnlock() // 被锁道具的位置和id
{
  ...
  // CS_UnlockItem( szMD5,
  //			   _lock_pos_,			  // 锁道具的位置
  //			   _lock_item_id_,	      // 锁道具的id
  //			   _lock_grid_id_,       // 被锁道具的位置
  //			   _lock_fusion_item_id_);// 被锁道具的id
  CS_UnlockItem(szMD5,
                _lock_pos_,                        // 锁道具的位置
                (dbc::Char)_lock_item_id_,         // 锁道具的id
                (dbc::Char)_lock_grid_id_,         // 被锁道具的位置
                (dbc::Char)_lock_fusion_item_id_); // 被锁道具的id
  // 关闭二次密码框
  ...
  if (strName == "btnYes") {
    ...
    if (!IsPwdValid(g_stUIDoublePwd.edtDoublePwdCreate->GetCaption())) {
      ...
      g_pGameApp->MsgBox(RES_STRING(
          CL_LANGUAGE_MATCH_797)); //"二次密码只能有 0 ~ 9 数字组成，请重新输入"
    ...
    if (6 > nPwdLen || 12 < nPwdLen) {
      // 密码长度不在 6 ~ 12 之间
      g_pGameApp->MsgBox(RES_STRING(
          CL_LANGUAGE_MATCH_798)); //"二次密码长度不在 6 ~ 12，请重新输入"
    ...
    if (0 != strcmp(g_stUIDoublePwd.edtDoublePwdCreate->GetCaption(),
      ...
      // 两次密码不同
      g_pGameApp->MsgBox(
          RES_STRING(CL_LANGUAGE_MATCH_799)); //"两次输入的密码不同，请重新输入"
