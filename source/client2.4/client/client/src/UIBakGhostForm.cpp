...
bool CBakGhostMgr::Init() {
  ...
  cmdRequirement[0] =
      static_cast<COneCommand *>(frmBakGhost->Find("cmdBakGhost0"));
  cmdRequirement[1] =
      static_cast<COneCommand *>(frmBakGhost->Find("cmdBakGhost1"));
  for (int i = 0; i < 2; i++) {
    if (cmdRequirement[i]) {
  ...
  CTextButton *btnHelp =
      static_cast<CTextButton *>(frmBakGhost->Find("btnHelp"));
  if (!btnHelp)
    return false;
...
void CBakGhostMgr::CreateSpiriteResponse(bool Success) {
  if (Success) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_979));
...
void CBakGhostMgr::evtRequirementEvent(CGuiData *pSender, CCommandObj *pItem,
                                       bool &isAccept) {
  isAccept = false;
  ...
  if (pCmd->nTag != 999) {
    ...
    if (pCmd == cmdRequirement[0]) {
      if (pItemRecord->sType == 59)
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_977));
        g_pGameApp->MsgBox(buf);
      }
      ...
      if (pItemRecord->sType == 78)
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_978));
        g_pGameApp->MsgBox(buf);
      }
    }
  }
...
void CBakGhostMgr::evtCreateSpirite(CGuiData *pSender, int x, int y,
                                    DWORD key) {
  char buf[256] = {0};
