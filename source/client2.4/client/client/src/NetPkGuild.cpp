...
BOOL MC_GUILD_LISTTRYPLAYER(LPRPACKET pk) {
  ...
  NetMC_LISTTRYPLAYER_BEGIN(l_gldid, l_gldname, l_motto, l_type, l_stat,
                            l_ldrname, l_memnum, l_maxmem, l_money, l_exp,
                            l_remain);
...
BOOL PC_GUILD(LPRPACKET pk) {
  ...
  case MSG_GUILD_START: {
    ...
    for (uChar i = 0; i < l_num; ++i) {
      ...
      uShort l_permission = pk.ReadShort();
      ...
      strncpy_s(info.szChaName, sizeof(info.szChaName), l_chaname, _TRUNCATE);
      strncpy_s(info.szJob, sizeof(info.szJob), l_job, _TRUNCATE);
      strncpy_s(info.szMotto, sizeof(info.szMotto), l_motto, _TRUNCATE);
    ...
    if (nGuildCount == (int)g_vecGuildInfo.size()) {
      for (int i = 0; i < nGuildCount; ++i) {
        NetPC_GUILD_START(g_vecGuildInfo[i].bOnline, g_vecGuildInfo[i].nChaID,
                          g_vecGuildInfo[i].szChaName,
                          g_vecGuildInfo[i].szMotto, g_vecGuildInfo[i].szJob,
                          g_vecGuildInfo[i].sDegree, g_vecGuildInfo[i].sIcon,
  ...
  case MSG_GUILD_ADD: {
    ...
    NetPC_GUILD_ADD(l_online, l_chaid, l_chaname, l_motto, l_job, l_degree,
                    l_icon, l_permission);
  } break;
  case MSG_GUILD_DEL: {
...
BOOL MC_GUILD_LEAVE(LPRPACKET pk) { return TRUE; }

BOOL MC_GUILD_KICK(LPRPACKET pk) { return TRUE; }
...
BOOL MC_GUILD_INFO(LPRPACKET pk) {
  ...
  NetMC_GUILD_INFO(dwCharID, dwGuildID, pszGuildName, pszGuildMotto);
  return TRUE;
}
...
BOOL MC_GUILD_LISTCHALL(LPRPACKET pk) {
  ...
  for (int i = 0; i < MAX_GUILD_CHALLLEVEL; i++) {
    ...
    if (Info.byLevel[i]) {
      ...
      strncpy_s(Info.szGuild[i], 64 - 1, pk.ReadString(), _TRUNCATE);
      strncpy_s(Info.szChall[i], 64 - 1, pk.ReadString(), _TRUNCATE);
...
BOOL PC_GUILD_INVITE(LPRPACKET pk) {
  const char *l_inviter_name = pk.ReadString();
  uLong l_inviter_chaid = pk.ReadLong();
  uShort l_inviter_icon = pk.ReadShort();
  NetGuildInvite(l_inviter_name, l_inviter_chaid, l_inviter_icon);
  return TRUE;
}
BOOL PC_GUILD_CANCEL(LPRPACKET pk) {
  unsigned char reason = pk.ReadChar();
  NetGuildCancel(pk.ReadLong(), reason);
  return TRUE;
}