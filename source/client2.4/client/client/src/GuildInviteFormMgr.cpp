...
bool CGuildInviteFormMgr::AddInviteForm(DWORD id, string inviterName) {
  ...
  CLabelEx *labManageName =
      dynamic_cast<CLabelEx *>(node->pForm->Find("labManageName"));
  if (labManageName) {
  ...
  CLabelEx *labManage =
      dynamic_cast<CLabelEx *>(node->pForm->Find("labManage"));
  if (labManage) {
...
void CGuildInviteFormMgr::_MainMousePlayerGuildEvent(CCompent *pSender,
