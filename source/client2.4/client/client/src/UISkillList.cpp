...
CSkillList::CSkillList(CForm &frmOwn)
    : CCompent(frmOwn), _nShowFirst(0), _nShowLast(0), _nRowHeight(36),
      _nShowCount(0), _dwFontColor(COLOR_BLACK), _nLeftMargin(0),
      _nTopMargin(0), _nRightMargin(0), _nBottomMargin(0), _nFontStart(40),
      _nFontYOff(0), _nSelectIndex(0), _nUnitHeight(32), _nUnitWidth(32),
      _nDragIndex(-1), _IsShowUpgrade(true), evtUpgrade(NULL),
      _nMouseUpgradeID(-1) {
  // _IsFocus = true;
...
CSkillList::CSkillList(const CSkillList &rhs) : CCompent(rhs) {
...
CSkillList::~CSkillList(void) {
  ...
  SAFE_DELETE(_pImage);
  SAFE_DELETE(_pScroll);
  SAFE_DELETE(_pButton);
}
...
void CSkillList::Init() {
  ...
  _pScroll->SetSize(_pScroll->GetWidth(),
                    GetHeight() - _nTopMargin - _nBottomMargin);
  _pScroll->SetPos(GetWidth() - _nRightMargin - _pScroll->GetWidth(),
                   _nTopMargin);
  _pScroll->Init();
  ...
  _pSelect->SetScale(GetWidth() - _nLeftMargin - _nRightMargin - _nUnitWidth -
                         _pScroll->GetWidth(),
                     _nUnitHeight);
...
void CSkillList::Render() {
  ...
  if (_nSelectIndex >= _nShowFirst && _nSelectIndex < _nShowLast) {
    _pSelect->Render(_nSX1 + _nUnitWidth,
                     _nSY1 + _nRowHeight * (_nSelectIndex - _nShowFirst));
  }
  ...
  for (int i = _nShowFirst; i < _nShowLast; i++) {
    pSkill = _skills[i];
    pSkill->Render(x, y);
    CGuiFont::s_Font.Render(pSkill->GetSkillName(), x + _nFontStart,
                            y + _nFontYOff, _dwFontColor);
  ...
  for (int i = _nShowFirst; i < _nShowLast; i++) {
    ...
    case enumSKILL_UPGRADE_CAN:
      if (_IsShowUpgrade &&
          !_skills[i]->GetIsSpecial(CSkillCommand::enumNotUpgrade)) {
...
void CSkillList::Refresh() {
  ...
  float fMax = (float)((int)_skills.size() - _nShowCount);
  if (fMax <= 0.0f)
    fMax = 0.0f;
...
bool CSkillList::MouseRun(int x, int y, DWORD key) {
  ...
  if (InRect(x, y)) {
    ...
    if (key & Mouse_LDown) {
      if (evtUpgrade && _IsShowUpgrade &&
          _skills[row]->GetSkillRecord()->GetIsUpgrade() &&
        ...
        if (tmpX >= _nButtonX1 && tmpX <= _nButtonX1 + _pButton->GetWidth() &&
            tmpY >= _nButtonOffY &&
            tmpY <= _nButtonOffY + _pButton->GetHeight()) {
...
void CSkillList::DragRender() {
  if (_nDragIndex >= 0 && _nDragIndex < (int)_skills.size()) {
    _skills[_nDragIndex]->Render(_pDrag->GetX() - _nDragOffX,
