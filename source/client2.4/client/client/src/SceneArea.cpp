...
long CSceneArea::Init(_TCHAR *ptcsAreaFile, bool bSilence) {
  ...
  if (m_fRdWr == NULL) {
    if (!bSilence) {
      //_stprintf(tcsPrint, _TEXT("%s %s"), ptcsAreaFile,
      //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_339)));
      _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s"),
                  ptcsAreaFile, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_339)));
  ...
  if (m_SFileHead.lVersion != SCENE_AREA_FILE_VER100) {
    if (!bSilence) {
      //_stprintf(tcsPrint, _TEXT("%s %s"), ptcsAreaFile,
      //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_340)));
      _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s"),
                  ptcsAreaFile, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_340)));
      MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
    }
    lRet = -2;
    goto end;
...
long CSceneArea::CreateFile(_TCHAR *ptcsAreaFile, int iSceneWidth,
                            int iSceneHeight) {
  if (m_bInitSuccess)
    Free();
  ...
  strncpy_s(SHead.tcsTitle, sizeof(SHead.tcsTitle),
            _TEXT("HF Scene Area File!"), _TRUNCATE);
  SHead.lVersion = SCENE_AREA_FILE_VER100;
  SHead.lFileSize =
      sizeof(SFileHead) + sizeof(SAreaUnit) * iSceneWidth * iSceneHeight;
...
// lUnitNO 从零开始
long CSceneArea::ReadAreaInfo(long lUnitNO, SAreaUnit *pUnitData,
                              long *lpUnitNum) {
  if (!m_bInitSuccess)
    return -1;
...
long CSceneArea::WriteAreaInfo(long lUnitNO, SAreaUnit *pUnitData,
                               long *lpUnitNum) {
  if (!m_bInitSuccess)
    return -1;
  if (lUnitNO >= m_lUnitNum)
