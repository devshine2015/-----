...
CReadyDieState::CReadyDieState(CActor *p)
    : CActionState(p), _state(enumDied), _IsActionEnd(false),
      _eDieState(enumInit), _nFallDownTime(0), _IsAlreadyEffect(false),
      _pAttack(NULL), _pArcTrack(NULL), _isFlyOff(false) {
...
bool CReadyDieState::_Start() {
  ...
  if (_pCha->IsMainCha()) {
    ...
    if ((rand() % 9) == 0) {
      ...
      int x, y;
      int dis = rand() % 15 + 5;
      GetInDistancePos(int(_pCha->GetPos().x * 10.0f),
                       int(_pCha->GetPos().y * 10.0f),
                       int(_pAttack->GetPos().x * 10.0f),
                       int(_pAttack->GetPos().y * 10.0f), dis, x, y);
      D3DXVECTOR3 end;
      end.x = (float)x / 10.0f;
      end.y = (float)y / 10.0f;
      ...
      _pCha->setYaw(GetLineAngle(_pAttack->GetCurX(), _pAttack->GetCurY(),
                                 _pCha->GetCurX(), _pCha->GetCurX()));
...
void CReadyDieState::FrameMove() {
  ...
  if (_eDieState == enumFallDown) {
    ...
    if (_nFallDownTime == _nDelayTime) {
      _pCha->PlayAni(_pCha->GetDefaultChaInfo()->nDiedBehave,
