...
public:
    CCamDither();
    void Init(const LEMatrix44* mat);
    LEMatrix44* GetRunningMatrix() { return &_RunningMat; }
    void FrameMove();
    BOOL GetState() { return _InitFlag; }
};

class CCameraCtrl : public LECamera
{
public:
	CCameraCtrl(void);
	~CCameraCtrl(void);
...
public:
	...
	float	m_fChaHeight;			//角色距水平地面高度
	float	m_fChaHeightCur;		//当前递增到的值
	float	m_fChaHeightTarget;		//递增Target
	// end

	float	m_fCameraTileHeight;	//摄像机所在处的地形高度
