...
bool CChaQeryInfo::Init() {
  ...
  m_pChangePhoto =
      dynamic_cast<CTextButton *>(m_pFrmSInfo->Find("btntouxiang"));
...
void CChaQeryInfo::SetShowChaInfo(bool _b) { m_pFrmSInfo->SetIsShow(_b); }

void CChaQeryInfo::SetShowQChaInfo(bool _b) { m_pFrmQInfo->SetIsShow(_b); }

void CChaQeryInfo::SetShowQRChaInfo(bool _b) { m_pFrmQRInfo->SetIsShow(_b); }
void CChaQeryInfo::_evSFrm(CGuiData *pSender, int x, int y, DWORD key) {
  if (pSender == (CGuiData *)g_ChaQeryInfo.m_pBtYes) // 详细信息   --确定
  {

  } else if (pSender ==
             (CGuiData *)g_ChaQeryInfo.m_pAddFlowerBt) //  --- 增加鲜花
  {

  } else if (pSender ==
             (CGuiData *)g_ChaQeryInfo.m_pAddEggBt) // -----增加臭鸡蛋
  {

  } else if (pSender ==
             (CGuiData *)g_ChaQeryInfo.m_pChangePhoto) // ----更改头像
  {
...
bool CChaQeryInfo::SetSearchFrmData(stPersonInfo *_info, CMember *_m) {
  ...
  if (m_pFrmSInfo) {
    ...
    m_pChaJob->SetCaption(
        _m->GetJobNameByID()); // modify by rock.wang 2009.05.07
    // 声望不知道是什么
    m_pReputation->SetCaption("");
    //
...
stPersonInfo *CChaQeryInfo::GetSearchFrmData() { return &m_ShowInfo; }

bool CChaQeryInfo::SetQueryFrmData(stQueryPersonInfo *_info) { return true; }

stQueryPersonInfo *CChaQeryInfo::GetQueryFrmData() { return &m_QeryInfo; }
...
bool CChaQeryInfo::SetQueryRFrmData(stQueryResoultPersonInfo *_info, int num) {
  ...
  if (m_pFrmQRInfo) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      if ((*_info).nMinAge == -1)
        continue;
      _snprintf_s(resoult, _countof(resoult), _TRUNCATE, "%s  %d  %s  %s  %s ",
                  (*_info).sChaName, (*_info).nMinAge, (*_info).cSex,
                  (*_info).szState, (*_info).nCity);
      m_pQRList->Add(resoult);
    }
  }
  return true;
}
void CChaQeryInfo::ClearQueryRFrmData() { m_pQRList->Clear(); }
...
void CChaQeryInfo::UpdateInfo() {
  ...
  if (m_pFrmSInfo) {
    ...
    m_ShowInfo.iBirthday =
        (atoi(birthmonth) << 5) + atoi(birthday); // Modify by sunny.sun20080825
  ...
  if (m_pFrmQInfo) {
    ...
    len = sizeof(m_QeryInfo.sChaName) > strlen(m_pQchaName->GetCaption())
              ? strlen(m_pQchaName->GetCaption())
              : sizeof(m_QeryInfo.sChaName);
    memcpy(m_QeryInfo.sChaName, m_pQchaName->GetCaption(), len);

    // 要把字符放到stringset.txt中去
    if (strcmp(m_pQHasPic->GetCaption(), RES_STRING(CL_UICHAINFO_CPP_00002)) ==
        0)
      m_QeryInfo.bHavePic = true;
    else
      m_QeryInfo.bHavePic = false;

    // m_pFrmQInfo.bHavePic =
    // (strcmp(m_pQHasPic->GetCaption(),"有")==0)？true:false;

    len = sizeof(m_QeryInfo.cSex) > strlen(m_pQSex->GetCaption())
              ? strlen(m_pQSex->GetCaption())
              : sizeof(m_QeryInfo.cSex);
    ...
    len = sizeof(m_QeryInfo.szAnimalZodiac) >
                  strlen(m_pQAnimalZodiac->GetCaption())
              ? strlen(m_pQAnimalZodiac->GetCaption())
              : sizeof(m_QeryInfo.szAnimalZodiac);
    memcpy(m_QeryInfo.szAnimalZodiac, m_pQAnimalZodiac->GetCaption(), len);

    // m_QeryInfo.iBirthday[0] =
    // (atoi(m_pQBirthmonth0->GetCaption())<<16)|(atoi(m_pQBirthday0->GetCaption()));
    // m_QeryInfo.iBirthday[1] =
    // (atoi(m_pQBirthmonth1->GetCaption())<<16)|(atoi(m_pQBirthday1->GetCaption()));
    // Modify by sunny.sun 20080825
    m_QeryInfo.iBirthday[0] = (atoi(m_pQBirthmonth0->GetCaption()) << 5) +
                              (atoi(m_pQBirthday0->GetCaption()));
    m_QeryInfo.iBirthday[1] = (atoi(m_pQBirthmonth1->GetCaption()) << 5) +
                              (atoi(m_pQBirthday1->GetCaption()));

    len = sizeof(m_QeryInfo.szState) > strlen(m_pQstate->GetCaption())
              ? strlen(m_pQstate->GetCaption())
              : sizeof(m_QeryInfo.szState);
    memcpy(m_QeryInfo.szState, m_pQstate->GetCaption(), len);

    len = sizeof(m_QeryInfo.szCity) > strlen(m_pQcity->GetCaption())
              ? strlen(m_pQcity->GetCaption())
              : sizeof(m_QeryInfo.szCity);
    memcpy(m_QeryInfo.szCity, m_pQcity->GetCaption(), len);

    len = sizeof(m_QeryInfo.szConstellation) >
                  strlen(m_pQConstellation->GetCaption())
              ? strlen(m_pQConstellation->GetCaption())
              : sizeof(m_QeryInfo.szConstellation);
    memcpy(m_QeryInfo.szConstellation, m_pQConstellation->GetCaption(), len);

    len = sizeof(m_QeryInfo.szCareer) > strlen(m_pQjob->GetCaption())
              ? strlen(m_pQjob->GetCaption())
              : sizeof(m_QeryInfo.szCareer);
