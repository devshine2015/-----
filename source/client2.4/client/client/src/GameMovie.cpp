...
bool CGameMovie::Play(HWND hWnd, TCHAR *pszFileName) {
  ...
  //_pVidWin->SetWindowPosition((rc.right - rc.left - 720)/2, (rc.bottom -
  //rc.top - 404)/2, 720, 404); 	int height = (rc.right - rc.left) * 9.0 / 16.0;
  int height = (int)((rc.right - rc.left) * 9.0 / 16.0);

  _pVidWin->SetWindowPosition(rc.left, (rc.bottom - rc.top - height) / 2,
