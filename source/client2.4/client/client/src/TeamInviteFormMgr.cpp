...
bool CTeamInviteFormMgr::AddInviteForm(DWORD id, string inviterName) {
  ...
  char str[256];
  _snprintf_s(str, _countof(str), _TRUNCATE, "frmAcceptGroup_Arcol_%d", id);
  node->pForm->SetName(str);
  m_FormLink.push_back(node);
  // node->pForm->SetPos(200,200);
  // node->pForm->Refresh();
  CLabelEx *labGroupName =
      dynamic_cast<CLabelEx *>(node->pForm->Find("labGroupName"));
  if (labGroupName) {
    _snprintf_s(str, _countof(str), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_62),
                inviterName.data());
...
void CTeamInviteFormMgr::_MainMousePlayerGroupEvent(CCompent *pSender,
