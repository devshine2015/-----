...
// 协议C->S : 发送行动信息
void CProCirculateCS::BeginAction(CCharacter *pCha, DWORD type, void *param,
                                  CActionState *pState) {
  ...
  try {
    ...
    case enumACTION_MOVE: {
      ...
      pk.WriteSequence((cChar *)pMove->pos_buf,
                       uShort(sizeof(Point) * pMove->pos_num));
      pCNetIf->SendPacketMessage(pk);

      // 				LG(szLogName,
      // "###Send(Move):\tTick:[%d]\n", GetTickCount()); 				LG(szLogName,
      // "Ping:\t%3d\n", pMove->dwAveragePing); 				LG(szLogName, "Point:\t%3d\n",
      // pMove->pos_num); 				for (DWORD i = 0; i < pMove->pos_num; i++)
      // 				{
      // 					LG(szLogName, "\t%d, \t%d\n",
    ...
    case enumACTION_SKILL: {
      ...
      if (pSkill->chMove == 2) {
        pk.WriteSequence((cChar *)pSkill->SMove.pos_buf,
                         uShort(sizeof(POINT) * pSkill->SMove.pos_num));
      }
      ...
      // 				LG(szLogName,
      // "###Send(Skill):\tTick:[%d]\n", GetTickCount()); 				if (pSkill->chMove ==
      // 2)
      // 				{
      // 					LG(szLogName, "Ping:\t%3d\n",
      // pSkill->SMove.dwAveragePing); 					LG(szLogName, "Point:\t%3d\n",
      // pSkill->SMove.pos_num); 					for (DWORD i = 0; i < pSkill->SMove.pos_num;
      // i++)
      // 					{
      // 						LG(szLogName, "\t%d, \t%d\n",
      ...
      // pSkill->lSkillID, pSkill->byFightID); 				LG(szLogName, "Target:\t%u,
      // \t%u\n", pSkill->lTarInfo1, pSkill->lTarInfo2); 				LG(szLogName, "\n");
      break;
    }
    case enumACTION_STOP_STATE: {
      ...
      // log
      // 				LG(szLogName, "###Send(Stop Skill State
      // %d):\tTick:[%d]\n", *((short *)param), GetTickCount()); 				LG(szLogName,
      // "\n");
      //
      break;
    }
    ...
    case enumACTION_ITEM_USE: {
      ...
      //              GetTickCount());
      // 				LG(szLogName,
      // RES_STRING(CL_LANGUAGE_MATCH_322), pUseItem->sGridID,
      // pUseItem->sTarGridID);
      //              LG(szLogName, "\n");
      break;
    }
    ...
    case enumACTION_ITEM_DELETE: {
      ...
      //              GetTickCount());
      // 				LG(szLogName,
      // RES_STRING(CL_LANGUAGE_MATCH_323), pDelItem->sGridID);
      //              LG(szLogName, "\n");
      break;
    }
    case enumACTION_ITEM_INFO: {
      ...
      //              GetTickCount());
      // 				LG(szLogName,
      // RES_STRING(CL_LANGUAGE_MATCH_324), pItemInfo->chType,
      // pItemInfo->sGridID);
      //              LG(szLogName, "\n");
      break;
    }
...
bool CProCirculate::Connect(const char *hostname, unsigned short port,
...
void CProCirculate::Login(const char *accounts, const char *password,
                          const char *passport) {
  /*strcpy(g_NetIF->m_accounts,accounts);
  strcpy(g_NetIF->m_passwd,password);*/
  strncpy_s(g_NetIF->m_accounts, sizeof(g_NetIF->m_accounts), accounts,
            _TRUNCATE);
  strncpy_s(g_NetIF->m_passwd, sizeof(g_NetIF->m_passwd), password, _TRUNCATE);

  KCHAPc l_chap(accounts, password);

  // if(strlen(g_Config.m_szMD5Pass) == 32) // 配置文件里直接输入MD5值密码登陆,
  // 方便诊断玩家报告的错误
  //{
  ...
  int l_len;
  char l_buf[1024];
  l_chap.GetStep1Data(&l_buf[512], sizeof(l_buf) - 512, l_len);
  ...
  if (g_TomServer.bEnable) {
    ...
    pk.WriteSequence(&l_buf[512], l_len);
  }
...
void CProCirculate::SynBaseAttribute(CChaAttr *pCAttr) {
  ...
  // strncpy_s(szReqChangeAttr,sizeof(szReqChangeAttr),
  // RES_STRING(CL_LANGUAGE_MATCH_325),_TRUNCATE);
  //
  // for (int i = ATTR_STR; i <= ATTR_LUK; i++)
  //{
  //	if (pCAttr->GetChangeBitFlag(i))
  //		LG(szReqChangeAttr, RES_STRING(CL_LANGUAGE_MATCH_326), i,
  //pCAttr->GetAttr(i));
  //}
  // LG(szReqChangeAttr, "\n");
...
void CProCirculate::SkillUpgrade(short sSkillID, char chAddLv) {
  ...
  // strncpy_s(szReqChangeAttr,sizeof(szReqChangeAttr),
  // RES_STRING(CL_LANGUAGE_MATCH_327),_TRUNCATE);
...
//	协议C->S : 发送请求挂机经验的请求
//-----------------------------------
void CProCirculate::RequestLeaveExp(char IsGet, int LeaveTime, int Position,
                                    int putNum) {
  WPacket pk = pCNetIf->GetWPacket();
