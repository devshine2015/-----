...
int UI_CreateCompent( int formId, int type, char *pszName, int w, int h, int x, int y )
{
	...
	case AMPHI_LIST_TYPE:	g = new CAmphitheaterList(*f);	break;//add by sunny.sun 20080718
	// Modify by lark.li 20081217 begin
	case WEB_BROWSER_TYPE:  g = new CWebBrowser(*f); break;
	// End
	default: 
		return R_FAIL;
	}
...
//#if _DEBUG
//	char buffer[255];
//	_snprintf_s( buffer, _countof( buffer ), _TRUNCATE, "UI_CreateForm\t%s\r\n", pszName);
...
//	if( lst ) return lst->GetScroll()->GetID();
	...
	CAmphitheaterList* lpList = dynamic_cast<CAmphitheaterList*> (p);
		if(lpList) return lpList->GetScroll()->GetID();
	return R_FAIL;
}
...
int UI_GetEXListObj(int id, int type)
{
	...
	switch( type )
	{
		case EXLIST_BUTTON: return p->GetButton()->GetID();
			break;
		default:
			break;
	}
...
int UI_LoadImage( int id, char * file, int frame, int w, int h, int tx, int ty )
{
	...
	if( img->LoadImage( file, w, h, frame, tx, ty ) ) return R_OK;
	return R_FAIL;
}
...
int UI_GridNodeAddItem( int nodeid, int itemid )
{
    ...
    if( pHint )
    {
		...
		if( _stricmp( obj->GetItem()->GetString(), RES_STRING(CL_LANGUAGE_MATCH_528) )==0 )
        {
            ...
            {
				char szBuf[128] = { 0 };
				_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE,  "%d.%s", pInfo->lID, pInfo->szName );
                pHint->SetHint( szBuf );
            }
        }
		//	以下stricmp 修改 同上
        ...
        else if( _stricmp( obj->GetItem()->GetString(), RES_STRING(CL_LANGUAGE_MATCH_529) )==0 )
        {
            ...
            {
				char szBuf[128] = { 0 };
				_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE,  "%d.%s", pInfo->nID, pInfo->szName );
                pHint->SetHint( szBuf );
            }
        }
        else if( pParent )            
