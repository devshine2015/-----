...
void UI_ReleaseItemScript() { g_ItemScript.ReleaseAll(); }
...
enum eHotKey { ALT_KEY = 0, CTRL_KEY, SHIFT_KEY };
...
int UI_CreateCompent(int formId, int type, char *pszName, int w, int h, int x,
                     int y) {
  CForm *f = dynamic_cast<CForm *>(CGuiData::GetGui(formId));
  if (!f)
    return R_FAIL;
...
int UI_CreateForm(char *pszName, int isModal, int w, int h, int x, int y,
                  int isTitle, int isShowFrame) {
  CForm *f = new CForm();
  f->SetName(pszName);
  f->SetSize(w, h);
  f->SetPos(x, y);
...
int UI_LoadFormImage(int id, const char *client, int cw, int ch, int tx, int ty,
...
int UI_LoadFrameImage(int id, const char *client, int cw, int ch, int tx,
                      int ty, const char *file, int w, int h) {
  CGuiData *p = CGuiData::GetGui(id);
  if (!p)
    return R_FAIL;
...
int UI_CreateListView(int formId, char *pszName, int w, int h, int x, int y,
                      int col, int style) {
  CForm *f = dynamic_cast<CForm *>(CGuiData::GetGui(formId));
  if (!f)
    return R_FAIL;
...
int UI_ListViewSetTitle(int listviewid, int index, int width,
                        const char *titleimage, int w, int h, int sx, int sy) {
...
int UI_LoadListFixSelect(int id, const char *imagefile, int w, int h, int sx,
                         int sy) {
...
int UI_LoadCheckFixListCheck(int id, const char *checkimage, int cw, int ch,
                             int csx, int csy, const char *uncheckimage, int uw,
                             int uh, int usx, int usy) {
...
int UI_GetEXListObj(int id, int type) {
  CAmphitheaterList *p =
      dynamic_cast<CAmphitheaterList *>(CGuiData::GetGui(id));
  if (!p)
    return R_FAIL;
  switch (type) {
  case EXLIST_BUTTON:
...
int UI_LoadComboImage(int id, char *edit, int ew, int eh, int ex, int ey,
                      char *button, int bw, int bh, int bx, int by,
                      int isHorizontal) {
  CGuiData *p = CGuiData::GetGui(id);
  if (!p)
    return R_FAIL;
...
int UI_LoadButtonImage(int id, const char *file, int w, int h, int sx, int sy,
                       int isHorizontal) {
  CGuiData *p = CGuiData::GetGui(id);
  if (!p)
    return R_FAIL;
...
int UI_LoadImage(int id, char *file, int frame, int w, int h, int tx, int ty) {
  CGuiData *p = CGuiData::GetGui(id);
  if (!p) {
    if (id != 0) // id=0时，表示 UI_CreateCompent 失败，可能该 ui
                 // 已经被杠掉，不需要关注
...
// 装载带比例的图象
int UI_LoadScaleImage(int id, char *file, int frame, int w, int h, int tx,
                      int ty, float scalex, float scaley) {
...
// 装载带比例的图象
int UI_LoadFlashScaleImage(int id, int flash, char *file, int frame, int w,
                           int h, int tx, int ty, float scalex, float scaley) {
  CGuiData *p = CGuiData::GetGui(id);
  if (!p)
    return R_FAIL;
...
int UI_ListLoadSelectImage(int id, char *file, int w, int h, int sx, int sy) {
  ...
  if (lst) {
    lst->GetItems()->GetSelect()->GetImage()->LoadImage(file, w, h, 0, sx, sy,
                                                        1.0f, 1.0f);
    return R_OK;
  }
...
int UI_LoadSkillListButtonImage(int id, char *file, int w, int h, int sx,
                                int sy, int item_w, int item_h) {
...
int UI_LoadListItemImage(int id, char *file, int w, int h, int sx, int sy,
...
int UI_AddFaceToGrid(int id, char *file, int w, int h, int sx, int sy,
                     int frame, int nTag) {
  CGrid *g = dynamic_cast<CGrid *>(CGuiData::GetGui(id));
  if (!g)
    return R_FAIL;
...
int UI_TreeLoadImage(int nTreeID, int nType, const char *imagefile, int w,
                     int h, int sx, int sy, int itemw, int itemh) {
...
int UI_CreateGraphItemTex(int tx, int ty, int tw, int th, float scale_x,
                          float scale_y, int nTextureID, int tag) {
  CHintGraph *item = new CHintGraph(1);
  LETexRect *pImage = item->GetImage()->GetImage();
  pImage->nTexSX = tx;
  pImage->nTexSY = ty;
  pImage->nTexW = tw;
  pImage->nTexH = th;
...
int UI_CreateNoteGraphItem(char *file, int w, int h, int sx, int sy, int frame,
                           const char *text, int TextX, int TextY) {
...
int UI_CreateGridNode(int treeid, int itemid, int maxcol, int uw, int uh,
                      int nodeid_parent) {
  CTreeView *g = dynamic_cast<CTreeView *>(CGuiData::GetGui(treeid));
  if (!g)
    return R_FAIL;
...
int UI_GridNodeAddItem(int nodeid, int itemid) {
  CTreeGridNode *obj =
      dynamic_cast<CTreeGridNode *>(CTreeNodeObj::GetNode(nodeid));
  if (!obj)
    return R_FAIL;
  ...
  if (pHint) {
    ...
    //	if( stricmp( obj->GetItem()->GetString(),
    //RES_STRING(CL_LANGUAGE_MATCH_528) )==0 )
    if (_stricmp(obj->GetItem()->GetString(),
                 RES_STRING(CL_LANGUAGE_MATCH_528)) == 0) {
      CChaRecord *pInfo = GetChaRecordInfo(pHint->nTag);
      if (pInfo) {
        char szBuf[128] = {0};
        _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%d.%s", pInfo->lID,
                    pInfo->szName);
        pHint->SetHint(szBuf);
      }
    }
    //	以下stricmp 修改 同上
    else if (_stricmp(obj->GetItem()->GetString(),
                      RES_STRING(CL_LANGUAGE_MATCH_532)) == 0) {
      CMapInfo *pInfo = GetMapInfo(pHint->nTag);
      if (pInfo) {
        pHint->SetHint(pInfo->szName);
      }
    } else if (_stricmp(obj->GetItem()->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_530)) == 0) {
      pHint->SetHint(g_GetAreaName(pHint->nTag));
    } else if (_stricmp(obj->GetItem()->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_529)) == 0) {
      CEffectInfo *pInfo = GetEffectInfo(pHint->nTag);
      if (pInfo) {
        char szBuf[128] = {0};
        _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%d.%s", pInfo->nID,
                    pInfo->szName);
        pHint->SetHint(szBuf);
      }
    } else if (pParent) {
      if (_stricmp(pParent->GetItem()->GetString(),
                   RES_STRING(CL_LANGUAGE_MATCH_540)) == 0) {
...
int UI_SetChatColor(int world, int road, int team, int guild, int gm,
                    int system, int trade, int person) {
...
// CHeadSay
//---------------------------------------------------------------------------
int UI_LoadHeadSayFaceImage(int num, int maxframe, int w, int h, char *file,
                            int cw, int ch, int tx, int ty) {
...
int UI_LoadHeadSayShopImage(int num, int w, int h, char *file, int cw, int ch,
                            int tx, int ty) {
...
int UI_LoadHeadSayLifeImage(int uw, int uh, char *file, int w, int h, int tx,
                            int ty, int isHorizontal) {
...
int UI_SetTextParse(int nIndex, char *file, int w, int h, int sx, int sy,
                    int frame) //建立文字与图象的映射
{
...
// 菜单
int UI_MenuLoadSelect(int id, const char *imagefile, int w, int h, int sx,
                      int sy) {
...
int UI_MenuLoadImage(int id, int IsShowFrame, int IsTitle,
                     const char *clientfile, int cw, int ch, int tx, int ty,
                     const char *framefile, int w, int h) {
  CMenu *f = dynamic_cast<CMenu *>(CGuiData::GetGui(id));
  if (!f)
    return R_FAIL;
...
int UI_LoadSkillActiveImage(char *file, int maxframe, int w, int h, int sx,
                            int sy) {
...
int UI_LoadChargeImage(int link, char *file, int maxframe, int w, int h, int sx,
                       int sy) {
...
const int AddMisDataFromC(int id, char *name, int type, int x1, int y1,
                          char *map1, int x2, int y2, char *map2) {
  CLU_CallScriptFunction("AddMisData", "int",
                         "int, char*, int, int, int, char*, int, int, char*",
                         id, name, type, x1, y1, map1, x2, y2, map2);
  return R_OK;
}
...
//---------------------------------------------------------------------------
void MPInitLua_Gui() {
  CLU_RegisterFunction("GetChaPhotoTexID", "int", "int", CLU_CDECL,
                       CLU_CAST(GetChaPhotoTexID));
  CLU_RegisterFunction("GetSceneObjPhotoTexID", "int", "int", CLU_CDECL,
                       CLU_CAST(GetSceneObjPhotoTexID));
  CLU_RegisterFunction("GetEffectPhotoTexID", "int", "int", CLU_CDECL,
                       CLU_CAST(GetEffectPhotoTexID));

  CLU_RegisterFunction("GetTextureID", "int", "char*", CLU_CDECL,
                       CLU_CAST(GetTextureID));
  CLU_RegisterFunction("GetTerrainTextureID", "int", "int", CLU_CDECL,
                       CLU_CAST(GetTerrainTextureID));
  CLU_RegisterFunction("GetTerrainTextureType", "int", "int", CLU_CDECL,
                       CLU_CAST(GetTerrainTextureType));
  CLU_RegisterFunction("GetSceneObjPhotoTexType", "int", "int", CLU_CDECL,
                       CLU_CAST(GetSceneObjPhotoTexType));

  // gui
  CLU_RegisterFunction("UI_LoadScript", "int", "char*", CLU_CDECL,
                       CLU_CAST(UI_LoadScript));
  CLU_RegisterFunction("UI_SetDragSnapToGrid", "int", "int,int", CLU_CDECL,
                       CLU_CAST(UI_SetDragSnapToGrid));

  CLU_RegisterFunction("UI_SetMaxImage", "int", "int,int", CLU_CDECL,
                       CLU_CAST(UI_SetMaxImage));

  CLU_RegisterFunction("UI_SetFormTempleteMax", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_SetFormTempleteMax));
  CLU_RegisterFunction("UI_SwitchTemplete", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_SwitchTemplete));
  CLU_RegisterFunction("UI_AddAllFormTemplete", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_AddAllFormTemplete));
  CLU_RegisterFunction("UI_AddFormToTemplete", "int", "int,int", CLU_CDECL,
                       CLU_CAST(UI_AddFormToTemplete));

  CLU_RegisterFunction("UI_CreateForm", "int",
                       "char*, int, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_CreateForm));
  CLU_RegisterFunction("UI_FormSetIsEscClose", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_FormSetIsEscClose));
  CLU_RegisterFunction("UI_FormSetEnterButton", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_FormSetEnterButton));
  CLU_RegisterFunction("UI_FormSetHotKey", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_FormSetHotKey));
  CLU_RegisterFunction("UI_LoadFormImage", "int",
                       "int, char*, int, int, int, int, char*, int, int",
                       CLU_CDECL, CLU_CAST(UI_LoadFormImage));
  CLU_RegisterFunction("UI_ShowForm", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_ShowForm));
  CLU_RegisterFunction("UI_SetIsDrag", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetIsDrag));
  CLU_RegisterFunction("UI_SetIsKeyFocus", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetIsKeyFocus));
  CLU_RegisterFunction("UI_CreateCompent", "int",
                       "int, int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_CreateCompent));

  // Add by lark.li 20090220 begin
  CLU_RegisterFunction("UI_SetFormInMianForm", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_SetFormInMianForm));
  // End

  CLU_RegisterFunction("UI_SetCaption", "int", "int, char*", CLU_CDECL,
                       CLU_CAST(UI_SetCaption));
  CLU_RegisterFunction("UI_CopyImage", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_CopyImage));
  CLU_RegisterFunction("UI_SetSize", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetSize));
  CLU_RegisterFunction("UI_SetPos", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetPos));
  CLU_RegisterFunction("UI_SetTag", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetTag));
  CLU_RegisterFunction("UI_SetAlign", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetAlign));
  CLU_RegisterFunction("UI_LoadImage", "int",
                       "int, char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadImage));
  CLU_RegisterFunction("UI_LoadScaleImage", "int",
                       "int, char*, int, int, int, int, int, float, float",
                       CLU_CDECL, CLU_CAST(UI_LoadScaleImage));
  CLU_RegisterFunction("UI_SetHint", "int", "int, char*", CLU_CDECL,
                       CLU_CAST(UI_SetHint));
  CLU_RegisterFunction("UI_LoadFlashScaleImage", "int",
                       "int, int ,char*, int, int, int, int, int, float, float",
                       CLU_CDECL, CLU_CAST(UI_LoadFlashScaleImage));
  CLU_RegisterFunction("UI_SetImageAlpha", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetImageAlpha));
  CLU_RegisterFunction("UI_ComboSetTextColor", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_ComboSetTextColor));
  CLU_RegisterFunction("UI_ComboSetStyle", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_ComboSetStyle));
  CLU_RegisterFunction(
      "UI_LoadComboImage", "int",
      "int, char*, int, int, int, int, char*, int, int, int, int, int",
      CLU_CDECL, CLU_CAST(UI_LoadComboImage));
  CLU_RegisterFunction("UI_LoadButtonImage", "int",
                       "int, char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadButtonImage));
  CLU_RegisterFunction("UI_GetScroll", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_GetScroll));
  CLU_RegisterFunction("UI_CopyCompent", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_CopyCompent));
  CLU_RegisterFunction("UI_GetList", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_GetList));
  CLU_RegisterFunction("UI_AddListText", "int", "int, char*", CLU_CDECL,
                       CLU_CAST(UI_AddListText));
  CLU_RegisterFunction("UI_ListLoadSelectImage", "int",
                       "int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_ListLoadSelectImage));
  CLU_RegisterFunction("UI_LoadListItemImage", "int",
                       "int, char*, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadListItemImage));
  CLU_RegisterFunction("UI_ListSetItemMargin", "int", "int, int, int",
                       CLU_CDECL, CLU_CAST(UI_ListSetItemMargin));
  CLU_RegisterFunction("UI_ListSetItemImageMargin", "int", "int, int, int",
                       CLU_CDECL, CLU_CAST(UI_ListSetItemImageMargin));
  CLU_RegisterFunction("UI_SetListFontColor", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetListFontColor));
  CLU_RegisterFunction("UI_SetProgressStyle", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetProgressStyle));
  CLU_RegisterFunction("UI_SetScrollStyle", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetScrollStyle));

  CLU_RegisterFunction("UI_AddGroupBox", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_AddGroupBox));
  CLU_RegisterFunction("UI_CreateListView", "int",
                       "int, char*, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_CreateListView));
  CLU_RegisterFunction("UI_ListViewSetTitle", "int",
                       "int, int, int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_ListViewSetTitle));
  CLU_RegisterFunction("UI_ListViewSetTitleHeight", "int", "int, int",
                       CLU_CDECL, CLU_CAST(UI_ListViewSetTitleHeight));
  CLU_RegisterFunction("UI_GoodGridLoadUnitImage", "int",
                       "int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_GoodGridLoadUnitImage));

  CLU_RegisterFunction("UI_SetChatColor", "int",
                       "int, int, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetChatColor));
  CLU_RegisterFunction("UI_SetListRowHeight", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetListRowHeight));
  CLU_RegisterFunction("UI_SetAlpha", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetAlpha));
  CLU_RegisterFunction("UI_GridLoadSelectImage", "int",
                       "int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_GridLoadSelectImage));
  CLU_RegisterFunction("UI_SetGridIsDragSize", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetGridIsDragSize));
  CLU_RegisterFunction("UI_GetScrollObj", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_GetScrollObj));
  CLU_RegisterFunction("UI_GetEXListObj", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_GetEXListObj)); // Add by sunny.sun20080721
  CLU_RegisterFunction("UI_SetGridUnitSize", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetGridUnitSize));
  CLU_RegisterFunction("UI_AddFaceToGrid", "int",
                       "int, char*, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_AddFaceToGrid));
  CLU_RegisterFunction("UI_SetIsShow", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetIsShow));
  CLU_RegisterFunction("UI_SetIsEnabled", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetIsEnabled));
  CLU_RegisterFunction("UI_SetMargin", "int", "int, int, int, int, int",
                       CLU_CDECL, CLU_CAST(UI_SetMargin));
  CLU_RegisterFunction("UI_LoadListFixSelect", "int",
                       "int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadListFixSelect));
  CLU_RegisterFunction("UI_FixListSetMaxNum", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_FixListSetMaxNum));
  CLU_RegisterFunction("UI_FixListSetText", "int", "int, int, char*", CLU_CDECL,
                       CLU_CAST(UI_FixListSetText));
  CLU_RegisterFunction("UI_FixListSetRowSpace", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_FixListSetRowSpace));
  CLU_RegisterFunction("UI_SetListIsMouseFollow", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetListIsMouseFollow));

  CLU_RegisterFunction("UI_CheckFixListSetCheckMargin", "int", "int, int, int",
                       CLU_CDECL, CLU_CAST(UI_CheckFixListSetCheckMargin));
  CLU_RegisterFunction(
      "UI_LoadCheckFixListCheck", "int",
      "int, char*, int, int, int, int, char*, int, int, int, int", CLU_CDECL,
      CLU_CAST(UI_LoadCheckFixListCheck));

  CLU_RegisterFunction("UI_SetEditCursorColor", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetEditCursorColor));
  CLU_RegisterFunction("UI_SetEditMaxNum", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetEditMaxNum));
  CLU_RegisterFunction("UI_SetEditEnterButton", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetEditEnterButton));
  CLU_RegisterFunction("UI_SetEditMaxNumVisible", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetEditMaxNumVisible));
  CLU_RegisterFunction("UI_SetProgressHintStyle", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetProgressHintStyle));

  CLU_RegisterFunction("UI_SetButtonModalResult", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetButtonModalResult));
  CLU_RegisterFunction("UI_SetTextColor", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetTextColor));
  CLU_RegisterFunction("UI_LoadFrameImage", "int",
                       "int, char*, int, int, int, int, char*, int, int",
                       CLU_CDECL, CLU_CAST(UI_LoadFrameImage));
  CLU_RegisterFunction("UI_CreatePageItem", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_CreatePageItem));
  CLU_RegisterFunction("UI_GetPageItemObj", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_GetPageItemObj));

  CLU_RegisterFunction("UI_AddCompent", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_AddCompent));
  CLU_RegisterFunction("UI_SetLabelExShadowColor", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetLabelExShadowColor));
  CLU_RegisterFunction("UI_SetPageButton", "int", "int, int, int, int",
                       CLU_CDECL, CLU_CAST(UI_SetPageButton));
  CLU_RegisterFunction("UI_ButtonSetHint", "int", "int, char*", CLU_CDECL,
                       CLU_CAST(UI_ButtonSetHint));

  CLU_RegisterFunction("UI_TreeLoadImage", "int",
                       "int, int, char*, int, int, int, int, int, int",
                       CLU_CDECL, CLU_CAST(UI_TreeLoadImage));
  CLU_RegisterFunction("UI_CreateTextItem", "int", "char*, int", CLU_CDECL,
                       CLU_CAST(UI_CreateTextItem));
  CLU_RegisterFunction("UI_CreateNoteGraphItem", "int",
                       "char*, int, int, int, int, int, char*, int, int",
                       CLU_CDECL, CLU_CAST(UI_CreateNoteGraphItem));
  CLU_RegisterFunction("UI_CreateGraphItem", "int",
                       "char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_CreateGraphItem));
  CLU_RegisterFunction("UI_CreateSingleNode", "int", "int, int, int", CLU_CDECL,
                       CLU_CAST(UI_CreateSingleNode));
  CLU_RegisterFunction("UI_CreateGridNode", "int", "int,int,int,int,int,int",
                       CLU_CDECL, CLU_CAST(UI_CreateGridNode));
  CLU_RegisterFunction("UI_GridNodeAddItem", "int", "int,int", CLU_CDECL,
                       CLU_CAST(UI_GridNodeAddItem));
  CLU_RegisterFunction("UI_CreateGraphItemTex", "int",
                       "int,int,int,int,float,float,int,int", CLU_CDECL,
                       CLU_CAST(UI_CreateGraphItemTex));
  CLU_RegisterFunction("UI_SetLabelExFont", "int", "int,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_SetLabelExFont));
  CLU_RegisterFunction("UI_SetGridSpace", "int", "int,int,int", CLU_CDECL,
                       CLU_CAST(UI_SetGridSpace));
  CLU_RegisterFunction("UI_SetGridContent", "int", "int,int,int", CLU_CDECL,
                       CLU_CAST(UI_SetGridContent));

  CLU_RegisterFunction("UI_ItemBarLoadImage", "int", "char*,int,int,int,int",
                       CLU_CDECL, CLU_CAST(UI_ItemBarLoadImage));
  CLU_RegisterFunction("UI_AddListBarText", "int", "int,char*,float", CLU_CDECL,
                       CLU_CAST(UI_AddListBarText));

  // headsay
  CLU_RegisterFunction("UI_LoadHeadSayFaceImage", "int",
                       "int,int,int,int,char*,int,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_LoadHeadSayFaceImage));
  CLU_RegisterFunction("UI_LoadHeadSayShopImage", "int",
                       "int,int,int,char*,int,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_LoadHeadSayShopImage));
  CLU_RegisterFunction("UI_LoadHeadSayLifeImage", "int",
                       "int,int,char*,int,int,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_LoadHeadSayLifeImage));

  // from style
  CLU_RegisterFunction("UI_SetFormStyle", "int", "int,int", CLU_CDECL,
                       CLU_CAST(UI_SetFormStyle));
  CLU_RegisterFunction("UI_SetFormStyleEx", "int", "int,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_SetFormStyleEx));

  // font
  CLU_RegisterFunction("UI_CreateFont", "int", "char*,int,int,int", CLU_CDECL,
                       CLU_CAST(UI_CreateFont));

  // add graph to text
  CLU_RegisterFunction("UI_SetTextParse", "int",
                       "int,  char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_SetTextParse));

  // memo
  CLU_RegisterFunction("UI_SetMemoMaxNumPerRow", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetMemoMaxNumPerRow));
  CLU_RegisterFunction("UI_SetMemoPageShowNum", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetMemoPageShowNum));
  CLU_RegisterFunction("UI_SetMemoRowHeight", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_SetMemoRowHeight));

  // Rich
  CLU_RegisterFunction("UI_RichSetClipRect", "int", "int, int, int, int, int",
                       CLU_CDECL, CLU_CAST(UI_RichSetClipRect));
  CLU_RegisterFunction("UI_RichSetMaxLine", "int", "int, int", CLU_CDECL,
                       CLU_CAST(UI_RichSetMaxLine));

  CLU_RegisterFunction("UI_SetProgressActiveMouse", "int", "int, int",
                       CLU_CDECL, CLU_CAST(UI_SetProgressActiveMouse));
  CLU_RegisterFunction("UI_LoadSkillListButtonImage", "int",
                       "int, char*, int, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadSkillListButtonImage));

  // 菜单
  CLU_RegisterFunction("UI_MenuLoadSelect", "int",
                       "int, char*, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_MenuLoadSelect));
  CLU_RegisterFunction(
      "UI_MenuLoadImage", "int",
      "int, int, int, char*, int, int, int, int, char*, int, int", CLU_CDECL,
      CLU_CAST(UI_MenuLoadImage));
  CLU_RegisterFunction("UI_MenuAddText", "int", "int, char*", CLU_CDECL,
                       CLU_CAST(UI_MenuAddText));
  CLU_RegisterFunction("UI_AddFilterTextToNameTable", "int", "char*", CLU_CDECL,
                       CLU_CAST(UI_AddFilterTextToNameTable));
  CLU_RegisterFunction("UI_AddFilterTextToDialogTable", "int", "char*",
                       CLU_CDECL, CLU_CAST(UI_AddFilterTextToDialogTable));

  CLU_RegisterFunction("UI_SetHeadSayBkgColor", "int", "int", CLU_CDECL,
                       CLU_CAST(UI_SetHeadSayBkgColor));

  CLU_RegisterFunction("UI_SetTitleFont", "int", "int, int, int, int",
                       CLU_CDECL, CLU_CAST(UI_SetTitleFont));

  CLU_RegisterFunction("UI_LoadSkillActiveImage", "int",
                       "char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadSkillActiveImage));
  CLU_RegisterFunction("UI_LoadChargeImage", "int",
                       "int, char*, int, int, int, int, int", CLU_CDECL,
                       CLU_CAST(UI_LoadChargeImage));

  // Add by lark.li
  char ret[] = "char*";
  CLU_RegisterFunction("UI_ResString", ret, "char*", CLU_CDECL,
                       CLU_CAST(UI_ResString));
  CLU_RegisterFunction("UI_SetCaptionEx", "int", "int, char*, int", CLU_CDECL,
                       CLU_CAST(UI_SetCaptionEx));
  CLU_RegisterFunction("AddMisDataFromC", "int",
                       "int, char*, int, int, int, char*, int, int, char*",
                       CLU_CDECL, CLU_CAST(AddMisDataFromC));
}
