...
CCharacter *CGameScene::AddCharacter(int nScriptID) {
  ...
  if (!pInfo) {
    LG("error",
       "msgCGameScene::AddCharacter() - GetChaRecordInfo(%d) return NULL\n",
       nScriptID);
    pCha = NULL;
    goto __ret;
  }
  ...
  {
    // added by clp 设置缩放
    D3DXVECTOR3 scale(pInfo->scaling[0], pInfo->scaling[1], pInfo->scaling[2]);
    ...
    if (pInfo->chModalType == enumMODAL_MAIN_CHA) {
      ...
      DWORD part_buf[6] = {
          ...
          pInfo->sSkinInfo[3], pInfo->sSkinInfo[4], pInfo->sSkinInfo[5],
      };
#else
      DWORD part_buf[5] = {
          pInfo->sSkinInfo[0], pInfo->sSkinInfo[1], pInfo->sSkinInfo[2],
      ...
      // load_info.pixelShader.data(), load_info.bone); 			for(DWORD i=0;
      // i<LW_MAX_SUBSKIN_NUM; i++)
      // 			{
...
CSceneObj *CGameScene::AddSceneObj(int nScriptID) {
  ...
  if (pInfo->bIsReallyBig) {
    ...
    if (tempChar) {
      float height =
          GetTerrainHeight(tempChar->getPos().x, tempChar->getPos().y);
      if (pObj)
...
CSceneItem *CGameScene::AddSceneItem(int nScriptID, int nType) {
  ...
  if (pObj) {
    // HandleSceneMsg(SCENEMSG_SCENEOBJ_DESTROY,
