...
CSkillCommand::CSkillCommand(CSkillRecord *pSkill)
  ...
  if (!_pSkill)
    LG("error",
       "msgCSkillCommand::CSkillCommand(CSkillRecord * pSkill) pSkill is NULL");
...
bool CSkillCommand::IsAtOnce() {
  return _pSkill->GetIsActive() || _pSkill->GetDistance() <= 0 ||
         enumSKILL_TYPE_SELF == _pSkill->chApplyTarget;
}
...
void CSkillCommand::AddHint(int x, int y, int index) {
  ...
  if (pMain) {
    ...
    case enumSKILL_LAND_LIVE:
      PushHint(RES_STRING(CL_LANGUAGE_MATCH_751),
               pMain->IsBoat() ? COLOR_RED : COLOR_WHITE);
      break;

    case enumSKILL_FIGHT:
      if (_pSkill->chType == 1) {
        if (_pSkill->chSrcType == 1) {
          PushHint(RES_STRING(CL_LANGUAGE_MATCH_752),
                   pMain->IsBoat() ? COLOR_RED : COLOR_WHITE);
        } else {
          PushHint(RES_STRING(CL_LANGUAGE_MATCH_753),
                   !pMain->IsBoat() ? COLOR_RED : COLOR_WHITE);
        }
      } else {
    ...
    case enumSKILL_SAIL:
      // PushHint( RES_STRING(CL_LANGUAGE_MATCH_755), pMain->IsBoat() ?
      // COLOR_WHITE : COLOR_RED );
      PushHint(RES_STRING(CL_LANGUAGE_MATCH_755), COLOR_WHITE);
      break;

    case enumSKILL_SEE_LIVE:
      PushHint(RES_STRING(CL_LANGUAGE_MATCH_756),
               pMain->IsBoat() ? COLOR_WHITE : COLOR_RED);
      break;
    }
  }

  PushHint(RES_STRING(CL_LANGUAGE_MATCH_757));
  StringNewLine(szBuf, sizeof(szBuf), HINT_WIDTH, _pSkill->szDescribeHint,
                (unsigned int)strlen(_pSkill->szDescribeHint));
  PushHint(szBuf);

  PushHint(RES_STRING(CL_LANGUAGE_MATCH_758));
  StringNewLine(szBuf, sizeof(szBuf), HINT_WIDTH, _pSkill->szEffectHint,
                (unsigned int)strlen(_pSkill->szEffectHint));
  PushHint(szBuf);

  PushHint(RES_STRING(CL_LANGUAGE_MATCH_759));
  StringNewLine(szBuf, sizeof(szBuf), HINT_WIDTH, _pSkill->szExpendHint,
                (unsigned int)strlen(_pSkill->szExpendHint));
  PushHint(szBuf);
...
bool CSkillCommand::_WriteNeed(int nType, int nValue, const char *szStr) {
  ...
  if (nType == enumSKILL_ITEM_NEED_TYPE) {
    ...
    if (pInfo) {
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s %s", szStr,
                  pInfo->szDataName);
...
const char *CSkillCommand::GetName() { return _pSkill->szName; }

const char *CSkillCommand::GetSkillName() {
  if (_pSkill->chType == 1) {
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s\nLV:%d, SP:%d",
                _pSkill->szName, _pSkill->GetLevel(), _pSkill->GetSPExpend());
  } else {
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s\nLV:%d", _pSkill->szName,
                _pSkill->GetLevel());
  }
