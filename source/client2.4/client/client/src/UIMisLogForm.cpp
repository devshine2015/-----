...
bool CMisLogForm::Init() {
  ...
  if (!m_pMisTree) {
    return Error(RES_STRING(CL_LANGUAGE_MATCH_473), m_pForm->GetName(),
                 "trvMission");
  }
...
void CMisLogForm::_MouseEvent(CCompent *pSender, int nMsgType, int x, int y,
                              DWORD dwKey) {
  ...
  if (_stricmp("frmMission", pSender->GetForm()->GetName()) == 0) {
    ...
    if (strName == "btnNo" || strName == "btnClose") {
      ...
      char szData[128];
      BYTE byType = 0;
      _snprintf_s(szData, _countof(szData), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_722), g_stUIMisLog.m_wMisID);
      ...
      //	{
      //		nMoney += pMain->getGameAttr()->get(ATTR_LV) *
      //pMain->getGameAttr()->get(ATTR_LV);
      //	}
      ...
      //{
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_723), szData);
      //}

      stSelectBox *pBox =
          CBoxMgr::ShowSelectBox(_evtBreakYesNoEvent, szBuf, true);
      pBox->dwTag = pNode->GetTag();
    }
  }
}

void CMisLogForm::_evtBreakYesNoEvent(CCompent *pSender, int nMsgType, int x,
                                      int y, DWORD dwKey) {
...
void CMisLogForm::_MouseDown(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (pNode) {
    CTextButton *pButton =
        dynamic_cast<CTextButton *>(g_stUIMisLog.m_pForm->Find("btnBreak"));
    ...
    if (dwTick - g_stUIMisLog.m_dwUpdateTick > MISLOG_REFRESH_TIME ||
        dwData != g_stUIMisLog.m_wMisID) {
...
BOOL CMisLogForm::AddNode(WORD wMisID, BYTE byState, BYTE &byType) {
  ...
  byType = 0;
  _snprintf_s(szData, _countof(szData), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_722), wMisID);
...
void CMisLogForm::MisLogList(const NET_MISLOG_LIST &List) {
  ...
  for (BYTE i = 0; i < m_LogList.byNumLog; i++) {
    AddNode(m_LogList.MisLog[i].wMisID, m_LogList.MisLog[i].byState,
            m_LogList.MisLog[i].byType);
  }
...
void CMisLogForm::GetMisData(WORD wMisID, BYTE &byType, char szBuf[],
                             USHORT sBufLen) {
  ...
  RetType *pRet = (RetType *)CLU_CallScriptFunction(
      "GetMisData", "uchar,uchar,char*", "ushort", wMisID);
  if (pRet == NULL) {
    return;
  }
...
void CMisLogForm::MisClear(WORD wMisID) {
  ...
  memset(m_LogList.MisLog + nIndex, 0,
         sizeof(NET_MISLOG) * (m_LogList.byNumLog - nIndex));
  memcpy(m_LogList.MisLog + nIndex, Log + nIndex + 1,
         sizeof(NET_MISLOG) * (m_LogList.byNumLog - nIndex - 1));
  m_LogList.byNumLog--;
...
void CMisLogForm::_ItemClickEvent(string strItem) {
  ...
  if (bmap && bx && by && index == strItem.length()) {
    ...
    if (strcmp((const char *)map,
               (const char *)RES_STRING(CL_LANGUAGE_MATCH_56)) == 0) {
      targetmap = new char[9];
      targetmap = RES_STRING(CL_LANGUAGE_MATCH_56);
    } else if (strcmp((const char *)map,
                      (const char *)RES_STRING(CL_LANGUAGE_MATCH_57)) == 0) {
      targetmap = new char[9];
      targetmap = RES_STRING(CL_LANGUAGE_MATCH_57);
    } else if (strcmp((const char *)map,
                      (const char *)RES_STRING(CL_LANGUAGE_MATCH_58)) == 0) {
      targetmap = new char[9];
      targetmap = RES_STRING(CL_LANGUAGE_MATCH_58);
    } else if (strcmp((const char *)map, (const char *)RES_STRING(
                                             CL_UIMISLOGFORM_CPP_00001)) == 0) {
      targetmap = new char[9];
