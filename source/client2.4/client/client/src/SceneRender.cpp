...
DR_RESULT __SceneTranspObjRenderProc(LEIPrimitive *obj, void *param) {
  ...
  if (g_Config.m_bEditor) {
    if (_vMousePos.x > 0) {
      if (g_Editor.IsEnable())
        g_Render.AddLine(
            _vMousePos,
            D3DXVECTOR3(_vMousePos.x, _vMousePos.y, _vMousePos.z + 4.0f),
            0xFFFF0000);
    }
    ...
    for (int i = 0; i < GetChaCnt(); i++) {
      ...
      if (pCha->IsValid()) {
        if (pCha->getPatrolX() > 0) {
          D3DXVECTOR3 vec1 =
              D3DXVECTOR3((float)(pCha->getPatrolX()) / 100.0f,
                          (float)(pCha->getPatrolY()) / 100.0f, 2.0f);
          D3DXVECTOR3 vec2 =
              D3DXVECTOR3((float)(pCha->GetCurX()) / 100.0f,
                          (float)(pCha->GetCurY()) / 100.0f, 2.0f);
          g_Render.AddLine(vec1, vec2, 0xFFFFFFFF);
          g_Render.AddLine(vec1, vec2 + D3DXVECTOR3(0.1f, 0.2f, 0.0f),
                           0xFFFF0000);
        }
      }
    }
...
// LEIDeviceObject* dev_obj = g_Render.GetInterfaceMgr()->dev_obj;
  ...
  SetupVertexFog(
      ...
      D3DFOG_EXP2, 1, g_Config.m_fExp2);
#endif
  ...
  amb =
      D3DCOLOR_XRGB((BYTE)(m_dwEnvColor.r * 255), (BYTE)(m_dwEnvColor.g * 255),
                    (BYTE)(m_dwEnvColor.b * 255));
  ...
  if (_bShowSceneObj) {
    ...
    LEVector3 org(0.0f, 0.0f, 0.0f);
    LEVector3 ray(0.0f, 0.0f, 0.0f);
    ...
    // drIViewFrustum* vf =
    // g_Render.GetInterfaceMgr()->sys_graphics->GetSceneMgr()->GetViewFrustum();
...
//		for( sceneobjs::iterator it=_vecHideObject.begin();
//it!=_vecHideObject.end(); ++it )
//		{}
...
// 			D3DXVECTOR3	EyePos =
// g_pGameApp->GetMainCam()->m_EyePos; 			D3DXVECTOR3 ObjPos = htObj->getPos(); 			int
// e2cDis = GetDistance( __int64( EyePos.x ), __int64( EyePos.y ), __int64(
// ChaPos.x ), __int64( ChaPos.y ) ); 			int	e2oDis = GetDistance( __int64(
// EyePos.x ), __int64( EyePos.y ), __int64( ObjPos.x ), __int64( ObjPos.y ) );
//
// 			if ( e2cDis > e2oDis )
// 			{
// 				htObj->SetHide( TRUE );
...
// 		}
    ...
    for (; it != it_end; ++it) {
      ...
      if (pObj->IsValid() && pObj->IsHide() == FALSE) {
        ...
        if (pObj->GetObject()->GetCullingPrimitiveNum() ==
        ...
        BOOL light_enable =
            info->bEnableEnvLight | info->bEnablePointLight | info->bShadeFlag;
        ...
        if (_pTerrain) {
          int a =
              _pTerrain
                  ->GetTile((int)(pObj->getPos().x), (int)(pObj->getPos().y))
                  ->getIsland();
          if (a != nArea) {
            nArea = a;
...
#if (defined OPEN_AMBIENT_ENV)
        if (info->bShadeFlag) {
          LEDwordByte4 db4_obj;
          LEDwordByte4 db4_env;
          LEDwordByte4 db4_mix;
  ...
  g_Render.GetInterfaceMgr()->sys_graphics->GetSceneMgr()->GetCullPrimitiveInfo(
      &cull_info);
...

  {
    ...
    if (g_IsShowShop) {
        ...
        if (cha_lgt) {
          ...
          if (pArea) {
            ...
            if (x) {
              ...
              env_light.Diffuse.r =
                  (1 - fLgt) * env_light.Diffuse.r + fLgt * c.r;
              env_light.Diffuse.g =
                  (1 - fLgt) * env_light.Diffuse.g + fLgt * c.g;
              env_light.Diffuse.b =
                  (1 - fLgt) * env_light.Diffuse.b + fLgt * c.b;
            }
            g_Render.SetLight(0, &env_light);
          }
        }
      ...
      for (i = 0; i < _nChaCnt; i++) {
        ...
        if (pCha->IsPlayer()) {
          // 如果是玩家,仅显示队友或主角
          if (!((nMainTeamID > 0 && nMainTeamID == pCha->GetTeamLeaderID()) ||
                (pCha == pMain))) {
            continue;
          }
        }
    ...
    //    D3DXVec3Normalize((D3DXVECTOR3*)&lgt.Direction,
    //    (D3DXVECTOR3*)&lgt.Direction);
  ...
  if (_bShowSceneObj) {
    ...
    for (; it != it_end; ++it) {
      ...
      if (pObj->IsValid() && pObj->IsHide() == FALSE) {
        ...
        // set region lighting color
        int a =
            _pTerrain->GetTile((int)(pObj->getPos().x), (int)(pObj->getPos().y))
                ->getIsland();
        if (a != nArea) {
          nArea = a;
