...
static void _Disconnect(CCompent *pSender, int nMsgType, int x, int y,
                        DWORD dwKey) {
...
static void _SwitchMapFailed(CCompent *pSender, int nMsgType, int x, int y,
                             DWORD dwKey) {
  CLoginScene *pLogin = dynamic_cast<CLoginScene *>(CGameApp::GetCurScene());
  if (!pLogin) {
    g_pGameApp->LoadScriptScene(enumLoginScene);
...
void CGameApp::_HandleMsg(DWORD dwTypeID, DWORD dwParam1, DWORD dwParam2) {
  ...
  case APP_NET_DISCONNECT: {
    ...
    if (g_TomServer.bEnable) {
      MessageBox(g_pGameApp->GetHWND(), RES_STRING(CL_LANGUAGE_MATCH_134),
                 "error", 0);
    ...
    if (dwParam1 == 1000) {
      ...
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_135), WSAGetLastError());
    ...
    // else if(dwParam1!=DS_DISCONN && dwParam1!=DS_SHUTDOWN )
    {
      if (!dynamic_cast<CLoginScene *>(scene)) {
        if (!g_ChaExitOnTime.TimeArrived()) {
          ...
          if (g_NetIF) {
            //_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE ,
            //"您现在已与服务器断开连接,确定后将返回登录界面\n原因:%s[%d]",
            //"您已在其他地方登陆！", dwParam1 );
            _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                        RES_STRING(CL_LANGUAGE_MATCH_138));
          } else {
            _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                        RES_STRING(CL_LANGUAGE_MATCH_139), dwParam1);
          }
          g_stUIBox.ShowMsgBox(_Disconnect, szBuf);
  ...
  case APP_SWITCH_MAP_FAILED: {
    ...
    char szBuf[256] = {0};
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s[%d]",
                g_GetServerError(dwParam1), dwParam1);
