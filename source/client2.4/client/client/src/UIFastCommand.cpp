...
void CFastCommand::DragRender() {
  if (_pCommand)
    _pCommand->Render(GetX() + CDrag::GetDrag()->GetDragX(),
                      GetY() + CDrag::GetDrag()->GetDragY());
}
...
COneCommand::COneCommand(CForm &frmOwn)
    : CCommandCompent(frmOwn), _pCommand(NULL), evtBeforeAccept(NULL),
      evtThrowItem(NULL), evtUseCommand(NULL), _pActive(NULL),
      _IsShowActive(false), _eShowStyle(enumSmall) {
  _SetSelf();
}
