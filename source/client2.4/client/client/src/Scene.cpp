...
int CGameScene::HitTestSceneObjTerrainForInfluence(D3DXVECTOR3 *t_pos,
  ...
  if (s &&
      h.z < fHeight - 0.3f) // 因为美术在拼物件时，通常会将物件埋入地下一段距离
  {
    // h.z = 0;
    s = 0;
  }
  ...
  if (g_pGameApp->IsKeyDown(DIK_V)) {
    ...
    TipI(this->IsSceneObjVisible(), RES_STRING(CL_LANGUAGE_MATCH_336),
         RES_STRING(CL_LANGUAGE_MATCH_337));
                 ...
                 DIK_F9)) //所有当前active的section做高度和阻挡计算并存盘
  {
    /*
// 修改此处一定要非常小心，
    //
请参考LEMap::DynamicLoading()，如果设置不正确，会导致需要的section没有读进内存；
    const int	ciSectionWidth = 8;			// 单位：tile（米）
    const int	ciSectionHeight = 8;		// 单位：tile
    const int	ciRefreshWidth = 3;			//
需要刷新的section宽，单位：section const int	ciRefreshHeight = 3;
// 需要刷新的section高，单位：section
