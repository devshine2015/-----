...
static PATH_LINK *SearchPath(BYTE *block_buf, short width, short height,
                             short sx, short sy, short tx, short ty,
                             long STEP_LIMIT, int diff_allow) {
#ifdef __FPS_DEBUG__
  IP("SearchPath(%08x, %dx%d, %d,%d -> %d,%d, limit=%d)\n", block_buf, width,
     height, sx, sy, tx, ty, STEP_LIMIT);
#endif
  if (block_buf == NULL)
    return NULL;
...
CFindPath::CFindPath(long step, int range) {
  ...
  memset(
      ...
      MAX_PATH_STEP * MAX_PATH_STEP *
          sizeof(
              BYTE)); //指针用sizeof得出4，整块内存大小 -Waiting Fix 2009-03-11
  _bblock = true;
  m_bFindPath = false;
  m_bLongPathFinding = false;
}

CFindPath::~CFindPath() { SAFE_DELETE_ARRAY(_byTempBlock); }
...
void CFindPath::SetShortPathFinding(long step, int range) {
  ...
  if (step == MAX_PATH_STEP && range == m_iRange) {
    ...
    _vecDirSave.clear();
    _vecPathPoint.clear();
    memset(_byTempBlock, 1,
           MAX_PATH_STEP * MAX_PATH_STEP *
               sizeof(BYTE)); //指针用sizeof得出4，整块内存大小 -Waiting Fix
                              //2009-03-11
    m_bFindPath = false;
    m_bLongPathFinding = false;
    return;
...
void CFindPath::SetTargetPos(int nCurX, int nCurY, int &nTargetX, int &nTargetY,
                             bool bback) {
  D3DXVECTOR2 vDir;
...
BYTE *CFindPath::GetTempTerrain(CGameScene *pScene, CCharacter *pCha, int iCurX,
                                int iCurY) {
  memset(
      _byTempBlock, 1,
      MAX_PATH_STEP * MAX_PATH_STEP *
        ...
        *pValue = pTer->GetBlock()->IsGridBlock(tm, tn); // jze
        ...
        if (*pValue == 0) {
          if (!g_IsMoveAble(pCha->getChaCtrlType(), Territory,
                            (EAreaMask)pTer->GetBlock()->GetTileRegionAttr(
                                tm * 50 / 100, tn * 50 / 100)))
            *pValue = 1;

          if (m_bLongPathFinding) {
            short sRegion = pTer->GetBlock()->GetTileRegionAttr(tm * 50 / 100,
                                                                tn * 50 / 100);
  ...
  //	{
  //		_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%d",_byTempBlock[i *
  //_nWidth + j]); 		fwrite(&buf,sizeof(char),1, pFile);
  //	}
...
bool PointPointRange(int px1, int py1, int px2, int py2, int range) {
  return ((abs(px1 - px2) < range) && (abs(py1 - py2) < range));
}
// nCurX,nCurY,nTargetX,nTargetY 的单位都是厘米
BOOL CFindPath::FindPath(CGameScene *pScene, CCharacter *pCha, int nSelfX,
                         int nSelfY, int nTargetX, int nTargetY,
                         bool &IsWalkLine, int diff_allow) {
  ...
  if (nSelfX < 0 || nSelfY < 0 || nTargetX < 0 || nTargetY < 0) {
    LG("path_find",
       "msginput coordinate less than 0, Self[%u, %u], Target[%u, %u], "
       "ChaPos[%u, %u]\n",
  ...
  if (!PointPointRange(nx, ny, showx, showy, m_iRange)) {
    LG("path_find",
       "input coordinate start is not in range, Self[%u, %u], Target[%u, %u], "
       "ChaPos[%u, %u]\n",
       nSelfX, nSelfY, nTargetX, nTargetY, pCha->GetCurX(), pCha->GetCurY());
    return FALSE;
  }
  if (!PointPointRange(ntx, nty, showx, showy, m_iRange)) {
    LG("path_find",
       "input coordinate target is not in range, Self[%u, %u], Target[%u, %u], "
       "ChaPos[%u, %u]\n",
       nSelfX, nSelfY, nTargetX, nTargetY, pCha->GetCurX(), pCha->GetCurY());
    return FALSE;
  }
  ...
  if (_nTargetY < 0 || _nTargetY >= MAX_PATH_STEP || _nTargetX < 0 ||
      _nTargetX >= MAX_PATH_STEP || _nWidth > MAX_PATH_STEP) {
#pragma message(                                                               \
    "[Waiting Note] FindPath error need fix") //当镜头速度跟地图跟不上时，就会出現这个情况，应该修改
    // IP("MAX_STEP(%d) Width(%d) Target(%d,%d)\n", MAX_PATH_STEP, _nWidth,
    // _nTargetX, _nTargetY);
  ...
  PATH_LINK *pLink =
      ::SearchPath(byBuf2, _nWidth, s1 * 2, (_nWidth / 2), (_nWidth / 2),
                   _nTargetX, _nTargetY, STEP_LIMIT, diff_allow);
...
void CFindPath::RenderPath(LEMap *pMap) {
  ...
  for (int m = 0; m < _vecPathPoint.size(); m++) {
    _vecPathPoint[m]->z =
        pMap->GetHeight(_vecPathPoint[m]->x, _vecPathPoint[m]->y);
...
bool CFindPath::Find(CGameScene *pScene, CCharacter *pCha, int nSelfX,
                     int nSelfY, int nTargetX, int nTargetY, bool &IsWalkLine,
                     int diff_allow) {
  UnLock();
  if (FindPath(pScene, pCha, nSelfX, nSelfY, nTargetX, nTargetY, IsWalkLine,
    ...
    for (int i = 1; i < count; i++)
      _nLength += GetDistance((__int64)(_vecPathPoint[i - 1]->x * 100.0f),
                              (__int64)(_vecPathPoint[i - 1]->y * 100.0f),
                              (__int64)(_vecPathPoint[i]->x * 100.0f),
                              (__int64)(_vecPathPoint[i]->y * 100.0f));
    ...
    if (count > 0) {
      ...
      if (fX >= (float)nSelfX - 0.05f && fX <= (float)nSelfX + 0.05f &&
          fY >= (float)nSelfY - 0.05f && fY <= (float)nSelfY + 0.05f) {
    ...
    if (count >= 2) {
      ...
      for (int i = 0; i < count; i++) {
        if (_vecPathPoint[i]->x == _vecPathPoint[i + 1]->x &&
            _vecPathPoint[i]->y == _vecPathPoint[i + 1]->y) {
...
void CFindPathEx::SetTarget(int CurX, int CurY, int TargetX, int TargetY) {
  ...
  g_bIsMouseWalk =
      false; //点击路径搜寻时，取消鼠标移动，避免互相干擾  by Waiting 2009-08-04
}

void CFindPathEx::SetDestDirection(int CurX, int CurY, int TargetX,
                                   int TargetY) {
  m_iDeltaFlagX =
      (TargetX - CurX) / (abs(TargetX - CurX) ? abs(TargetX - CurX) : 1);
  m_iDeltaFlagY =
      (TargetY - CurY) / (abs(TargetY - CurY) ? abs(TargetY - CurY) : 1);
}
...
void CFindPathEx::CalculateGuideStraight() {
  ...
  if (deltaX == 0) {
    if (deltaY > 0) {
      ...
      while (1) {
        if (!pScene->GetTerrain()->GetBlock()->IsGridBlock(TempX * 2,
                                                           TempY * 2)) {
          if (!g_IsMoveAble(
                  pScene->GetMainCha()->getChaCtrlType(),
                  pScene->GetMainCha()->GetDefaultChaInfo()->chTerritory,
                  (EAreaMask)pScene->GetTerrain()
                      ->GetBlock()
                      ->GetTileRegionAttr(TempX * 50 / 100,
                                          TempY * 50 / 100))) {
...
void CFindPathEx::Move() {
  ...
  COneMoveState *pMoveState = dynamic_cast<COneMoveState *>(
      pScene->GetMainCha()->GetActor()->GetCurState());
  ...
  if (!m_bReach) {
    ...
    if (m_bReachGuide) {
      ...
      pMouse->ActMove(pScene->GetMainCha(), TargetX * 100, TargetY * 100,
                      g_pGameApp->IsAltPress() ? true : false);
  ...
  int dc = (m_iTargetX - m_iCurX) * (m_iTargetX - m_iCurX) +
           (m_iTargetY - m_iCurY) * (m_iTargetY - m_iCurY);
  int ds = (m_iTargetX - ServerX) * (m_iTargetX - ServerX) +
           (m_iTargetY - ServerY) * (m_iTargetY - ServerY);
  int diff = dc - ds;
