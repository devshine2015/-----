...
static DWORD BoyInvisibleSet[] = {1, 3};
static DWORD GirlInvisibleSet[] = {2, 4, 110, 111, 114, 115};

bool CCreateChaScene::IsGirl = false;
inline bool Error(const char *strInfo, const char *strFormName,
...
int LoginScene_CreateCha::_CreateArrowItem(const char *file) {
  DR_IF_RELEASE(_arrow);

  _res_mgr->CreateItem(&_arrow);
  if (DR_FAILED(_arrow->Load(file)))
    return 0;
...
bool LoginScene_CreateCha::LoadArrowMark(const char *Filename,
                                         const drVector3 *Offsets) {
  memcpy(_ArrowMarksOffset, Offsets, sizeof(_ArrowMarksOffset));

  for (int Index = 0; Index < 4; Index++) {
    DR_IF_RELEASE(_ArrowMarks[Index]);

    if ((CCreateChaScene::IsGirl && (Index == 0 || Index == 2)) ||
        (!CCreateChaScene::IsGirl && (Index == 1 || Index == 3)))
      continue;

    assert(_cha_obj[Index]);

    _res_mgr->CreateItem(&(_ArrowMarks[Index]));
    if (DR_FAILED(_ArrowMarks[Index]->Load(Filename)))
      return false;
    ...
    if (_cha_obj[Index]) {
      drMatrix44 matWorld = *(_cha_obj[Index]->GetWorldMatrix());
      drMatrix44 matTranslate = drMatrix44Translate(_ArrowMarksOffset[Index]);
      drMatrix44Multiply(&matWorld, &matTranslate, &matWorld);
...
int LoginScene_CreateCha::_InitChaObjSeq() {
  ...
  // init pose
  int x[5][5] = {
      {1, 0, 125, 500, 600}, {2, 0, 200, 500, 600}, {3, 0, 360, 500, 600},
      {4, 0, 500, 500, 600}, {0, 0, 360, 500, 600},
  };

  drModelNodeQueryInfo mnqi;
  ...
  for (DWORD i = 0; i < cha_num; i++) {
    ...
    if (DR_FAILED(_model_lxo->QueryTreeNode(&mnqi)))
      return 0;

    if (mnqi.node == NULL)
      return 0;

    _cha_obj[i] = (drINodePrimitive *)mnqi.node->GetData();
  }
  ...
  drIPoseCtrl *pose_ctrl;
  drIAnimCtrlObjBone *ctrl_obj;

  drPlayPoseInfo play_info;
  drPlayPoseInfo_Construct(&play_info);
  ...
  for (DWORD i = 0; i < _cha_num; i++) {
    drINodeBoneCtrl *bone_ctrl = (drINodeBoneCtrl *)_cha_obj[i]->GetParent();
    if (bone_ctrl == 0)
      return 0;
  ...
  int StartIndex = SetInvisibleArray(false, 0);
  SetInvisibleArray(true, StartIndex);

  return 1;
}

int LoginScene_CreateCha::SetInvisibleArray(bool bGirl, int StartIndex) {
  if (!StartIndex)
    memset(&ignoreStruct, 0x0, sizeof(IgnoreStruct));
  ...
  int SetSize =
      bGirl ? ARRAY_SIZE(BoyInvisibleSet) : ARRAY_SIZE(GirlInvisibleSet);
  DWORD *SetArray = bGirl ? BoyInvisibleSet : GirlInvisibleSet;
  for (int Index = 0; Index < SetSize; Index++) {
    mnqi.node = NULL;
    mnqi.id = SetArray[Index];

    if (DR_FAILED(_model_lxo->QueryTreeNode(&mnqi)))
      continue;

    if (StartIndex >= 0 && StartIndex < MAX_IGNORE_NODES)
      ignoreStruct.nodes[StartIndex] = mnqi.node;
...
void LoginScene_CreateCha::Destroy() {
  DR_SAFE_RELEASE(_model_lxo);
  DR_SAFE_RELEASE(_model_lmo);
}
...
void LoginScene_CreateCha::Update() {
  ...
  if (_ArrowMarks[3]) {
    drMatrix44 matWorld = *(_cha_obj[_act_obj[3]]->GetWorldMatrix());
    drMatrix44 matTranslate = drMatrix44Translate(_ArrowMarksOffset[3]);
    drMatrix44Multiply(&matWorld, &matTranslate, &matWorld);
...
int LoginScene_CreateCha::LoadModelLXO(const char *file) {
  ...
  drAssObjInfo assinfo;
  assinfo.size = drVector3(1.0f, 1.0f, 1.0f);
  _res_mgr->SetAssObjInfo(ASSOBJ_MASK_SIZE, &assinfo);

  _res_mgr->SetTexturePath(".\\texture\\scene\\");

  char path[260];
  _snprintf_s(path, _countof(path), _TRUNCATE, ".\\model\\scene\\%s", file);
  _model_lxo->Load(path, MODELOBJECT_LOAD_RESET, 0);

  drINodeObjectA::PlayDefaultPose(_model_lxo);
  // drINodeObjectA::ShowBoundingObject(_model_lxo, 1);

  // drINodeObjectA::DumpObjectTree(_model_lxo, "test.txt");

  drITreeNode *tn_bonectrl = 0;
  drITreeNode *tn_dummy = 0;
  drINodeBoneCtrl *node_bonectrl = 0;
  drINodeDummy *node_dummy = 0;
  ...
  if (DR_SUCCEEDED(_model_lxo->QueryTreeNode(&mnqi))) {
    tn_bonectrl = mnqi.node;
    node_bonectrl = (drINodeBoneCtrl *)mnqi.node->GetData();
  }
  ...
  if (DR_SUCCEEDED(_model_lxo->QueryTreeNode(&mnqi))) {
    tn_dummy = mnqi.node;
    node_dummy = (drINodeDummy *)mnqi.node->GetData();
  }
...

  {
    ...
    pci[0].pose_id = 41, pci[0].subset = -1, pci[0].stage = -1,
    pci[0].anim_type = 0, pci[0].play_type = 1, pci[0].velocity = 1.0f;
    pci[1].pose_id = 41, pci[1].subset = 0, pci[1].stage = -1,
    pci[1].anim_type = 4, pci[1].play_type = 1, pci[1].velocity = 1.0f;
    pci[2].pose_id = 54, pci[2].subset = -1, pci[2].stage = -1,
    pci[2].anim_type = 0, pci[2].play_type = 1, pci[2].velocity = 1.0f;
    pci[3].pose_id = 54, pci[3].subset = 0, pci[3].stage = -1,
    pci[3].anim_type = 4, pci[3].play_type = 1, pci[3].velocity = 1.0f;
    pci[4].pose_id = 27, pci[4].subset = 0, pci[4].stage = 0,
    pci[4].anim_type = 2, pci[4].play_type = 2, pci[4].velocity = 0.8f;
    pci[5].pose_id = 28, pci[5].subset = 0, pci[5].stage = 0,
    pci[5].anim_type = 2, pci[5].play_type = 2, pci[5].velocity = 0.5f;
    pci[6].pose_id = 106, pci[6].subset = -1, pci[6].stage = -1,
    pci[6].anim_type = 1, pci[6].play_type = 2, pci[6].velocity = 0.3f;
    pci[7].pose_id = 60, pci[7].subset = -1, pci[7].stage = -1,
    pci[7].anim_type = 1, pci[7].play_type = 2, pci[7].velocity = 0.8f;
    pci[8].pose_id = 61, pci[8].subset = -1, pci[8].stage = -1,
    pci[8].anim_type = 1, pci[8].play_type = 2, pci[8].velocity = 0.8f;
    pci[9].pose_id = 62, pci[9].subset = -1, pci[9].stage = -1,
    pci[9].anim_type = 1, pci[9].play_type = 2, pci[9].velocity = 0.8f;
    pci[10].pose_id = 63, pci[10].subset = -1, pci[10].stage = -1,
    pci[10].anim_type = 1, pci[10].play_type = 2, pci[10].velocity = 0.8f;
    pci[11].pose_id = 64, pci[11].subset = -1, pci[11].stage = -1,
    pci[11].anim_type = 1, pci[11].play_type = 2, pci[11].velocity = 0.8f;
    pci[12].pose_id = 110, pci[12].subset = -1, pci[12].stage = -1,
    pci[12].anim_type = 0, pci[12].play_type = 2, pci[12].velocity = 1.5f;
    pci[13].pose_id = 111, pci[13].subset = -1, pci[13].stage = -1,
    pci[13].anim_type = 0, pci[13].play_type = 2, pci[13].velocity = 1.5f;
    ...
    for (DWORD i = 0; i < pci_num; i++) {
      ...
      if (pci[i].anim_type == ANIM_CTRL_TYPE_BONE) {
        ...
        if (DR_SUCCEEDED(_model_lxo->QueryTreeNode(&mnqi))) {
          drINodeBoneCtrl *nb = (drINodeBoneCtrl *)mnqi.node->GetData();

          drIAnimCtrlObj *ctrl_obj = nb->GetAnimCtrlObj();
          if (ctrl_obj == 0)
            continue;
        ...
        if (DR_SUCCEEDED(_model_lxo->QueryTreeNode(&mnqi))) {
          ...
          np = (drINodePrimitive *)mnqi.node->GetData();

          drIAnimCtrlAgent *anim_agent = np->GetAnimCtrlAgent();
          if (anim_agent == 0)
            continue;
...
int LoginScene_CreateCha::LoadModelLMO(const char *file) {
...

  {
    ...
    if (pose_enable) {
      ...
      pci[0].pose_id = 0, pci[0].subset = -1, pci[0].stage = -1,
      pci[0].anim_type = 1, pci[0].play_type = 1, pci[0].velocity = 0.5;
      pci[1].pose_id = 1, pci[1].subset = -1, pci[1].stage = -1,
      pci[1].anim_type = 1, pci[1].play_type = 1, pci[1].velocity = 0.5;
      pci[2].pose_id = 2, pci[2].subset = -1, pci[2].stage = -1,
      pci[2].anim_type = 1, pci[2].play_type = 1, pci[2].velocity = 0.5;
      pci[3].pose_id = 3, pci[3].subset = -1, pci[3].stage = -1,
      pci[3].anim_type = 1, pci[3].play_type = 1, pci[3].velocity = 0.5;
      pci[4].pose_id = 4, pci[4].subset = -1, pci[4].stage = -1,
      pci[4].anim_type = 1, pci[4].play_type = 1, pci[4].velocity = 0.5;
      pci[5].pose_id = 26, pci[5].subset = -1, pci[5].stage = -1,
      pci[5].anim_type = 1, pci[5].play_type = 1, pci[5].velocity = 0.5;
...
int LoginScene_CreateCha::LoadArrowItem(const char *file,
                                        const drVector3 *offset_pos4) {
...
void LoginScene_CreateCha::OnMouseMove(int flag, int x, int y) {
  ...
  drPlayPoseInfo_Construct(&play_info);
  play_info.bit_mask =
      PPI_MASK_POSE | PPI_MASK_TYPE | PPI_MASK_VELOCITY | PPI_MASK_FRAME;
  ...
  for (DWORD i = 0; i < _act_num; i++) {
    ...
    if (DR_SUCCEEDED(pri->HitTest(&info, &org, &ray))) {
      play_info.pose = 2;
      ...
      /*if(_arrow)
      {
          ...
          m = drMatrix44Translate(_arrow_offset_pos[i]);
    ...
    drINodeBoneCtrl *bone_ctrl = (drINodeBoneCtrl *)pri->GetParent();
    bone_ctrl->GetAnimCtrlObj()->PlayPose(&play_info);

    if (i == 2) // octopus
    {
      /*drINodeBoneCtrl* */ bone_ctrl =
          (drINodeBoneCtrl *)_cha_obj[4]->GetParent();
      bone_ctrl->GetAnimCtrlObj()->PlayPose(&play_info);
    }
  }

  { // dummy 99 obj
    drINodeDummy *node_dummy =
        (drINodeDummy *)_cha_obj[_act_obj[3]]->GetParent()->GetParent();
    drIAnimCtrlObjMat *ctrl_mat =
        (drIAnimCtrlObjMat *)node_dummy->GetAnimCtrlObj();
...
int LoginScene_CreateCha::OnLButtonDown(int flag, int x, int y) {
  ...
  CCreateChaScene *pCreateChaScene =
  ...
  for (DWORD i = 0; i < _act_num; i++) {
    ...
    if (DR_SUCCEEDED(pri->HitTest(&info, &org, &ray))) {
      if (CCreateChaScene::IsGirl && (i == 1 || i == 3))
        return i;
      else if (!CCreateChaScene::IsGirl && (i == 0 || i == 2))
        return i;
...
const char *CCreateChaScene::GetDescripts(int index) {
  switch (index) {
  case 0:
...
const char *CCreateChaScene::GetCityNames(int index) {
  switch (index) {
  case 0:
...
const char *CCreateChaScene::GetCitiesDescripts(int index) {
  switch (index) {
  case 0:
...
void CCreateChaScene::_FrameMove(DWORD dwTimeParam) {
  ...
  if (frmChaCity->GetIsShow() && !bShowDialog &&
      CFormMgr::s_Mgr.ModalFormNum() == 1) {
    if (m_bFirstShow && iMouseX != m_iFirstMourseX &&
        iMouseY != m_iFirstMourseY) {
      m_bFirstShow = false;
    }
...
bool CCreateChaScene::_MouseButtonDown(int nButton) {
  ...
  if (frmLanchInfo && frmAimiInfo && frmFelierInfo && frmCaxiusInfo &&
      frmRoleAllInfo) {
    DarkScene();
...
bool CCreateChaScene::_InitUI() {
  // 角色创建界面
  {
    ...
    if (!btnLeftHair) {
      Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmChaFound->GetName(),
            "btnLeftHair");
      return false;
    }
    btnLeftHair->evtMouseClick = __gui_event_left_hair;
    CTextButton *btnRightHair =
        (CTextButton *)frmChaFound->Find("btnRightHair");
    if (!btnRightHair) {
      Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmChaFound->GetName(),
            "btnRightHair");
      return false;
    ...
    CTextButton *btnRightFace =
        (CTextButton *)frmChaFound->Find("btnRightFace");
    if (!btnRightFace) {
      Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmChaFound->GetName(),
            "btnRightFace");
      return false;
    ...
    if (!memChaDescribe) {
      return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmChaFound->GetName(),
                   "memChaDescribe");
    }
    C3DCompent *ui3dCreateCha =
        (C3DCompent *)frmChaFound->Find("ui3dCreateCha");
    if (ui3dCreateCha) {
  ...
  // 角色城市选择界面
  {
    ...
    for (int i(0); i < MAX_CITY_NUM + 1; ++i) {
      for (int j(0); j < CITY_PICTURE_NUM; ++j) {
        _snprintf_s(szPicName, _countof(szPicName), _TRUNCATE, szPicNameBase, i,
                    j);
        imgCities[i][j] = (CImage *)frmChaCity->Find(szPicName);
        if (!imgCities[i][j]) {
          return Error(RES_STRING(CL_LANGUAGE_MATCH_46), frmChaCity->GetName(),
                       szPicName);
        }
        if (i == 0)
    ...
    for (int i(0); i < MAX_CITY_NUM; ++i) {
      _snprintf_s(szPicName, _countof(szPicName), _TRUNCATE, szCityBlockName,
                  i + 1);
  ...
  // 退出菜单界面
  {
    ...
    frmQuit->SetPos((g_pGameApp->GetWindowWidth() - frmQuit->GetWidth()) / 2,
  ...
  // 新手流程界面
  {
    ...
    // 新手帮助 MEMO
    memChaDescribeUp =
        dynamic_cast<CMemo *>(frmRoleAllInfo->Find("memChaDescribeUp"));
    if (!memChaDescribeUp) {
      LG("xXx", "msgInit memChaDescribeUp UI error\n");
      return false;
    }
    memChaDescribeDown =
        dynamic_cast<CMemo *>(frmRoleAllInfo->Find("memChaDescribeDown"));
    ...
    for (int i = 0; i < ROLE_ALL_INFO_COUNT; ++i) {
      _snprintf_s(szChaView, _countof(szChaView), _TRUNCATE, "imgChaView%d",
                  i + 1);
...
void CCreateChaScene::_SelectCity(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
  bShowDialog = false;
...
void CCreateChaScene::_ChaFoundFrmMouseEvent(CCompent *pSender, int nMsgType,
  ...
  if (_stricmp("frmFound", pSender->GetForm()->GetName()) != 0) {
    return;
  }
...
void CCreateChaScene::_ChaCityFrmMouseEvent(CCompent *pSender, int nMsgType,
  ...
  if (_stricmp("frmCity", pSender->GetForm()->GetName()) != 0) {
    return;
  }
  ...
  if (frmChaCity->GetIsShow()) {
    ...
    if (CITY_BY_INDEX <= index && index <= CITY_BL_INDEX) {
      ...
      g_stUIBox.ShowSelectBox(_SelectCity, GetCitiesDescripts(index - 1), true);
    }
  }
...
//-----------------------------------------------------------------------
void CCreateChaScene::_QuitFrmMouseEvent(CCompent *pSender, int nMsgType, int x,
                                         int y, DWORD dwKey) {
  string strName = pSender->GetName();

  //	if ( stricmp ("frmQuit", pSender->GetForm()->GetName()) != 0 )
  if (_stricmp("frmQuit", pSender->GetForm()->GetName()) != 0) {
    return;
  }
...
//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_left_face(CGuiData *sender, int x, int y,
                                            DWORD key) {
  GetCurrScene().ChangeFace(LEFT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_right_face(CGuiData *sender, int x, int y,
                                             DWORD key) {
  GetCurrScene().ChangeFace(RIGHT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_left_hair(CGuiData *sender, int x, int y,
                                            DWORD key) {
  GetCurrScene().ChangeHair(LEFT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_right_hair(CGuiData *sender, int x, int y,
                                             DWORD key) {
  GetCurrScene().ChangeHair(RIGHT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_left_city(CGuiData *sender, int x, int y,
                                            DWORD key) {
  GetCurrScene().ChangeCity(LEFT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_right_city(CGuiData *sender, int x, int y,
                                             DWORD key) {
  GetCurrScene().ChangeCity(RIGHT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_left_rotate(CGuiData *sender, int x, int y,
                                              DWORD key) {
  GetCurrScene().RotateChar(LEFT);
}

//-----------------------------------------------------------------------
void CCreateChaScene::__gui_event_right_rotate(CGuiData *sender, int x, int y,
                                               DWORD key) {
...
void CCreateChaScene::ChangeFace(eDirectType enumDirect) {
  ...
  if (NULL == m_pChaForUI[m_nSelChaIndex])
    return;
  ...
  m_nCurFaceIndex = (m_nCurFaceIndex + nSelFaceNum[m_nSelChaIndex]) %
                    nSelFaceNum[m_nSelChaIndex];
  m_nCurFaceIndex += nBeginIndex;

  // 改变脸形
  BOOL bOK =
      m_pChaForUI[m_nSelChaIndex]->ChangePart(enumEQUIP_FACE, m_nCurFaceIndex);
  if (bOK) {
...
void CCreateChaScene::ChangeHair(eDirectType enumDirect) {
  ...
  m_nCurHairIndex = (m_nCurHairIndex + nSelHairNum[m_nSelChaIndex]) %
                    nSelHairNum[m_nSelChaIndex];
  m_nCurHairIndex += nBeginIndex;

  // 改变头发
  BOOL bOK =
      m_pChaForUI[m_nSelChaIndex]->ChangePart(enumEQUIP_HEAD, m_nCurHairIndex);
  if (bOK) {
...
void CCreateChaScene::ChangeCity(eDirectType enumDirect) {
  ...
  memChaDescribe2->SetCaption(GetCitiesDescripts(m_nCurCityIndex));
...
void CCreateChaScene::RenderCha(int x, int y) {
  ...
  g_Render.LookAt(D3DXVECTOR3(11.0f, 36.0f, 10.0f),
                  D3DXVECTOR3(8.70f, 12.0f, 8.0f), LERender::VIEW_3DUI);
  y += 100;

  LEMatrix44 old_mat = *m_pChaForUI[m_nSelChaIndex]->GetMatrix();
  m_pChaForUI[m_nSelChaIndex]->SetUIYaw(180 + m_nChaRotate);
  m_pChaForUI[m_nSelChaIndex]->SetUIScaleDis(9.0f * g_Render.GetScrWidth() /
                                             800);
...
void CCreateChaScene::InitChaFoundFrm() {
  ...
  memChaDescribe->SetCaption(GetDescripts(m_nSelChaIndex));
...
bool CCreateChaScene::IsValidCheckChaName(const char *name) {
  ...
  if (_stricmp(name, RES_STRING(CL_LANGUAGE_MATCH_49)) == 0) {
...
void CCreateChaScene::SendChaToServ() {
  ...
  if (LookData2String(&part, szLookbuf, defLOOK_DATA_STRING_LEN)) {
    CS_NewCha(edtName->GetCaption(), GetCityNames(m_nCurCityIndex), part);
    CGameApp::Waiting();
  }
...
int CCreateChaScene::GetCityZone(int x, int y) {
  ...
  for (int i(0); i < MAX_CITY_NUM; ++i) {
    ...
    if (x >= pSize->GetX() && x <= pSize->GetX2() && y >= pSize->GetY() &&
        y <= pSize->GetY2()) {
      return i + 1;
    }
  }
...
void CCreateChaScene::ShowAllRoleInfo(int nRoleInfo) {
  if (0 < nRoleInfo && nRoleInfo <= ROLE_ALL_INFO_COUNT) {
    for (int i = 0; i < ROLE_ALL_INFO_COUNT; ++i) {
      if (imgChaView[i] && (i + 1) == nRoleInfo && memChaDescribeUp &&
          memChaDescribeDown) {
...
void CCreateChaScene::_evtRoleInfoFormMouseEvent(CCompent *pSender,
                                                 int nMsgType, int x, int y,
                                                 DWORD dwKey) {
  string strName = pSender->GetName();

  IsGirl = strName == "btnfemale" ? true : false;

  CCreateChaScene *pCreateChaScene =
      dynamic_cast<CCreateChaScene *>(g_pGameApp->GetCurScene());
  if (pCreateChaScene) {
    pCreateChaScene->m_LoginSceneCreateCha.SetInvisibleArray(IsGirl, 0);
    ...
    pCreateChaScene->m_LoginSceneCreateCha.LoadArrowMark("hand.lgo",
                                                         OffsetPos4);
...
// 蓝崎界面 （1 ~ 7）
void CCreateChaScene::_evtLanchInfoFormMouseEvent(CCompent *pSender,
                                                  int nMsgType, int x, int y,
                                                  DWORD dwKey) {
  string strName = pSender->GetName();
  CCreateChaScene *pCreateChaScene =
...
// 艾米界面 （19 ~ 24）
void CCreateChaScene::_evtAimiInfoFormMouseEvent(CCompent *pSender,
                                                 int nMsgType, int x, int y,
                                                 DWORD dwKey) {
  string strName = pSender->GetName();
  CCreateChaScene *pCreateChaScene =
...
// 菲利尔界面 （11 ~ 18）
void CCreateChaScene::_evtFelierInfoFormMouseEvent(CCompent *pSender,
                                                   int nMsgType, int x, int y,
                                                   DWORD dwKey) {
  string strName = pSender->GetName();
  CCreateChaScene *pCreateChaScene =
...
// 卡西斯界面 （8 ~ 10）
void CCreateChaScene::_evtCaxiusInfoFormMouseEvent(CCompent *pSender,
                                                   int nMsgType, int x, int y,
                                                   DWORD dwKey) {
  string strName = pSender->GetName();
  CCreateChaScene *pCreateChaScene =
...
void CCreateChaScene::_evtRoleAllInfoFormMouseEvent(CCompent *pSender,
                                                    int nMsgType, int x, int y,
                                                    DWORD dwKey) {
  string strName = pSender->GetName();
  CCreateChaScene *pCreateChaScene =
