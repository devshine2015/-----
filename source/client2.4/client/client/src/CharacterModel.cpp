...
int CCharacterModel::LoadPose()
{
	...
	if(_TypeID == 1)
	{
		...
		if(anim_agent)
		{
			...
			LEIAnimCtrlObjBone* ctrl_obj_bone = (LEIAnimCtrlObjBone*)anim_agent->GetAnimCtrlObj(&type_info);
			LEIAnimCtrlBone* ctrl_bone = (LEIAnimCtrlBone*)ctrl_obj_bone->GetAnimCtrl();
...
DWORD CCharacterModel::GetCurPoseID()
{
	...
	{
		LEPlayPoseInfo* info = LECharacter::GetObjImpPlayPoseInfo( 0, ANIM_CTRL_TYPE_MAT );
		return info ? info->pose : -1;
	}
}
...
int CCharacterModel::IsPosePlaying( DWORD pose )
{
	...
	{
		LEPlayPoseInfo* info = LECharacter::GetObjImpPlayPoseInfo(1, ANIM_CTRL_TYPE_MAT);
