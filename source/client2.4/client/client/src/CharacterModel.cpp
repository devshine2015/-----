...
int CCharacterModel::LoadCha(DWORD type_id, DWORD bone_id,
                             const DWORD *part_buf) {
  _TypeID = type_id;
...
int CCharacterModel::LoadPart(DWORD part_id, const char *file) {
  ...
  LEIPrimitive* p = LECharacter::GetPhysique()->GetPrimitive(part_id);
  drPrimitiveTexLitA(p, l->str_buf[0], l->str_buf[1], ".\\texture\\lit\\",
  l->anim_type);
...
int CCharacterModel::ChangePart(DWORD part_id, DWORD tab_id) {
  ...
  if (_ModelType == MODEL_WARSHIP) {
    if (part_id == 0) {
      ...
      //	_snprintf_s( str, _countof( str ), _TRUNCATE,  "%s.lgo",
      //ir->chModule[0] ); 	LoadPart( 3, str );
...
int CCharacterModel::LoadPose() {
  ...
  if (false ==
      GetCharacterAction(cha_type, &SCharAct)) // 没有在文件中找到对应的ID
  {
...
#if 1
  if (_TypeID == 1) {
    LEIAnimCtrlAgent *anim_agent = GetPhysique()->GetAnimCtrlAgent();
    if (anim_agent) {
      LEAnimCtrlObjTypeInfo type_info;
      type_info.type = ANIM_CTRL_TYPE_BONE;
      type_info.data[0] = DR_INVALID_INDEX;
      type_info.data[1] = DR_INVALID_INDEX;
      LEIAnimCtrlObjBone *ctrl_obj_bone =
          (LEIAnimCtrlObjBone *)anim_agent->GetAnimCtrlObj(&type_info);
      LEIAnimCtrlBone *ctrl_bone =
          (LEIAnimCtrlBone *)ctrl_obj_bone->GetAnimCtrl();
      LEIPoseCtrl *pc = ctrl_bone->GetPoseCtrl();
      LEPoseInfo *pi = pc->GetPoseInfo(0);
...
void CCharacterModel::PlayPose(DWORD pose, DWORD type, int time, int fps,
                               DWORD blend_flag, DWORD blend_src_num) {
  DWORD p = 0;
  _PoseType = pose;
  ...
  LEPoseInfo *pi = c->GetPoseInfo(_SmallPoseID);
  if (pi == 0) {
  ...
  if (time > 0) {
    float v =
        1000.0f / (float)fps / ((float)time / (float)(pi->end - pi->start));
    velocity = v;
  } else {
  ...
  switch (_ModelType) {
  case MODEL_CHARACTER:
    if (FAILED(LECharacter::PlayPose(p, type, 0.0f, velocity, blend_flag,
                                     blend_src_num))) {
  ...
  case MODEL_WARSHIP: //船只只有obj_0有外部pose，其余部分都为default pose
    if (FAILED(LECharacter::PlayObjImpPose(0, ANIM_CTRL_TYPE_MAT, p, type, 0.0f,
                                           velocity))) {
    ...
    // LECharacter::PlayObjImpPose( 3, ANIM_CTRL_TYPE_TEXTURE0, 0, type,
    // 0.0f, 1.0f ); LECharacter::PlayObjImpPose( 3, ANIM_CTRL_TYPE_TEXTURE1, 0,
    // type, 0.0f, 1.0f ); LECharacter::PlayObjImpPose( 3,
    // ANIM_CTRL_TYPE_TEXTURE2, 0, type, 0.0f, 1.0f );
    // LECharacter::PlayObjImpPose( 3, ANIM_CTRL_TYPE_TEXTURE3, 0, type,
    // 0.0f, 1.0f );
    break;
...
void CCharacterModel::PlayPose(const LEPlayPoseInfo *info) {
  if (_ModelType != MODEL_CHARACTER) {
...
int CCharacterModel::AttachItem(DWORD link_id, CSceneItem *obj, DWORD part_type,
                                DWORD item_link_id) {
  ItemLinkInfo info;
  info.obj = obj;
  info.data = part_type;
  ...
  if (AttachItem(link_id, &info) == 0) {
    LG("error", "msgType: %s, link_id: %d, item_id: %d",
       obj->GetItemInfo()->szName, link_id, item_link_id);
    return 0;
  }
  return 1;
}
...
DWORD CCharacterModel::GetCurPoseID() {
  if (_ModelType == MODEL_CHARACTER) {
    LEPlayPoseInfo *info = LECharacter::GetPlayPoseInfo();
    return info ? info->pose : -1;
  } else {
    LEPlayPoseInfo *info =
        LECharacter::GetObjImpPlayPoseInfo(0, ANIM_CTRL_TYPE_MAT);
    return info ? info->pose : -1;
  }
}
...
int CCharacterModel::IsPosePlaying(DWORD pose) {
  if (_ModelType == MODEL_CHARACTER) {
    LEPlayPoseInfo *info = LECharacter::GetPlayPoseInfo();
    LEIPoseCtrl *ctrl = LECharacter::GetPoseCtrl();
    ...
    LEPlayPoseInfo *info =
        LECharacter::GetObjImpPlayPoseInfo(0, ANIM_CTRL_TYPE_MAT);
    LEIPoseCtrl *ctrl = LECharacter::GetObjImpPoseCtrl(0, ANIM_CTRL_TYPE_MAT);
    if (info == 0 || info->pose != pose)
      return 0;
    ...
    LEPlayPoseInfo *info =
        LECharacter::GetObjImpPlayPoseInfo(1, ANIM_CTRL_TYPE_MAT);
    LEIPoseCtrl *ctrl = LECharacter::GetObjImpPoseCtrl(1, ANIM_CTRL_TYPE_MAT);
    if (info == 0 || info->pose != pose)
      return 0;
...
void CCharacterModel::SetPoseKeyFrameProc(drPoseKeyFrameProc proc,
                                          void *proc_param) {
  _KeyFrameProc = proc;
  ;
  ...
  if (this->_ModelType == MODEL_CHARACTER) {
    ...
    LECharacter::SetObjImpPoseKeyFrameProc(1, ANIM_CTRL_TYPE_MAT, proc,
                                           proc_param);
  } else {
    LECharacter::SetObjImpPoseKeyFrameProc(0, ANIM_CTRL_TYPE_MAT, proc,
                                           proc_param);
  }
}
DWORD CCharacterModel::GetCurPoseKeyFrameNum() {
  ...
  LEPoseInfo *info = c->GetPoseInfo(_SmallPoseID);
  if (info == 0)
    return 0;
...
LEIPoseCtrl *CCharacterModel::GetPoseCtrl() {
  LEIPoseCtrl *c = 0;
...
LEPoseInfo *CCharacterModel::GetPoseInfo(DWORD pose_id) {
  LEIPoseCtrl *pose_ctrl = GetPoseCtrl();
  if (pose_ctrl == 0)
    return 0;

  return pose_ctrl->GetPoseInfo(pose_id);
}
LEPoseInfo *CCharacterModel::GetPoseInfoBig(DWORD big_pose_id) {
  DWORD p;
...
int CCharacterModel::Unlit(DWORD part_id) {
  LEIPrimitive *p = GetPhysique()->GetPrimitive(part_id);
  if (p == 0)
    return 0;
...
int CCharacterModel::Lit(DWORD part_id, DWORD lit_id) {
  ...
  if (_ModelType == MODEL_CHARACTER) {
    ...
    drPrimitiveTexLitA(p, l->mask, l->str_buf[lit_id], ".\\texture\\lit\\",
                       l->anim_type);
    DWORD vst = p->GetRenderCtrlAgent()->GetVertexShader();
...
BOOL CCharacterModel::Cull() {
  ...
  drISceneMgr *scn_mgr =
      g_Render.GetInterfaceMgr()->sys_graphics->GetSceneMgr();
  drIViewFrustum *vf = scn_mgr->GetViewFrustum();

  DWORD i;
  if (pInfo->chModalType == enumMODAL_MAIN_CHA ||
      pInfo->chModalType == enumMODAL_OTHER) // character
  {
    drSphere s;
    ...
    return !vf->IsSphereInFrustum(&s);
             ...
             pInfo->chModalType == enumMODAL_EMPL) // ship
  {
    LEIPhysique *p = GetPhysique();

    DWORD pri_num = 8;
    for (i = 0; i < pri_num; i++) {
      LEIPrimitive *pri = p->GetPrimitive(i);
      if (pri == 0)
        continue;
