...
BOOL SC_CHARSTR(LPRPACKET pk) {
  T_B
      ...
      strncpy_s(g_NetIF->m_chapstr, sizeof(g_NetIF->m_chapstr), pk.ReadString(),
                _TRUNCATE);
...
// 网络协议是否加密
#if NET_PROTOCOL_ENCRYPT
  {
    int nNoise =
        g_sKeyData *
        int(*(int *)(g_NetIF->m_chapstr + strlen(g_NetIF->m_chapstr) - 4));
...
BOOL SC_Login(LPRPACKET pk) {
  T_B uShort l_errno = pk.ReadShort();
...
#ifdef _TEST_CLIENT
    CTestClient *pClient = reinterpret_cast<CTestClient *>(
        g_NetIF->m_connect.GetDatasock()->GetPointer());
  ...
  } else //成功
  {
    ...
    g_stUIChat._PictureCChat.setServerKey(
        const_cast<char *>(sessionKey)); // add guojiangang 20090211
    ...
    g_stUIChat._PictureCChat.setUserName(
        g_NetIF->m_accounts); // add guojiangang 20090211

    // if(strlen(g_Config.m_szMD5Pass) == 32) //
    // 配置文件里直接输入MD5值密码登陆, 方便诊断玩家报告的错误
    //{
    //	l_chap.m_strPwdDig = g_Config.m_szMD5Pass; // 中途修改输入密码的MD5值
    //}

    l_chap.GetSessionClrKey((void *)l_sKey, l_keylen, l_buf, sizeof(l_buf),
                            l_len);
    ...
    for (int i = 0; i < l_chanum; i++) {
      ...
      l_netcha[i].sLook =
          reinterpret_cast<const LOOK *>(pk.ReadSequence(l_looklen));
    ...
    CTestClient *pClient = reinterpret_cast<CTestClient *>(
        g_NetIF->m_connect.GetDatasock()->GetPointer());
    pClient->OnLogin(g_NetIF->_enc, g_NetIF->_comm_enc, g_NetIF->_key,
                     g_NetIF->_key_len);
...
BOOL SC_EnterMap(LPRPACKET pk) {
  T_B
      ...
      CWorldScene *pWorldScene =
          dynamic_cast<CWorldScene *>(CGameApp::GetCurScene());
  if (pWorldScene)
  ...
  SMapInfo.chEnterType =
      pk.ReadChar(); // 进入地图类型，参见CompCommand.h EEnterMapType
  ...
  NetSynAttr(SCreateInfo.ulWorldID, SChaAttr.chType, SChaAttr.sNum,
             SChaAttr.SEff);
  ...
  XTrap_C_SetUserInfoEx(lpUserID, lpServerName, lpCharacterName,
                        lpCharacterClass, dwCharacterLevel, Socket);
#endif
  ...
  //    CTestClient* pClient = reinterpret_cast<CTestClient*>(
  //    g_NetIF->m_connect.GetDatasock()->GetPointer() );
  //    pClient->StartGame(SCreateInfo.SArea.centre.x,
...
BOOL PC_Ping(LPRPACKET pk) {
  T_B WPacket l_wpk = g_NetIF->GetWPacket();
...
BOOL SC_CheckPing(LPRPACKET pk) {
  T_B WPacket wpk = g_NetIF->GetWPacket();
...
BOOL SC_ChaBeginSee(LPRPACKET pk) {
  T_B stNetActorCreate SCreateInfo;
  ...
  TBegin CCharacter *pCha = SCreateInfo.CreateCha();
  if (!pCha)
    return FALSE;
  TEnd
      // 	char* szLogName = g_LogName.GetLogName( SCreateInfo.ulWorldID );
...
BOOL SC_ItemCreate(LPRPACKET pk) {
  T_B stNetItemCreate SCreateInfo;
  ...
  SCreateInfo.chAppeType =
      pk.ReadChar(); // 出现原因（参见CompCommand.h EItemAppearType）
  ...
  // log
  //	LG("SC_Item", "CreateType = %d, WorldID:%u, ItemID = %d, Pos = [%d,%d],
  //SrcID = %u, \n", SCreateInfo.chAppeType, SCreateInfo.lWorldID,
  //SCreateInfo.lID, SCreateInfo.SPos.x, SCreateInfo.SPos.y,
  //SCreateInfo.lFromID);
  //
  return TRUE;
  T_E
...
BOOL SC_AStateBeginSee(LPRPACKET pk) {
  ...
  //
  // 	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_296), SynAState.sAreaX,
  // SynAState.sAreaY, SynAState.chStateNum); 	for (char j = 0; j <
  // SynAState.chStateNum; j++) 		LG(szLogName, "\t%d\t%d\n",
  // SynAState.State[j].chID, SynAState.State[j].chLv); 	LG(szLogName, "\n");
  //
...
BOOL SC_AStateEndSee(LPRPACKET pk) {
  ...
  // log
  //	LG(g_LogName.GetMainLogName(), RES_STRING(CL_LANGUAGE_MATCH_296),
  //SynAState.sAreaX, SynAState.sAreaY, 0);
  //
...
BOOL SC_Cha_Emotion(LPRPACKET pk) {
  T_B uLong l_id = pk.ReadLong();
  uShort sEmotion = pk.ReadShort();

  NetChaEmotion(l_id, sEmotion);
  //	LG( g_LogName.GetLogName( l_id ), RES_STRING(CL_LANGUAGE_MATCH_297),
  //sEmotion );
  return TRUE;
  T_E
}
...
BOOL SC_CharacterAction(LPRPACKET pk) {
  ...
  // try
  {
    ...
    case enumACTION_MOVE: {
      ...
      // 					if( isMainCha )
      // 					{
      // 						g_pGameApp->GetDrawPoints()->Add(
      // SMoveInfo.SPos[i].x, SMoveInfo.SPos[i].y, 0xffff0000, 0.5f ); 						if
      // (SMoveInfo.sState && i==SMoveInfo.nPointNum-1)
      // 						{
      // 							g_pGameApp->GetDrawPoints()->Add(
      // SMoveInfo.SPos[i].x, SMoveInfo.SPos[i].y, 0xff000000, 0.3f );
      // 						}
      // 					}
      // #endif
      //
      // 					if (i > 0)
      // 					{
      // 						lDistX = SMoveInfo.SPos[i].x
      // - SMoveInfo.SPos[i - 1].x; 						lDistY = SMoveInfo.SPos[i].y -
      // SMoveInfo.SPos[i - 1].y; 						lDist = (long)sqrt((double)lDistX * lDistX +
      // lDistY * lDistY);
      // 					}
      //					LG(szLogName, "\t%d, %d\t%d\n",
      ...
      CTestClient *pClient = reinterpret_cast<CTestClient *>(
          g_NetIF->m_connect.GetDatasock()->GetPointer());
      pClient->MoveTo(l_id, SMoveInfo.SPos[SMoveInfo.nPointNum - 1].x,
      ...
      // CGameScene::GetMainCha(); 					if( pCMainCha )
      // 					{
      // 						lDist = (pCMainCha->GetCurX()
      // - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].x) * (pCMainCha->GetCurX() -
      // SMoveInfo.SPos[SMoveInfo.nPointNum - 1].x)
      // 							+
      // (pCMainCha->GetCurY() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].y) *
      // (pCMainCha->GetCurY() - SMoveInfo.SPos[SMoveInfo.nPointNum - 1].y);
      // 						LG(szLogName,
    ...
    case enumACTION_SKILL_SRC: {
      ...
      if (lResNum > 0) {
        ...
        for (long i = 0; i < lResNum; i++) {
          ...
          if (SSkillInfo.SEffect[i].lAttrID == ATTR_CEXP ||
              SSkillInfo.SEffect[i].lAttrID == ATTR_NLEXP ||
              SSkillInfo.SEffect[i].lAttrID == ATTR_CLEXP)
      ...
      // log
      // 				LG(szLogName, "===Recieve(Skill
      // Represent):\tTick:[%u]\n", GetTickCount()); 				LG(szLogName,
      // "Angle:\t%d\tFightID:%d\n", SSkillInfo.sAngle, SSkillInfo.byFightID );
      // 				LG(szLogName,
      // "SkillID:\t%u\tSkillSpeed:%d\n", SSkillInfo.lSkillID,
      // SSkillInfo.lSkillSpeed); 					LG(szLogName, "TargetInfo(ID, PosX,
      // PosY):\t%d\n", SSkillInfo.lTargetID, SSkillInfo.STargetPoint.x,
      // SSkillInfo.STargetPoint.y); 				LG(szLogName, "Effect:[ID, Value]\n"); 				for
      // (DWORD i = 0; i < SSkillInfo.SEffect.GetCount(); i++) 					LG(szLogName,
      // "\t%d,\t%i64\n", SSkillInfo.SEffect[i].lAttrID,
      // SSkillInfo.SEffect[i].lVal); 				if (SSkillInfo.SState.GetCount() > 0)
      // 					LG(szLogName, "Skill State:[ID,
      // LV]\n"); 				for (DWORD chNum = 0; chNum < SSkillInfo.SState.GetCount();
      // chNum++) 					LG(szLogName, "\t%d, %d\n", SSkillInfo.SState[chNum].chID,
      // SSkillInfo.SState[chNum].chLv); 				if (SSkillInfo.sState) 					LG(szLogName,
      // "@@@End Skill\tState:0x%x\n", SSkillInfo.sState); 				LG(szLogName, "\n");
      //
...
//SSkillInfo.SEffect[i].lAttrID,SSkillInfo.SEffect[i].lVal);
      ...
      if (pk.ReadChar()) {
        ...
        if (lSrcResNum > 0) {
          ...
          for (long i = 0; i < lSrcResNum; i++) {
            ...
            if (SSkillInfo.SSrcEffect[i].lAttrID == ATTR_CEXP ||
                SSkillInfo.SSrcEffect[i].lAttrID == ATTR_NLEXP ||
                SSkillInfo.SSrcEffect[i].lAttrID == ATTR_CLEXP)
      ...
      // 				LG(szLogName, "===Recieve(Skill
      // Effect):\tTick:[%u]\n", GetTickCount()); 				LG(szLogName,
      // "SourceChaID:\t%u\tFightID:%d\n", SSkillInfo.lSrcID,
      // SSkillInfo.byFightID); 				LG(szLogName, "SkillID:\t%u\n",
      // SSkillInfo.lSkillID); 				LG(szLogName, "State: 0x%x, Double: %d, Miss: %d,
      // Pos[%d, %d], ExecTime: %d, ResNum: %d\n", SSkillInfo.sState,
      // SSkillInfo.bDoubleAttack, SSkillInfo.bMiss, SSkillInfo.SSrcPos.x,
      // SSkillInfo.SSrcPos.y, SSkillInfo.sExecTime,
      // SSkillInfo.SEffect.GetCount() ); 				if (SSkillInfo.bBeatBack)
      // 					LG(szLogName,
      // RES_STRING(CL_LANGUAGE_MATCH_298), SSkillInfo.SPos.x,
      // SSkillInfo.SPos.y); 				LG(szLogName, "Effect:[ID, Value]\n"); 				for (long i
      // = 0; i < lResNum; i++) 					LG(szLogName, "\t%d,\t%i64\n",
      // SSkillInfo.SEffect[i].lAttrID, SSkillInfo.SEffect[i].lVal); 				if
      // (chSStateNum > 0) 					LG(szLogName, "Skill State:[ID, LV]\n"); 				for (short
      // chNum = 0; chNum < chSStateNum; chNum++) 					LG(szLogName, "\t%d, %d\n",
      // SSkillInfo.SState[chNum].chID, SSkillInfo.SState[chNum].chLv);
      //
      // 				LG(szLogName, "Src Effect:[ID, Value],
      // State: 0x%x\n", SSkillInfo.sSrcState ); 				for (long i = 0; i <
      // lSrcResNum; i++) 					LG(szLogName, "\t%d,\t%d\n",
      // SSkillInfo.SSrcEffect[i].lAttrID, SSkillInfo.SSrcEffect[i].lVal); 				if
      // (chSrcSStateNum > 0) 					LG(szLogName, "Src Skill State:[ID, LV]\n"); 				for
      // (short chNum = 0; chNum < chSrcSStateNum; chNum++) 					LG(szLogName, "\t%d,
      // %d\n", SSkillInfo.SSrcState[chNum].chID,
      // SSkillInfo.SSrcState[chNum].chLv); 				if (SSkillInfo.sState &
      // enumFSTATE_DIE) 					LG(szLogName, "@@@Die\n"); 				LG(szLogName, "\n");
      //
    ...
    case enumACTION_LEAN: // 倚靠
    {
      ...
      // log
      // 				LG(szLogName,
      // "===Recieve(Lean):\tTick:[%u]\n", GetTickCount()); 				if (SLean.chState)
      // 					LG(szLogName, "@@@End
      // Lean\tState:%d\n", SLean.chState); 				LG(szLogName, "\n");
      //
  ...
  // catch (...)
  //{
  //	MessageBox(0, "!!!!!!!!!!!!!!!!!!!!exception: Notice Action", "error",
  //0);
  //}
...
BOOL SC_SynAttribute(LPRPACKET pk) {
  ...
  //_i64toa(SChaAttr.SEff[0].lVal , val, 10)); OutputDebugStr(buff);
...
BOOL SC_SynTeam(LPRPACKET pk) {
  ...
  // LG( "Team", "Refresh, ID[%u], HP[%d], MaxHP[%d], SP[%d], MaxSP[%d],
  // LV[%d]\n", STeamState.ulID, STeamState.lHP, STeamState.lMaxHP,
  // STeamState.lSP, STeamState.lMaxSP, STeamState.lLV );
...
BOOL SC_SynTLeaderID(LPRPACKET pk) {
  ...
  // log
  //	LG(g_LogName.GetLogName( lID ), RES_STRING(CL_LANGUAGE_MATCH_300),
  //lLeaderID, lID);
  //
...
BOOL SC_HelpInfo(LPRPACKET packet) {
  T_B NET_HELPINFO Info;
  memset(&Info, 0, sizeof(NET_HELPINFO));

  Info.byType = packet.ReadChar();
  if (Info.byType == mission::MIS_HELP_DESP ||
      Info.byType == mission::MIS_HELP_IMAGE ||
...
BOOL SC_TalkInfo(LPRPACKET packet) {
  T_B DWORD dwNpcID = packet.ReadLong();
...
BOOL SC_FuncInfo(LPRPACKET packet) {
  ...
  strncpy_s(FuncPage.szTalk, ROLE_MAXNUM_DESPSIZE - 1, packet.ReadString(),
            _TRUNCATE);
  ...
  for (int i = 0; i < byCount; i++) {
    ...
    // strncpy( FuncPage.FuncItem[i].szFunc, pszFunc, ROLE_MAXNUM_FUNCITEMSIZE -
    // 1 );
    strncpy_s(FuncPage.FuncItem[i].szFunc, ROLE_MAXNUM_FUNCITEMSIZE - 1,
              pszFunc, _TRUNCATE);
  }
  ...
  for (int i = 0; i < byMisCount; i++) {
    ...
    // );
    strncpy_s(FuncPage.MisItem[i].szMis, ROLE_MAXNUM_FUNCITEMSIZE - 1, pszMis,
              _TRUNCATE);
...
BOOL SC_TradeResult(LPRPACKET packet) {
  T_B BYTE byType = packet.ReadChar();
  BYTE byIndex = packet.ReadChar();
  BYTE byCount = packet.ReadChar();
  USHORT sItemID = packet.ReadShort();
  DWORD dwMoney = packet.ReadLong();
  LG("trade", RES_STRING(CL_LANGUAGE_MATCH_301), byType, byIndex, byCount,
     sItemID, dwMoney);
...
BOOL SC_CharTradeInfo(LPRPACKET packet) {
  T_B USHORT usCmd = packet.ReadShort();
  switch (usCmd) {
  ...
  case CMD_MC_CHARTRADE_ITEM: {
    ...
    if (byOpType == mission::TRADE_DRAGTO_ITEM) {
      ...
      NetTradeAddItem(dwRequestID, byOpType, 0, byIndex, byCount, byItemIndex,
                      Data);
      ...
      if (sType == enumItemTypeBoat) {
        ...
        if (packet.ReadChar() == 0) {
          ...
          // strncpy( Data.szName, packet.ReadString(), BOAT_MAXSIZE_BOATNAME -
          // 1 );
          strncpy_s(Data.szName, BOAT_MAXSIZE_BOATNAME - 1, packet.ReadString(),
                    _TRUNCATE);
          ...
          NetTradeAddBoat(dwRequestID, byOpType, sItemID, byIndex, byCount,
                          byItemIndex, Data);
        }
      } else {
        // 物品实例属性
        ...
        NetTradeAddItem(dwRequestID, byOpType, sItemID, byIndex, byCount,
                        byItemIndex, Data);
      }
    } else {
      MessageBox(NULL, RES_STRING(CL_LANGUAGE_MATCH_303),
                 RES_STRING(CL_LANGUAGE_MATCH_25), MB_OK);
      return FALSE;
    }
...
BOOL SC_MisPage(LPRPACKET packet) {
  T_B BYTE byCmd = packet.ReadChar();
  DWORD dwNpcID = packet.ReadLong();
  NET_MISPAGE page;
  ...
  case ROLE_MIS_BTNPENDING: {
    ...
    for (int i = 0; i < page.byNeedNum; i++) {
      ...
      if (page.MisNeed[i].byType == mission::MIS_NEED_ITEM ||
          page.MisNeed[i].byType == mission::MIS_NEED_KILL) {
        ...
        // strncpy( page.MisNeed[i].szNeed, pszTemp, ROLE_MAXNUM_NEEDDESPSIZE -
        // 1 );
        strncpy_s(page.MisNeed[i].szNeed, ROLE_MAXNUM_NEEDDESPSIZE - 1, pszTemp,
                  _TRUNCATE);
      } else {
        // 未知的数据类型
...
BOOL SC_MisLogClear(LPRPACKET packet) {
  T_B WORD wMisID = packet.ReadShort();
...
BOOL SC_EntityStateChange(LPRPACKET packet) {
  T_B DWORD dwEntityID = packet.ReadLong();
  BYTE byState = packet.ReadChar();
...
BOOL SC_Amphitheater(LPRPACKET packet) {
  ...
  if (byState) {
    ...
    for (int i = 0; i < ProSize; i++) {
      ...
      // strncpy(Punit.PromCapName, packet.ReadString(),
      // sizeof(Punit.PromCapName));
      strncpy_s(Punit.PromCapName, sizeof(Punit.PromCapName),
                packet.ReadString(), _TRUNCATE);
    ...
    for (int i = 0; i < RelSize; i++) {
      TeamRelUnit Runit;
      // strncpy(Runit.RelCapName, packet.ReadString(),
      // sizeof(Runit.RelCapName));
      strncpy_s(Runit.RelCapName, sizeof(Runit.RelCapName), packet.ReadString(),
                _TRUNCATE);
...
BOOL SC_Fix(LPRPACKET packet) {
  T_B NetShowRepairOven();
  return TRUE;
  T_E
}

BOOL SC_GMSend(LPRPACKET packet) {
  T_B g_stUIMail.ShowQuestionForm();
  return TRUE;
  T_E
}
...
BOOL SC_CreateBoat(LPRPACKET packet) {
  T_B DWORD dwBoatID = packet.ReadLong();
  xShipBuildInfo Info;
  ...
  strncpy_s(Info.szBerth, BOAT_MAXSIZE_NAME - 1, packet.ReadString(),
            _TRUNCATE);
  ...
  if (Info.byIsUpdate) {
    ...
    strncpy_s(Info.szHeader, BOAT_MAXSIZE_NAME - 1, packet.ReadString(),
              _TRUNCATE);

    Info.byEngine = packet.ReadChar();
    Info.dwEngine = packet.ReadLong();
    // strncpy( Info.szEngine, packet.ReadString(), BOAT_MAXSIZE_NAME - 1 );
    strncpy_s(Info.szEngine, BOAT_MAXSIZE_NAME - 1, packet.ReadString(),
              _TRUNCATE);
  ...
  strncpy_s(Info.szCannon, BOAT_MAXSIZE_NAME - 1, packet.ReadString(),
            _TRUNCATE);

  Info.byEquipment = packet.ReadChar();
  // strncpy( Info.szEquipment, packet.ReadString(), BOAT_MAXSIZE_NAME - 1 );
  strncpy_s(Info.szEquipment, BOAT_MAXSIZE_NAME - 1, packet.ReadString(),
            _TRUNCATE);
...
BOOL SC_BoatList(LPRPACKET packet) {
  ...
  for (BYTE i = 0; i < byNumBoat; i++) {
    // strncpy( Data.szName[i], packet.ReadString(), BOAT_MAXSIZE_BOATNAME +
    // BOAT_MAXSIZE_DESP - 1 );
    strncpy_s(Data.szName[i], BOAT_MAXSIZE_BOATNAME + BOAT_MAXSIZE_DESP - 1,
              packet.ReadString(), _TRUNCATE);
  }
...
//	for( BYTE i = 0; i < byNumBoat; i++ )
//	{
//		strncpy( Data.szName[i], packet.ReadString(),
//BOAT_MAXSIZE_BOATNAME - 1 );
//	}
//
...
BOOL SC_StallInfo(LPRPACKET packet) {
  T_B DWORD dwCharID = packet.ReadLong();
...
BOOL SC_StartExit(LPRPACKET packet) {
  T_B DWORD dwExitTime = packet.ReadLong();
...
BOOL SC_TeamFightAsk(LPRPACKET packet) {
  T_B
      ...
      // 	strncpy_s(szLogName,sizeof(szLogName),
      // RES_STRING(CL_LANGUAGE_MATCH_305),_TRUNCATE);

          stNetTeamFightAsk SFightAsk;
  SFightAsk.chSideNum2 = packet.ReverseReadChar();
  SFightAsk.chSideNum1 = packet.ReverseReadChar();
  // 	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_306), SFightAsk.chSideNum1,
  // SFightAsk.chSideNum2);
  for (char i = 0; i < SFightAsk.chSideNum1 + SFightAsk.chSideNum2; i++) {
    ...
    // 		LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_307),
    // SFightAsk.Info[i].szName, SFightAsk.Info[i].chLv,
    // SFightAsk.Info[i].szJob);
  }
  // 	LG(szLogName, "\n");
  SFightAsk.Exec();
  return TRUE;
...
BOOL SC_BeginItemRepair(LPRPACKET packet) {
  T_B NetBeginRepairItem();
  return TRUE;
  T_E
}

BOOL SC_ItemRepairAsk(LPRPACKET packet) {
  T_B stNetItemRepairAsk SRepairAsk;
...
BOOL SC_ItemForgeAsk(LPRPACKET packet) {
  T_B stSCNetItemForgeAsk SForgeAsk;
...
BOOL SC_ItemForgeAnswer(LPRPACKET packet) {
  T_B stNetItemForgeAnswer SForgeAnswer;
...
BOOL SC_ItemLotteryAnswer(LPRPACKET packet) {
  T_B stNetItemLotteryAnswer SLotteryAnswer;
...
BOOL SC_ItemLotteryAsk(LPRPACKET packet) {
  T_B stSCNetItemLotteryAsk SLotteryAsk;
...
BOOL SC_JewelryUpAnswer(LPRPACKET packet) {
  ...
  if (result == '1') {
    ...
    g_stJewelryUpForm.ShowConfirmDialog(OldJeyID, OldJeyLev, NewJeyID,
                                        NewJeyLev);
  } else {
    char buf[246] = {0};
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_991));
...
BOOL SC_QueryCha(LPRPACKET pk) {
  ...
  _snprintf_s(szInfo, _countof(szInfo), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_308), pChaName, lChaID, pMapName,
              lPosX, lPosY);
...
BOOL SC_QueryChaPing(LPRPACKET pk) {
  ...
  _snprintf_s(szInfo, _countof(szInfo), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_309), pMapName, lPing);
  SShowInfo.m_sysinfo = szInfo;
  NetSysInfo(SShowInfo);
...
BOOL SC_PreMoveTime(LPRPACKET pk) {
  T_B uLong ulPreMoveTime = pk.ReadLong();
...
BOOL SC_ExchangeAnswer(LPRPACKET packet) {
  T_B char ReturnCase = packet.ReadChar(); // 商城是否开启
  if (ReturnCase == 1) {
    ...
    g_stRumDealForm.AddMyDealInfo(MyDealSize, MyDealPrices, MyDealCounts,
                                  MyDealTyes, MyDealTotalPrices);
...
BOOL SC_RequestBuyAsk(LPRPACKET packet) {
  T_B char Result = packet.ReadChar();
  if (Result == '0') {
    ...
    if (Result == '1') {
      ...
      if (g_stRumDealForm.IsOpen())
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1003),
                           (int)CrystalCount, (int)TotalCost);
    } else if (Result == '0')
...
BOOL SC_RequestSaleAsk(LPRPACKET packet) {
  ...
  if (Result == '0') {
    ...
    if (Result == '1') {
      ...
      if (g_stRumDealForm.IsOpen())
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1005),
                           (int)CrystalCount, (int)TotalEarning);
    } else if (Result == '0')
...
BOOL SC_RequesttCrystalState(LPRPACKET packet) {
  ...
  if (Result == 0) // buy
  {
    ...
    if (State0 == 2 || State0 == 3) {
      ...
      if (State0 == 2)
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1016), (int)Num,
                           (int)Num, 0, (int)(Num * Price));
      else
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1017),
                           (int)(Num * Price));
    } else if (State0 == 1) {
      ...
      if (flag == 'T') {
        ...
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1016), (int)Total,
                           (int)Num, (int)(Total - Num), int(Num * Price));
      }
    }
  ...
  } else if (Result == 1) //卖
  {
    ...
    if (State1 == 2 || State1 == 3) {
      ...
      LONG64 Price = packet.ReadLongLong();
      if (State1 == 2)
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1018), (int)Num,
                           (int)Num, 0, (int)(Num * Price));
      else
      ...
      if (flag == 'T') {
        ...
        LONG64 Price = packet.ReadLongLong();
        LONG64 Total = packet.ReadLongLong();
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1018), (int)Total,
                           (int)(Total - Num), (int)Num,
                           (int)((Total - Num) * Price));
      }
    }
    ...
    if (State0 == 3 || State0 == 2) {
      ...
      if (State0 == 2)
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1016), (int)buyNum,
                           (int)buyNum, 0, (int)(buyNum * buyPrice));
      else
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1017),
                           (int)(buyNum * buyPrice));
    } else if (State0 == 1) {
      ...
      if (flag == 'T') {
        ...
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1016), (int)buyTotal,
                           (int)buyNum, (int)(buyTotal - buyNum),
                           (int)(buyNum * buyPrice));
      }
    }
    ...
    if (State1 == 2 || State1 == 3) {
      ...
      if (State1 == 2)
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1018), (int)sellNum,
                           (int)sellNum, 0, (int)(sellNum * sellPrice));
      else
      ...
      if (flag == 'T') {
        ...
        g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1018), (int)(sellTotal),
                           (int)(sellTotal - sellNum), (int)(sellNum),
                           (int)((sellTotal - sellNum) * sellPrice));
      }
    }
...
BOOL SC_StoreListAnswer(LPRPACKET packet) {
  T_B g_stUIStore.ClearStoreItemList();
  ...
  for (i = 0; i < sComNum; i++) {
    ...
    // 添加到商城
    g_stUIStore.AddStoreItemInfo(i, lComID, szComName, lPrice, szRemark, isHot,
                                 nTime, lComNumber, lComExpire);
    ...
    for (j = 0; j < sItemClsNum; j++) {
      ...
      // 商品细节内容
      g_stUIStore.AddStoreItemDetail(i, j, sItemID, sItemNum, sFlute,
                                     pItemAttrID, pItemAttrVal);
    }
  }
...
BOOL SC_BlackMarketExchangeUpdate(LPRPACKET packet) {
  ...
  for (short sIndex = 0; sIndex < sCount; ++sIndex) {
    ...
    if (g_stUIBlackTrade.GetIsShow() &&
        g_stUIBlackTrade.GetNpcID() == dwNpcID) {
      g_stUIBlackTrade.SetItem(&SBlackTrade);
    }
  }
...
BOOL SC_SyncKitbagTemp(LPRPACKET packet) {
  ...
  // CWorldScene* pWorldScene =
  // dynamic_cast<CWorldScene*>(g_pGameApp->GetCurScene()); if(pWorldScene)
...
BOOL SC_XtrapStepOne(LPRPACKET packet) {
  ...
  XTrap_CS_Step2(szIn, szOut,
                 XTRAP_PROTECT_PE | XTRAP_PROTECT_TEXT | XTRAP_PROTECT_RDATA |
                     XTRAP_PROTECT_EDATA | XTRAP_PROTECT_RSRC |
                     XTRAP_PROTECT_RELOC | XTRAP_PROTECT_EXCEPT_VIRUS);
...
BOOL PC_MasterRefresh(LPRPACKET packet) {
  ...
  case MSG_PRENTICE_REFRESH_START: {
    ...
    NetPrenticeStart(l_self, l_nfs,
                     min(l_nfnum, (sizeof(l_nfs) / sizeof(l_nfs[0]))));
  } break;
  }
  return TRUE;
}
...
BOOL SC_ListAuction(LPRPACKET pk) {
  ...
  for (int i = 0; i < sNum; i++) {
    ...
    strncpy_s(stInfo.szChaName, sizeof(stInfo.szChaName) - 1, szChaName,
              _TRUNCATE);
...
void ReadChaBasePacket(LPRPACKET pk, stNetActorCreate &SCreateInfo) {
  T_B memset(&SCreateInfo, 0, sizeof(SCreateInfo));
  // 基本数据
  ...
  SCreateInfo.lHandle = pk.ReadLong();
  SCreateInfo.chCtrlType =
      pk.ReadChar(); // 角色类型（参考CompCommand.h EChaCtrlType）
  ...
  //	char* szLogName = NULL; //g_LogName.SetLogName( SCreateInfo.ulWorldID,
  //SCreateInfo.szName );
  ...
  // SCreateInfo.SArea.centre.y); 	LG(szLogName, "CtrlType:%d, TeamdID:%u\n",
...
BOOL ReadChaSkillBagPacket(LPRPACKET pk, stNetSkillBag &SCurSkill) {
  T_B memset(&SCurSkill, 0, sizeof(SCurSkill));
  stNetDefaultSkill SDefaultSkill;
  SDefaultSkill.sSkillID = pk.ReadShort();
  SDefaultSkill.Exec();
  ...
  // GetTickCount()); 	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_310)); 	char
  // szRange[256]; 	for (i = 0; i < sSkillNum; i++)
  // 	{
  // 		_snprintf_s(szRange,_countof( szRange ), _TRUNCATE, "%d",
  // pSBag[i].sRange[0]); 		if (pSBag[i].sRange[0] != enumRANGE_TYPE_NONE) 			for
  // (short j = 1; j < defSKILL_RANGE_PARAM_NUM; j++) 				_snprintf_s(szRange +
  // strlen(szRange), 256- strlen(szRange),_TRUNCATE, ",%d",
  // pSBag[i].sRange[j]); 		LG(szLogName,
  // "\t%4d\t%4d\t%4d\t%6d\t%6d\t%6d\t%18d\t%s\n", pSBag[i].sID,
  // pSBag[i].chState, pSBag[i].chLv, pSBag[i].sUseSP, pSBag[i].sUseEndure,
  // pSBag[i].sUseEnergy, pSBag[i].lResumeTime, szRange);
  // 	}
  // 	LG(szLogName, "\n");
  //
...
void ReadChaSkillStatePacket(LPRPACKET pk, stNetSkillState &SCurSState) {
  T_B memset(&SCurSState, 0, sizeof(SCurSState));
  SCurSState.chType = 0;
  ...
  // GetTickCount()); 	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_311)); 	for
  // (char i = 0; i < sNum; i++) 		LG(szLogName, "\t%8d\t%4d\n",
  // SCurSState.SState[i].chID, SCurSState.SState[i].chLv); 	LG(szLogName, "\n");
  //
  T_E
}

void ReadChaAttrPacket(LPRPACKET pk, stNetChaAttr &SChaAttr) {
  T_B memset(&SChaAttr, 0, sizeof(SChaAttr));
  SChaAttr.chType = pk.ReadChar();
  SChaAttr.sNum = pk.ReadShort();
  for (short i = 0; i < SChaAttr.sNum; i++) {
    SChaAttr.SEff[i].lAttrID = pk.ReadChar();
    if (SChaAttr.SEff[i].lAttrID == ATTR_CEXP ||
        SChaAttr.SEff[i].lAttrID == ATTR_NLEXP ||
        SChaAttr.SEff[i].lAttrID == ATTR_CLEXP)
  ...
  //SChaAttr.sNum, SChaAttr.chType, GetTickCount()); 	LG(szLogName,
  //RES_STRING(CL_LANGUAGE_MATCH_312));
  // 	for (short i = 0; i < SChaAttr.sNum; i++)
  // 	{
  // 		LG(szLogName, "\t%d\t%i64\n", SChaAttr.SEff[i].lAttrID,
  // SChaAttr.SEff[i].lVal);
  // 	}
  // 	LG(szLogName, "\n");
  //
  T_E
...
void ReadChaLookPacket(LPRPACKET pk, stNetLookInfo &SLookInfo) {
  T_B memset(&SLookInfo, 0, sizeof(SLookInfo));
  ...
  if (pk.ReadChar() == 1) // 船的外观
  {
    ...
    // SChaPart.sPosID); 		LG(szLogName, "\tPart: Boat:%u, Header:%u, Body:%u,
    // Engine:%u, Cannon:%u, Equipment:%u\n", SChaPart.sBoatID,
    // SChaPart.sHeader, SChaPart.sBody, SChaPart.sEngine, SChaPart.sCannon,
    // SChaPart.sEquipment ); 		LG(szLogName, "\n");
    //
  } else {
    ...
    // SChaPart.sHairID); 		for (int i = 0; i < enumEQUIP_NUM; i++) 			LG(szLogName,
    // "\tLink: %d\n", SChaPart.SLink[i].sID); 		LG(szLogName, "\n");
    //
  }
  T_E
}
...
void ReadChaLookEnergyPacket(LPRPACKET pk, stLookEnergy &SLookEnergy) {
  T_B memset(&SLookEnergy, 0, sizeof(SLookEnergy));
  for (int i = 0; i < enumEQUIP_NUM; i++) {
...
void ReadEntEventPacket(LPRPACKET pk, stNetEvent &SNetEvent) {
  T_B SNetEvent.lEntityID = pk.ReadLong();
...
void ReadChaKitbagPacket(LPRPACKET pk, stNetKitbag &SKitbag) {
  T_B memset(&SKitbag, 0, sizeof(SKitbag));
  ...
  // 	LG(szLogName, "===Recieve(Update
  // Kitbag):\tGridNum:%d\tType:%d\tTick:[%u]\n", SKitbag.nKeybagNum,
  // SKitbag.chType, GetTickCount());
  ...
  while (1) {
    ...
    pItem->sID = pk.ReadShort();
    // 		LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_313),
    // Grid[nGridNum].sGridID, pItem->sID);
    if (pItem->sID > 0) // 存在道具
    {
      pItem->dwDBID = pk.ReadLong();
      pItem->sNum = pk.ReadShort();
      ...
      //			pItem->sEnergy[0]	=	pk.ReadShort();
      //			pItem->sEnergy[1]	=	pk.ReadShort();
      // 			LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_314),
      // pItem->sNum, pItem->sEndure[0], pItem->sEndure[1], pItem->sEnergy[0],
      // pItem->sEnergy[1]);
      pItem->chForgeLv = pk.ReadChar();
      pItem->SetValid(pk.ReadChar() != 0 ? true : false);
      ...
      if (pItemRec == NULL) {
        ...
        _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_315), pItem->sID);
        MessageBox(0, szBuf, "Error", 0);
#ifdef __SOUND__
        /*#ifdef USE_DSOUND
                                        if( g_dwCurMusicID )
                                        {
                                                AudioSDL::get_instance()->stop(
        g_dwCurMusicID ); g_dwCurMusicID = 0; Sleep( 60 );
                                        }
        #endif*/
      ...
      if (pItemRec->sType ==
          enumItemTypeBoat) // 船道具，写入船的WorldID，用于客户端将道具与船角色挂钩
      {
        pItem->SetDBParam(enumITEMDBP_INST_ID, pk.ReadLong());
      }
      ...
      // 			LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_316),
      // pItem->GetDBParam(enumITEMDBP_FORGE));
      if (pk.ReadChar()) // 存在实例属性
      {
        for (int j = 0; j < defITEM_INSTANCE_ATTR_NUM; j++) {
          pItem->sInstAttr[j][0] = pk.ReadShort();
          pItem->sInstAttr[j][1] = pk.ReadShort();
          // 					LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_317),
          // pItem->sInstAttr[j][0], pItem->sInstAttr[j][1]);
        }
      }
    }
    nGridNum++;
    ...
    {
      // 			LG(RES_STRING(CL_LANGUAGE_MATCH_318),
      // RES_STRING(CL_LANGUAGE_MATCH_319), nGridNum,
      // defMAX_KBITEM_NUM_PER_TYPE);
      break;
    }
  }
...
void ReadChaShortcutPacket(LPRPACKET pk, stNetShortCut &SShortcut) {
  T_B memset(&SShortcut, 0, sizeof(SShortcut));
  ...
  for (int i = 0; i < SHORT_CUT_NUM; i++) {
    ...
    // 		LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_321), SShortcut.chType[i],
    // SShortcut.byGridID[i]);
  }
  T_E
}

BOOL PC_PKSilver(LPRPACKET packet) {
  T_B std::string szName;
  long sLevel;
...
BOOL SC_LeaveExpShow(LPRPACKET packet) {
  T_B short sSelectTime = packet.ReadShort();
  ...
  g_stUIStart.RefreshLeftExpName(sSelectTime, baseExp, addExp, allExp, addLev,
                                 addPercent, sNum, amplitude, coin);
  return TRUE;
  T_E
}
...
BOOL SC_LifeSkill(LPRPACKET packet) {
  ...
  case 2: //  锻造
  {
    ...
    g_stUIFound.CheckResult(ret, strVer[0].c_str(), strVer[1].c_str(),
                            strVer[2].c_str());
  } break;
  case 3: //  烹饪
  {
...
BOOL SC_DropLockAsr(LPRPACKET pk) {
  ...
  if (g_yyy_add_lock_item_wait_return_state) {
    ...
    if (success) {
      ...
      // g_pGameApp->MsgBox("该类道具不可加锁！");// modify by ning.yan
      // 2008-11-13
...
BOOL SC_UnlockItemAsr(LPRPACKET pk) {
  ...
  if (g_yyy_add_lock_item_wait_return_state) {
    ...
    // 		{
    // 			char* szLogName = g_LogName.GetMainLogName();
    // 			LG(szLogName, "unlock item answer (file: PacketCmd_SC.cpp, function:
    // SC_UnlockItemAsr ) error! return value: %d.\n", success);
    // 		}
  }
