...
bool CMailMgr::Init() {
  ...
  edtQuestionTitle =
      dynamic_cast<CEdit *>(frmQuestion->Find("edtQuestionTitle"));
  if (!edtQuestionTitle) {
...
void CMailMgr::SubmitQuestion() {
  if (strlen(edtQuestionTitle->GetCaption()) < 16) {
    g_pGameApp->MsgBox(RES_STRING(
        CL_LANGUAGE_MATCH_914)); // 邮件内容不得少于8个汉字（16个英文字）！
    return;
  }
...
void CMailMgr::_evtQuestionFormEvent(CCompent *pSender, int nMsgType, int x,
                                     int y, DWORD dwKey) {
