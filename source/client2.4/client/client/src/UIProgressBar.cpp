...
CProgressBar::CProgressBar(CForm &frmOwn, eStyle style)
    : CCompent(frmOwn), _bActiveMouse(false), _style(style), _pTex(NULL),
      _nFlash(1), _eHintStyle(hsHintPercent), evtMouseDown(NULL),
      evtTimeArrive(NULL), _dwEndTime(0), _dwStartTime(0) {
...
void CProgressBar::RenderHint(int x, int y) {
  ...
  if (_eHintStyle == hsHintNum) {
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%d/%d", _strHint.c_str(),
                (int)_step.GetPosition(), (int)_step.GetRange());
  } else {
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%.02f%%", _strHint.c_str(),
                _step.GetPosition() / _step.GetRange() * 100.0f);
  }
  _RenderHint(buf, x, y);
}
