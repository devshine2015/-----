...
// class CNpcState
//---------------------------------------------------------------------------
CNpcState::CNpcState(CActor *p) : CActionState(p), _pNpc(NULL) {
  _IsSend = true;
  _IsOver = true;
}
...
bool CNpcState::_Start() {
  ...
  _pNpc->FaceTo(pSelf->GetCurX(), pSelf->GetCurY());
...
// class CNpcState
//---------------------------------------------------------------------------
CShopState::CShopState(CActor *p) : CActionState(p), _pShop(NULL) {
  _IsSend = true;
  _IsOver = true;
}
...
// CPickState
//---------------------------------------------------------------------------
CPickState::CPickState(CActor *p) : CActionState(p), _pItem(NULL) {
...
bool CPickState::_Start() {
  ...
  if (GetDistance(GetActor()->GetCha()->GetCurX(),
                  GetActor()->GetCha()->GetCurY(), _pItem->GetCurX(),
                  _pItem->GetCurY()) > defPICKUP_DISTANCE)
    return false;
...
bool CEventState::_Start() {
  ...
  CCharacter *pSelf = GetActor()->GetCha();
  if (GetDistance(pSelf->GetCurX(), pSelf->GetCurY(), _pNode->GetCurX(),
                  _pNode->GetCurY()) > 200)
    return false;
...
// CRepairState
//---------------------------------------------------------------------------
CRepairState::CRepairState(CActor *p) : CActionState(p) {}
...
// CFeedState
//---------------------------------------------------------------------------
CFeedState::CFeedState(CActor *p) : CActionState(p) {}
...
//---------------------------------------------------------------------------
CFeteState::CFeteState(CActor *p) : CActionState(p) {}
