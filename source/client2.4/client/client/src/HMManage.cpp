...
// class CServerHarm
//---------------------------------------------------------------------------
CServerHarm::CServerHarm(CActor *pActor) : _pActor(pActor) {
...
void CServerHarm::ExecAll(int nCount) {
  //	LG( _pCha->getLogName(), "CServerHarm ExecAll(int), FightID[%d],
  //Ready[%d], Size[%d], Count[%d]\n", GetFightID(), _nReadyExec, _harm.size(),
  //nCount );
...
void CServerHarm::ExecAll() {
  //	LG( _pCha->getLogName(), "CServerHarm ExecAll, FightID[%d], Ready[%d],
...
void CServerHarm::Exec() {
  //	LG( _pCha->getLogName(), "CServerHarm Exec, FightID[%d], Ready[%d],
  //Size[%d], IsOuter[%d]\n", GetFightID(), _nReadyExec, _harm.size(), _IsOuter
  //);
  ...
  // 	{
  // 		LG( _pCha->getLogName(), "!!!!!!!!!!!!!!!!!CServerHarm Exec,
  // FightID[%d], Ready[%d], Size[%d], IsOuter[%d]\n", GetFightID(),
  ...
  if (!_IsOuter && _harm.empty() && _nReadyExec <= 0) {
    //		LG( _pCha->getLogName(), "CServerHarm Over, FightID[%d], Ready[%d],
    //Size[%d], IsOuter[%d]\n", GetFightID(), _nReadyExec, _harm.size(),
    //_IsOuter );
  ...
  for (synchros::iterator it = _harm.begin(); it != _harm.end(); it++) {
    ...
    if (pCha) {
      ...
      if (pSkill->sTargetEffectID > 0)
        pCha->SelfEffect(pSkill->sTargetEffectID, pSkill->sTargetDummyLink,
                         false, -1, nAngle);
    }
  }
}
...
void CServerHarm::SetIsOuter(bool v) {
  ...
  //	LG( _pCha->getLogName(), "CServerHarm SetIsOuter, FightID[%d],
  //Ready[%d], Size[%d], IsOuter[%d], OldOuter[%d]\n", GetFightID(),
  //_nReadyExec, _harm.size(), v, _IsOuter );
  ...
  if (!v) {
    ...
    // 有外部执行时,由外部执行结束
    if (_nReadyExec <= 0) {
      //			LG( _pCha->getLogName(), "CServerHarm
      //Over(Outer), FightID[%d]\n", GetFightID() );
      _nFightID = ERROR_FIGHT_ID;
    }
    return;
  }
...
bool CServerHarm::AddHarm(CAttackEffect *s, CSkillRecord *pSkill) {
  if (pSkill != _pSkill) {
    // 		LG( _pCha->getLogName(), "AddHarm skill false, FightID[%d], Ready[%d],
    // Size[%d], IsOuter[%d]\n", GetFightID(), _nReadyExec, _harm.size(),
    // _IsOuter );
    return false;
  }
  ...
  if (!_IsOuter) {
    if (!_pSkill->IsHarmRange()) {
      if (_nReadyExec <= (int)_harm.size()) {
        // 				LG( _pCha->getLogName(), "AddHarm attrange false,
        // FightID[%d], Ready[%d], Size[%d], IsOuter[%d]\n", GetFightID(),
        // _nReadyExec, _harm.size(), _IsOuter );
        return false;
      }
    } else if (_nReadyExec <= 0) {
      // 			LG( _pCha->getLogName(), "AddHarm false,
      // FightID[%d], Ready[%d], Size[%d], IsOuter[%d]\n", GetFightID(),
      // _nReadyExec, _harm.size(), _IsOuter );
      return false;
    }
  ...
  //	LG( _pCha->getLogName(), "AddHarm, FightID[%d], Ready[%d], Size[%d],
  //IsOuter[%d]\n", GetFightID(), _nReadyExec, _harm.size(), _IsOuter );
  return true;
}
