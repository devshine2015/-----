...
bool CBankMgr::Init() //用户银行信息初始化
{
  ...
  grdBank->evtBeforeAccept =
      CUIInterface::_evtDragToGoodsEvent; // 消息的处理 有拖入银行就会调用
...
void CBankMgr::ShowBank() // 显示物品
{
  ...
  _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s%s",
              ...
              RES_STRING(CL_LANGUAGE_MATCH_440)); //显示人物名及专用
...
#define BANK_TYPE 1
  ...
  if (pkItemCmd->GetItemInfo()->GetIsPile() &&
    ...
    return true;
    // char buf[256] = { 0 };
    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,  "您确认放入银行\n[%s]?",
...
bool CBankMgr::PopFromBank(CGoodsGrid &rkDrag, CGoodsGrid &rkSelf, int nGridID,
  ...
  if (pkItemCmd->GetItemInfo()->GetIsPile() &&
      pkItemCmd->GetTotalNum() > 1) { /*取出多个物品*/
    m_pkNumberBox =
        g_stUIBox.ShowNumberBox(_MoveItemsEvent, pkItemCmd->GetTotalNum(),
    ...
    // char buf[256] = { 0 };
    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,  "您确认取出\n[%s]?",
    //pkItemCmd->GetName()); g_stUIBox.ShowSelectBox(_MoveAItemEvent, buf,
    // true); return true;
  }
}
                               ...
                               DWORD dwKey) // 单个道具移动
{
  ...
  g_stUIBank.m_kNetBank.sSrcNum = 1;
  CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_BANK,
                 (void *)&(g_stUIBank.m_kNetBank)); //更新银行信息
}
