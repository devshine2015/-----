...
namespace GUI
{
	...
	void CBankMgr::ShowBank() // 显示物品
	{
		...
		char szBuf[32];
		_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE,  "%s%s", g_stUIBoat.GetHuman()->getName(), RES_STRING(CL_LANGUAGE_MATCH_440));//显示人物名及专用
...
#define BANK_TYPE 1
		...
		if( pkItemRecord->lID == 2520 || pkItemRecord->lID == 2521 || pkItemRecord->lID == 6341 || pkItemRecord->lID == 6343
		 ...
		 // modify by ning.yan 20080820 策划绵羊、李恒、萝卜等提需求，增加一些道具不准存银行
		{
			//g_pGameApp->MsgBox(g_oLangRec.GetString(958));	// "该道具不允许存入银行！请重新选择"
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_958));	// "该道具不允许存入银行！请重新选择"
			return false;
		}
