...
void CTextHint::Render() {
  ...
  if (_IsHeadShadow) {
    ...
    CGuiFont::s_Font.BRender((*it)->font, (*it)->hint.c_str(),
                             _nStartX + (*it)->offx, yy, (*it)->color, color);
    yy += (*it)->height;
    ++it;
  }
  for (; it != _hint.end(); ++it) {
    CGuiFont::s_Font.Render((*it)->font, (*it)->hint.c_str(),
                            _nStartX + (*it)->offx, yy, (*it)->color);
    yy += (*it)->height;
  }
  ...
  for (INT i = 0; i < enumEQUIP_NUM; ++i) {
    ...
    if (_IsHeadShadow) {
      ...
      color = (0xff000000 & color) | ~color;
      CGuiFont::s_Font.BRender((*it)->font, (*it)->hint.c_str(),
                               nStartX + (*it)->offx, yy, (*it)->color, color);
      yy += (*it)->height;
      ++it;
    }
...
void CTextHint::ReadyForHint(int x, int y) {
  ...
  for (int i = 0; i < enumEQUIP_NUM; ++i) {
    ...
    for (hints::iterator it = _hint_related[i].begin();
         it != _hint_related[i].end(); ++it) {
      CGuiFont::s_Font.GetSize((*it)->font, (*it)->hint.c_str(), w, h);
  ...
  for (int i = 0; i < enumEQUIP_NUM; ++i) {
    ...
    int tempoff =
        GetRender().GetScreenHeight() - _nStartY - _nRelatedHintH[i] - 10;
    off = min(off, tempoff);
  }
...
void CTextHint::PushHint(const char *str, DWORD color, int height, int font,
                         int index) {
  if (str && strlen(str) > 0) {
    if (index == -1)
...
void CTextScrollHint::Render() {
  ...
  try {
    ...
    try {
      for (vector<stHint *>::iterator it = _hint.begin(); it != _hint.end();
           it++) {
        stHint *st = *it;
