...
bool DrawBMPFile(RECT rect, LPTSTR pszFile, HDC hDC) {
  ...
  //装载图片
  hBitmap =
      (HBITMAP)LoadImage(NULL, pszFile, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
  ...
  BitBlt(hDC, rect.right - bm.bmWidth, rect.bottom - bm.bmHeight, bm.bmWidth,
         bm.bmHeight, hdcCompatible, 0, 0, SRCCOPY);
...
LRESULT CALLBACK LoadingProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                             LPARAM lParam) {
  static int i = 0;
  switch (uMsg) {
  case WM_INITDIALOG: {
  ...
  case WM_PAINT: {
    ...
    rect.bottom = rc.top + (LONG)(height / 2.0) + 1;
...
unsigned __stdcall LoadingThread(void *param) {
  ...
  if (g_stUISystem.m_sysProp.m_videoProp.bFullScreen) // 伪全屏
  {
  ...
  int nWindowHeight =
      rc.bottom - rc.top; // nScrHeight + nFrameSize + nCaptionSize;

  if (g_stUISystem.m_sysProp.m_videoProp.bFullScreen) {
    load->m_hLoading = CreateWindowEx(WS_EX_TOPMOST, "Loading", "Loading",
                                      WS_POPUP /*& ~WS_TILED*/, 0, 0, nSizeW,
                                      nSizeH, NULL, NULL, g_hInstance, NULL);
  } else {
    load->m_hLoading = CreateWindowEx(
        WS_EX_TOPMOST, "Loading", "Loading", WS_POPUP /*& ~WS_TILED*/,
        // load->m_hLoading =  CreateWindow("Loading", "Loading", WS_POPUP /*&
        // ~WS_TILED*/  ,
        (int)((nSizeW - nWindowWidth) / 2.0),
        (int)((nSizeH - nWindowHeight) / 2.0) + nCaptionSize + nFrameSize - 1,
        nWindowWidth, nWindowHeight, NULL, NULL, g_hInstance, NULL);
  }
  ...
  // SetWindowPos(load->m_hLoading, HWND_TOP, (nSizeW - nWindowWidth) / 2,
  // (nSizeH - nWindowHeight) / 2 + nCaptionSize + nFrameSize - 1, nWindowWidth,
  // nWindowHeight, SWP_NOMOVE| WS_EX_TOPMOST);
...
void GameLoading::Destory() { SAFE_DELETE(_instance); }
...
GameLoading::GameLoading(string param) {
  ...
  m_hLoading = NULL; //变量初始化 -Waiting Add 2009-03-10
  _instance = NULL;  //变量初始化 -Waiting Add 2009-03-10
}
...
void GameLoading::Close() {
  ...
  // if(g_stUISystem.m_sysProp.m_startOption.bFirst)
  {
    //增加m_pMovie指针非空的判断 -Waiting Fix 2009-03-10
    while (m_pMovie && m_pMovie->IsPlaying()) {
      ::Sleep(1);
    }
  }
