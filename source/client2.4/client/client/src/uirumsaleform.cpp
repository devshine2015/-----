...
CRumSaleMgr::CRumSaleMgr()
    : frmRumSale(NULL), labelCount(NULL), labelPrice(NULL), labelTotal(NULL),
      labelTax(NULL) {}
...
bool CRumSaleMgr::Init() {
  ...
  CTextButton *btnDeal03 =
      static_cast<CTextButton *>(frmRumSale->Find("btnDeal11"));
  assert(btnDeal03);
...
void CRumSaleMgr::evtSale(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (CRumDealMgr::GetIndex(false) >
      (g_stRumDealForm.GoldCoins + GameGoldCoins)) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_1020));
    return;
  }

  if (g_stRumDealForm.SellBidPrice * g_stRumDealForm.SellBidCount +
          g_stRumDealForm.GoldCoins >
      2000000000) {
  ...
  CS_RequestSaleAsk(g_stRumDealForm.SellBidPrice, g_stRumDealForm.SellBidCount,
                    0);
