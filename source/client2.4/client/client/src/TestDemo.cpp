...
void CGameApp::MouseContinue(int nButton) { _dwMouseDownTime[nButton]++; }
...
void CGameApp::HandleSuperKey() {
  ...
  try {
    ...
    if (g_pGameApp->IsKeyDown(DIK_C)) {
      ...
      //GetCurScene()->GetMainCha()->IsBoat() ? 1 : 0 );
      //}
    ...
    } else if (g_pGameApp->IsKeyDown(DIK_F3) && g_Config.m_bEditor &&
               g_pGameApp->IsCtrlPress()) {
      // 切换场景 by lh test
      static int type = 0;
      ...
      if (g_pGameApp->IsKeyDown(DIK_Y)) {
        ...
        g_pGameApp->GetDrawPoints()->SetIsEnabled(
            !g_pGameApp->GetDrawPoints()->GetIsEnabled());
...
void CGameApp::HandleContinueSuperKey() {
  ...
  if (IsKeyContinue(DIK_W)) {
    ...
    cha->GetFaceDir(&dir);
    pos += dir * 2.0f;
    ((CWorldScene *)GetCurScene())
        ->GetMouseDown()
        .ActMove(cha, int(pos.x * 100), (int)(pos.y * 100), 0, 1);
    ...
    ((CWorldScene *)GetCurScene())
        ->GetMouseDown()
        .ActBackMove(cha, (int)(pos.x * 100), (int)(pos.y * 100), 0);
  }
...
CTestDemo::CTestDemo() { _skybox = 0; }
CTestDemo::~CTestDemo() { SAFE_RELEASE(_skybox); }
...
void CTestDemo::FrameMove() {
  if (_skybox) {
    drMatrix44 mat = drMatrix44Translate(g_pGameApp->GetMainCam()->m_RefPos);
...
void ReleaseTestDemo() { SAFE_DELETE(g_pTestDemo); }
