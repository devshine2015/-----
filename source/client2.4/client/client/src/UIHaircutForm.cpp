...
namespace GUI {
//~ ==================================================================
CHaircutMgr::CHaircutMgr() { m_pHairTools = new CHairTools; }
...
bool CHaircutMgr::Init() {
  ...
  for (int i(0); i < defHAIR_MAX_ITEM; i++) {
    ...
    if (!cmdProp[i])
      return Error(RES_STRING(CL_LANGUAGE_MATCH_616), frmHaircut->GetName(),
                   szBuf);
  }
  ...
  if (!btnLeftColor) {
    Error(RES_STRING(CL_LANGUAGE_MATCH_616), frmHaircut->GetName(),
          "btnLeftColor");
    return false;
  }
  ...
  if (!btnLeft3d) {
    Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmHaircut->GetName(), "btnLeft3d");
    return false;
  }
  ...
  if (!btnRight3d) {
    Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmHaircut->GetName(),
          "btnRight3d");
    return false;
  }
...
void CHaircutMgr::SwitchMap() { m_pCurrMainCha = 0; }
...
void CHaircutMgr::ShowHaircutForm() {
  ...
  CCharacter *pCha =
      CGameApp::GetCurScene()->AddCharacter(pMainCha->getTypeID());
  if (!pCha) {
...
void CHaircutMgr::Refresh(DWORD dwHairType, DWORD dwHairColor) {
  ...
  for (int i(0); i < defHAIR_MAX_ITEM; i++) {
    ...
    propItem =
        new CItemCommand(pItemRecord, false); // modified guojiangang 20090108
...
void CHaircutMgr::RenderCha(int x, int y) {
  ...
  LEMatrix44 old_mat = *m_pCurrMainCha->GetMatrix();
...
void CHaircutMgr::ChangeHairColor(eDirectType enumDirect) {
  ...
  // 循环移动
  m_dwHairColorIndex =
      (m_dwHairColorIndex + m_dwHairColorMaxNum) % m_dwHairColorMaxNum;
...
void CHaircutMgr::ChangeHairType(eDirectType enumDirect) {
  ...
  // 循环移动
  m_dwHairTypeIndex =
      (m_dwHairTypeIndex + m_dwHairTypeMaxNum) % m_dwHairTypeMaxNum;

  Refresh(m_dwHairTypeIndex, 0);
}

//~ ==================================================================
void CHaircutMgr::_MainMouseHaircutEvent(CCompent *pSender, int nMsgType, int x,
                                         int y, DWORD dwKey) {
...
//~ ==================================================================
void CHaircutMgr::_gui_event_left_color(CGuiData *sender, int x, int y,
                                        DWORD key) {
  g_stUIHaircut.ChangeHairColor(LEFT);
}

//~ ==================================================================
void CHaircutMgr::_gui_event_right_color(CGuiData *sender, int x, int y,
                                         DWORD key) {
  g_stUIHaircut.ChangeHairColor(RIGHT);
}

//~ ==================================================================
void CHaircutMgr::_gui_event_left_hair(CGuiData *sender, int x, int y,
                                       DWORD key) {
  g_stUIHaircut.ChangeHairType(LEFT);
}

//~ ==================================================================
void CHaircutMgr::_gui_event_right_hair(CGuiData *sender, int x, int y,
                                        DWORD key) {
  g_stUIHaircut.ChangeHairType(RIGHT);
}

//~ ==================================================================
void CHaircutMgr::_gui_event_left_rotate(CGuiData *sender, int x, int y,
                                         DWORD key) {
  g_stUIHaircut.RotateCha(LEFT);
}

//~ ==================================================================
void CHaircutMgr::_gui_event_right_rotate(CGuiData *sender, int x, int y,
                                          DWORD key) {
