...
void CSceneObjFile::_Serialize_RBO() {
  if (!filename_RBO.empty()) {
    ...
    // std::set < struct ReallyBigObjectInfo >::iterator itr =
    // RBOinfoList.begin(); std::set < struct ReallyBigObjectInfo >::iterator end
    // = RBOinfoList.end(); while( itr != end )
    //{
    //	file << *(itr++);
    //}
    // fclose ( file );
...
void CSceneObjFile::_Serialize_RBO_ToMap() {
  ...
  while (itr != end) {
    ...
    int sectionX =
        (int)(itr->position.x / (100.f * terrain->GetSectionWidth()));
    int sectionY =
        (int)(itr->position.y / (100.f * terrain->GetSectionHeight()));
...
long CSceneObjFile::Init(_TCHAR *ptcsFileName, bool bSilence) {
  ...
  if (fFile == NULL) {
    if (err_no == ENOENT) // No such file or directory
    {
      if (CreateFile(ptcsFileName) == 0) {
        if (!bSilence) {
          //_stprintf(tcsPrint, _TEXT("%s %s %s"),
          //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_355)), ptcsFileName,
          //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_356)));
          _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s %s"),
                      _TEXT(RES_STRING(CL_LANGUAGE_MATCH_355)), ptcsFileName,
                      _TEXT(RES_STRING(CL_LANGUAGE_MATCH_356)));
          MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)),
                     0);
        }
        lRet = 0;
        goto end;
  ...
  if (m_SFileHead.lFileSize != lFileSize) {
    if (!bSilence) {
      //_stprintf(tcsPrint, _TEXT("%s %s"), ptcsFileName,
      //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_357)));
      _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s"),
                  ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_357)));
  ...
  if (m_SFileHead.lVersion == OBJ_FILE_VER500) //需要版本转换
  {
    ...
    if (fFile == NULL) {
      if (!bSilence) {
        //_stprintf(tcsPrint, _TEXT("%s %s"), ptcsFileName,
        //_TEXT(RES_STRING(CL_LANGUAGE_MATCH_358)));
        _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s"),
                    ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_358)));
        MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
      }
      lRet = 0;
      goto end;
  ...
  if (m_SFileHead.lVersion != OBJ_FILE_VER600) {
    if (!bSilence) {
      _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE, _TEXT("%s %s"),
                  ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_340)));
      MessageBox(NULL, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
    }
    lRet = 0;
    goto end;
  ...
  m_SSectionIndex =
      new (SSectionIndex[m_SFileHead.iSectionCntX * m_SFileHead.iSectionCntY]);
  if (m_SSectionIndex == NULL) {
    lRet = 0;
    goto end;
  }
  fread((void *)m_SSectionIndex, sizeof(SSectionIndex),
        m_SFileHead.iSectionCntX * m_SFileHead.iSectionCntY, fFile);
  m_fRdWr = fFile;
  fFile = NULL;
  ...
  // 		{
  // 			_snprintf_s(tcsPrint,sizeof(tcsPrint) ,_TRUNCATE , _TEXT("%s %s
  // (%d)"), ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_339)),
  // GetLastError()); 			MessageBox(NULL, tcsPrint,
  ...
  // 		{
  // 			_snprintf_s(tcsPrint,sizeof(tcsPrint) ,_TRUNCATE , _TEXT("%s %s"),
  // ptcsFileName, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_359))); 			MessageBox(NULL,
  // tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
  // 		}
  // 		lRet = 0;
  // 		goto end;
  // 	}
...
long CSceneObjFile::CreateFile(_TCHAR *ptcsFileName, int iSectionCntX,
                               int iSectionCntY, int iSectionWidth,
  ...
  strncpy_s(SHead.tcsTitle, sizeof(SHead.tcsTitle), _TEXT("HF Object File!"),
            _TRUNCATE);
  SHead.lVersion = OBJ_FILE_VER600;
  SHead.lFileSize =
      sizeof(SFileHead) + sizeof(SSectionIndex) * iSectionCntX * iSectionCntY;
  ...
  memset(lFileSectionIndex, 0,
         sizeof(SSectionIndex) * iSectionCntX * iSectionCntY);

  fwrite((const void *)&SHead, sizeof(SFileHead), 1, fFile);
  fwrite((const void *)lFileSectionIndex, sizeof(SSectionIndex),
         iSectionCntX * iSectionCntY, fFile);
...
long CSceneObjFile::ConvertObjFileVer(
    _TCHAR *ptcsFileName,
    bool
        bBackUp) // 从版本500转换到600(物件的绝对坐标转换成相对section的相对坐标)
{
  long lRet = 2;
  ...
  _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE,
              _TEXT(RES_STRING(CL_LANGUAGE_MATCH_360)), ptcsFileName);
  ...
  fread((void *)pSSectionIndex, sizeof(SSectionIndex), lMaxSectionNum,
        fFileOld);
  fseek(fFileNew, sizeof(SFileHead) + sizeof(SSectionIndex) * lMaxSectionNum,
        SEEK_SET);

  int nSectionX, nSectionY;
  for (i = 0;
       i < lMaxSectionNum && ulFileSize >= (unsigned long)ftell(fFileOld);
       i++) {
    if (pSSectionIndex[i].iObjNum > 0) //存在有效数据
    {
      fseek(fFileOld, pSSectionIndex[i].lObjInfoPos, SEEK_SET);
      fread(pSObjInfo, sizeof(SSceneObjInfo) * SHead.iSectionObjNum, 1,
            fFileOld);
      ...
      fwrite(pSObjInfo, sizeof(SSceneObjInfo) * SHead.iSectionObjNum, 1,
             fFileNew);
    } else {
  ...
  fwrite((const void *)pSSectionIndex, sizeof(SSectionIndex), lMaxSectionNum,
         fFileNew);
  ...
  if (lRet == 2) {
    _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE,
                _TEXT(RES_STRING(CL_LANGUAGE_MATCH_362)), ptcsFileName,
                tcsBackUpName);
...
long CSceneObjFile::ReadSectionObjInfo(int nSectionNO, SSceneObjInfo *SSceneObj,
                                       long *lSectionObjNum) {
  if (!m_bInitSuccess)
    return 0;
  ...
  if ((*lSectionObjNum = m_SSectionIndex[nSectionNO].iObjNum) > 0) {
    ...
    //      m_SFileHead.iSectionCntX, nSectionNO / m_SFileHead.iSectionCntX,
    //      m_SSectionIndex[nSectionNO].lObjInfoPos);
    fread(SSceneObj, sizeof(SSceneObjInfo), m_SSectionIndex[nSectionNO].iObjNum,
          m_fRdWr);
    // 转换成绝对坐标
    int nSectionX, nSectionY;
    for (int i = 0; i < m_SSectionIndex[nSectionNO].iObjNum; i++) {
      nSectionX = nSectionNO % m_SFileHead.iSectionCntX *
                  m_SFileHead.iSectionWidth * 100;
      nSectionY = nSectionNO / m_SFileHead.iSectionCntX *
...
long CSceneObjFile::WriteSectionObjInfo(int nSectionNO,
                                        SSceneObjInfo *SSceneObj,
                                        long lSectionObjNum) {
  //	FILE	*fFile;
  ...
  if (lSectionObjNum > 0) {
    if (m_SSectionIndex[nSectionNO].lObjInfoPos > 0) {
      ...
      m_SFileHead.lFileSize +=
          sizeof(SSceneObjInfo) * m_SFileHead.iSectionObjNum;
    ...
    // 写入obj信息
    if (result == S_OK) {
      fwrite((const void *)SSceneObj, sizeof(SSceneObjInfo),
             m_SFileHead.iSectionObjNum, m_fRdWr);
      fflush(m_fRdWr);
    }
  } else {
  ...
  result =
      fseek(m_fRdWr, sizeof(SFileHead) + sizeof(SSectionIndex) * nSectionNO,
            SEEK_SET);
  if (result == S_OK) {
    fwrite((const void *)(m_SSectionIndex + nSectionNO), sizeof(SSectionIndex),
           1, m_fRdWr);
    fflush(m_fRdWr);
  }
  ...
  if (result == S_OK) {
    ...
    if (lFileSize != m_SFileHead.lFileSize) {
      MessageBox(NULL, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_365)),
                 _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
    }
  }
  //
...
long CSceneObjFile::TrimFile(_TCHAR *ptcsFileName, bool bBackUp) {
  ...
  for (i = 0;
    ...
    {
      fseek(fFileOld, pSSectionIndex[i].lObjInfoPos, SEEK_SET);
      fread(pszSectionInfo, sizeof(SSceneObjInfo) * SHead.iSectionObjNum, 1,
            fFileOld);
      pSSectionIndex[i].lObjInfoPos = ftell(fFileNew);
      fwrite(pszSectionInfo, sizeof(SSceneObjInfo) * SHead.iSectionObjNum, 1,
             fFileNew);
    } else {
      pSSectionIndex[i].lObjInfoPos = 0;
    }
...
long CSceneObjFile::TrimDirectory(_TCHAR *ptcsDirectory, bool bBackUp) {
  ...
  if (bBackUp)
    _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE,
                _TEXT(RES_STRING(CL_LANGUAGE_MATCH_366)), ptcsDirectory);
  else
    _snprintf_s(tcsPrint, sizeof(tcsPrint), _TRUNCATE,
                _TEXT(RES_STRING(CL_LANGUAGE_MATCH_367)), ptcsDirectory);
  if (IDYES != MessageBox(NULL, tcsPrint,
                          _TEXT(RES_STRING(CL_LANGUAGE_MATCH_361)), MB_YESNO))
    return 1;
  ...
  _snprintf_s(tcsFileName, sizeof(tcsFileName), _TRUNCATE, _TEXT("%s%s"),
              tcsPath, ptcsRecordFile);
  if (g_Config.m_bEditor)
