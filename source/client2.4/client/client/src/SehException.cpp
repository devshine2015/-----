...
#pragma warning(disable : 4312) // warning C4312: 'type cast' : conversion from
                                // 'DWORD' to 'void *' of greater size
...
void SehExceptionBase::TransFunc(unsigned int u, EXCEPTION_POINTERS *pExp) {
  ...
  case EXCEPTION_ACCESS_VIOLATION:
    throw SehExceptionAccessViolation(pExp, msg);
    break;
  case EXCEPTION_INT_DIVIDE_BY_ZERO:
    throw SehExceptionDivideByZero(pExp, msg);
    break;
  case EXCEPTION_INVALID_HANDLE:
    throw SehExceptionInvalidHandle(pExp, msg);
    break;
  default:
    throw SehExceptionBase(pExp, msg);
    break;
  }
}
...
LONG WINAPI UnExcept(
    EXCEPTION_POINTERS
        *pExceptionPointers) ///< Pointer to information about the exception
{
...
string SehExceptionBase::GetRegisterInfo() {
  ...
  _snprintf_s(tim, sizeof(tim), _TRUNCATE, "\
   ...
   ESI=0x%08x  EDI=0x%08x  CTF=0x%08x\n\n",
              m_ContextRecord.Eip, m_ContextRecord.Ebp, m_ContextRecord.Esp,
              m_ContextRecord.EFlags, m_ContextRecord.Eax, m_ContextRecord.Ebx,
              m_ContextRecord.Ecx, m_ContextRecord.Edx, m_ContextRecord.Esi,
              m_ContextRecord.Edi, m_ContextRecord.ContextFlags);
  str += tim;
