...
bool CUIGuildList::Init() {
  ...
  FORM_CONTROL_LOADING_CHECK(m_pGuildList, m_pGuildListForm, CListView,
                             "npc.clu", "lstAsk");
  m_pGuildList->GetList()->evtSelectChange = OnSelectChange;
  CTextButton *btn;
  FORM_CONTROL_LOADING_CHECK(btn, m_pGuildListForm, CTextButton, "npc.clu",
                             "btnName");
  ...
  FORM_CONTROL_LOADING_CHECK(m_pGuildApplyBtn, m_pGuildListForm, CTextButton,
                             "npc.clu", "btnApply");
  m_pGuildApplyBtn->SetIsEnabled(false);
  m_pGuildApplyBtn->evtMouseClick = OnClickApply;
  FORM_CONTROL_LOADING_CHECK(m_pGuildNavyLab, m_pGuildListForm, CLabelEx,
                             "npc.clu", "labInfoJ");
  FORM_CONTROL_LOADING_CHECK(m_pGuildNavyImg, m_pGuildListForm, CImage,
                             "npc.clu", "imgName1");
  // FORM_CONTROL_LOADING_CHECK(m_pGuildNavyImg1,m_pGuildListForm,CImage,"npc.clu","imgbackJ");
  // FORM_CONTROL_LOADING_CHECK(m_pGuildNavyImg2,m_pGuildListForm,CImage,"npc.clu","imgTitleJ");
  // // 修改成 Label 了
  FORM_CONTROL_LOADING_CHECK(m_pGuildNavyImg3, m_pGuildListForm, CImage,
                             "npc.clu", "imgBakAskH");
  FORM_CONTROL_LOADING_CHECK(m_pGuildNavyImg4, m_pGuildListForm, CImage,
                             "npc.clu", "imgBakAskH1");
  FORM_CONTROL_LOADING_CHECK(m_pGuildPirateImg, m_pGuildListForm, CImage,
                             "npc.clu", "imgName2");
  // FORM_CONTROL_LOADING_CHECK(m_pGuildPirateImg1,m_pGuildListForm,CImage,"npc.clu","imgbackD");
  // FORM_CONTROL_LOADING_CHECK(m_pGuildPirateImg2,m_pGuildListForm,CImage,"npc.clu","imgTitleD");
  // // 修改成 Label 了
  FORM_CONTROL_LOADING_CHECK(m_pGuildPirateImg3, m_pGuildListForm, CImage,
                             "npc.clu", "imgBakAsk");
  FORM_CONTROL_LOADING_CHECK(m_pGuildPirateImg4, m_pGuildListForm, CImage,
                             "npc.clu", "imgBakAsk1");

  FORM_CONTROL_LOADING_CHECK(m_lblGuildNavyTitle, m_pGuildListForm, CLabelEx,
                             "npc.clu", "labTitleJ");
  FORM_CONTROL_LOADING_CHECK(m_lblGuildPirateTitle, m_pGuildListForm, CLabelEx,
                             "npc.clu", "labTitleD");
...
void CUIGuildList::ShowGuildList() {
  ...
  for (DWORD i = 0; i < CGuildListMgr::GetTotalGuilds(); i++) {
    ...
    CItem *pGuildNameItem =
        new CItem(pListData->GetGuildName().c_str(), COLOR_BLACK);
    CItem *pGuildMottoNameItem =
        new CItem(pListData->GetGuildMottoName().c_str(), COLOR_BLACK);
    //	CItem*	pGuildMemberCountItem=new
    //CItem(itoa(pListData->GetMembers(),buf,10),COLOR_BLACK);
    _itoa_s(pListData->GetMembers(), buf, sizeof(buf), 10);
    CItem *pGuildMemberCountItem = new CItem(buf, COLOR_BLACK);
    // CItem*	pGuildExperienceItem=new
    // CItem(_i64toa(pListData->GetExperence(),buf,10),COLOR_BLACK);
    _i64toa_s(pListData->GetExperence(), buf, sizeof(buf), 10);
    CItem *pGuildExperienceItem = new CItem(buf, COLOR_BLACK);
    CItem *pGuildMasterItem =
        new CItem(pListData->GetGuildMasterName().c_str(), COLOR_BLACK);
...
void CUIGuildList::OnClickSortByGuildName(CGuiData *pSender, int x, int y,
                                          DWORD key) {
...
void CUIGuildList::OnClickSortByGuildMemberCount(CGuiData *pSender, int x,
                                                 int y, DWORD key) {
...
void CUIGuildList::OnClickSortByGuildExperience(CGuiData *pSender, int x, int y,
                                                DWORD key) {
...
void CUIGuildList::OnClickApply(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (!pItemRow)
    return;
  CGuildListData *pListData =
      static_cast<CGuildListData *>(pItemRow->GetPointer());
  if (!pListData)
    return;
...
void CUIGuildList::OnMsgReplaceApplySelectClick(CCompent *pSender, int nMsgType,
