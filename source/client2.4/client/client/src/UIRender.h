...
public:
	...
	void RenderTextureRect(int nX, int nY, LETexRect *pRect);

	void RenderTextureAbsRect(int nX, int nY, LETexRect *pRect);
...
extern bool UIGetChooseColor( DWORD &c, DWORD* custcolor );					// 颜色对话框,c为默认颜色,同时也作为执行成功时的返回值
