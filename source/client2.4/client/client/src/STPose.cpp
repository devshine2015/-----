...
CInsertState::CInsertState(CActor *p)
    : CActionState(p), _nAngle(DEFAULT_NUM), _IsPlayPose(false), _IsFirst(true),
      _eAngle(enumInit) {}
...
// class CEquipState
//---------------------------------------------------------------------------
CEquipState::CEquipState(CActor *p) : CActionState(p), _eType(enumInit) {
...
bool CEquipState::_Start() {
  ...
  switch (_eType) {
  case enumUseItem:
    CS_BeginAction(GetActor()->GetCha(), enumACTION_ITEM_USE,
                   (void *)_pUseItem);
    ...
    break;
  case enumUnfix:
    CS_BeginAction(GetActor()->GetCha(), enumACTION_ITEM_UNFIX,
                   (void *)_pUnfix);
    break;
  }
  return true;
