...
#define GETPROC(_x)                                                            \
  CheckBassFunction(*((unsigned int *)&_x) =                                   \
                        (unsigned int)GetProcAddress(mModule, #_x),            \
                    #_x)
  ...
  *((unsigned int *)&BASS_SetGlobalVolumes) =
      (unsigned int)GetProcAddress(mModule, "BASS_SetGlobalVolumes");
  *((unsigned int *)&BASS_SetVolume) =
      (unsigned int)GetProcAddress(mModule, "BASS_SetVolume");

  if ((BASS_SetVolume == NULL) && (BASS_SetGlobalVolumes == NULL)) {
    MessageBox(
        NULL,
        "Neither BASS_SetGlobalVolumes or BASS_SetVolume found in bass.dll",
        "Error", MB_OK | MB_ICONERROR);
    exit(0);
  }

  *((unsigned int *)&BASS_SetConfig) =
      (unsigned int)GetProcAddress(mModule, "BASS_SetConfig");
