...
void CItemReMgr::ItemResetResponse(bool Success) {
  ...
  if (!Success) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_988));
...
void CItemReMgr::evtRequirementEvent(CGuiData *pSender, CCommandObj *pItem,
                                     bool &isAccept) {
  isAccept = false;
  ...
  if (pCmd->nTag != 999) {
    ...
    if (pCmd == cmdRequirement[0]) {
      if (pItemRecord->sType == 59)
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_985));
        g_pGameApp->MsgBox(buf);
      }
      ...
      if (pItemRecord->sType == 27 || pItemRecord->sType <= 24)
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_986));
        g_pGameApp->MsgBox(buf);
      }
      ...
      if (pItemRecord->sType == 80)
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_987));
        g_pGameApp->MsgBox(buf);
      }
    }
  }
