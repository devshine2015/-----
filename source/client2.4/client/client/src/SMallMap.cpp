...
//#ifdef MGR
 ...
 //   drISurfaceStreamMgr* surmgr = g_Render.GetInterfaceMgr()->res_mgr->GetSurfaceStreamMgr();
...
#ifdef MGR
	//LEIResourceMgr* res_mgr = g_Render.GetInterfaceMgr()->res_mgr;
	if(FAILED(drLoadTex(&_pTexMask,res_mgr, "texture\\minimap\\mapmask.bmp", 0, D3DFMT_A4R4G4B4)))
	{
		LG("ERROR","msgmapmask.bmp");
	}
	if(FAILED(drLoadTex(&_pTexDefault,res_mgr, "texture\\minimap\\mapsea.bmp", 0, D3DFMT_A4R4G4B4)))
	{
...
void	CSMallMap2D::RenderScene()
{
	...
	for( m = 0; m < 3; m++)
	{
		for(int n = 0; n < 3; n++)
		{
			_snprintf_s( filename, _countof( filename ), _TRUNCATE,"texture\\minimap\\%s\\sm%d%d.bmp",
...
void	CBigMap::Create()
{
	...
	for (int m = 0; m < 3; ++m)
	{
		for (int n = 0; n < 4; ++n)
		{
			_snprintf_s( filename, _countof( filename ), _TRUNCATE,"texture\\bigmap\\%d%d.bmp",m , n+1);
...
void	CMinimap::InitScene()
{
	...
	//	LG("ERROR","msgmapmask.bmp");
	//}
	if(FAILED(drLoadTex(&_pTexDefault,res_mgr, "texture\\ui\\minimap\\mapsea.bmp", 0, D3DFMT_A4R4G4B4)))
	{
		LG("ERROR","msgmapsea.bmp");
	}
	...
	if(m_pScene)
	{
		...
		char filename[128];
		_snprintf_s( filename, _countof( filename ), _TRUNCATE,"texture\\minimap\\%s\\%s.pk",
	...
	for( m = 0; m < VIEWRANGE; m++)
	{
		...
		for(int n = 0; n < VIEWRANGE; n++)
		{
			...
			}else
			{
				if(!_pMiniPack)
				{
					_snprintf_s( filename, _countof( filename ), _TRUNCATE,"texture\\minimap\\%s\\sm_%d_%d.bmp",
						m_pScene->GetTerrainName(),_sx + n,_sy + m);
...
void	CLargerMap::InitScene()
{
	...
	//if(FAILED(drLoadTex(&_pTexMask,res_mgr, "texture\\ui\\minimap\\mask.tga", 0, D3DFMT_A4R4G4B4)))
	//{
...
void	CLargerMap::RenderScene()
{
	...
	//sprintf_s(psz,RES_STRING(CL_LANGUAGE_MATCH_394), _nCenterX/100, _nCenterY/100, fInf );
	_snprintf_s(psz, _countof( psz ), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_394), _nCenterX/100, _nCenterY/100);
