...
bool CSMallWnd::Create(LPDIRECT3DDEVICE8 pDev, RECT rcwnd, CGameScene *pScene,
                       int bksize) {
  m_pDev = pDev;
  _rcWnd = rcwnd;
  ...
  //	info.width = texsize;
  //	info.height = texsize;
  //	if(FAILED(hr = drLoadTex(&_pCurSuf,res_mgr,&info)))
  //#else
  //	if(FAILED(hr = m_pDev->CreateTexture( texsize, texsize, 1,
  //D3DUSAGE_RENDERTARGET , 		_BufParam.Format, D3DPOOL_DEFAULT, &_pCurSuf ) ))
  //#endif
  //	{
  //		IP("CreateTexture hr=%d", hr);
  ...
  //   drISurfaceStreamMgr* surmgr =
  //   g_Render.GetInterfaceMgr()->res_mgr->GetSurfaceStreamMgr();
  //   surmgr->CreateDepthStencilSurface(&handle,texsize, texsize, depth_fmt, \
 //       D3DMULTISAMPLE_NONE/*d3dcp->present_param.MultiSampleType*/, 0,
  //       false, 0);
...
////		_pCurSuf->GetTex()->GetSurfaceLevel(0,&surface);
////		if(FAILED(hr =
///m_pDev->SetRenderTarget(surface,NULL/*_pDSSuf->GetSurface()*/))) /        {
////            int i = 0;
////        }
////#else
...
void CSMallWnd::MoveTo(D3DXVECTOR3 &vPos, D3DXVECTOR3 &vCameraDir,
                       float fCameraAngle, float fCameradDist, int iAngle) {
...
void CSMallMap2D::InitScene() {
  ...
  mi.subset_num = 1;
  mi.vertex_num = 4;
  mi.vertex_seq = LE_NEW(LEVector3[mi.vertex_num]);
  mi.blend_seq = LE_NEW(drBlendInfo[mi.vertex_num]);
  mi.vercol_seq = LE_NEW(DWORD[mi.vertex_num]);
  mi.texcoord0_seq = LE_NEW(LEVector2[mi.vertex_num]);
  mi.texcoord1_seq = LE_NEW(LEVector2[mi.vertex_num]);
  mi.subset_seq = DR_NEW(drSubsetInfo[mi.subset_num]);

  mi.vertex_seq[0] = LEVector3(0, 0, 0);
  mi.vertex_seq[1] = LEVector3(0, 0, 0);
  mi.vertex_seq[2] = LEVector3(0, 0, 0);
  mi.vertex_seq[3] = LEVector3(0, 0, 0);
  ...
  mi.texcoord0_seq[0] = LEVector2(0, 0);
  mi.texcoord0_seq[1] = LEVector2(1, 0);
  mi.texcoord0_seq[2] = LEVector2(1, 1);
  mi.texcoord0_seq[3] = LEVector2(0, 1);

  mi.texcoord1_seq[0] = LEVector2(0, 0);
  mi.texcoord1_seq[1] = LEVector2(1, 0);
  mi.texcoord1_seq[2] = LEVector2(1, 1);
  mi.texcoord1_seq[3] = LEVector2(0, 1);
  ...
  mi.vertex_seq[0] = LEVector3(float(_rcWnd.left), float(_rcWnd.top), 0.9f);
  mi.vertex_seq[1] = LEVector3(float(_rcWnd.right), float(_rcWnd.top), 0.9f);
  mi.vertex_seq[2] = LEVector3(float(_rcWnd.right), float(_rcWnd.bottom), 0.9f);
  mi.vertex_seq[3] = LEVector3(float(_rcWnd.left), float(_rcWnd.bottom), 0.9f);
  ...
  (*pVers++).SetValue(
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.top), 0.9f, 1), 0xffffffff,
      vUV[0]);
  (*pVers++).SetValue(
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.top), 0.9f, 1), 0xffffffff,
      vUV[1]);
  (*pVers++).SetValue(
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.bottom), 0.9f, 1),
      0xffffffff, vUV[2]);
  (*pVers++).SetValue(
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.bottom), 0.9f, 1),
      0xffffffff, vUV[3]);
  _pVBWnd->Unlock();
#endif
  ...
  if (ResMgr.m_bUseSoft)
    ...
    //	{
    //		D3DVSD_STREAM( 0 ),
    //			D3DVSD_REG( D3DVSDE_POSITION ,		D3DVSDT_FLOAT3 ), // Position of
    //first mesh 			D3DVSD_REG( D3DVSDE_BLENDWEIGHT,	D3DVSDT_FLOAT1),
    //			D3DVSD_REG( D3DVSDE_DIFFUSE,		D3DVSDT_D3DCOLOR ), //
    //diffuse 			D3DVSD_REG( D3DVSDE_TEXCOORD0,		D3DVSDT_FLOAT2 ), // Tex
    //coords 			D3DVSD_END()
    //	};
    //	if(_dwVShader != 0)
    //	{
    ...
    //		_dwVShader = 0;
    //	}
    //	sprintf_s(t_Path,"shader\\minimap.vsh");
...
#ifdef MGR
  // LEIResourceMgr* res_mgr = g_Render.GetInterfaceMgr()->res_mgr;
  if (FAILED(drLoadTex(&_pTexMask, res_mgr, "texture\\minimap\\mapmask.bmp", 0,
                       D3DFMT_A4R4G4B4))) {
    LG("ERROR", "msgmapmask.bmp");
  }
  if (FAILED(drLoadTex(&_pTexDefault, res_mgr, "texture\\minimap\\mapsea.bmp",
                       0, D3DFMT_A4R4G4B4))) {
...
void CSMallMap2D::RenderScene() {
  ...
  for (m = 0; m < 3; m++) {
    for (int n = 0; n < 3; n++) {
...
#ifdef MGR
          drLoadTex(&_pTex[n][m].pTex, g_Render.GetInterfaceMgr()->res_mgr,
      ...
      if (_dwVShader == NULL) {
        M2D_VER *pVertices;
#ifdef MGR
        drILockableStreamVB *_lpSVB = _pVB->GetLockableStreamVB();
        ...
        (*pVertices++).m_vPos =
            D3DXVECTOR4(float(sx) + SHOWRSIZE, float(sy), 0, 1);
        (*pVertices++).m_vPos =
            D3DXVECTOR4(float(sx) + SHOWRSIZE, float(sy) + SHOWRSIZE, 0, 1);
        (*pVertices++).m_vPos =
            D3DXVECTOR4(float(sx), float(sy) + SHOWRSIZE, 0, 1);
#ifdef MGR
        _lpSVB->Unlock();
#else
        _pVB->Unlock();
        ...
        g_Render.SetVertexShaderConstant(
            9, &D3DXVECTOR4(float(sx), float(sy), 0, 1), 1);
        g_Render.SetVertexShaderConstant(
            10, &D3DXVECTOR4(float(sx) + SHOWRSIZE, float(sy), 0, 1), 1);
        ...
        g_Render.SetVertexShaderConstant(
            ...
            1);
        g_Render.SetVertexShaderConstant(
            12, &D3DXVECTOR4(float(sx), float(sy) + SHOWRSIZE, 0, 1), 1);
      }
#ifdef MGR
      _pVB->DrawSubset(0);
#else
    ...
    for (m = 0; m < m_pScene->GetChaCnt(); m++) {
      ...
      if (pCha && pCha->IsInMiniMap() && pCha != m_pScene->GetMainCha()) {
        if (PosInRange(pCha->GetPos(), _vEyePt, SHOWRSIZE)) {
          ...
          // g_Render.SetTextureStageState( 0, D3DTSS_MAGFILTER,
          // D3DTEXF_LINEAR); g_Render.SetTextureStageState( 0, D3DTSS_MINFILTER,
          // D3DTEXF_LINEAR);

          if (pCha->IsNPC()) {
            // g_Render.SetTextureStageState( 0, D3DTSS_MAGFILTER,
            // D3DTEXF_POINT); g_Render.SetTextureStageState( 0,
  ...
  // g_Render.LookAt(g_pGameApp->GetMainCam()->m_EyePos,
  // g_pGameApp->GetMainCam()->m_RefPos);
...
CAniWnd::~CAniWnd() { SAFE_RELEASE(_pVBWnd); }
...
void CAniWnd::MoveWnd(int x, int y) {
  ...
  M2D_AVER *pVers;
#ifdef MGR
  drILockableStreamVB *_lpSVB = _pVBWnd->GetLockableStreamVB();
  _lpSVB->Lock(0, 0, (void **)&pVers, 0);
#else
  _pVBWnd->Lock(0, 0, (BYTE **)&pVers, D3DLOCK_DISCARD);
#endif
  _vWndVer[0].SetValue(
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.top), 0.9f, 1), 0x80ffffff,
      vUV[0]);
  _vWndVer[0].SetValue(
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.top), 0.9f, 1), 0x80ffffff,
      vUV[1]);
  _vWndVer[0].SetValue(
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.bottom), 0.9f, 1),
      0x80ffffff, vUV[2]);
...
void CAniWnd::RenderScene() {
  ...
  g_Render.GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 4, &_vVertex,
                                        sizeof(ClockVer));
}
...
CAniClock::~CAniClock() { SAFE_RELEASE(_pVBWnd); }
...
bool CAniClock::Create(int width, DWORD dwColor) {
  ...
  _vVertex[0].vPos = D3DXVECTOR4(float(_rcWnd.left + len / 2),
                                 float(_rcWnd.top + hei / 2), 0.9f, 1);
  _vVertex[1].vPos =
      D3DXVECTOR4(_vVertex[0].vPos.x, float(_rcWnd.top), 0.9f, 1);
  _vVertex[2].vPos =
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.top), 0.9f, 1);
  _vVertex[3].vPos =
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.top + hei / 2), 0.9f, 1);
  _vVertex[4].vPos =
      D3DXVECTOR4(float(_rcWnd.right), float(_rcWnd.bottom), 0.9f, 1);
  _vVertex[5].vPos =
      D3DXVECTOR4(_vVertex[0].vPos.x, float(_rcWnd.bottom), 0.9f, 1);
  _vVertex[6].vPos =
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.bottom), 0.9f, 1);
  _vVertex[7].vPos =
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.top + hei / 2), 0.9f, 1);
  _vVertex[8].vPos =
      D3DXVECTOR4(float(_rcWnd.left), float(_rcWnd.top), 0.9f, 1);
  _vVertex[9].vPos =
      D3DXVECTOR4(_vVertex[0].vPos.x, float(_rcWnd.top), 0.9f, 1);
...
void CAniClock::FrameMove(DWORD dwDailTime) {
  ...
  } else if (_fCurAngle >= 3.141592f + 0.7853981f &&
             _fCurAngle < 3.141592f + 1.570796f) {
    flerp = (_fCurAngle - (3.141592f + 0.7853981f)) / 0.7853981f;
  ...
  } else if (_fCurAngle >= 3.141592f + 1.570796f &&
             _fCurAngle < 6.283185f - 0.7853981f) {
...
void CAniClock::Render(int x, int y) {
  ...
  g_Render.GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 8, &_vTempVer,
                                        sizeof(ClockVer));
}
//////////////////////////////////////////////////////////////////////////
...
void CCharacter2D::Create(RECT rc) {
  ...
  //_vWndVer[0].m_vPos =
  //D3DXVECTOR4(float(_rcWnd.left),float(_rcWnd.top),0.9f,1); _vWndVer[1].m_vPos
  //= D3DXVECTOR4(float(_rcWnd.right),float(_rcWnd.top),0.9f,1);
  //_vWndVer[2].m_vPos =
  //D3DXVECTOR4(float(_rcWnd.right),float(_rcWnd.bottom),0.9f,1);
  //_vWndVer[3].m_vPos =
...
void CCharacter2D::Render() {
  ...
  ////drMatrix44Scale( _pModel->GetMatrix(), 1.0f / (float)1 );
...
void CCharacter2D::LoadCha(DWORD dwID) {
  ...
  if (pInfo->chModalType == enumMODAL_MAIN_CHA) {
    ...
    if (((CCharacterModel *)pCha)->LoadTower(pInfo->chModalType, part_buf) ==
        0) {
      return;
    }
  } else {
    ...
    _snprintf_s(load_info.bone, _countof(load_info.bone), _TRUNCATE, "%04d.lab",
                pInfo->sModel);
    ...
    for (DWORD i = 0; i < 5; i++) {
      ...
      _snprintf_s(load_info.part[i], _countof(load_info.part[i]), _TRUNCATE,
                  "%010d.lgo", file_id);
    }
...
void CCharacter2D::UpdataFace(stNetTeamChaPart &stPart) {
  ...
  int nHairID = stPart.SLink[enumEQUIP_HEAD].sID != 0
                    ...
                    : stPart.sHairID;
  int nSelfID = _pChaPart->SLink[enumEQUIP_HEAD].sID != 0
                    ? _pChaPart->SLink[enumEQUIP_HEAD].sID
                    : _pChaPart->sHairID;
  if (nHairID != nSelfID) {
    _pModel->ChangePart(0, nHairID);
  }

  // const int nMaxPart = 3;
  // for( int i=1; i<nMaxPart; i++ )
  //{
  //    if( stPart.SLink[i].sID!=_pChaPart->SLink[i].sID )
  //        _pModel->ChangePart( i, stPart.SLink[i].sID );
  //}
...
CBigMap::~CBigMap() { Destory(); }

void CBigMap::Create() {
  CGameScene *pScene = g_pGameApp->GetCurScene();
  LETerrain *pTerr = pScene->GetTerrain();
  if (!pTerr)
    return;
  ...
  _pTex = new LEITex *[4 * 3];

  char filename[128];
  for (int m = 0; m < 3; ++m) {
    for (int n = 0; n < 4; ++n) {
      _snprintf_s(filename, _countof(filename), _TRUNCATE,
                  "texture\\bigmap\\%d%d.bmp", m, n + 1);
      drLoadTex(&_pTex[m * 4 + n], g_Render.GetInterfaceMgr()->res_mgr,
                filename, 0, D3DFMT_A4R4G4B4);
    }
  }
}
...
void CBigMap::Render() {
  ...
  for (int m = 0; m < 3; ++m) {
    for (int n = 0; n < 4; ++n) {
      ...
      g_Render.GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, &_vWndVer,
                                            sizeof(M2D_AVER));
    }
  }
...
Ctemp::~Ctemp() { SAFE_RELEASE(_pTex); }

void Ctemp::Render() {
  if (!_pTex) {
    //_pTex = CGuiFont::s_Font.GetFont(0)->GetTexture();
    ////g_pGameApp->GetFont()->GetTexture();

    drLoadTex(&_pTex, g_Render.GetInterfaceMgr()->res_mgr,
...
void CMinimap::InitScene() {
  ...
  //	LG("ERROR","msgmapmask.bmp");
  //}
  if (FAILED(drLoadTex(&_pTexDefault, res_mgr,
                       "texture\\ui\\minimap\\mapsea.bmp", 0,
                       D3DFMT_A4R4G4B4))) {
    LG("ERROR", "msgmapsea.bmp");
  }
  ...
  if (m_pScene) {
    ...
    char filename[128];
    _snprintf_s(filename, _countof(filename), _TRUNCATE,
                "texture\\minimap\\%s\\%s.pk", m_pScene->GetTerrainName(),
                m_pScene->GetTerrainName());
...
void CMinimap::RenderScene() {
  ...
  //		{
  //			_snprintf_s( filename, _countof( filename ), _TRUNCATE,
  //,"texture\\minimap\\%s\\sm%d%d.bmp", 				m_pScene->GetTerrainName(),_sx + n,_sy
  //+ m);
  //
  //			if(_access(filename,0)== -1)
  //			{
  ...
  //			}else
  //			{
  //				if((_pTex[n][m].x != _sx + n) || (_pTex[n][m].y != _sy +
  //m))
  //				{
  //					_pTex[n][m].ReleaseTex();
  //
  ...
  //				}
  //				if(_pTex[n][m].pTex)
  //					g_Render.SetTexture(0,
  //_pTex[n][m].pTex->GetTex()); 				else 					g_Render.SetTexture(0, NULL);
  //			}
  //			rc.left =  tempx + n * 64;
  //			rc.right =  rc.left + 64 - 1;
  //
  //			_vPicVer[0].m_vPos =
  //D3DXVECTOR4(float(rc.left),float(rc.top),0.9f,1); 			_vPicVer[1].m_vPos =
  //D3DXVECTOR4(float(rc.right),float(rc.top),0.9f,1); 			_vPicVer[2].m_vPos =
  //D3DXVECTOR4(float(rc.right),float(rc.bottom),0.9f,1); 			_vPicVer[3].m_vPos =
  //D3DXVECTOR4(float(rc.left),float(rc.bottom),0.9f,1);
  //
  ...
  for (m = 0; m < VIEWRANGE; m++) {
    ...
    for (int n = 0; n < VIEWRANGE; n++) {
      if (_sx + n < 0 || _sy + m < 0) {
        ...
        if (!_pMiniPack) {
          _snprintf_s(filename, _countof(filename), _TRUNCATE,
                      "texture\\minimap\\%s\\sm_%d_%d.bmp",
                      m_pScene->GetTerrainName(), _sx + n, _sy + m);
      ...
      _vPicVer[0].m_vPos =
          D3DXVECTOR4(float(rct.left), float(rct.top), 0.9f, 1);
      _vPicVer[1].m_vPos =
          D3DXVECTOR4(float(rct.right), float(rct.top), 0.9f, 1);
      _vPicVer[2].m_vPos =
          D3DXVECTOR4(float(rct.right), float(rct.bottom), 0.9f, 1);
      _vPicVer[3].m_vPos =
          D3DXVECTOR4(float(rct.left), float(rct.bottom), 0.9f, 1);

      g_Render.GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, &_vPicVer,
                                            sizeof(M2D_AVER));
    }
  }
  ...
  for (m = 0; m < nChaCnt; m++) {
    ...
    if (pCha && pCha->IsInMiniMap() && pCha != pMainCha) {
      if (PosInRange(pCha->GetPos(), _vEyePt, SHOWRSIZE)) {
        ...
        _cSmNpc.setPos(pCha->GetCurX(), pCha->GetCurY(), tnx, tny, _rcWnd.left,
                       _rcWnd.top, lenw);
  ...
  //////////////////////////////////////////////////////////////////////////
  //_vCameraTar[0] = D3DXVECTOR4(float(lenw + _rcWnd.left), float(lenw + 24 +
  //_rcWnd.top),0,1);

  // g_Render.SetTexture(0, NULL);
  // D3DXMATRIX matIden;
  // GetMatrixRotation(&matIden,&D3DXVECTOR3(float(lenw +
  // _rcWnd.left),float(lenw + _rcWnd.top),0),\
	//	&D3DXVECTOR3(0,0,1),_fCameraAngle);
  // D3DXVec4Transform(&_vCameraTar[0], &_vCameraTar[0], &matIden);

  //_vCameraTar[1] = D3DXVECTOR4(float(lenw / 2 + 6 + _rcWnd.left),float(lenw /
  //2 + 6 + _rcWnd.top),0,1); D3DXVec4Transform(&_vCameraTar[1],
  // &_vCameraTar[1], &matIden);
  ...
  //_vCameraTar[1] = D3DXVECTOR4(float(lenw  + 26 + _rcWnd.left),float(lenw / 2
  //+ 6 + _rcWnd.top),0,1); D3DXVec4Transform(&_vCameraTar[1], &_vCameraTar[1],
  // &matIden);
...
void CLargerMap::InitScene() {
  ...
  // if(FAILED(drLoadTex(&_pTexMask,res_mgr, "texture\\ui\\minimap\\mask.tga",
  // 0, D3DFMT_A4R4G4B4)))
  //{
  ...
  //_pTex = new LEITex*[_TexWnum * _TexHnum];
  ...
  std::vector<DWORD>::pointer pt = &vecColor[0];
  memset(pt, 0, sizeof(DWORD) * (_TexWnum + 1) * (_TexHnum + 1));
...
void CLargerMap::Update(int x, int y) {
  ...
  if (y < 0)
    y = 0;
  if (x > defMAP_GARNER_WIDTH * 100)
    x = defMAP_GARNER_WIDTH * 100;
  if (y > defMAP_GARNER_WIDTH * 100)
    y = defMAP_GARNER_WIDTH * 100;
  ...
  float sxue =
      ((float)(tnx - sx * (SHOWRSIZE * 100))) / (float(SHOWRSIZE * 100));
  float syue =
      ((float)(tny - sy * (SHOWRSIZE * 100))) / (float(SHOWRSIZE * 100));
  ...
  int tempx =
      (lenw) -
      ((_TexWnum / 2 * TILE_SIZE) +
       int(sxue *
           float(TILE_SIZE))); // int(( -HALF_TILE * 100 - sxue) / 100 * 1.6f) ;
  int tempy =
      (lenh) -
      ((_TexHnum / 2 * TILE_SIZE) +
       int(syue *
           float(TILE_SIZE))); // int(( -HALF_TILE * 100 - syue) / 100 * 1.6f) ;
  ...
  ZeroMemory(pt, sizeof(DWORD) * vecColor.size());
  for (m = 0; m < _TexHnum; m++) {
        ...
        if (!_pMiniPack) {
          ...
          if (!_pTex[idx] || strcmp(_pTex[idx]->GetFileName(), filename) != 0) {
            if (_access(filename, 0) == -1) {
              ...
              drLoadTex(&_pTex[idx], g_Render.GetInterfaceMgr()->res_mgr,
                        filename, 0, D3DFMT_A4R4G4B4);
            }
          }
        } else {
  ...
  if (CMaskData::g_MaskData) {
    ...
    fInf /=
        float((CMaskData::g_MaskData->iNumX) * (CMaskData::g_MaskData->iNumY));
    fInf *= 100;
    // id1 = (int)fInf * 100;
...
void CLargerMap::RenderScene() {
  ...
  if (g_pGameApp->GetMouseKey() & M_RDown) {
    Update(_nInitX, _nInitY);
  } else if (g_pGameApp->IsMouseContinue(0) ||
             (g_pGameApp->GetMouseKey() & M_LDown)) {
  ...
  for (m = 0; m < _TexHnum; m++) {
    for (int n = 0; n < _TexWnum; n++) {
      ...
      _vPicVer[0].m_vPos =
          D3DXVECTOR4(float(prc->left), float(prc->top), 0.9f, 1);
      _vPicVer[1].m_vPos =
          D3DXVECTOR4(float(prc->right), float(prc->top), 0.9f, 1);
      _vPicVer[2].m_vPos =
          D3DXVECTOR4(float(prc->right), float(prc->bottom), 0.9f, 1);
      _vPicVer[3].m_vPos =
          D3DXVECTOR4(float(prc->left), float(prc->bottom), 0.9f, 1);
  ...
  CLEFont *pfont = g_pGameApp->GetFont();
  if (pfont) {
    pfont->DrawTextShadow((char *)strMapName.c_str(), _rcWnd.left + 60,
                          _rcWnd.top + 30, _rcWnd.left + 61, _rcWnd.top + 31,
                          0xff808000, 0xffffff00);
  }

  static char psz[128] = {0};
  // Modify by sunny.sun 20080904 绵羊要求去掉探索度
  // sprintf_s(psz,RES_STRING(CL_LANGUAGE_MATCH_394), _nCenterX/100,
  // _nCenterY/100, fInf );
  _snprintf_s(psz, _countof(psz), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_394),
              _nCenterX / 100, _nCenterY / 100);

  pfont->DrawTextShadow(psz, _rcWnd.left + 350, _rcWnd.top + 30,
                        _rcWnd.left + 351, _rcWnd.top + 31, 0xff808000,
                        0xffffff00);
}
