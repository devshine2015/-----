...
int CImageBMP::GetWidth(void) { return _nWidth; }

int CImageBMP::GetHeight(void) { return _nHeight; }

DWORD CImageBMP::GetColor(int x, int y) {
  static BYTE BitMask[] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};

  if (_pData) {
    //  位置 = 数据起始  + ( 转换对应BMP的行  *    每一行的占用的字节数   ) +
    //  列偏移字节
    int nPos =
        _nBmpData + (_nHeight - y - 1) * (((_nWidth >> 5) + 1) << 2) + (x >> 3);
...
CNumAnswerMgr::~CNumAnswerMgr() { SAFE_RELEASE(_pNumTexture); }
...
bool CNumAnswerMgr::Init() {
  ...
  ui3dCheckCode =
      dynamic_cast<C3DCompent *>(frmNumAnswer->Find("ui3dCheckCode"));
  if (!ui3dCheckCode)
    return false;
  ...
  if (FAILED(g_Render.GetDevice()->CreateTexture(
          256, 256, 1, 0, D3DFMT_R5G6B5, D3DPOOL_MANAGED, &_pNumTexture))) {
...
void CNumAnswerMgr::FrameMove(DWORD dwTime) {
  ...
  if (frmNumAnswer && frmNumAnswer->GetIsShow()) {
    if (frmNumAnswer->GetBottom() > frmFast->GetTop()) {
      int nLeft = (frmFast->GetWidth() - frmNumAnswer->GetWidth()) / 2 +
                  frmFast->GetLeft();
    ...
    if (_dwTickCount) {
      ...
      char szTime[64] = {0};
      _snprintf_s(szTime, _countof(szTime), _TRUNCATE, "%d", nSecond);
...
void CNumAnswerMgr::RenderCheckCode(void) {
  if (_pNumTexture && frmNumAnswer && ui3dCheckCode) {
    ...
    for (int i = 0; i < NUMBER_COUNT; ++i) {
      ...
      rc.bottom = _imgBMP[i].GetHeight() > rc.bottom ? _imgBMP[i].GetHeight()
                                                     : rc.bottom;
    }
...
void CNumAnswerMgr::_evtCheckCodeMouseEvent(CCompent *pSender, int nMsgType,
...
void CNumAnswerMgr::_evtCheckCodeRenderEvent(C3DCompent *pSender, int x,
                                             int y) {
