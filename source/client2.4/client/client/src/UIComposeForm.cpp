...
namespace GUI {
char *g_prompt[] = {"Thankyou", "Abysmal",  "Terrible", "Bad",
...
void CComposeMgr::CheckResult(short sRet, const char *txt) {
  ...
  if (sRet > 0) {
    pItemCommand =
        dynamic_cast<CItemCommand *>(g_stUIEquip.GetGoodsGrid()->GetItem(sRet));
    if (pItemCommand) {
  ...
  /*if(g_stUIEquip.GetGoodsGrid()->GetItem(iComposePos[1]))
  {
          for(int i = 0; i < (COMPOSE_COUNT - 1); i++)
          {
                  ...
                  {
                          pItemCommand =
  dynamic_cast<CItemCommand*>(g_stUIEquip.GetGoodsGrid()->GetItem(iComposePos[i]));
                          if(pItemCommand)
                          {
                                  if(3 == i)
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUICompose.cmdCompose[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_STR);
                                          if(pItemCommand->GetItemInfo()->lID ==
  iItem)
                                          {
                                                  PushItem(i, *pItemCommand,
  true);
                                          }
                                  }
                                  else if(4 == i)
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUICompose.cmdCompose[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_CON);
                                          if(pItemCommand->GetItemInfo()->lID ==
  iItem)
                                          {
                                                  PushItem(i, *pItemCommand,
  true);
                                  ...
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUICompose.cmdCompose[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_DEX);
                                          if(pItemCommand->GetItemInfo()->lID ==
  iItem)
                                          {
                                                  PushItem(i, *pItemCommand,
  ...
  if ((sRet > 0) && (lTimes > 0)) {
    char buff[16] = {0};
    _snprintf_s(buff, _countof(buff), _TRUNCATE, "%li", lTimes);
    edtNumInput->SetCaption(buff);
    // CS_LifeSkill(0, g_stUINpcTalk.GetNpcId());
    CS_Compose(g_stUINpcTalk.GetNpcId(), g_stUICompose.iComposePos,
               (COMPOSE_COUNT - 1), true);
  } else {
...
void CComposeMgr::DragToEquipGrid(int index) { PopItem(index); }
...
void CComposeMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
  ...
  CItemCommand *pItemCommand =
      dynamic_cast<CItemCommand *>(cmdCompose[iIndex]->GetCommand());
  if (!pItemCommand)
    return;
...
void CComposeMgr::PushItem(int iIndex, CItemCommand &rItem,
                           bool bRetry /* = false*/) {
  if (iIndex < 0 || iIndex >= COMPOSE_COUNT)
    return;
...
void CComposeMgr::SetComposeUI() {
  //      CItemCommand* pItem1 =
  //      dynamic_cast<CItemCommand*>(cmdCompose[0]->GetCommand());
  // CItemCommand* pItem2 =
  // dynamic_cast<CItemCommand*>(cmdCompose[1]->GetCommand());
  //      CItemCommand* pItem3 =
  //      dynamic_cast<CItemCommand*>(cmdCompose[2]->GetCommand());
  //      CItemCommand* pItem4 =
  //      dynamic_cast<CItemCommand*>(cmdCompose[3]->GetCommand());
  //      CItemCommand* pItem5 =
  //      dynamic_cast<CItemCommand*>(cmdCompose[4]->GetCommand());
  //      CItemCommand* pItem6 =
  //      dynamic_cast<CItemCommand*>(cmdCompose[5]->GetCommand());

  // if ((NULL == pItem1) || (NULL == pItem2) || (NULL == pItem3) || (NULL ==
  // pItem4) || (NULL == pItem5) || (NULL == pItem6))
  //{
  //	btnForgeYes->SetIsEnabled(false);
  //	return;
  //}
...
void CComposeMgr::_evtbtnForgeYes(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
  ...
  if (szName == "btnForgeYes") {
    ...
    if (g_stUICompose.lTimes > 1) {
      ...
      //	   g_stUICompose.edtNumInput->SetCaption(itoa(g_stUICompose.lTimes,
      //buff, 10));
      _itoa_s(g_stUICompose.lTimes, buff, sizeof(buff), 10);
      g_stUICompose.edtNumInput->SetCaption(buff);
    }
  } else if (szName == "btnRightPage") {
    char buff[16] = {0};
    ++g_stUICompose.lTimes;
    //	 g_stUICompose.edtNumInput->SetCaption(itoa(g_stUICompose.lTimes, buff,
    //10));
    _itoa_s(g_stUICompose.lTimes, buff, sizeof(buff), 10);
    g_stUICompose.edtNumInput->SetCaption(buff);
  }
...
void CComposeMgr::_evtDragItemBase1(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
...
void CComposeMgr::_evtDragItemBase2(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
  CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(pItem);
  if (!pItemCommand || !pItemCommand->GetIsValid())
    return;
...
void CComposeMgr::_evtDragItemBase3(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
  if (!g_stUICompose.cmdCompose[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
    return;
...
void CComposeMgr::_evtDragItemBase4(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
  //  材料1
  if (!g_stUICompose.cmdCompose[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
  ...
  if (!pItemCommand || !pItemCommand->GetIsValid())
    return;
  int iItem =
      (dynamic_cast<CItemCommand *>(g_stUICompose.cmdCompose[1]->GetCommand()))
          ->GetData()
          .GetInstAttr(ITEMATTR_VAL_STR);
...
void CComposeMgr::_evtDragItemBase5(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
  //  材料2
  if (!g_stUICompose.cmdCompose[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
...
void CComposeMgr::_evtDragItemBase6(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isAccept) {
  //  材料3
  if (!g_stUICompose.cmdCompose[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
