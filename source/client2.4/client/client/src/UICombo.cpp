...
CCombo::CCombo(CForm &frmOwn)
    : CCompent(frmOwn), _pEdit(new CEdit(frmOwn)), _pList(new CList(frmOwn)),
      _pButton(new CTextButton(frmOwn)), _eListStyle(lsDown),
      evtSelectChange(NULL), evtMouseClick(NULL) {
  _IsFocus = false;
  _SetSelf();
}

CCombo::CCombo(const CCombo &rhs)
    : CCompent(rhs), _pEdit(new CEdit(*rhs._pEdit)),
      _pList(new CList(*rhs._pList)), _pButton(new CTextButton(*rhs._pButton)),
