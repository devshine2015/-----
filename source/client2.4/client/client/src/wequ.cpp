...
paramlist::paramlist(void) { elm = NULL; }
...
static REAL bands[] = {(REAL)65.406392, (REAL)92.498606, (REAL)130.81278,
                       (REAL)184.99721, (REAL)261.62557, (REAL)369.99442,
                       (REAL)523.25113, (REAL)739.9884,  (REAL)1046.5023,
                       (REAL)1479.9768, (REAL)2093.0045, (REAL)2959.9536,
                       (REAL)4186.0091, (REAL)5919.9072, (REAL)8372.0181,
                       (REAL)11839.814, (REAL)16744.036};
...
static REAL alpha(REAL a) {
  if (a <= 21)
    return 0;
  if (a <= 50)
    return (REAL)(0.5842 * pow(a - 21.0, 0.4) + 0.07886 * (a - 21));
...
static REAL hn(int n, paramlist &param2, REAL fs) {
  ...
  for (e = param2.elm->next; e->next != NULL && e->upper < fs / 2;
       e = e->next) {
...
void process_param(REAL *bc, paramlist *param, paramlist &param2, REAL fs,
                   int ch) {
...
void equ_makeTable(REAL *lbc, REAL *rbc, paramlist *param, REAL fs) {
  ...
  for (i = 0; i < winlen; i++)
    ...
    irest[i] =
        hn(i - winlen / 2, param2, fs) * win((REAL)(i - winlen / 2), winlen);
...
unsigned int equ_latency() { return tabsize - 4098; }
    ...
    for (ch = 0; ch < nch; ch++) {
      ...
      if (weq_enable) {
        ...
        for (i = 1; i < tabsize / 2; i++) {
          ...
          re = ires[i * 2] * fsamples[i * 2] -
               ires[i * 2 + 1] * fsamples[i * 2 + 1];
          im = ires[i * 2 + 1] * fsamples[i * 2] +
               ires[i * 2] * fsamples[i * 2 + 1];
...
void bitrv216(REAL *a) {
  REAL x1r, x1i, x2r, x2i, x3r, x3i, x4r, x4i, x5r, x5i, x7r, x7i, x8r, x8i,
...
void cftf1st(int n, REAL *a, REAL *w) {
  ...
  REAL x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r, y1i, y2r, y2i,
      y3r, y3i;
...
void cftf161(REAL *a, REAL *w) {
  REAL wn4r, wk1r, wk1i, x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r,
...
void cftf162(REAL *a, REAL *w) {
  REAL wn4r, wk1r, wk1i, wk2r, wk2i, wk3r, wk3i, x0r, x0i, x1r, x1i, x2r, x2i,
...
void cftf081(REAL *a, REAL *w) {
  REAL wn4r, x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r, y1i, y2r,
...
void cftf082(REAL *a, REAL *w) {
  REAL wn4r, wk1r, wk1i, x0r, x0i, x1r, x1i, y0r, y0i, y1r, y1i, y2r, y2i, y3r,
...
void rfft(int n, int isign, REAL x[]) {
  ...
  // newipsize = 2+sqrtf(n/2);
  newipsize = 2 + (int)sqrtf((float)(n / 2));
  if (newipsize > ipsize) {
    if (ip != NULL)
      LocalFree(ip);
