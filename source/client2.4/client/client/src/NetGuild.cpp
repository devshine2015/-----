...
void NetMC_LISTGUILD_BEGIN(uChar gldtype) {
  ...
  CGuildListMgr::SetType((gldtype == 0) ? CGuildData::navy
                                        : CGuildData::pirate);
}
...
void NetMC_LISTGUILD(uLong id, cChar *name, cChar *motto, cChar *leadername,
                     uShort memtotal, LLong exp) {
  CGuildListMgr::AddGuild(
      new CGuildListData(id, name, motto, leadername, memtotal, exp));
}
...
void NetMC_LISTTRYPLAYER_BEGIN(uLong gldid, cChar *gldname, cChar *motto,
                               char type, char stat, cChar *ldrname,
                               uShort memnum, uShort maxmem, LLong money,
                               LLong exp, uLong remain) {
...
void NetMC_LISTTRYPLAYER_END() { CUIGuildMgr::ShowForm(); }

void NetMC_LISTTRYPLAYER(uLong chaid, cChar *chaname, cChar *job,
                         uShort degree) {
...
void NetPC_GUILD_START(bool online, uLong chaid, cChar *chaname, cChar *motto,
                       cChar *job, uShort degree, uShort icon,
                       uShort permission) {
...
void NetPC_GUILD_ADD(bool online, uLong chaid, cChar *chaname, cChar *motto,
                     cChar *job, uShort degree, uShort icon,
                     uShort permission) {
...
void NetMC_GUILD_INFO(DWORD dwCharID, DWORD dwGuildID, const char szGuildName[],
                      const char szGuildMotto[]) {
  //	const char* pszLogName = g_LogName.GetLogName( dwCharID );
  //	LG( pszLogName, "Guild Info:%u, Name:%s, Motto:%s\n", dwGuildID,
  //szGuildName, szGuildMotto  );
...
void NetGuildInvite(const char *inviter_name, unsigned long inviter_chaid,
                    unsigned short icon) {
  CCharacter *mCha = CGameScene::GetMainCha();
  if (mCha->getGuildID() <= 0) {
    g_stGuildInviteFormMgr.AddInviteForm(inviter_chaid, inviter_name);
  }
}

void NetGuildCancel(unsigned long inviter_chaid, char reason) {
  switch (reason) {
  case MSG_GUILD_CANCLE_BUSY:
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_1031));
    break;
  case MSG_GUILD_CANCLE_TIMEOUT:
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_1032));
    break;
  case MSG_GUILD_CANCLE_OFFLINE:
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_1033));
    break;
  case MSG_GUILD_CANCLE_SELF_ISFULL:
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_1034));
    break;
  case MSG_GUILD_CANCLE_CANCEL:
    // g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_1035));
    break;
  }
  g_stGuildInviteFormMgr.RemoveInviteForm(inviter_chaid);
}