...
CMemo::CMemo(CForm &frmOwn)
    : CCompent(frmOwn), _IsTextCenter(true), _color(0xff000000),
      _nItemRowNum(0), _nMisRowNum(0), evtSelectChange(NULL), _nMaxNum(40),
      _nRowNum(0), _nFirst(0), _nLast(0), _nPageShowNum(4), _bIsHaveItem(false),
      _bIsHaveMis(false), _nLeftMargin(0), _nTopMargin(4), _nRightMargin(4),
      _nBottomMargin(4), _nRowHeight(20), _nSelectItem(-1), _nSelectMis(-1),
      _nMemoNum(0) {
  int i = 0;
...
CMemo::CMemo(const CMemo &rhs)
    : CCompent(rhs), _color(rhs._color), _strCaption(rhs._strCaption),
      _IsTextCenter(true) {
...
void CMemo::AddItemRowContent(int row, const char *szFunc,
                              const char *szItemEx) {
  if (!szFunc) {
...
void CMemo::ProcessCaption() {
    ...
    if (s[i] & 0x80) {
      ...
      if ((int)strlen(strTemp.c_str()) > _nMaxNum) {
        while ((int)strlen(strTemp.c_str()) > _nMaxNum) {
          ...
          strTemp = strTemp.substr(_str[_nRowNum].length(),
                                   strTemp.length() - _str[_nRowNum].length());
          _nRowNum += 1;
        }
      } else {
  ...
  else if (!bNotFlag) { // modify by ning.yan 20081006 NPC对话框断字处理 begin
    UnicodeString str = _strCaption.c_str();
    int32_t lineStarts[20];
    int32_t lineEnds[20];

    const char *resName =
        CResourceBundleManage::Instance()->GetLocale().getName();
    int maxNum = _nMaxNum;
    if (strcmp(resName, "zh_Hans_CN") == 0 ||
        strcmp(resName, "zh_Hant_TW") == 0 ||
        strcmp(resName, "zh_Hant_HK") == 0)
      maxNum = _nMaxNum / 2;

    int32_t maxLines = str.length() / maxNum + 1;

    int32_t lineNum = CBreakLine::WrapParagraph(
        str, CResourceBundleManage::Instance()->GetLocale(), maxNum, lineStarts,
        lineEnds, maxLines);
    char lineBuf[200];
    for (_nRowNum = 0; _nRowNum < lineNum; _nRowNum++) {
      UnicodeString string(str, lineStarts[_nRowNum],
                           lineEnds[_nRowNum] - lineStarts[_nRowNum]);
      string.extract(0, string.length(), lineBuf, sizeof(lineBuf) - 1, 0);
      lineBuf[sizeof(lineBuf) - 1] = 0;
      _str[_nRowNum] = lineBuf;
    }
...
void CMemo::Render() {
  ...
  for (i = _nFirst; i < _nLast; i++) {
    if (i >= _nRowInfo[0][0] && i <= _nRowInfo[0][1]) {
      CGuiFont::s_Font.Render((char *)_str[i].c_str(), GetX() + _nLeftMargin,
                              GetY() + _nTopMargin +
                                  _nRowHeight * (i - _nFirst),
                              0xFF000000); // 0xFFFFFFFF );
    } else if (i >= _nRowInfo[1][0] && i <= _nRowInfo[1][1]) {
      if (i == _nSelectMis + _nRowInfo[1][0]) {
        int nPosX = GetX() + _nLeftMargin + 12;
        int nPosY = GetY() + _nTopMargin + _nRowHeight * (i - _nFirst);

        // TODO
        if (((i - _nRowInfo[1][0]) < _files.size()) &&
            _files[i - _nRowInfo[1][0]].pGraph && _str[i].length() > 0)
          _files[i - _nRowInfo[1][0]].pGraph->Render(nPosX, nPosY - 3);

        CGuiFont::s_Font.Render((char *)_str[i].c_str(), nPosX + 12, nPosY,
                                0xFFFF00FF);
      } else {
        int nPosX = GetX() + _nLeftMargin + 12;
        int nPosY = GetY() + _nTopMargin + _nRowHeight * (i - _nFirst);

        if (_files.size() > (i - _nRowInfo[1][0]) &&
            _files[i - _nRowInfo[1][0]].pGraph && _str[i].length() > 0)
          _files[i - _nRowInfo[1][0]].pGraph->Render(nPosX, nPosY - 2);

        CGuiFont::s_Font.Render((char *)_str[i].c_str(), nPosX + 12, nPosY + 1,
                                0xFFcF00cF);
      }
    }
    if (i >= _nRowInfo[2][0] && i <= _nRowInfo[2][1]) {
      if (i == _nSelectItem + _nRowInfo[2][0]) {
        ...
        int nPosY = GetY() + _nTopMargin + _nRowHeight * (i - _nFirst);
        CGuiFont::s_Font.Render((char *)_str[i].c_str(), nPosX + 24, nPosY,
                                0xFF0000FF);
      } else {
        int nPosX = GetX() + _nLeftMargin;
        int nPosY = GetY() + _nTopMargin + _nRowHeight * (i - _nFirst);
        CGuiFont::s_Font.Render((char *)_str[i].c_str(), nPosX + 24, nPosY + 1,
                                0xFF0000aF);
      }
    }
  }
...
bool CMemo::MouseRun(int x, int y, DWORD key) {
  ...
  if (InRect(x, y)) {
    ...
    } else if (_bIsHaveItem && nRow >= _nRowInfo[2][0] &&
               nRow <= _nRowInfo[2][1]) {
...
void CMemo::_SetScrollRange() { _pScroll->Refresh(); }
...
CMemoEx::CMemoEx(CForm &frmOwn)
    : CCompent(frmOwn), _color(0xff000000), evtSelectChange(NULL), _nMaxNum(-1),
      _nRowNum(0), _nFirst(0), _nLast(0), _nPageShowNum(4), _nLeftMargin(0),
      _nTopMargin(4), _nRightMargin(4), _nBottomMargin(4), _nRowHeight(14),
      _nTitleHeight(24), _nIconHeight(18), _nPhraseNum(0), evtClickItem(NULL) {
  _pScroll = new CScroll(frmOwn);
  _SetSelf();
...
CMemoEx::CMemoEx(const CMemoEx &rhs) : CCompent(rhs), _color(rhs._color) {
  _pScroll = new CScroll(*rhs._pScroll);
  _nLeftMargin = rhs._nLeftMargin;
  _nTopMargin = rhs._nTopMargin;
  _nRightMargin = rhs._nRightMargin;
...
void CMemoEx::_Copy(const CMemoEx &rhs) { _SetSelf(); }
...
BOOL CMemoEx::SelPrizeItem(int nxPos, int nyPos, DWORD dwKey) {
  ...
  for (int i = 0; i < m_sNumInfo; i++) {
    ...
    if (m_MemoInfo[i].byType == MEMO_LINE_TITLE) {
      // CGuiFont::s_Font.Render( 2, m_MemoInfo[i].strDesp.c_str(), x, y,
      // m_MemoInfo[i].dwColor );
      nInterval++;
    } else if (m_MemoInfo[i].byType == MEMO_LINE_DESP) {
      // CGuiFont::s_Font.Render( m_MemoInfo[i].strDesp.c_str(), x, y,
      // m_MemoInfo[i].dwColor );
    } else if (m_MemoInfo[i].byType == MEMO_LINE_INTERVAL) {
      nInterval += 2;
    } else if (m_MemoInfo[i].byType == MEMO_LINE_ICON) {
      // m_MemoInfo[i].pIcon->Render( x, y );
      if (nxPos >= x - 2 && nxPos <= x + 25 * nWidth && nyPos >= y &&
          nyPos <= y + 32) {
...
int CMemoEx::SelMemInfo(int x, int y, DWORD key) {
  ...
  for (int i = 0; i < m_sNumInfo; i++) {
    ...
    startY =
        (m_MemoInfo[i].syPos - _nFirst) * _nRowHeight + nPosY + nInterval * 2;

    if (m_MemoInfo[i].byType == MEMO_LINE_TITLE) {
      nInterval++;
    } else if (m_MemoInfo[i].byType == MEMO_LINE_DESP) {
      //	if( startX < x && ( x < startX + nWidth *
      //m_MemoInfo[i].strDesp.length()) && startY < y && ( y < startY + nHeight
      //))
      if (startX < x &&
          (x < startX + nWidth * (int)m_MemoInfo[i].strDesp.length()) &&
          startY < y && (y < startY + nHeight)) {
        return i;
      }
    } else if (m_MemoInfo[i].byType == MEMO_LINE_INTERVAL) {
      nInterval += 2;
...
void CMemoEx::Render() {
  ...
  // 裁剪显示区
  GetRender().SetClipRect(nPosX, nPosY,
                          (_nMaxNum == -1 ? 32 : _nMaxNum) * nWidth,
                          (_nPageShowNum + 1) * nHeight);
  ...
  for (int i = 0; i < m_sNumInfo; i++) {
    ...
    if (m_MemoInfo[i].byType == MEMO_LINE_TITLE) {
      ...
      // CGuiFont::s_Font.Draw( m_MemoInfo[i].strDesp.c_str(), x, y,
      // m_MemoInfo[i].dwColor );
    } else if (m_MemoInfo[i].byType == MEMO_LINE_INTERVAL) {
      nInterval += 2;
    } else if (m_MemoInfo[i].byType == MEMO_LINE_ICON) {
...
void CMemoEx::ParseMisPage() {
  // USHORT sMaxCom = 32;
  USHORT sMaxCom = _nMaxNum == -1 ? g_Config.maxNumPerLine
                                  : _nMaxNum; //	读配置文件里的设置。
  USHORT sRow = 0, sCom = 0;
  DWORD dwColor = 0xFF000000; // 黑色

  // 任务名称标题
  if (m_PageInfo.szName[0] != 0) {
  ...
  ParseScript(pszData, m_sNumInfo, sRow, sCom, 0, sMaxCom, 0xFF000000,
              m_MemoInfo, MEMO_MAX_LINE);
    ...
    for (int i = 0; i < m_PageInfo.byNeedNum; i++) {
      ...
      if (m_PageInfo.MisNeed[i].byType == mission::MIS_NEED_ITEM) {
        ...
        if (!pItem) {
          ...
          _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE, "0x%X",
                      m_PageInfo.MisNeed[i].wParam1);
          strTemp += szNumber;
          strTemp += "]>";
        } else {
        ...
        szNumber[0] = 0;
        _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE, "<r%d/%d>",
                    m_PageInfo.MisNeed[i].wParam3,
                    m_PageInfo.MisNeed[i].wParam2);
      ...
      strncpy_s(szNeed, 255, strTemp.c_str(), _TRUNCATE);
      ParseScript(szNeed, m_sNumInfo, sRow, sCom, 0, sMaxCom, 0xFF000000,
                  m_MemoInfo, MEMO_MAX_LINE);
  ...
  if (m_PageInfo.byPrizeNum > 0) {
    ...
    for (int n = 0; n < m_PageInfo.byPrizeNum; n++) {
      ...
      strncpy_s(szItem, sizeof(szItem), "./texture/ui/system/error.tga",
                _TRUNCATE);
      if (m_PageInfo.MisPrize[n].byType == mission::MIS_PRIZE_ITEM) {
        ...
        if (pItem) {
          ...
          strncpy_s(szItem, sizeof(szItem), pItem->GetIconFile(), _TRUNCATE);
        }
        ...
        if (pItem) {
          ...
          _snprintf_s(szItem, _countof(szItem), _TRUNCATE,
                      RES_STRING(CL_LANGUAGE_MATCH_713),
                      m_PageInfo.MisPrize[n].wParam1);
        }

        _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_714),
                    m_PageInfo.MisPrize[n].wParam2, szItem);
        m_MemoInfo[m_sNumInfo].strDesp = "";
        m_MemoInfo[m_sNumInfo].dwColor = 0xFF808080;
        m_MemoInfo[m_sNumInfo].byType = MEMO_LINE_DESP;
        m_MemoInfo[m_sNumInfo].sxPos = sCom;
        m_MemoInfo[m_sNumInfo].syPos = sRow;
        ParseScript(szNumber, m_sNumInfo, sRow, sCom, 6, sMaxCom, 0xFF808080,
                    m_MemoInfo, MEMO_MAX_LINE);
        sRow += 2;
        sCom = 1;
        ...
        m_sNumInfo++;
        _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_715),
                    m_PageInfo.MisPrize[n].wParam1);
        m_MemoInfo[m_sNumInfo].strDesp = "";
        m_MemoInfo[m_sNumInfo].dwColor = 0xFF808080;
        m_MemoInfo[m_sNumInfo].byType = MEMO_LINE_DESP;
        ...
        m_sNumInfo++;
        _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_716),
                    m_PageInfo.MisPrize[n].wParam1);
        m_MemoInfo[m_sNumInfo].strDesp = "";
        m_MemoInfo[m_sNumInfo].dwColor = 0xFF808080;
        m_MemoInfo[m_sNumInfo].byType = MEMO_LINE_DESP;
        ...
        m_sNumInfo++;
        _snprintf_s(szNumber, _countof(szNumber), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_717),
                    m_PageInfo.MisPrize[n].wParam1);
        m_MemoInfo[m_sNumInfo].strDesp = "";
        m_MemoInfo[m_sNumInfo].dwColor = 0xFF808080;
        m_MemoInfo[m_sNumInfo].byType = MEMO_LINE_DESP;
        ...
        g_pGameApp->MsgBox(
            RES_STRING(CL_LANGUAGE_MATCH_718), m_PageInfo.MisPrize[n].byType,
            m_PageInfo.MisPrize[n].wParam1, m_PageInfo.MisPrize[n].wParam2);
      }
    }
  }
  m_sNumInfo++;
...
BOOL CMemoEx::ParseScript(char *pszTemp, USHORT &sNumLine, USHORT &sRow,
                          USHORT &sCom, USHORT sStartCom, USHORT sMaxCom,
...
BOOL CMemoEx::IsSelPrize() {
  return m_PageInfo.byPrizeNum > 0 &&
         m_PageInfo.byPrizeSelType == mission::PRIZE_SELONE;
}
    ...
    if (m_SelMem > 0 && (key & Mouse_LDown)) {
      if (evtClickItem) {
        if (m_MemoInfo[m_SelMem - 1].dwColor == m_MemoInfo[m_SelMem].dwColor)
          evtClickItem(m_MemoInfo[m_SelMem - 1].strDesp +
                       m_MemoInfo[m_SelMem].strDesp);
        else if (m_MemoInfo[m_SelMem + 1].dwColor ==
                 m_MemoInfo[m_SelMem].dwColor)
          evtClickItem(m_MemoInfo[m_SelMem].strDesp +
                       m_MemoInfo[m_SelMem + 1].strDesp);
        else
          evtClickItem(m_MemoInfo[m_SelMem].strDesp);
      }
    }
...
void CMemoEx::_SetScrollRange() { _pScroll->Refresh(); }

// CRichMemo implement
CRichMemo::CRichMemo(CForm &frmOwn) : CCompent(frmOwn), m_pScroll(NULL) {
  m_ShowRect.left = 0;
  m_ShowRect.right = 0;
  m_ShowRect.top = 0;
  m_ShowRect.bottom = 0;
...
CRichMemo::CRichMemo(const CRichMemo &rhs) : CCompent(rhs) {
...
void CRichMemo::Render() {
  ...
  GetRender().SetClipRect(nPosX + m_ShowRect.left, nPosY + m_ShowRect.top,
                          GetWidth() - m_ShowRect.right + 2,
                          GetHeight() - m_ShowRect.bottom);
  ...
  for (pos = m_LineList.begin(); pos != m_LineList.end(); pos++) {
    ...
    for (colpos = pInfo->ColInfoArray.begin();
         colpos != pInfo->ColInfoArray.end(); colpos++) {
      ...
      case COL_TEXT: {
        ...
        nyPos += nPosY;
        CGuiFont::s_Font.Render(pInfo->byFontType, (*colpos).strDesp.c_str(),
                                nxPos, nyPos, (*colpos).dwColor);
      } break;
      case COL_ICON: {
...
void CRichMemo::AddText(const char szTitle[], const char szText[],
                        BYTE byColorType, BYTE byFontType) {
  // 判断是否超出显示范围
...
void CRichMemo::ParseScript(const char szInfo[], DWORD dwDefColor, BYTE byFont,
                            USHORT sStartCom, USHORT sMaxCom) {
  char szData[3];
  memset(szData, 0, 3);
  ...
  // add by ning.yan 聊天内容断字处理 20081007 begin
  UnicodeString str = szInfo;
  ...
  for (int row = 0; row < lineNum; row++) {
    ...
    strDest += string;
    if (row < lineNum - 1)
      strDest += "<n>"; //遇到字符串<n>时换行，所以在需要换行处人为增加此字符串
  }
  strDest.extract(0, strDest.length(), lineBuf, sizeof(lineBuf) - 1, 0);
  lineBuf[sizeof(lineBuf) - 1] = 0;
  // ning.yan end
  ...
  const char *pszTemp = lineBuf; // modify by ning.yan
  ...
  while (pszTemp[0]) {
    ...
    if (pszTemp[0] == '<') {
      ...
      if (pszTemp[0] == 'p') {
        ...
        strncpy_s(szIcon, pszEnd - pszTemp, pszTemp + 1, _TRUNCATE);
        pszTemp = pszEnd;
        // 获取图标
