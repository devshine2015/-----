...
CDrawPointList::CDrawPointList(void) : _nMaxCount(10), _bIsEnabled(false) {}

CDrawPointList::~CDrawPointList(void) { Clear(); }
...
void CDrawPointList::Reader() {
  ...
  for (unsigned int m = 0; m < points.size(); m++) {
    ...
    if (points[m].color == 0xffff0000) {
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d,%d", (int)points[m].pos.x,
                  (int)points[m].pos.y);
...
void CDrawPointList::OnLostDevice() { CPathBox.ReleaseBox(); }
