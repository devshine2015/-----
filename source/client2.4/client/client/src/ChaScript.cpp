...
int GetChaPhotoTexID(int nTypeID) {
  ...
  if (pInfo) {
    char szPhoto[80] = {0};
    _snprintf_s(szPhoto, _countof(szPhoto), _TRUNCATE, "texture/photo/%s.bmp",
                pInfo->szIconName);
...
//---------------------------------------------------------------------------
void MPInitLua_Cha() {
  CLU_RegisterFunction("CH_Create", "int", "int,int", CLU_CDECL,
                       CLU_CAST(CH_Create));
  CLU_RegisterFunction("CH_SetPos", "int", "int,int,int", CLU_CDECL,
                       CLU_CAST(CH_SetPos));
  CLU_RegisterFunction("CH_SetYaw", "int", "int,int", CLU_CDECL,
                       CLU_CAST(CH_SetYaw));
  CLU_RegisterFunction("CH_PlayPos", "int", "int,int,int", CLU_CDECL,
                       CLU_CAST(CH_PlayPos));
}
