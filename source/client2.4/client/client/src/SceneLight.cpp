...
// lwAnimKeySetPRS
template <class T>
HRESULT lwIndexDataSearch(DWORD *ret_min, DWORD *ret_max, DWORD key,
                          T *data_seq, DWORD data_num) {
  DWORD low = 0;
...
void AnimCtrlLight::SetData(IndexDataSceneLight *buf, DWORD num) {
  ...
  _data_seq = LE_NEW(IndexDataSceneLight[num]);
  ...
  LEGUIDCreateObject((DR_VOID **)&_pose_ctrl, DR_GUID_POSECTRL);
...
int AnimCtrlLight::Load(const char *file) {
  ...
  fscanf_s(fp, "%d", &_data_num);

  _data_seq = LE_NEW(IndexDataSceneLight[_data_num]);
  ...
  for (DWORD i = 0; i < _data_num; i++) {
    ...
    fscanf_s(fp, "%d, %d, %f %f %f, %f, %f %f %f\n", &p->id, &p->light.type,
             &p->light.amb.r, &p->light.amb.g, &p->light.amb.b, &p->light.range,
             &p->light.attenuation0, &p->light.attenuation1,
             &p->light.attenuation2);
...
int AnimCtrlLight::UpdateObject(SceneLight *ret_obj) {
  ...
  lwIndexDataSearch<IndexDataSceneLight>(&key[0], &key[1], i, _data_seq,
                                         _data_num);

  float t = (float)(i - _data_seq[key[0]].id) /
            (float)(_data_seq[key[1]].id - _data_seq[key[0]].id);

  SceneLight *u = &_data_seq[key[0]].light;
  SceneLight *v = &_data_seq[key[1]].light;

  ret_obj->type = u->type;
  LEVector3Slerp((LEVector3 *)&ret_obj->pos, (LEVector3 *)&u->pos,
                 (LEVector3 *)&v->pos, t);
  LEVector3Slerp((LEVector3 *)&ret_obj->amb.r, (LEVector3 *)&u->amb.r,
                 (LEVector3 *)&v->amb.r, t);
  LEVector3Slerp((LEVector3 *)&ret_obj->dif.r, (LEVector3 *)&u->dif.r,
                 (LEVector3 *)&v->dif.r, t);
