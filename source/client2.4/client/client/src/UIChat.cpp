...
bool CChat::Init() {
  ...
  for (int i = 0; i < MAX_MEMBER; i++) {
    ...
    proTeamMenberHP[i] =
        dynamic_cast<CProgressBar *>(frmTeamMenber[i]->Find(szBuf));
    if (!proTeamMenberHP[i])
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), szBuf);
    ...
    if (!labLv[i])
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), "labFLv");

    imgWork[i] = dynamic_cast<CImage *>(frmTeamMenber[i]->Find("imgobj"));
    if (!imgWork[i])
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), "imgobj");

    imgLeader[i] = dynamic_cast<CImage *>(frmTeamMenber[i]->Find("imgLeader"));
    if (!imgLeader[i])
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), "imgLeader");

    C3DCompent *p3DDown =
        dynamic_cast<C3DCompent *>(frmTeamMenber[i]->Find("d3dDown"));
    if (!p3DDown)
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), "d3dDown");
    p3DDown->nTag = i;
    p3DDown->evtMouseDown = _MemberMouseDownEvent;
    p3DDown->SetMouseAction(enumMA_Skill);

    C3DCompent *p3D =
        dynamic_cast<C3DCompent *>(frmTeamMenber[i]->Find("d3dHead"));
    if (!p3D)
      return _Error(RES_STRING(CL_LANGUAGE_MATCH_45),
                    frmTeamMenber[i]->GetName(), "d3dHead");
  ...
  if (!pbtnAdd)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmQQ->GetName(),
                  "btnAdd");
  ...
  CTextButton *btnAddFriend =
      dynamic_cast<CTextButton *>(frmjieguo2->Find("btnAddFriend"));
  if (!btnAddFriend)
    return false;
  btnAddFriend->evtMouseClick = _evtAddFriend;

  CTextButton *btnViewFriend =
      dynamic_cast<CTextButton *>(frmjieguo2->Find("btnViewFriend"));
  if (!btnViewFriend)
    return false;
  btnViewFriend->evtMouseClick = _evtViewFriend;

  CTextButton *btnInfo =
      dynamic_cast<CTextButton *>(frmchainfo2->Find("btnInfo"));
  if (!btnInfo)
    return false;
  ...
  CTextButton *btnshow =
      dynamic_cast<CTextButton *>(frmDetail->Find("btnshow"));
  ...
  int nWidth =
      m_pQQTreeView->GetWidth() - m_pQQTreeView->GetScroll()->GetWidth() - 28;
  int nHeight = SMALL_ICON_SIZE + 3;

  /*_pSessionNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_465) )); _pSessionNode->GetItem()->SetColor(
  COLOR_BLACK ); _pSessionNode->GetUpImage()->UnLoadImage();
  ...
  /*_pFrndNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_466) )); _pFrndNode->GetItem()->SetColor(
  COLOR_BLACK ); _pFrndNode->GetUpImage()->UnLoadImage();
  ...
  /*_pGroupNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_467) )); _pGroupNode->GetItem()->SetColor(
  COLOR_BLACK ); _pGroupNode->GetUpImage()->UnLoadImage();
  ...
  _pGuildNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_468) )); _pGuildNode->GetItem()->SetColor(
  COLOR_BLACK ); _pGuildNode->GetUpImage()->UnLoadImage();
  ...
  _pMasterNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_850) )); _pMasterNode->GetItem()->SetColor(
  COLOR_BLACK ); _pMasterNode->GetUpImage()->UnLoadImage();
  ...
  _pPrenticeNode =
  dynamic_cast<GUI::CTreeGridNode*>(m_pQQTreeView->GetRootNode()->FindNode(
  RES_STRING(CL_LANGUAGE_MATCH_851) )); _pPrenticeNode->GetItem()->SetColor(
  COLOR_BLACK ); _pPrenticeNode->GetUpImage()->UnLoadImage();
  ...
  if (!pbtnAdd)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmAddFriend->GetName(),
                  "btnYes");
  ...
  if (!pbtnAdd)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411),
                  _frmAddFrndGroup->GetName(), "btnYes");
  pbtnAdd->evtMouseClick = _evtAddFrndGroup;
  _pAddFrndGroupName =
      dynamic_cast<CEdit *>(_frmAddFrndGroup->Find("edtGroupName"));
  if (!_pAddFrndGroupName)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411),
                  _frmAddFrndGroup->GetName(), "edtGroupName");
  ...
  if (!pbtnAdd)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411),
                  _frmEditFrndGroup->GetName(), "btnYes");
  pbtnAdd->evtMouseClick = _evtEditFrndGroup;
  _pEditFrndGroupName =
      dynamic_cast<CEdit *>(_frmEditFrndGroup->Find("edtGroupName"));
  if (!_pEditFrndGroupName)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411),
                  _frmEditFrndGroup->GetName(), "edtGroupName");
  // End
  ...
  if (!pbtnAdd)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmEditMotto->GetName(),
                  "btnYes");
  ...
  if (!_imgFaceOfMottoFrm)
    return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmEditMotto->GetName(),
                  "imgMhead");
  _chkForbidOfMottoFrm =
      dynamic_cast<CCheckBox *>(_frmEditMotto->Find("chkChat"));
  ...
  // if( !pbtnAdd ) return _Error(RES_STRING(CL_LANGUAGE_MATCH_473),
  // _frmDetails->GetName(), "btnRefurbish" );
 ...
 */
CTreeGridNode *CChat::GetFrndGroupNode(const char *szGroupName,
                                       CTreeGridNode *pNode) {
  ...
  for (vector<pair<string, CTreeGridNode *>>::iterator it = vFrndNode.begin();
       it != vFrndNode.end(); it++) {
...
void CChat::ChangeFrndGroup(const char *szOldGroupName,
                            const char *szNewGroupName) {
...
void CChat::RefreshTeam() {
  ...
  if (CGameScene::GetMainCha() &&
      CGameScene::GetMainCha()->getHumanID() == CTeamMgr::GetTeamLeaderID()) {
    // 在自已的头上显示队长标志
  ...
  for (int i = MAX_MEMBER - 1; i >= 0; i--) {
    if (i < Num) {
      ...
      if (pCurMember && pCurMember->GetPointer()) {
        ...
        imgLeader[i]->SetIsShow(pCurMember->GetID() ==
...
int CChat::TeamSend(DWORD dwMsg, void *pData, DWORD dwParam) {
  ...
  case enumSTM_ADD_MEMBER: {
    ...
    if (pIconInfo) {
      ...
      if (pMember->IsOnline()) {
        pPic->LoadImage((strPath + pIconInfo->szSmall).c_str(), SMALL_ICON_SIZE,
                        SMALL_ICON_SIZE, 0, pIconInfo->nSmallX,
                        pIconInfo->nSmallY);
        pPic->SetFrame(0);
        pItem->SetColor(0xfffc2f20);
      } else {
        pPic->LoadImage((strPath + pIconInfo->szSmallOff).c_str(),
                        SMALL_ICON_SIZE, SMALL_ICON_SIZE, 0, pIconInfo->nSmallX,
                        pIconInfo->nSmallY);
        pPic->SetFrame(0);
  ...
  case enumSTM_NODE_CHANGE: {
    ...
    CChatIconInfo *pIconInfo = GetChatIconInfo(pMember->GetIconID());
    if (pIconInfo &&
        pItem) // modify by Philip.Wu  2006-08-13  修改去除路人后的当机
    {
      CGuiPic *pPic = pItem->GetImage();
      string strPath = "texture/ui/HEAD/";
      if (pMember->IsOnline()) {
  ...
  for (; i < nCount; i++) {
    ...
    for (; j < nCount; j++) {
      ...
      if (pMember2->IsOnline()) {
        ...
        string strPath = "texture/ui/HEAD/";
        pPic2->LoadImage((strPath + pIconInfo1->szSmallOff).c_str(),
                         SMALL_ICON_SIZE, SMALL_ICON_SIZE, 0,
                         pIconInfo1->nSmallX, pIconInfo1->nSmallY);
        pPic2->SetFrame(0);
        pItem2->SetColor(0xff000000);
        CGuiPic *pPic1 = pItem1->GetImage();
        pPic1->LoadImage((strPath + pIconInfo2->szSmall).c_str(),
                         SMALL_ICON_SIZE, SMALL_ICON_SIZE, 0,
                         pIconInfo2->nSmallX, pIconInfo2->nSmallY);
        pPic1->SetFrame(0);
...
void CChat::SortOnlineFrnd() {
  ...
  //			pItem2->SetPointer(pMember1);
  //			CChatIconInfo
  //*pIconInfo1=GetChatIconInfo(pMember1->GetIconID()); 			CChatIconInfo
  //*pIconInfo2=GetChatIconInfo(pMember2->GetIconID()); 			if (!pIconInfo1 ||
  //!pIconInfo2)
  //			{
  //				continue;
  //			}
...
void CChat::_MemberMouseDownEvent(CGuiData *pSender, int x, int y, DWORD key) {
  if (key & Mouse_LDown) {
    ...
    CCharacter *pCha =
        pScene->SearchByHumanName(labMenberName[pSender->nTag]->GetCaption());
    if (!pCha) {
      g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_474),
                          labMenberName[pSender->nTag]->GetCaption());
      return;
    }

    if (g_pGameApp->IsAltPress())
      pScene->GetMouseDown().GetAutoAttack()->Follow(CGameScene::GetMainCha(),
                                                     pCha);
    else
  ...
  if (key & Mouse_RDown) {
    if (_groupMouseRight) {
      ...
      /*		if( pMember )
                      {
                              ...
                              CMenuItem* pItem =
         _groupMouseRight->GetMenuItem(3); if (pItem && stricmp(
         pItem->GetString(), RES_STRING(CL_LANGUAGE_MATCH_475) ) == 0)
                              {
      ...
      if (pMember) {
        ...
        //	if( stricmp( MapName,"starena1") == 0 || stricmp(
        //MapName,"starena2") == 0 || stricmp( MapName,"starena3") == 0 )
        if (_stricmp(MapName, "starena1") == 0 ||
            _stricmp(MapName, "starena2") == 0 ||
            _stricmp(MapName, "starena3") == 0) {
          //	if (pItem && stricmp( pItem->GetString(),
          //RES_STRING(CL_LANGUAGE_MATCH_475) ) == 0)
          if (pItem && _stricmp(pItem->GetString(),
                                RES_STRING(CL_LANGUAGE_MATCH_475)) == 0) {
            pItem->SetIsEnabled(false);
          }
        } else {
    ...
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_476),
                        labMenberName[pSender->nTag]->GetCaption());
  }
}
...
void CChat::_MainMouseClick(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  // Add by lark.li 20080804 begin
  CNoteGraph *pSelectNodeItem =
      dynamic_cast<CNoteGraph *>(pTree->GetHitItem(x, y));
  if (pSelectNodeItem) {
    for (vector<pair<string, CTreeGridNode *>>::iterator it =
             g_stUIChat.vFrndNode.begin();
         it != g_stUIChat.vFrndNode.end(); it++) {
      if (pSelectNodeItem == it->second->GetItem()) {
        if (key & Mouse_RDown) {
          _curSelectNode = it->second;

          if (x + _frndGroupMouseRight->GetWidth() >
              g_pGameApp->GetWindowWidth())
            sx =
                g_pGameApp->GetWindowWidth() - _frndGroupMouseRight->GetWidth();
          if (y + _frndGroupMouseRight->GetHeight() >
              g_pGameApp->GetWindowHeight())
            sy = g_pGameApp->GetWindowHeight() -
                 _frndGroupMouseRight->GetHeight();
  ...
  CTreeGridNode *pSelectNode =
      dynamic_cast<CTreeGridNode *>(pTree->GetSelectNode());
  if (!pSelectNode)
    return;
  // End
    ...
    for (DWORD i = 0; i < pTeam->GetCount(); i++) {
      ...
      if (pMember->GetPointer() == pSelectItem) {
        ...
        break;
             ...
             _pTeamMgr->Find(enumTeamMaster)->GetPointer()) // 导师
  {
    ...
    for (DWORD i = 0; i < pTeam->GetCount(); i++) {
      ...
      if (pMember->GetPointer() == pSelectItem) {
        ...
        break;
             ...
             _pTeamMgr->Find(enumTeamPrentice)->GetPointer()) // 学徒
  {
    ...
    if (key & Mouse_LDB) {
      ...
      CTalkSessionForm *sessForm =
          static_cast<CTalkSessionForm *>(pSelectItem->GetPointer());
      if (sessForm) {
    ...
    while (pMemberData = CGuildMembersMgr::FindGuildMemberByIndex(i)) {
      if (pMemberData->GetPointer() == pSelectItem) {
        if (key & Mouse_LDB) {
          CTalkSessionFormMgr::sApplyMember *pMember =
              new CTalkSessionFormMgr::sApplyMember[1];
    ...
    g_pGameApp->SysInfo(
        "Unknow Hit! File: UIChat.cpp   Class: CChat   Code: Arcol");
  }
}
...
void CChat::_evtQQMainShow(CGuiData *pSender) {
  ...
  if (pMain) {
    CLabelEx *pMineName = dynamic_cast<CLabelEx *>(
        CFormMgr::s_Mgr.Find("frmQQ")->Find("labMineName"));
    if (pMineName) {
      pMineName->SetCaption(pMain->getHumanName());
    } else {
      _Error(RES_STRING(CL_LANGUAGE_MATCH_473), _frmQQ->GetName(),
             "labMineName");
    }
  }
}

void CChat::_OnMouseAddFrndGroupMenu(CGuiData *pSender, int x, int y,
                                     DWORD key) {
  ...
  if (str == "修改名称") {
    ...
    if (count > 0) {
      ...
      _snprintf_s(buffer, _countof(buffer), _TRUNCATE,
                  "该分组已经有%d 个成员，不能删除", count);
      g_stUIBox.ShowMsgBox(NULL, buffer);
    } else {
      g_stUIBox.ShowSelectBox(_evtAskDelGroupEvent, "确定删除该分组吗？",
                              false);
    }
  } else if (str == "新增分类") {
...
void CChat::_evtAskDelGroupEvent(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
...
void CChat::ShowDetailForm(CMember *pMember) //右击好友显示的详细信息
{
  if (pMember) {
    ...
    g_stUIChat._PictureCChat.setVisitorCharacterID(
        pMember->GetID()); // add by guojiangang 20090211
    //_frmDetails->Show();
...
void CChat::_OnMouseFrndMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (str == RES_STRING(CL_LANGUAGE_MATCH_477)) {
    ...
    stSelectBox *pSelectBox =
        g_stUIBox.ShowSelectBox(_OnFrndDeleteConfirm, str.c_str(), true);
...
void CChat::_OnMouseSessMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  {
    CTalkSessionFormMgr::OnClickCloseSession(
        static_cast<CTalkSessionForm *>(_sessMouseRight->GetPointer()));
  }
...
void CChat::_OnMouseMasterMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_482)) // 添加好友
  {
    ...
    if (pMainCha && pMainCha->getGameAttr() &&
        pMainCha->getGameAttr()->get(ATTR_LV) >= 7) {
      CS_Frnd_Invite(_curSelectMember->GetName());
    } else {
      // 七级以下禁止添加好友
  ...
  } else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_484)) // 邀请组队
  {
    CCharacter *pMainCha = CGameScene::GetMainCha();
    if (pMainCha &&
        (pMainCha->IsBoat() || (pMainCha->getGameAttr() &&
                                pMainCha->getGameAttr()->get(ATTR_LV) >= 8))) {
      CS_Team_Invite(_curSelectMember->GetName());
    } else {
      // 八级以下禁止组队
  ...
  } else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_478)) // 发送消息
  {
    CTalkSessionFormMgr::ApplySession(&_curSelectMember);
             ...
             RES_STRING(CL_LANGUAGE_MATCH_854)) // 解除关系（徒弟解除师傅）
  {
    CCharacter *pMain = CGameScene::GetMainCha();
    if (pMain && pMain->getGameAttr()) {
      ...
      _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_889),
                  _curSelectMember->GetName(),
                  nLevel * 500); // "解除师傅 %s 您将扣除\n金钱:%d\n确定吗？"

      stSelectBox *pSelectBox =
          g_stUIBox.ShowSelectBox(_OnMasterDeleteConfirm, szBuffer, true);
      pSelectBox->dwTag = _curSelectMember->GetID();
...
void CChat::_OnMouseStudentMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  } else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_478)) // 发送消息
  {
    CTalkSessionFormMgr::ApplySession(&_curSelectMember);
             ...
             RES_STRING(CL_LANGUAGE_MATCH_854)) // 解除关系（师傅解除徒弟）
  {
    CCharacter *pMain = CGameScene::GetMainCha();
    if (pMain && pMain->getGameAttr()) {
      // long nLevel = pMain->getGameAttr()->get(ATTR_LV);
      ...
      char szBuffer[256] = {0};
      _snprintf_s(
          szBuffer, _countof(szBuffer), _TRUNCATE,
          RES_STRING(CL_LANGUAGE_MATCH_856), _curSelectMember->GetName(),
          nLevel * 5); // 解除徒弟 %s 您将扣除\n金钱:%d  声望:%d\n确定吗？

      stSelectBox *pSelectBox =
          g_stUIBox.ShowSelectBox(_OnStudentDeleteConfirm, szBuffer, true);
      pSelectBox->dwTag = _curSelectMember->GetID();
      pSelectBox->pointer = (void *)_curSelectMember->GetName();
    }
  }
...
void CChat::_evtAddFrndGroup(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (!name.empty()) {
    ...
    //	if(strcmp(name.c_str(),RES_STRING( CL_LANGUAGE_MATCH_466) ) ==0);//
    //ning.yan modify
    if (_stricmp(name.c_str(), RES_STRING(CL_LANGUAGE_MATCH_466)) == 0) {
      // g_stUIBox.ShowMsgBox( NULL, "已经存在这个分组了！" );
      g_stUIBox.ShowMsgBox(
          NULL, RES_STRING(CL_LANGUAGE_MATCH_970)); // ning.yan  modify
      return;
    }
...
void CChat::_evtQQMainEditMotto(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (!g_pGameApp->GetCurScene()->GetMainCha())
    return;
  CTextButton *pbtnAdd =
      dynamic_cast<CTextButton *>(_frmQQ->Find("btnscr")); // 搜索面板
  if (pSender == pbtnAdd) {
  ...
  } else // 查询自己的信息
  {
    ...
    _labNameOfMottoFrm->SetCaption(g_pGameApp->GetCurScene()
                                       ->GetMainCha()
                                       ->getHumanName()); //设定相册上的角色名
    CImage *m_pChaPhoto =
        dynamic_cast<CImage *>(_frmEditMotto->Find("imgMhead"));
    m_pChaPhoto->GetImage()->UnLoadImage();

    _chkForbidOfMottoFrm->SetIsChecked(_bForbid);
    _frmEditMotto->Refresh();
    _frmEditMotto->SetIsShow(
        !_frmEditMotto->GetIsShow()); //	Modify by alfred.shi 20080909
  }
}
...
void CChat::_evtChangeMotto(CGuiData *pSender, int x, int y, DWORD key) {
  CTextButton *pbtnAdd =
      dynamic_cast<CTextButton *>(_frmEditMotto->Find("btnYes"));
  CTextButton *pbtnAdd1 =
      dynamic_cast<CTextButton *>(_frmEditMotto->Find("btnEgg"));
  CTextButton *pbtnAdd2 =
      dynamic_cast<CTextButton *>(_frmEditMotto->Find("btnFlower"));
  CTextButton *pbtnAdd3 =
      dynamic_cast<CTextButton *>(_frmEditMotto->Find("btnxiangce01"));
  CTextButton *pbtnAdd4 =
      dynamic_cast<CTextButton *>(_frmEditMotto->Find("btnxiangce02"));
  if (pSender == pbtnAdd) {
    ...
    CEdit *m_pEdtTradeGold =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("edtTradeGold"));
    ...
    CEdit *m_pAnimalZodiac =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chashengxiao"));

    CEdit *m_pBloodType =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chaxuexing"));

    CEdit *m_pBirthMonth =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chashengriyue"));

    CEdit *m_pBirthDay =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chashengriri"));

    CEdit *m_pState = dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chashengfen"));

    CEdit *m_pCity = dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chaquyu"));

    CEdit *m_pConstellation =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chaxingzuo"));

    CEdit *m_pJob = dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chajob"));

    CEdit *m_pIdiograph =
        dynamic_cast<CEdit *>(m_pFrmSInfo->Find("chaqianming"));

    CCheckBox *m_pStopChat =
        dynamic_cast<CCheckBox *>(m_pFrmSInfo->Find("chkChat"));

    CLabel *m_pSupport =
        dynamic_cast<CLabel *>(m_pFrmSInfo->Find("chaxianhua"));

    CLabel *m_pOppose = dynamic_cast<CLabel *>(m_pFrmSInfo->Find("chajidan"));

    len = sizeof(tradeGold) > strlen(m_pEdtTradeGold->GetCaption())
              ? strlen(m_pEdtTradeGold->GetCaption())
              : sizeof(tradeGold);
    memcpy(tradeGold, m_pEdtTradeGold->GetCaption(), len);

    len = sizeof(m_ShowInfo.szMotto) > strlen(m_pIdiograph->GetCaption())
              ? strlen(m_pIdiograph->GetCaption())
              : sizeof(m_ShowInfo.szMotto);
    memcpy(m_ShowInfo.szMotto, m_pIdiograph->GetCaption(), len);

    m_ShowInfo.bShowMotto = len > 0 ? true : false;

    len = sizeof(m_ShowInfo.szSex) > strlen(m_pChaSex->GetCaption())
              ? strlen(m_pChaSex->GetCaption())
              : sizeof(m_ShowInfo.szSex);
    ...
    if (strcmp(m_ShowInfo.szSex, RES_STRING(CL_LANGUAGE_MATCH_966)) != 0 &&
        strcmp(m_ShowInfo.szSex, RES_STRING(CL_LANGUAGE_MATCH_967)) != 0)
      g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_968));
    // end
    // MessageBox( NULL, "性别必须是:男或女!", RES_STRING(CL_LANGUAGE_MATCH_25),
    // MB_OK );

    m_ShowInfo.sAge = atoi(m_pChaAge->GetCaption());

    len = sizeof(m_ShowInfo.szName) > strlen(m_pChaName->GetCaption())
              ? strlen(m_pChaName->GetCaption())
              : sizeof(m_ShowInfo.szName);
    memcpy(m_ShowInfo.szName, m_pChaName->GetCaption(), len);

    len = sizeof(m_ShowInfo.szAnimalZodiac) >
                  strlen(m_pAnimalZodiac->GetCaption())
              ? strlen(m_pAnimalZodiac->GetCaption())
              : sizeof(m_ShowInfo.szAnimalZodiac);
    memcpy(m_ShowInfo.szAnimalZodiac, m_pAnimalZodiac->GetCaption(), len);

    len = sizeof(m_ShowInfo.szBloodType) > strlen(m_pBloodType->GetCaption())
              ? strlen(m_pBloodType->GetCaption())
              : sizeof(m_ShowInfo.szBloodType);
    memcpy(m_ShowInfo.szBloodType, m_pBloodType->GetCaption(), len);

    len = sizeof(birthmonth) > strlen(m_pBirthMonth->GetCaption())
              ? strlen(m_pBirthMonth->GetCaption())
              : sizeof(birthmonth);
    memcpy(birthmonth, m_pBirthMonth->GetCaption(), len);

    len = sizeof(birthday) > strlen(m_pBirthDay->GetCaption())
              ? strlen(m_pBirthDay->GetCaption())
              : sizeof(birthday);
    memcpy(birthday, m_pBirthDay->GetCaption(), len);

    // m_ShowInfo.iBirthday = atoi(birthmonth)<<16 |atoi(birthday);
    m_ShowInfo.iBirthday = (atoi(birthmonth) << 5) +
                           atoi(birthday); // Modify by sunny.sun 20080825

    len = sizeof(m_ShowInfo.szState) > strlen(m_pState->GetCaption())
              ? strlen(m_pState->GetCaption())
              : sizeof(m_ShowInfo.szState);
    memcpy(m_ShowInfo.szState, m_pState->GetCaption(), len);

    len = sizeof(m_ShowInfo.szCity) > strlen(m_pCity->GetCaption())
              ? strlen(m_pCity->GetCaption())
              : sizeof(m_ShowInfo.szCity);
    memcpy(m_ShowInfo.szCity, m_pCity->GetCaption(), len);

    len = sizeof(m_ShowInfo.szConstellation) >
                  strlen(m_pConstellation->GetCaption())
              ? strlen(m_pConstellation->GetCaption())
              : sizeof(m_ShowInfo.szConstellation);
    memcpy(m_ShowInfo.szConstellation, m_pConstellation->GetCaption(), len);

    len = sizeof(m_ShowInfo.szCareer) > strlen(m_pJob->GetCaption())
              ? strlen(m_pJob->GetCaption())
              : sizeof(m_ShowInfo.szCareer);
  ...
  {
    CLabel *m_pSupport =
        dynamic_cast<CLabel *>(_frmEditMotto->Find("chaxianhua"));
...
void CChat::setURLPicture(DWORD chaID) {
  ...
  if (chaID == _dwSelfID) {
    _snprintf_s(m_URLPicture, sizeof(m_URLPicture), _TRUNCATE,
                "%sAID=%d&SID=%d&KEY=%s&F_User=%s&F_CharID=%ld&To_User=%s&To_"
                "CharID=%ld",
...
bool CChat::_UpdateSelfInfo() {
  ...
  //玩家名称
  _snprintf_s(ChCharaterName, _countof(ChCharaterName), _TRUNCATE, "%s",
              pCha->getName());

  _snprintf_s(ChCharaterNickName, _countof(ChCharaterNickName), _TRUNCATE, "%s",
              pCChaAttr->get(ATTR_TITLE));
  ...
  //玩家职业
  _snprintf_s(ChCharaterJob, _countof(ChCharaterJob), _TRUNCATE, "%s",
              g_GetJobName((short)pCChaAttr->get(ATTR_JOB)));

  //玩家等级
  _snprintf_s(ChCharaterLev, _countof(ChCharaterLev), _TRUNCATE, "%d",
              pCChaAttr->get(ATTR_LV));

  // 声望
  _snprintf_s(ChCharaterFame, _countof(ChCharaterFame), _TRUNCATE, "%d",
              pCChaAttr->get(ATTR_FAME));
  ...
  CLabel *m_pReputation =
      dynamic_cast<CLabel *>(m_pFrmSInfo->Find("chashengwang"));
  CLabel *m_pChaNickName =
      dynamic_cast<CLabel *>(m_pFrmSInfo->Find("chachenghao"));
  ...
  // 清空数据
  CLabel *m_pAttention =
      dynamic_cast<CLabel *>(m_pFrmSInfo->Find("chaguanzhu"));
...
// Add by lark.li 20080805 begin
void CChat::_OnDragMouseMove(CForm *pTargetForm, CTreeView *pTree,
                             CTreeNodeObj *pNode1, CTreeNodeObj *pNode2,
                             CItemObj *pItem, int x, int y, DWORD key) {
...
void CChat::_OnDragEnd(CForm *pTargetForm, CTreeView *pTree,
                       CTreeNodeObj *pNode1, CTreeNodeObj *pNode2,
                       CItemObj *pItem, int x, int y, DWORD key) {
  // Add by lark.li 20080805 begin
  CTreeGridNode *pTreeNode1 = dynamic_cast<CTreeGridNode *>(pNode1);
    ...
    if (g_stUIChat.IsFrndGroupNode(pTreeNode2)) {
      if (g_stUIChat._pDropTreeNode != pTreeNode1) {
        ...
        if (pText) {
          ...
          CP_Frnd_Move_Group(pMember->GetID(), pTreeNode1->GetCaption(),
                             pTreeNode2->GetCaption());
        }
      }
    }
    return;
  ...
  if (pText) {
    ...
    if (pSessForm->IsActiveSession()) {
      ...
      if (string(pMember->GetName()) ==
          pSessForm->GetMemberByIndex(0)->GetName()) {
...
void CChat::_OnFrndDeleteConfirm(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;

  stSelectBox *pSelect =
      static_cast<stSelectBox *>(pSender->GetForm()->GetPointer());
  if (pSelect) {
    CS_Frnd_Delete(pSelect->dwTag);
  }
}

void CChat::_OnMasterDeleteConfirm(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CChat::_OnStudentDeleteConfirm(CCompent *pSender, int nMsgType, int x,
                                    int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CChat::MasterAsk(const char *szName, DWORD dwCharID) {
  char szBuffer[512] = {0};
  _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_860), szName);

  stSelectBox *pSelectBox =
      g_stUIBox.ShowSelectBox(_OnStudentAskConfirm, szBuffer, true);
  pSelectBox->pointer = (void *)szName;
  pSelectBox->dwTag = dwCharID;
}

void CChat::_OnStudentAskConfirm(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
  stSelectBox *pSelect =
      static_cast<stSelectBox *>(pSender->GetForm()->GetPointer());
  if (pSelect && pSelect->pointer) {
    CS_MasterAsr(nMsgType == CForm::mrYes, (const char *)pSelect->pointer,
                 pSelect->dwTag);
  }
}
...
void CChat::PrenticeAsk(const char *szName, DWORD dwCharID) {
  char szBuffer[512] = {0};
  _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_861), szName);

  stSelectBox *pSelectBox =
      g_stUIBox.ShowSelectBox(_OnMasterAskConfirm, szBuffer, true);
  pSelectBox->pointer = (void *)szName;
  pSelectBox->dwTag = dwCharID;
}

void CChat::_OnMasterAskConfirm(CCompent *pSender, int nMsgType, int x, int y,
                                DWORD dwKey) {
  stSelectBox *pSelect =
      static_cast<stSelectBox *>(pSender->GetForm()->GetPointer());
  if (pSelect && pSelect->pointer) {
    CS_PrenticeAsr(nMsgType == CForm::mrYes, (const char *)pSelect->pointer,
                   pSelect->dwTag);
  }
}
...
void CChat::CaptainConfirm(DWORD dwTeamID) {
  stMsgTimeBox *pBox = g_stUIBox.ShowMsgTime(
      _OnCaptainAskConfirm, RES_STRING(CL_UI_CHAT_CPP_00001), 5);
  // pBox->teamID= dwTeamID;
  pBox->teamID = (short)dwTeamID;
}

void CChat::_OnCaptainAskConfirm(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
  stMsgTimeBox *pBox =
      static_cast<stMsgTimeBox *>(pSender->GetForm()->GetPointer());
  if (pBox) {
...
void CChat::ResetSelfInfo(const stPersonInfo *_info) {
  ...
  //_snprintf_s( buff, _countof( buff ), _TRUNCATE, "%d",(_info->iBirthday
  //&&0xffff0000)); m_pBirthMonth->SetCaption(buff); _snprintf_s( buff,
  //_countof( buff ), _TRUNCATE, "%d",(_info->iBirthday &&0xffff)); Modify by
  // sunny.sun 20080825
  _snprintf_s(buff, _countof(buff), _TRUNCATE, "%d", (_info->iBirthday >> 5));
  m_pBirthMonth->SetCaption(buff);
  _snprintf_s(buff, _countof(buff), _TRUNCATE, "%d",
              (_info->iBirthday - (_info->iBirthday >> 5 << 5)));
  ...
  if (pIconInfo) {
    ...
    if (strcmp(_info->szSex, RES_STRING(CL_LANGUAGE_MATCH_967)) ==
        0) // ning.yan modify
      pPic->LoadImage((strPath + "1.bmp").c_str(), 100, 100, 0,
                      pIconInfo->nBigX, pIconInfo->nBigY);
    else
      pPic->LoadImage((strPath + "2.bmp").c_str(), 100, 100, 0,
                      pIconInfo->nBigX, pIconInfo->nBigY);
