...
bool Connection::Connect(dbc::cChar *hostname, dbc::uShort port,
                         dbc::uLong timeout) {
...
/*
        ...
        if (cbNameAppendInfo)
        {
                ...
                sprintf_s((char *)chIp, "%s", m_datasock->GetLocalIP());
                sprintf_s((char *)chPort, "%d", m_datasock->GetLocalPort());
        ...
        pk.WriteCmd(CMD_CM_SELROLE);			//命令
        pk.WriteLong(m_cat);
        pk.WriteSequence(m_name, uShort(strlen(m_name) + 1));
        if (cbNameAppendInfo)
                pk.WriteSequence(chAppendName, uShort(strlen(chAppendName) +
   1)); m_nt->SendData(m_datasock,pk); return true;
*/
