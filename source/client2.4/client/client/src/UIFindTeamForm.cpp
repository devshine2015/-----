...
CFindTeamMgr::CFindTeamMgr() { m_dwLastTick = 0; }
...
bool CFindTeamMgr::Init() {
  ...
  for (int i = 0; i < FINDTEAM_PAGE_SIZE; ++i) {
    ...
    _snprintf_s(szName, _countof(szName), _TRUNCATE, "labLevel_%d", i);
    ...
    _snprintf_s(szName, _countof(szName), _TRUNCATE, "labJob_%d", i);
    ...
    _snprintf_s(szName, _countof(szName), _TRUNCATE, "labPlace_%d", i);
    ...
    _snprintf_s(szName, _countof(szName), _TRUNCATE, "btnSubmit_%d", i);
...
void CFindTeamMgr::CloseForm() { frmFindTeam->SetIsShow(false); }
...
void CFindTeamMgr::SetFindTeamPage(int nCurPage, int nPageNum) {
  ...
  char szPage[32] = {0};
  _snprintf_s(szPage, _countof(szPage), _TRUNCATE, "%d/%d", nCurPage, nPageNum);
  labListPage->SetCaption(szPage);
}

void CFindTeamMgr::AddFindTeamInfo(int nSeq, const char *szName, long nLevel,
                                   long nJob, const char *szPlace) {
  if (0 <= nSeq && nSeq < FINDTEAM_PAGE_SIZE) {
    labName[nSeq]->SetCaption(szName);
    labName[nSeq]->SetIsShow(true);

    char szLevel[32] = {0};
    _snprintf_s(szLevel, _countof(szLevel), _TRUNCATE, "%d", nLevel);
    ...
    if (0 <= nJob && nJob < MAX_JOB_TYPE) {
      ...
      labJob[nSeq]->SetCaption(CommonRes::GetJobName(nJob));
...
void CFindTeamMgr::FindTeamAsk(const char *szName) {
  ...
  _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_863), szName);
  CBoxMgr::ShowSelectBox(_evtFindTeamCheckEvent, szBuffer, true);
}

// 寻找组队界面按钮消息处理
void CFindTeamMgr::_evtFindTeamMouseButton(CCompent *pSender, int nMsgType,
  ...
  if (strName == "btnLeftPage") {
    ...
    if (g_stUIFindTeam.m_dwLastTick + 1000 * FINDTEAM_INTERVAL > dwCurTick) {
      g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_883), FINDTEAM_INTERVAL,
                         FINDTEAM_INTERVAL -
                             (dwCurTick - g_stUIFindTeam.m_dwLastTick) / 1000);
      return;
    }
...
// 组队申请回调
void CFindTeamMgr::_evtFindTeamCheckEvent(CCompent *pSender, int nMsgType,
                                          int x, int y, DWORD dwKey) {
  CS_VolunteerAsr(nMsgType == CForm::mrYes,
                  g_stUIFindTeam.m_strTeamLeader.c_str());
}
