...
CSelectItem::~CSelectItem() { SAFE_DELETE(_pImage); }
...
CListItems::CListItems()
    : _pSelect(new CSelectItem()), _dwCount(0), _nItemLeft(0), _nItemTop(0),
      _dwFirstShowRow(0), _pList(NULL), _dwLastShowRow(0), _nShowCount(1000),
      _nMaxRow(0), _IsMouseFollow(false), _pItemPic(NULL), _nPicLeft(0),
      _nPicTop(0), _nPicX(0) {}

CListItems::CListItems(const CListItems &rhs)
    : _pSelect(new CSelectItem(*rhs._pSelect)), _dwCount(rhs._dwCount),
      _dwFirstShowRow(rhs._dwFirstShowRow), _pList(rhs._pList),
      _dwLastShowRow(rhs._dwLastShowRow), _nShowCount(rhs._nShowCount),
      _nX1(rhs._nX1), _nX2(rhs._nX2), _nY1(rhs._nY1), _nY2(rhs._nY2),
...
bool CListItems::MouseRun(int x, int y, DWORD key) {
  if (x >= _nX1 && x <= _nX2 && y >= _nY1 && y <= _nY2) {
    if (_IsMouseFollow || (key & Mouse_LDown) || (key & Mouse_LDB) ||
        (key & Mouse_RDown)) {
...
void CListItems::Render() {
  assert(_pSelect);
  if (_pSelect->GetIsEnabled() && _pSelect->GetIsSelect() &&
      _pSelect->GetIndex() >= (int)_dwFirstShowRow &&
      _pSelect->GetIndex() < (int)_dwLastShowRow) {
    _pSelect->Render(_nX1, (_pSelect->GetIndex() - _dwFirstShowRow) * _nHeight +
                               _nY1);
  }
...
// class CListItemsEx
//---------------------------------------------------------------------------
CListItemsEx::CListItemsEx(unsigned int nCol) : CListItems(), _nCol(nCol) {
...
void CListItemsEx::Clear() { CListItems::Clear(); }
...
CList::CList(CForm &frmOwn)
    : CCompent(frmOwn), _nRowHeight(22), _nSpaceBottom(0),
      _dwFontColor(COLOR_BLACK), _dwSelectColor(0xFF00FF71), _nLeftMargin(5),
      _nTopMargin(5), _nRightMargin(5), _nBottomMargin(5), _IsChangeColor(true),
...
CList::CList(const CList &rhs)
    : CCompent(rhs), _pImage(new CGuiPic(*rhs._pImage)),
      ...
      _pItems(new CListItems(*rhs._pItems)),
      _pItemImage(new CGuiPic(*rhs._pItemImage)) {
  _Copy(rhs);
...
void CList::Refresh() {
  ...
  _pItems->SetRect(GetX() + _nLeftMargin, GetY() + _nTopMargin,
                   GetX2() - _pScroll->GetWidth() - _nRightMargin,
                   GetY2() - _nBottomMargin, _nRowHeight);
  _pItems->Refresh();
...
void CList::ResetScrollChange() { _pItems->SetFirstShowRow(0); }
...
void CList::_SetScrollRange() {
  _pScroll->SetRange(0.0, (float)_pItems->GetCount() -
                              (float)_pItems->GetShowCount() +
                              (float)_nSpaceBottom);
  _pScroll->Refresh();
  _pItems->Refresh();
}

void CList::OnActive() { CCompent::OnActive(); }

void CList::OnLost() { CCompent::OnLost(); }
