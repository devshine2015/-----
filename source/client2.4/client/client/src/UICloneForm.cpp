...
// class CCloneForm
//---------------------------------------------------------------------------
CCloneForm::CCloneForm() : _pSample(NULL), _nCount(0) {}
...
CForm *CCloneForm::Clone() {
  ...
  if (_nCount >= (int)_vfrm.size()) {
    ...
    if (rv) {
      ...
      char buf[80] = {0};
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s%d", rv->GetName(),
                  nCountForm++);
      rv->SetName(buf);
...
CForm *CHideForm::GetHide() {
  ...
  if (frm) {
    ...
    char buf[80] = {0};
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s%d", frm->GetName(),
                nCountForm++);
    frm->SetName(buf);
    frm->Init();
  ...
  if (_vfrm.size() > 30 && frm) {
    LG("error", RES_STRING(CL_LANGUAGE_MATCH_491), frm->GetName(),
       _vfrm.size());
  }
