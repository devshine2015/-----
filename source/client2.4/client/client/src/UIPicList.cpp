...
CFixList::CFixList(CForm &frmOwn)
    : CCompent(frmOwn), _nRowHeight(22), _nRowSpace(0), _pImage(NULL),
      _pSelect(NULL), _nLeftMargin(5), _nTopMargin(5), _nRightMargin(5),
      _nBottomMargin(5), evtSelectChange(NULL), evtListMouseDown(NULL) {
...
CFixList::CFixList(const CFixList &rhs) : CCompent(rhs) { _SetSelf(rhs); }
...
bool CFixList::MouseRun(int x, int y, DWORD key) {
  ...
  if (IsNoDrag(x, y, key)) {
    if ((key & Mouse_LDown) &&
        (x >= _nHitX1 && x <= _nHitX2 && y >= _nHitY1 && y <= _nHitY2)) {
      // 计算点击后的行数
...
CCheckFixList::CCheckFixList(const CCheckFixList &rhs) : CFixList(rhs) {
  _SetSelf(rhs);
}
