...
static void __timer_frame(DWORD time) { g_pGameApp->FrameMove(time); }
static void __timer_render(DWORD time) { g_pGameApp->Render(); }

#ifdef USE_TIMERPERIOD
void CALLBACK __timer_period_proc(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser,
  ...
  while (_isRun) {
    ...
    if (_pCurScene) {
      ...
      if (g_Render.IsRealFPS() || _pSteady->Run()) {
        ...
        BEGIN_TICK(PR_FRAME_MOVE);
        FrameMove(_dwCurTick);
        END_TICK(PR_FRAME_MOVE);
        BEGIN_TICK(PR_RENDER);
        Render();
        END_TICK(PR_RENDER);
        ...
        eShowWhat =
            g_Render.IsRealFPS() ? _GetShowWhat() : _pSteady->GetShowWhat();
#ifdef __SOUND__
#ifdef USE_DSOUND
      ...
      if (eShowWhat != eShowAll) {
        ...
        if (eShowWhat == eShowNo &&
            !PeekMessage(&msg, NULL, 0U, 0U,
                         PM_NOREMOVE | PM_NOYIELD)) // PM_NOYIELD 不让出控制权
        {
...
void CGameApp::GotoScene(CGameScene *scene, bool isDelCurScene,
                         bool IsShowLoading) {
  if (!scene) {
...
void CGameApp::ResetCamera() {
  ...
  GetMainCam()->ResetCamera(
      INIT_CAMERA_ANGLE); // 0.5235987f + 0.0872664f); 双击鼠标右键镜头摆正 by
                          // Waiting 2009-07-09
...
void CGameApp::CreateCharImg() {
  ...
  if (be) {
    ...
    _snprintf_s(fileName, sizeof(fileName), _TRUNCATE, "%s/%s.bmp", szPath,
                pInfo->szDataName);
...
BOOL CGameApp::_CreateSmMap(LETerrain *pTerr) {
  ...
  if (fTime > 0.5f) {
    ...
    _snprintf_s(szPath, sizeof(szPath), _TRUNCATE, "texture/minimap/%s",
                _pCurScene->GetTerrainName());
    Util_MakeDir(szPath);
    /*sprintf(fileName,"%s/sm_%d_%d.bmp", szPath,
            int(vEyePt.x / SHOWRSIZE),int(vEyePt.y / SHOWRSIZE));*/
    _snprintf_s(fileName, sizeof(fileName), _TRUNCATE, "%s/sm_%d_%d.bmp",
                szPath, int(vEyePt.x / SHOWRSIZE), int(vEyePt.y / SHOWRSIZE));
...
BOOL CGameApp::_PrintScreen() {
  if (IsEnableSpAvi()) {
    ...
    while (1) {
      ...
      _snprintf_s(pszName, sizeof(pszName), _TRUNCATE, "screenshot\\%d\\",
                  nidx);
    ...
    _snprintf_s(fileName, sizeof(fileName), _TRUNCATE, "%scap%05d.bmp", pszName,
                g_nScreenCap);
    g_Render.CaptureScreen(fileName);

    char szTip[64];
    // sprintf(szTip, RES_STRING(CL_LANGUAGE_MATCH_75), fileName);
    _snprintf_s(szTip, sizeof(szTip), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_75), fileName);
    Tip(szTip);
    g_nScreenCap++;
...
void CGameApp::_RenderTipText() {
  if (!_TipText.empty()) {
    ...
    int nTextY = 220;
    SIZE sz;
    for (list<STipText *>::iterator it = _TipText.begin(); it != _TipText.end();
         it++) {
      STipText *pText = (*it);
      ...
      g_CFont.DrawText(pText->szText, (GetWindowWidth() - sz.cx) / 2, nTextY,
                       dwColor);
      nTextY += 24;
    }
...
#else
  ulong musid =
      AudioSDL::get_instance()->get_resID(pInfo->szDataName, TYPE_WAV);
...
void CGameApp::_ShowLoading() {
  ...
  if (GetCurScene()->GetTerrain() && _nSwitchScene > LOAD_DELAY) {
    ...
    if (pMain) {
      // LG("load", "camera pos = %.1f  %.1f  %.1f\n", _pMainCam->m_vCurPos.x,
      // _pMainCam->m_vCurPos.y, _pMainCam->m_vCurPos.z);
      ...
      if (isOK) {
        ...
        // GetMainCam()->InitModel(pMain->IsBoat() ? 3 : 0,
        // &D3DXVECTOR3(fX,fY,fHeight)); CCameraCtrl *pCam =
        // g_pGameApp->GetMainCam(); LETerrain *pTerr = GetTerrain();
  ...
  if (frmLoading) {
    for (int i = 0; i < 32; i++) {
      ...
      if (i == 32 - _nSwitchScene / 4) {
        flashProgress[i]->SetPos(0,
                                 (int)(GetRender().GetScreenHeight() * 0.9f));
...
void CGameApp::RefreshLoadingProgress() {
  ...
  if (frmLoading) {
    ...
    if (imgLoading) {
      float fPrecent = ((GetCurTick() - _dwLoadingTick) % PROGRESS_CRYCLE) /
                       (float)(PROGRESS_CRYCLE); // 在当前进度条的百分比
      int nX = (int)((PROGRESS_WIDTH - imgLoading->GetWidth()) * fPrecent +
                     PROGRESS_X);
      int nY = PROGRESS_Y;
...
void CGameApp::ResetCaption() {
  ...
  char szCaption[350];
  //_snprintf_s(szCaption, _countof( szCaption ), _TRUNCATE,
  //RES_STRING(CL_LANGUAGE_MATCH_77), (float)(g_sClientVer % 1000) / 100,
  //szSpace ); // , curr_ver.c_str());
  _snprintf_s(szCaption, _countof(szCaption), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_77),
              szSpace); // , curr_ver.c_str());
  SetCaption("King of Pirates Online");
}
...
void CGameApp::Loading(int nFrame) {
  ...
  frmLoading->SetSize(GetRender().GetScreenWidth(),
  ...
  for (int i = 1; i <= 12; ++i) {
    ...
    //	imgLoading = dynamic_cast<CImage *>(frmLoading->Find((sImgFile + itoa(i,
    //szBuf, 10)).c_str()))  ;
    _itoa_s(i, szBuf, sizeof(szBuf), 10);
    imgLoading =
        dynamic_cast<CImage *>(frmLoading->Find((sImgFile + szBuf).c_str()));
    if (imgLoading) {
...
void CGameApp::InitAllTable() {
  ...
  if (g_bBinaryTable) {
    ...
    default:
      // CLanguageRecord t;
      // t.MadeBinFile("./scripts/table/serverset.bin",
      // "./scripts/table/serverset.txt");

      pServer->LoadRawDataInfo("scripts/table/ServerSet", bBinary);
    }
    //  end
  } else {
    // add by Philip.Wu  2006-06-12  修改 makebin 不能完整生成全部的
    // serverset.txt （策划需求）
    pServer->LoadRawDataInfo("scripts/table/ServerSet", bBinary);

    // Delete by lark.li 20080305
    // add by Philip.Wu  2006-07-31  多语言统一后的字符串资源生成 BIN 文件
    // g_oLangRec.MadeBinFile("./scripts/table/StringSet.bin",
    // "./scripts/table/StringSet.txt");
  }
  ...
  CItemRefineSet *pItemRefineSet =
      new CItemRefineSet(0, g_Config.m_nMaxItemType);
  pItemRefineSet->LoadRawDataInfo("scripts/table/ItemRefineInfo", bBinary);

  CItemRefineEffectSet *pItemRefineEffectSet =
      new CItemRefineEffectSet(0, 5000);
  pItemRefineEffectSet->LoadRawDataInfo("scripts/table/ItemRefineEffectInfo",
                                        bBinary);
...
void CGameApp::ReleaseAllTable() { //安全释放资源表 by Waiting 2009-06-18
...
bool CGameApp::LoadRes4() {
  ...
  for (int i = 1; i < 1200; i++) {
    ...
    if (pInfo && (pInfo->chCtrlType == 1 || pInfo->chCtrlType == 2)) {
      _snprintf_s(szBone, _countof(szBone), _TRUNCATE, "%04d.lab",
                  pInfo->sModel);
...
void LoadResModelBuf(LEIResourceMgr *res_mgr) {
  LEIResBufMgr *buf_mgr = res_mgr->GetResBufMgr();
  LEIPathInfo *path_info =
      res_mgr->GetSysGraphics()->GetSystem()->GetPathInfo();
  ...
  for (DWORD i = 0; i < model_num; i++) {
    ...
    //      }
    handle = obj_info->nID;
    _snprintf_s(path, _countof(path), _TRUNCATE, "%s%s", model_path,
                obj_info->szDataName);
...
void CGameApp::_SceneError(const char *info, CGameScene *p) {
  if (p) {
    MsgBox(RES_STRING(CL_LANGUAGE_MATCH_78), p->GetInitParam()->strName.c_str(),
           info);
  } else {
      ...
      for (int j = 0; j < nMax; j++) {
        if (pHair->IsChaUse[j]) {
          if (!pHairCha[j]->ChangePart(enumEQUIP_HEAD, pHair->dwItemID))
            SysInfo(RES_STRING(CL_LANGUAGE_MATCH_85), i, j + 1,
                    pHair->dwItemID);

          for (int k = 0; k < pHair->GetFailItemNum(); k++) {
            if (!pHairCha[j]->ChangePart(enumEQUIP_HEAD,
                                         pHair->dwFailItemID[k]))
              SysInfo(RES_STRING(CL_LANGUAGE_MATCH_86), i, j + 1,
                      pHair->dwFailItemID[k]);
          }
          AutoTestUpdate();
        }
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      for (int j = 0; j < 5; j++) {
        if (strcmp(pInfo->chModule[j], "0") != 0) {
          ...
          if (!IsExistFile(file.c_str())) {
            ...
            if (!IsExistFile(file.c_str())) {
              if (j == 0) {
                AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_88), i, pInfo->szName,
                             pInfo->chModule[0]);
              } else {
                AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_89), i, pInfo->szName,
                             j, pInfo->chModule[j]);
              }
            }
          }
      ...
      if (strcmp(pInfo->szICON, "0") != 0) {
        if (!IsExistFile(pInfo->GetIconFile()))
          AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_90), i, pInfo->szName,
                       pInfo->szICON);
      }
      ...
      if (nEffectID > 0) {
        ...
        if (!pEffectInfo) {
          AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_91), i,
                       pItem->GetItemInfo()->szName, nEffectID);
        }
      }
      for (int j = 0; j < pInfo->sEffNum; j++) {
        ...
        if (nEffectID > 0) {
          pEffectInfo = GetMagicInfo(nEffectID);
          if (!pEffectInfo) {
            AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_92), i,
                         pItem->GetItemInfo()->szName, nEffectID);
          }
        }
      }
      ...
      if (nEffectID > 0) {
        pEffectInfo = GetMagicInfo(nEffectID);
        if (!pEffectInfo) {
          AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_93), i,
                       pItem->GetItemInfo()->szName, nEffectID);
        }
      }
      ...
      if (nEffectID > 0) {
        pEffectInfo = GetMagicInfo(nEffectID);
        if (!pEffectInfo) {
          AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_94), i,
                       pItem->GetItemInfo()->szName, nEffectID);
        }
      }
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      for (int k = 0; k < ITEM_REFINE_NUM; k++) {
        ...
        if (nEffectID > 0) {
          ...
          if (!pEffectInfo) {
            AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_97), i,
                         pRefine->szDataName, nEffectID);
          }
        }
      }
    }
      ...
      for (int k = 0; k < 4; k++) {
        ...
        for (int j = 0; j < nEffectNum; j++) {
          ...
          for (int level = 0; level < 4; level++) {
            ...
            pEffectInfo = GetMagicInfo(nEffectID);
            if (!pEffectInfo) {
              AutoTestInfo(RES_STRING(CL_LANGUAGE_MATCH_99), i,
                           pInfo->szDataName, nEffectID);
            }
          }
        }
      }
