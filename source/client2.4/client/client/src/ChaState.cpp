...
CChaStateMgr::CChaStateMgr(CCharacter *pCha) : _pCha(pCha) {
  if (!_IsInit) {
    _IsInit = true;
...
void CChaStateMgr::ChaDestroy() {
  for (states::iterator it = _states.begin(); it != _states.end(); it++) {
    if ((*it)->pEffect) {
      // 			LG( _pCha->getLogName(),
      // RES_STRING(CL_LANGUAGE_MATCH_29), (*it)->pEffect->getIdxID(),
      // (*it)->pEffect );
...
CBoolSet &CChaStateMgr::Synchro(stSkillState *pState, int nCount) {
  ...
  for (unsigned int i = 0; i < nTmpCount; i++) {
    ...
    if (pChaState->IsDel) {
      //			LG( _pCha->getLogName(),
      //RES_STRING(CL_LANGUAGE_MATCH_30), pChaState->pInfo->nID,
      //pChaState->pInfo->szName, pChaState->pInfo->sEffect );

      // 已有的删除
      if (pChaState->pEffect) {
        //				LG( _pCha->getLogName(),
        //RES_STRING(CL_LANGUAGE_MATCH_31), pChaState->pEffect->getIdxID(),
        //pChaState->pEffect );
      ...
      //			LG( _pCha->getLogName(),
      //RES_STRING(CL_LANGUAGE_MATCH_32), pChaState->pInfo->nID,
      //pChaState->pInfo->szName, pChaState->pInfo->sEffect );
      ...
      if (pInfo->sEffect > 0 && !pChaState->pEffect) {
        pChaState->pEffect =
            _pCha->SelfEffect(pInfo->sEffect, pInfo->sDummy1, true);
        //				LG( _pCha->getLogName(),
        //RES_STRING(CL_LANGUAGE_MATCH_33), pInfo->sEffect, pInfo->sDummy1,
        //pChaState->pEffect );
      }
    }
  }
