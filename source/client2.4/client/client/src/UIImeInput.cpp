...
CImeInput::CImeInput(void)
    : _pImage(NULL), _ImmNameColor(0xf00000ff), _pList(NULL), _nShowX(0),
      _nShowY(0), _bIsFull(false), _hImc(NULL) {
...
CImeInput::~CImeInput(void) { Clear(); }
...
bool CImeInput::HandleWindowMsg(DWORD dwMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INPUTLANGCHANGE: {
    ...
    if (len > 0) {
      ...
      strncpy_s(_strImmName, sizeof(_strImmName),
                RES_STRING(CL_LANGUAGE_MATCH_622), _TRUNCATE);
    }
    return true;
  } break;
...
void CImeInput::Render() {
  ...
  CGuiFont::s_Font.Render(_strInterpunction, _nShowX + 82, _nShowY + 7,
                          _ImmNameColor);
  CGuiFont::s_Font.Render(_strComposition, _nShowX + 7, _nShowY + 25,
                          _ImmNameColor);
    ...
    if (dwSize = ImmGetCandidateList(_hImc, 0x0, NULL, 0)) {
      ...
      if (_pList) {
        ...
        if (!_pList->dwPageStart) {
          for (unsigned int i = _pList->dwPageStart + _pList->dwSelection;
               (i < _pList->dwCount && i < _pList->dwPageStart +
                                               _pList->dwPageSize +
                                               _pList->dwSelection);
               i++) {
            _snprintf_s(temp, _countof(temp), _TRUNCATE, " %d:%s\n",
                        i + 1 - _pList->dwSelection,
                        (char *)_pList + _pList->dwOffset[i]);
            // strcat(_strCandidate, temp);
            strncat_s(_strCandidate, _countof(_strCandidate), temp, _TRUNCATE);
          }
        } else {
          for (unsigned int i = _pList->dwSelection;
               (i < _pList->dwCount &&
                i < _pList->dwPageStart + _pList->dwPageSize);
               i++) {
            _snprintf_s(temp, _countof(temp), _TRUNCATE, " %d:%s\n",
                        i + 1 - _pList->dwSelection,
                        (char *)_pList + _pList->dwOffset[i]);
...
bool CImeInput::_GetConversion() {
  ...
  if (_lConversion & 0x01) {
    ...
    strncpy_s(&_strInput[1], 2, "中", _TRUNCATE);
    _strInput[3] = ']';
    _strInput[4] = 0;
  } else {
    _strInput[0] = '[';
    // strncpy(&_strInput[1], "英", 2);
    strncpy_s(&_strInput[1], 2, "英", _TRUNCATE);
    _strInput[3] = ']';
    _strInput[4] = 0;
  }
  if (_lConversion & 0x08) {
    ...
    strncpy_s(&_strSBC[1], 2, "全", _TRUNCATE);
    _strSBC[3] = ']';
    _strSBC[4] = 0;
  } else {
    _strSBC[0] = '[';
    // strncpy(&_strSBC[1], "半", 2);
    strncpy_s(&_strSBC[1], 2, "半", _TRUNCATE);
    _strSBC[3] = ']';
    _strSBC[4] = 0;
  }
  if (_lConversion & 0x400) {
    ...
    strncpy_s(&_strInterpunction[1], 4, "。，", _TRUNCATE);
    _strInterpunction[5] = ']';
    _strInterpunction[6] = 0;
  } else {
    _strInterpunction[0] = '[';
    // strncpy(&_strInterpunction[1], ".,", 2);
    strncpy_s(&_strInterpunction[1], 2, ".,", _TRUNCATE);
    _strInterpunction[3] = ']';
