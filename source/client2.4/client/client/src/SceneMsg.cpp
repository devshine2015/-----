...
void CGameScene::HandleSceneMsg(int nMsgType, int nParam1, int nParam2,
                                int nParam3) {
  ...
  case SCENEMSG_CHA_CREATE: {
    ...
    for (int n = 0; n < pInfo->sEeffID; n++) {
      ...
      for (int m = 0; m < peffInfo->nDummyNum; m++) {
        ...
        if (pCha->IsBoat() && (pInfo->sFeffID[n] == EFFECT_RIPPLE1 ||
                               pInfo->sFeffID[n] == EFFECT_RIPPLE2)) {
          // pEffect->setFollow();
        ...
        if (nParam2 == CHA1 || nParam2 == CHA2 || nParam2 == CHA3 ||
            nParam2 == CHA4) {
          pEffect->SetHide(TRUE);
        }
  ...
  case SCENEMSG_CHA_BEGINMOVE: {
    ...
    if (pCha->IsBoat()) {
      ...
      for (int n = 0; n < size; n++) {
        ...
        if (pEffect && ((pEffect->getIdxID() == CHA_EFFECT1) ||
                        (pEffect->getIdxID() == CHA_EFFECT2) ||
                        (pEffect->getIdxID() == CHA_EFFECT3) ||
                        (pEffect->getIdxID() == CHA_EFFECT4))) {
          pEffect->SetHide(false);
        }
      }
    }
    // CEffectObj *pEff = GetEffect(pCha->GetEffectID(0));
    // if(pEff && ((pEff->getIdxID()==CHA_EFFECT1)
    // ||(pEff->getIdxID()==CHA_EFFECT2)
  ...
  case SCENEMSG_CHA_ENDMOVE: {
    ...
    if (pCha->IsBoat()) {
      ...
      // if(pEff && ((pEff->getIdxID()==CHA_EFFECT1)
      // ||(pEff->getIdxID()==CHA_EFFECT2)
      //	||(pEff->getIdxID()==CHA_EFFECT3)
      //||(pEff->getIdxID()==CHA_EFFECT4)))
      //{
      //	pEff->SetHide(true);
      //	pEff = GetEffect(pCha->GetEffectID(1));
      //	if(pEff)
  ...
  case SCENEMSG_SCENEOBJ_DESTROY: {
    pObj = GetSceneObj(nParam1);
    if (pObj->getObjType() == SCENEOBJ_TYPE_ENVSOUND &&
        pObj->GetMusicID() != -1) {
  ...
  case SCENEMSG_SCENEITEM_CREATE: {
    ...
    if (nParam2 != 0) {
      ...
      for (int n = 0; n < pInfo->sEffNum; n++) {
        ...
        pItem->bindEffect(pInfo->sEffect[n][1], pInfo->sEffect[n][0], true);
        ...
        //{
        //	LG("ERROR","create cha`s effect fail,ID %d",
        //pInfo->sEffect[n][0]); 	return;
        //}
