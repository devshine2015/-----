...
CAmphitheaterList::CAmphitheaterList(CForm &frmOwn)
    : CList(frmOwn), _ButtonLen(10) {
...
void CAmphitheaterList::Render() {
  ...
  for (int i = 0; i < _ButtonLen; i++) {
    if (_button[i] && _button[i]->GetIsShow())
      _button[i]->Render();
  }
}
  ...
  if (_buttonPip) {
    ...
    for (int i = 0; i < _ButtonLen; i++) {
      if (!_button[i]) {
        ...
        _button[i]->SetPos(_buttonPip->GetLeft(),
                           _buttonPip->GetTop() + _nRowHeight * i);
...
bool CAmphitheaterForm::Init() {
  ...
  if (!frmNPCAmphitheater)
    return false;

  m_pSecPromotionList = dynamic_cast<CAmphitheaterList *>(
      frmNPCAmphitheater->Find("SecPromotionList"));
  if (!m_pSecPromotionList)
    return false;

  m_pSecReliveList = dynamic_cast<CAmphitheaterList *>(
      frmNPCAmphitheater->Find("SecReliveList"));
  if (!m_pSecReliveList)
    return false;
...
//投票按钮响应
void CAmphitheaterForm::ListMouseRun(CGuiData *pSender, int x, int y,
                                     DWORD key) {
  int ID = 6068;
  ...
  if (IdNum <= 0) {
    ...
    for (; i < 20; i++) {
      if (pSender ==
          (CGuiData *)(m_pStaticAmphiForm->m_pSecReliveList->_button[i])) {
        CSelectItem *item =
            m_pStaticAmphiForm->m_pSecReliveList->GetItems()->GetSelect();
        int index = item->GetIndex();
        int ReID = 0;
        TeamRelUnit *data2 =
            m_pStaticAmphiForm->m_pAmphitheaterData.GetTeamRel(index);
        ReID = data2->RelID;
...
void CAmphitheaterForm::CloseForm() { frmNPCAmphitheater->Hide(); }
