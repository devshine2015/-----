...
CRumBuyMgr::CRumBuyMgr()
    : frmRumBuy(NULL), editCount(NULL), labelPrice(NULL), labelTotal(NULL),
      labelTax(NULL), BidPrice(0), BidCount(0) {}
...
bool CRumBuyMgr::Init() {
  ...
  CTextButton *btnDeal01 =
      static_cast<CTextButton *>(frmRumBuy->Find("btnDeal01"));
  assert(btnDeal01);
...
void CRumBuyMgr::evtBuy(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if ((CRumDealMgr::GetIndex(true) +
       g_stRumBuyForm.BidPrice * g_stRumBuyForm.BidCount) >
      (g_stRumDealForm.GoldCoins + GoldCoins)) {
