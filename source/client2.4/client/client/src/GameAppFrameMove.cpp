...
void CGameApp::_FrameMove(DWORD dwTimeParam, bool camMove) // Vim
{
  ...
  if (m_AddSceneObjList.size() > 0) {
    ...
    if (pObj) {
      ...
      //			LG("frameload",
      //RES_STRING(CL_LANGUAGE_MATCH_64), pAdd->nTypeID, pAdd->nPosX,
      //pAdd->nPosY);
    }
    delete pAdd;
  }
  ...
  case enumOldMusic:
    ...
    if (_nCurMusicSize <= 1) {
      ...
      if (_szBkgMusic[0] == '\0') {
        ...
        g_dwCurMusicID =
            AudioSDL::get_instance()->get_resID(_szBkgMusic, TYPE_MP3);
  ...
  if (g_Config.m_bEditor) {
    ...
    if (_pCurScene->GetTerrain()) {
      g_Render.Print(INFO_DEBUG, 290, 120, "showcenter = %f,%f",
                     _pCurScene->GetTerrain()->GetShowCenterX(),
                     _pCurScene->GetTerrain()->GetShowCenterY());
    }

    if (pCha) {
      g_Render.Print(INFO_DEBUG, 290, 410, "cha pos = %f,%f,%f",
                     pCha->GetPos().x * 100, pCha->GetPos().y * 100,
                     pCha->GetPos().z * 100);
      g_Render.Print(
          INFO_DEBUG, 290, 430, "cha angle = %d",
          (int)(int(((float)pCha->getYaw() * 0.01745329f /*+ D3DX_PI*/) *
                    57.29577f) %
                360));
    }
  }
...
void CGameApp::_PreMouseRun(DWORD dwMouseKey) {
  ...
  case enumMA_Drill:
    _MouseInScene =
        CCozeForm::GetInstance()->IsMouseOnList(GetMouseX(), GetMouseY());
    break;
  }
}
