...
DSoundInstance::DSoundInstance(DSoundManager *theSoundManager,
                               LPDIRECTSOUNDBUFFER theSourceSound) {
  ...
  if (mSourceSoundBuffer != NULL) {
    hr = mSoundManagerP->mDirectSound->DuplicateSoundBuffer(mSourceSoundBuffer,
                                                            &mSoundBuffer);
    if (hr != DS_OK) {
      switch (hr) {
...
void DSoundInstance::RehupVolume() {
  if (mSoundBuffer != NULL)
    mSoundBuffer->SetVolume(mSoundManagerP->VolumeToDB(
        mBaseVolume * mVolume * mSoundManagerP->mMasterVolume *
        mSoundManagerP->m_fFadeByWindow));
}
...
double DSoundInstance::GetVolume() { return mVolume; }