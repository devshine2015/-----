...
bool CBoxMgr::Init() {
  ...
  if (!edtNumber)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmTrade->GetName(),
                 "edtNumber");
  ...
  if (!labInfo)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmSelect->GetName(),
                 "labInfo");
  ...
  if (!edtNumber)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmNumber->GetName(),
                 "edtNumber");
  edtNumber->SetIsDigit(true);
  ...
  if (!edtPassword)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPassword->GetName(),
                 "edtPassword");
  ...
  if (!labError)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmError->GetName(),
                 "labError");
...
stTradeBox *CBoxMgr::ShowTradeBox(FormMouseEvent evtForm, float fUnitMoney,
                                  int nTotalNum, const char *szName) {
  ...
  if (nTotalNum > 0) {
    char buf[32] = {0};
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d", nTotalNum);
...
stSelectBox *CBoxMgr::ShowSelectBox(FormMouseEvent evtForm, const char *szTitle,
                                    bool bModal) {
  ...
  // frm->SetSize( CGuiFont::s_Font.GetWidth( t->labInfo->GetCaption() ) +
  // t->labInfo->GetLeft() * 2, frm->GetHeight() );
...
stNumBox *CBoxMgr::ShowNumberBox(FormMouseEvent evtForm, int nTotalNum,
                                 const char *szTitle, bool IsMax, bool bModal) {
...
stMsgTimeBox *CBoxMgr::ShowMsgTime(FormMouseEvent evtForm, const char *szTitle,
                                   int iSeconds) {
  ...
  _cfrmError->SetSize(CGuiFont::s_Font.GetWidth(t->labInfo->GetCaption()) +
                          t->labInfo->GetLeft() * 2,
                      _cfrmError->GetHeight());
  _cfrmError->SetPos(
      (GetRender().GetScreenWidth() - _cfrmError->GetWidth()) / 2,
...
stMsgBox *CBoxMgr::ShowMsgBox(FormMouseEvent evtForm, const char *szTitle,
                              bool bModal) {
...
stPasswordBox *CBoxMgr::ShowPasswordBox(FormMouseEvent evtForm,
                                        const char *szTitle,
                                        const char *szPassword, bool bModal) {
...
void CBoxMgr::FrameMove(DWORD dwTime) {
  ...
  if (_cStart) {
    if (_cTick == 0) {
      ...
      if (dwTime - _cTick > (DWORD)_cSeconds * 1000) {
        if (_cfrmError) {
          CLabel *labInfo =
              dynamic_cast<CLabel *>(_cfrmError->Find("labError"));
          // labInfo->SetCaption(
...
void stTradeBox::Refresh() {
  ...
  if (nTotalNum > 0) {
    if (n > nTotalNum) {
      ...
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%d", n);
  ...
  _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s",
              StringSplitNum((int)((float)n * fUnitMoney)));
