...
DR_RESULT lwItemLit::Load(const char *file) {
  ...
  DR_ZERO_RET(fp);
  DR_FAILED_RET(
      fp->CreateFile(file, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING));
  ...
  for (i = 0; i < item_num; i++) {
    ...
    DR_FAILED_RET(
        fp->Read(&item_info->descriptor, sizeof(item_info->descriptor), 0));
...
DR_RESULT lwItemLit::Save(const char *file) {
  ...
  DR_FAILED_RET(drGUIDCreateObject((lxvoid **)&fp, DR_GUID_FILE));
  DR_ZERO_RET(fp);
  DR_FAILED_RET(fp->CreateFile(file, GENERIC_WRITE, FILE_SHARE_READ, NULL,
                               CREATE_ALWAYS));
  ...
  for (i = 0; i < item_num; i++) {
    ...
    DR_FAILED_RET(
        fp->Write(&item_info->descriptor, sizeof(item_info->descriptor), 0));
...
DR_RESULT lwItemLit::AddItem(DWORD id, const char *desc,
                             lwItemLitInfo **ret_info) {
  DR_RESULT ret = DR_RET_FAILED;
  lwItemLitInfo *info;
