...
bool CPKSilverMgr::AddFormAttribute(int idx, const std::string &szName,
                                    long sLevel, const std::string &szJob,
                                    long lPkval) {
  if ((idx < 0) || (idx >= MAX_PKSILVER_PLAYER)) {
    return false;
  }

  if ((!labName[idx]) || (!labLevel[idx]) || (!labJob[idx]) ||
      (!labDate[idx])) {
    return false;
  }
...
// void CPKSilverMgr::_evtPKSilverSortBtn(CCompent *pSender, int nMsgType, int
// x, int y, DWORD dwKey)
// {
