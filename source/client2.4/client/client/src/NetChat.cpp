...
void NetSay2You(stNetSay2You &say2you) //私聊
{
  ...
  CCozeForm::GetInstance()->OnPrivateMsg(say2you.m_src, say2you.m_dst,
                                         say2you.m_content);
}
...
void NetPCTeam(stNetPCTeam &pcteam) {
  ...
  switch (pcteam.kind) {
  case TEAM_MSG_KICK:
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_218),
                        pcteam.cha_name[pcteam.count - 1]);
  case TEAM_MSG_LEAVE:
  ...
  case TEAM_MSG_GROUP_ADD: // 成立队伍
  {
    ...
    if (pTeam) {
      for (int i = 0; i < pcteam.count; i++) {
        pTeam->Add(pcteam.cha_dbid[i], pcteam.cha_name[i], pcteam.motto[i],
                   pcteam.cha_icon[i], "", true);
      }
    }
  } break;
  ...
  case TEAM_MSG_ADD: // 子类型:增加新队员
  {
    CTeam *pTeam = pTeamMgr->Find(enumTeamGroup);
    if (pTeam) {
      pTeam->Add(pcteam.cha_dbid[pcteam.count - 1],
                 pcteam.cha_name[pcteam.count - 1],
                 pcteam.motto[pcteam.count - 1],
                 pcteam.cha_icon[pcteam.count - 1], "", true);
    }
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_221),
                        pcteam.cha_name[pcteam.count - 1]);
  } break;
  // case TEAM_MSG_OFFLINE:      // 子类型:队员下线
  case TEAM_MSG_LEAVE: // 子类型:队员离开
    g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_222),
                        pcteam.cha_name[pcteam.count - 1]);
  case TEAM_MSG_KICK: {
    CTeam *pTeam = pTeamMgr->Find(enumTeamGroup);
    if (pTeam) {
...
void NetTeamInvite(const char *inviter_name, unsigned long inviter_chaid,
                   unsigned short icon) {
...
void NetFrndInvite(const char *inviter_name, unsigned long inviter_chaid,
                   unsigned short icon) {
...
void NetFrndAdd(unsigned long cha_id, const char *cha_name, const char *motto,
                unsigned short icon_id, const char *grp) { // grp-所属好友分组
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_241),cha_name,cha_id);
  // Modify by lark.li 20080813 begin
  // g_stUIChat.GetTeamMgr()->Find( enumTeamFrnd
  // )->Add(cha_id,cha_name,motto,icon_id);
  g_stUIChat.GetTeamMgr()
      ->Find(enumTeamFrnd)
      ->Add(cha_id, cha_name, motto, icon_id, grp, true);
  // End
  // g_stUIChat.ReceiveMsg(
  // enumSCM_FRND_ADD,(void*)cha_name,cha_id,icon_id,(void*)motto);
}

// Modify by lark.li 20080804 begin
// void NetFrndStart(stNetFrndStart &self,stNetFrndStart arrCha[],unsigned short
// arrnum)
void NetFrndStart(stNetFrndStart &self, stNetFrndStart arrCha[],
                  unsigned short arrnum, string groupNames[],
...
// End
{
  ...
  for (int i = 0; i < groupNum; i++) {
    g_stUIChat.GetTeamMgr()
        ->Find(enumTeamFrnd)
        ->AddGroupName(groupNames[i].c_str());
  }
  // End
  ...
  for (int i = 0; i < arrnum; i++) {
    ...
    // CMember *pMember=g_stUIChat.GetTeamMgr()->Find( enumTeamFrnd
    // )->Add(arrCha[i].lChaid,arrCha[i].szChaname,arrCha[i].szMotto,arrCha[i].sIconID);
    CMember *pMember =
        g_stUIChat.GetTeamMgr()
            ->Find(enumTeamFrnd)
            ->Add(arrCha[i].lChaid, arrCha[i].szChaname, arrCha[i].szMotto,
                  arrCha[i].sIconID, arrCha[i].szGroup);
    // End
...
void NetFrndMoveGroup(unsigned long cha_id, const char *oldgroup,
                      const char *newgroup) {
  g_stUIChat.GetTeamMgr()
      ->Find(enumTeamFrnd)
      ->MoveGroup(cha_id, oldgroup, newgroup);
}

void NetFrndChangeGroup(const char *oldgroup, const char *newgroup) {
  g_stUIChat.GetTeamMgr()
      ->Find(enumTeamFrnd)
      ->ChangeGroupName(oldgroup, newgroup);
}
// End

// Add by lark.li 20080808 begin
void NetFrndRefreshInfo(unsigned long cha_id, stPersonInfo *pInfo,
                        const char *motto, unsigned short icon,
                        unsigned short degree, const char *job,
  ...
  if (g_stUIChat._dwSelfID == cha_id) // 是自己
  {
    ...
    CMember *pMember =
        g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(cha_id);
    if (pMember) {
...
// void NetFrndRefreshInfo(unsigned long cha_id,const char * motto,unsigned
// short icon,unsigned short degree,const char * job,const char *guildname)
//{
//	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_243),cha_id,motto,icon,degree,job);
//	CMember
//*pMember=g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(cha_id); 	if
//(pMember)
//	{
//		pMember->ModifyAttr(motto, job, degree, icon, guildname);
//
//		// todo
//
//		/*if (g_stUIChat._frmDetails->GetIsShow() &&
//g_stUIChat._frmDetails->nTag==(int)pMember->GetID())
//		{
...
void NetChangePersonInfo(const char *motto, unsigned short icon,
                         bool refuse_sess) {
...
void NetSessCreate(unsigned long newsessid, stNetSessCreate cha[],
                   short chanum) {
  ...
  if (cha[0].lChaID == g_stUIChat._dwSelfID) {
    ...
    CTalkSessionFormMgr::CreateHiddenSessionForm(newsessid, pArrName,
                                                 chanum - 1);
  }
...
extern void NetSessLeave(unsigned long sessid, unsigned long chaid) {
  ...
  if (g_stUIChat._dwSelfID != chaid) {
    if (!pSession) {
      // char szBuf[2048] = { 0 };
      // sprintf( szBuf, "NetSessLeave(sessid:%d, chaid:%d, selfid:%d) error,
      // pSession==NULL SessionCount=%d\n", sessid, chaid, g_stUIChat._dwSelfID,
      // CTalkSessionFormMgr::GetSessionCount() );
      // MessageBox(g_pGameApp->GetHWND(), "error", szBuf, MB_OK);
      // CTalkSessionFormMgr::PrintTraceInfo();
      return;
    }
    if (pSession->GetTotalMembers() == 1 &&
        pSession->GetStyle() == enumTalkSessionHidden) {
...
extern void NetSessSay(unsigned long sessid, unsigned long chaid,
                       const char *word) {
  CTalkSessionForm *pSession = CTalkSessionFormMgr::GetSessionFormByID(sessid);
  // CMember *pMember=g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(chaid);
  // if (pMember)
  //{
  //	if (pSession && pSession->GetTotalMembers()==1 &&
  //pSession->GetStyle()!=enumTalkSessionNormal) 	pMember->SetIsFlash(true);
  //}
...
void NetMasterOnline(unsigned long cha_id) {
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_236),cha_id);
  CMember *pMember =
      g_stUIChat.GetTeamMgr()->Find(enumTeamMaster)->Find(cha_id);
  if (pMember) {
    pMember->SetOnline(true);
...
void NetMasterAdd(unsigned long cha_id, const char *cha_name, const char *motto,
                  unsigned short icon_id, const char *grp) {
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_241),cha_name,cha_id);
  // g_stUIChat.GetTeamMgr()->Find( enumTeamMaster
  // )->Add(cha_id,cha_name,motto,icon_id);
  g_stUIChat.GetTeamMgr()
      ->Find(enumTeamMaster)
      ->Add(cha_id, cha_name, motto, icon_id, grp,
            true); // modify by ning.yan
                   // 解决拜师（收徒）时客户端没有及时显示在线信息  2008-11-14
}

void NetMasterStart(stNetFrndStart &self, stNetFrndStart arrCha[],
                    unsigned short arrnum) {
  g_stUIChat._dwSelfIcon = self.sIconID;
  g_stUIChat._strSelfMottoName = self.szMotto;
  g_stUIChat._dwSelfID = self.lChaid;
  for (int i = 0; i < arrnum; i++) {
    //		LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_242),i,arrCha[i].szChaname,arrCha[i].lChaid);
    CMember *pMember = g_stUIChat.GetTeamMgr()
                           ->Find(enumTeamMaster)
                           ->Add(arrCha[i].lChaid, arrCha[i].szChaname,
                                 arrCha[i].szMotto, arrCha[i].sIconID);
    pMember->SetOnline(arrCha[i].cStatus != 0);
  }
  g_stUIChat._UpdateSelfInfo();
}
...
void NetMasterRefreshInfo(unsigned long cha_id, const char *motto,
                          unsigned short icon, unsigned short degree,
...
void NetPrenticeOnline(unsigned long cha_id) {
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_236),cha_id);
  CMember *pMember =
      g_stUIChat.GetTeamMgr()->Find(enumTeamPrentice)->Find(cha_id);
  if (pMember) {
    pMember->SetOnline(true);
...
void NetPrenticeAdd(unsigned long cha_id, const char *cha_name,
                    const char *motto, unsigned short icon_id,
                    const char *grp) {
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_241),cha_name,cha_id);
  // g_stUIChat.GetTeamMgr()->Find( enumTeamPrentice
  // )->Add(cha_id,cha_name,motto,icon_id);
  g_stUIChat.GetTeamMgr()
      ->Find(enumTeamPrentice)
      ->Add(cha_id, cha_name, motto, icon_id, grp,
            true); // modify by ning.yan  2008-11-14
                   // 解决收徒（拜师）客户端及时显示
}

void NetPrenticeStart(stNetFrndStart &self, stNetFrndStart arrCha[],
                      unsigned short arrnum) {
  g_stUIChat._dwSelfIcon = self.sIconID;
  g_stUIChat._strSelfMottoName = self.szMotto;
  g_stUIChat._dwSelfID = self.lChaid;
...
void NetPrenticeRefreshInfo(unsigned long cha_id, const char *motto,
                            unsigned short icon, unsigned short degree,
                            const char *job, const char *guildname) {
  //	LG("test_frnd",RES_STRING(CL_LANGUAGE_MATCH_243),cha_id,motto,icon,degree,job);
  CMember *pMember =
