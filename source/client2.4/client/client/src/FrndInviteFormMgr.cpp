...
bool CFrndInviteFormMgr::AddInviteForm(DWORD id, string inviterName) {
  ...
  char str[256];
  _snprintf_s(str, _countof(str), _TRUNCATE, "frmAcceptFrnd_Arcol_%d", id);
  node->pForm->SetName(str);
  m_FormLink.push_back(node);
  // node->pForm->SetPos(200,200);
  CLabelEx *labFrndName =
      dynamic_cast<CLabelEx *>(node->pForm->Find("labFrndName"));
  if (labFrndName) {
...
void CFrndInviteFormMgr::_MainMousePlayerFrndEvent(CCompent *pSender,
