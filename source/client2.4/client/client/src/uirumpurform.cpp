...
CRumPurMgr::CRumPurMgr()
    : frmRumPur(NULL), labelCount(NULL), labelPrice(NULL), labelTotal(NULL),
      labelTax(NULL) {}
...
bool CRumPurMgr::Init() {
  ...
  CTextButton *btnDeal03 =
      static_cast<CTextButton *>(frmRumPur->Find("btnDeal13"));
  assert(btnDeal03);
...
void CRumPurMgr::evtPur(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if ((CRumDealMgr::GetIndex(true) +
       g_stRumDealForm.BuyBidPrice * g_stRumDealForm.BuyBidCount) >
