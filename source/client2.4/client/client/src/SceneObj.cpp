...
bool  CSceneObj::IsBoxVisible_LineWithPlane(  LEVector3 vecMin, LEVector3 vecMax , DWORD &index)
{
//	return true;
...
int CSceneObj::HitTestForInfluence(int* flag, D3DXVECTOR3* t_pos, const D3DXVECTOR3* nPos)
{
    ...
    *flag = 0;
    ...
    if(DR_SUCCEEDED(HitTestHelperMesh(&p, (LEVector3*)&org, (LEVector3*)&ray, "block")))
    {
        *flag = 2;
        *t_pos = *(D3DXVECTOR3*)&p.pos;
    }
    else if(DR_SUCCEEDED(HitTestHelperMesh(&p, (LEVector3*)&org, (LEVector3*)&ray, "terrain")))
    {
        *flag = 1;
        *t_pos = *(D3DXVECTOR3*)&p.pos;
    }
...
int CSceneObj::UpdateObjFadeInFadeOut(const LEVector3* org, const LEVector3* ray)
{
    //if(_dwTranspFlag == 0)
    //    return 0;
