...
BOOL CSceneObj::_Create(int nScriptID, int nType) {
  ...
  //用途：在地表高度＝0的地方，贴一层半透明特效光影，因为与地表位置过于贴近，需要使用ZBIAS
  //避免破片，最多支持16层，数字越大越上层
  if (pInfo->nAttachEffectID > 0 && pInfo->nAttachEffectID <= 16) {
    ...
    for (DWORD i = 0; i < num; i++) {
      ...
      GetPrimitive(i)->GetMeshAgent()->GetMesh()->AddStateToSet(
          D3DRS_ZBIAS, pInfo->nAttachEffectID);
    }
  }
...
void CSceneObj::Render() {
  if (_dwCullingFlag) {
    ...
    for (DWORD i = 0; i < pri_num; i++) {
      ...
      LEVector3 v = box_info->box.c + box_info->box.r;
      ...
      if (pTerrain) {
        if (pTerrain->IsPointVisible(u.x, u.y) ||
            pTerrain->IsPointVisible(u.x, v.y) ||
            pTerrain->IsPointVisible(v.x, v.y) ||
            pTerrain->IsPointVisible(v.x, u.y)) {
  ...
  // int nX, nY; g_Render.GetScreenPos(nX, nY, D3DXVECTOR3((float)GetCurX() /
  // 100.0f, (float)GetCurY() / 100.0f, 0.6f)); g_Render.Print(INFO_GAME, nX, nY
  // - 40, "%d", _dwID);
}
...
bool CSceneObj::IsBoxVisible(LEVector3 vecMin, LEVector3 vecMax, DWORD &index) {
  static LEVector3 vecBounds[8];

  vecBounds[0] = LEVector3(vecMin.x, vecMin.y, vecMin.z);
  vecBounds[1] = LEVector3(vecMax.x, vecMin.y, vecMin.z);
  vecBounds[2] = LEVector3(vecMin.x, vecMax.y, vecMin.z);
  vecBounds[3] = LEVector3(vecMax.x, vecMax.y, vecMin.z);
  vecBounds[4] = LEVector3(vecMin.x, vecMin.y, vecMax.z);
  vecBounds[5] = LEVector3(vecMax.x, vecMin.y, vecMax.z);
  vecBounds[6] = LEVector3(vecMin.x, vecMax.y, vecMax.z);
  vecBounds[7] = LEVector3(vecMax.x, vecMax.y, vecMax.z);
...
bool CSceneObj::IsBoxVisible_LineWithPlane(LEVector3 vecMin, LEVector3 vecMax,
                                           DWORD &index) {
  //	return true;
  ...
  D3DXVECTOR3 face2[6][4] = {
      pCull->vecFrustum[0], pCull->vecFrustum[2],
      pCull->vecFrustum[3], pCull->vecFrustum[1], // front
      pCull->vecFrustum[4], pCull->vecFrustum[5],
      pCull->vecFrustum[7], pCull->vecFrustum[6], // back
      pCull->vecFrustum[0], pCull->vecFrustum[4],
      pCull->vecFrustum[6], pCull->vecFrustum[2], // left
      pCull->vecFrustum[1], pCull->vecFrustum[3],
      pCull->vecFrustum[7], pCull->vecFrustum[5], // right
      pCull->vecFrustum[2], pCull->vecFrustum[6],
      pCull->vecFrustum[7], pCull->vecFrustum[3], // top
      pCull->vecFrustum[0], pCull->vecFrustum[4],
      pCull->vecFrustum[5], pCull->vecFrustum[1], // bottom
  };
...
void CSceneObj::_UpdateValid(BOOL bValid) {
  ...
  _pScene->HandleSceneMsg(SCENEMSG_SCENEOBJ_UPDATEVALID, bValid, _dwObjType,
                          _dwID);
}
...
void CSceneObj::MoveLight(D3DXVECTOR3 *SVerPos) {
  ...
  for (int y = (int)_fy; y < (int)_fty; y++) {
    for (int x = (int)_fx; x < (int)_ftx; x++) {
      ...
      if (!pTile)
        continue;
      float fd = D3DXVec2LengthSq(
          &(D3DXVECTOR2(float(x), float(y)) - D3DXVECTOR2(_vPos.x, _vPos.y)));
...
int CSceneObj::HitTestForInfluence(int *flag, D3DXVECTOR3 *t_pos,
  ...
  *flag = 0;
  ...
  if (DR_SUCCEEDED(HitTestHelperMesh(&p, (LEVector3 *)&org, (LEVector3 *)&ray,
                                     "block"))) {
    *flag = 2;
    *t_pos = *(D3DXVECTOR3 *)&p.pos;
  } else if (DR_SUCCEEDED(HitTestHelperMesh(&p, (LEVector3 *)&org,
                                            (LEVector3 *)&ray, "terrain"))) {
    *flag = 1;
    *t_pos = *(D3DXVECTOR3 *)&p.pos;
  }
...
int CSceneObj::UpdateObjFadeInFadeOut(const LEVector3 *org,
                                      const LEVector3 *ray) {
  // if(_dwTranspFlag == 0)
  //    return 0;
