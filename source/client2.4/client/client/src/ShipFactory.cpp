...
//---------------------------------------------------------------------------
void xShipFactory::__ProcShipName(CGuiData *pSender) { __xsf->CheckShipName(); }

//---------------------------------------------------------------------------
//打开关闭船只货舱UI事件(Michael Chen 2005-05-26)
void xShipFactory::__ButtonOpenRoom(CGuiData *pSender, int x, int y,
                                    DWORD dwKey) {
  if (!__xsf)
    return;
...
//---------------------------------------------------------------------------
void xShipFactory::__ButtonCloseRoom(CGuiData *pSender, int x, int y,
                                     DWORD dwKey) {
  if (!__xsf)
    return;
...
//---------------------------------------------------------------------------
void xShipFactory::__CheckShip(CGuiData *pSender) { __xsf->UpdateBoatAttr(); }
...
//---------------------------------------------------------------------------
xShipFactory::~xShipFactory() { SAFE_DELETE(ship); }
...
//---------------------------------------------------------------------------
BOOL xShipFactory::IsVisible() { return sbf.wnd ? sbf.wnd->GetIsShow() : 0; }
...
void xShipFactory::Render3DView(int x, int y) {
  ...
  g_Render.LookAt(D3DXVECTOR3(0.0f, 10.0f, 10.0f),
                  D3DXVECTOR3(0.0f, 0.0f, 0.0f), LERender::VIEW_3DUI);

  drMatrix44 old_mat = *ship->GetMatrix();
...
BOOL xShipFactory::UpdateShipPart(CGuiData *sender) {
  ...
  CS_UpdateBoat(item_id[ITEM_HEAD], item_id[ITEM_POWER], item_id[ITEM_WEAPON],
                item_id[ITEM_GOODS]);
  /*
  if(i == 0)
  {
  ...
  */
  ...
  char buf[128];
  _snprintf_s( buf, _countof( buf ), _TRUNCATE,  "%d / %d", prop_att[0],
  prop_att[1]); sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf); _snprintf_s(
  buf, _countof( buf ), _TRUNCATE,  "%d / %d", prop_res[0], prop_res[1]);
  sbf.lbl_prop[SBF_TEXT_RESISTANCE]->SetCaption(buf);
  _snprintf_s( buf, _countof( buf ), _TRUNCATE,  "%d / %d", prop_supply[0],
  prop_supply[1]); sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf); _snprintf_s(
  buf, _countof( buf ), _TRUNCATE,  "%d / %d", prop_cap[0], prop_cap[1]);
...
BOOL xShipFactory::CheckShipName() {
  ...
  strncpy_s(ship_name, sizeof(ship_name), sbf.name->GetCaption(), _TRUNCATE);
  size_t l = _tcslen(ship_name);
  if (l == 0) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_390));
    return 0;
  } else if (l < MIN_LENGTH || l > MAX_LENGTH) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_391), MIN_LENGTH,
                       MAX_LENGTH);
    return 0;
  }
...
BOOL xShipFactory::Update(xShipBuildInfo *info, BOOL flag, const char *name) {
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", info->dwMinAttack,
              info->dwMaxAttack);
  sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", info->dwCurEndure,
              info->dwMaxEndure);
  sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", info->dwCurSupply,
              info->dwMaxSupply);
  sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", 0, info->sCapacity);
  sbf.lbl_prop[SBF_TEXT_CAPACITY]->SetCaption(buf);
  ...
  sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(itoa(info->dwAttackTime, buf,
  10)); sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(itoa(info->dwConsume, buf,
  10));*/
  _itoa_s(info->dwDefence, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);

  _itoa_s(info->dwSpeed, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);

  _itoa_s(info->dwAttackTime, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(buf);

  _itoa_s(info->dwConsume, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
  ...
  _itoa_s(info->dwMoney, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_MONEY]->SetCaption(buf);
...
//---------------------------------------------------------------------------
BOOL xShipFactory::UpdateBoatCreate(xShipBuildInfo *info, BOOL flag,
                                    const char *name) {
  ClearBoatAttr();
...
//---------------------------------------------------------------------------
BOOL xShipFactory::UpdateBoatInfo(xShipBuildInfo *info, BOOL flag,
                                  const char *name) {
  //得到船舱
  if (!GetCabinByID())
    return FALSE;
...
//---------------------------------------------------------------------------
BOOL xShipFactory::UpdateBoatFreedomTrade(const char *name, DWORD *dwModelInfo,
                                          size_t size) {
  //清空原来船的属性
  ClearBoatAttr();
  ...
  if (!SetShipModelInfo(dwModelInfo, size)) // 我猜他是笔误
    return 0;
...
void xShipFactory::SetBoatAttr() {
  char buf[128];
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", m_pkBoatInfo->dwMinAttack,
              m_pkBoatInfo->dwMaxAttack);
  sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", m_pkBoatInfo->dwCurEndure,
              m_pkBoatInfo->dwMaxEndure);
  sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", m_pkBoatInfo->dwCurSupply,
              m_pkBoatInfo->dwMaxSupply);
  sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

  /*sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(itoa(m_pkBoatInfo->dwSpeed, buf,
  10));
  sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(itoa(m_pkBoatInfo->dwDistance,
  buf, 10));
  sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(itoa(m_pkBoatInfo->dwDefence, buf,
  10));*/
  _itoa_s(m_pkBoatInfo->dwSpeed, buf, sizeof(buf), 10);
  _itoa_s(m_pkBoatInfo->dwDistance, buf, sizeof(buf), 10);
  _itoa_s(m_pkBoatInfo->dwDefence, buf, sizeof(buf), 10);
  sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
  sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
  sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
}
...
void xShipFactory::UpdateBoatAttr() {
  ...
  char buf[128];
  if (sbf.chkShip->GetIsChecked()) { /*显示船只原始属性*/
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_BMNATK), (int)pkAttr->get(ATTR_BMXATK));
    sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_HP), (int)pkAttr->get(ATTR_BMXHP));
    sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_SP), (int)pkAttr->get(ATTR_BMXSP));
    sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

    /*sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(itoa((int)pkAttr->get(ATTR_BMSPD),
    buf, 10));
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(itoa((int)pkAttr->get(ATTR_BDEF),
    buf, 10));
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(itoa((int)pkAttr->get(ATTR_BSREC),
    buf, 10));*/
    _itoa_s((int)pkAttr->get(ATTR_BMSPD), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
    _itoa_s((int)pkAttr->get(ATTR_BDEF), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
    _itoa_s((int)pkAttr->get(ATTR_BSREC), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
  } else { /*显示船只属性(有人物影响)*/
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_MNATK), (int)pkAttr->get(ATTR_MXATK));
    sbf.lbl_prop[SBF_TEXT_ATTACK]->SetCaption(buf);
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_HP), (int)pkAttr->get(ATTR_MXHP));
    sbf.lbl_prop[SBF_TEXT_ENDURE]->SetCaption(buf);
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
                (int)pkAttr->get(ATTR_SP), (int)pkAttr->get(ATTR_MXSP));
    sbf.lbl_prop[SBF_TEXT_SUPPLY]->SetCaption(buf);

    /*sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(itoa((int)pkAttr->get(ATTR_MSPD),
    buf, 10));
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(itoa((int)pkAttr->get(ATTR_DEF),
    buf, 10));
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(itoa((int)pkAttr->get(ATTR_SREC),
    buf, 10));*/
    _itoa_s((int)pkAttr->get(ATTR_MSPD), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(buf);
    _itoa_s((int)pkAttr->get(ATTR_DEF), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(buf);
    _itoa_s((int)pkAttr->get(ATTR_SREC), buf, sizeof(buf), 10);
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(buf);
  }
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", nCurCapacity,
              nMaxCapacity);
  ...
  if (v == 0)
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "-1");
  else
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d", 100000 / v);
  sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(buf);

  //船只等级和熟练度
  /*sbf.lbl_ship_level->SetCaption(itoa(pkAttr->get(ATTR_LV), buf, 10));
  sbf.lbl_ship_exp->SetCaption(itoa(pkAttr->get(ATTR_CEXP), buf, 10));*/
  _itoa_s((int)pkAttr->get(ATTR_LV), buf, sizeof(buf), 10);
  sbf.lbl_ship_level->SetCaption(buf);
  _itoa_s((int)pkAttr->get(ATTR_CEXP), buf, sizeof(buf), 10);
  sbf.lbl_ship_exp->SetCaption(buf);
...
//---------------------------------------------------------------------------
void xShipLaunchList::__ButtonClose(CGuiData *pSender, int x, int y,
                                    DWORD dwKey) {
  __xsl->wnd->Close();
}
...
void xShipLaunchList::SelectItem(CGuiData *pSender) {
  ...
  } else if (eTrade == m_eFlag || eRepair == m_eFlag || eSalvage == m_eFlag ||
             eSupply == m_eFlag || eUpgrade == m_eFlag) {
...
void xShipLaunchList::Test() {
  ...
  strncpy_s(data.szName[0], sizeof(data.szName[0]), "aaaa", _TRUNCATE);
  strncpy_s(data.szName[1], sizeof(data.szName[1]), "bbbb", _TRUNCATE);
  strncpy_s(data.szName[2], sizeof(data.szName[2]), "cccc", _TRUNCATE);
