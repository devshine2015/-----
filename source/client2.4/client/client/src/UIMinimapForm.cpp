...
bool CMiniMapMgr::Init() {
  ...
  if (!MinimatRect)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMinimap->GetName(),
                 "imgMinimapRect");
  ...
  for (int i = 0; i < MAP_POS_MAX; i++) {
    ...
    labMapPosRand[i] =
        dynamic_cast<CLabelEx *>(frmMinimap->Find(szName)); //地图坐标
    if (!labMapPosRand[i])
      return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMinimap->GetName(),
                   szName);
  }
  // labMapPos = dynamic_cast<CLabelEx*>(frmMinimap->Find( "labMapPos" ));
  // //地图坐标 if( !labMapPos ) return
  // Error("msgui.clu界面<%s>上找不到控件<%s>", frmMinimap->GetName(),
  // "labMapPos");
  labMapPos = labMapPosRand[0];

  labMapName =
      dynamic_cast<CLabelEx *>(frmMinimap->Find("labMapName")); //地图名称
  if (!labMapName)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMinimap->GetName(),
                 "labMapName");
  ...
  if (!d3dBigmap)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmBigmap->GetName(),
                 "d3dCompent");
  ...
  if (!edtX)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_719), frmRadar->GetName(),
                 "edtLeft");
...
void CMiniMapMgr::_evtRadarEnter(CGuiData *pSender) { ShowRadar(); }
...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent *pSender, int nMsgType, int x,
                                      int y, DWORD dwKey) {
  ...
  //		{
  //			memCtrl->Init();
  //			memCtrl->SetCaption( "《那个啥Online》操作指南_行
  //走：单击左键，按住不放可持续行走_战
  //斗：在城外对怪物单击左键可战斗_NPC对话：左键点击城里NPC可进行对话_视角转换：按住右键可水平旋转。滑动滚轮可推_
  //进拉远，双击右键回到默认的视角_组    队：对其他玩家点右键，选择邀请组队，_
  //组队后，只有队长才能邀请其他玩家加入_聊 天：Alt+F或点界面右下方按钮可打开_
  //聊天界面点击玩家头像，会出现对话框_交 易：对其他玩家点击右键，选择邀请交易_
  //获对方同意后，弹出玩家间交易界面" ); 			memCtrl->ProcessCaption();
  //		}
  //		frmHelp->Refresh();
  //		frmHelp->Show();
  //	}
...
void CMiniMapMgr::_RadarFormEvent(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
  string name = pSender->GetName();
...
void CMiniMapMgr::_RadarFormBeforeShowEvent(CForm *pForm, bool &IsShow) {
  ...
  //得到玩家目前所在的地图名字
  CMapInfo *pMapInfo =
      GetMapInfo(CGameApp::GetCurScene()->GetInitParam()->strMapFile.c_str());
  if (!pMapInfo)
...
void CMiniMapMgr::ShowRadar(const char *szX, const char *szY) {
  ...
  if ((strcmp(curMap, _strName) == 0) && (!pScene->GetMainCha()->IsBoat())) {
    if (g_stUIStart.chkID->GetIsChecked()) {
      ...
      g_cFindPathEx.SetDestDirection((int)pScene->GetMainCha()->GetPos().x,
                                     (int)pScene->GetMainCha()->GetPos().y, x,
                                     y);
      g_cFindPathEx.SetTarget((int)pScene->GetMainCha()->GetPos().x,
                              (int)pScene->GetMainCha()->GetPos().y, x, y);
    }
  } else {
...
void CMiniMapMgr::ShowRadar() {
  ...
  edtX->SetActive(
      edtX); // 焦点应在X输入框，而不是先在Y输入框 add by rock.wang 090629
  // 坐标;
  int x = atoi(szX), y = atoi(szY);
  D3DXVECTOR3 target((float)x, (float)y, 0);
  // 地点
...
void CMiniMapMgr::SwitchMap() { frmBigmap->Close(); }

bool CMiniMapMgr::IsShowBigmap() { return frmBigmap->GetIsShow(); }
...
bool CMiniMapMgr::IsPKSilver() {
  if (0 == strcmp(GetMapName(),
                  RES_STRING(CL_LANGUAGE_MATCH_900))) // 乱斗白银城 //"白银城"))
    return true;
  ...
  if (0 == strcmp(pScene->GetCurMapInfo()->szName,
                  RES_STRING(CL_LANGUAGE_MATCH_900))) // 乱斗白银城 //"白银城"))
    return true;
...
bool CMiniMapMgr::IsGuildWar() {
  if (0 == strcmp(GetMapName(), RES_STRING(CL_LANGUAGE_MATCH_934)) ||
      0 == strcmp(GetMapName(),
                  RES_STRING(CL_LANGUAGE_MATCH_934))) // "圣战" "小圣战"
    return true;
...
void CMiniMapMgr::_RenderBigMapHint(void) {
  ...
  // CLEFont				g_CFont
  if (/*!pfont ||*/ !CGameApp::GetCurScene() ||
      !CGameApp::GetCurScene()->GetLargerMap())
    return;
  ...
  static const float MAP_SCALE =
      1.25f; // 大地图像素比例，一个推测的 magic number  :)

  // 绘制主城和补给站，昊爷要求
  /*if(0 == stricmp(CGameApp::GetCurScene()->GetTerrainName(), "garner")
              ||0 == stricmp(CGameApp::GetCurScene()->GetTerrainName(),
     "magicsea")
              ||0 == stricmp(CGameApp::GetCurScene()->GetTerrainName(),
     "darkblue"))*/
  if (0 == _stricmp(CGameApp::GetCurScene()->GetTerrainName(), "garner") ||
      0 == _stricmp(CGameApp::GetCurScene()->GetTerrainName(), "magicsea") ||
      0 == _stricmp(CGameApp::GetCurScene()->GetTerrainName(), "darkblue")) {
    struct SApplyInfo {
    ...
    rcBigMapArea.left =
        ptCenter.x - ((int)((rcBigMap.right - rcBigMap.left) * MAP_SCALE) >> 1);
    rcBigMapArea.right =
        ptCenter.x + ((int)((rcBigMap.right - rcBigMap.left) * MAP_SCALE) >> 1);
    rcBigMapArea.top =
        ptCenter.y - ((int)((rcBigMap.bottom - rcBigMap.top) * MAP_SCALE) >> 1);
    rcBigMapArea.bottom =
        ptCenter.y + ((int)((rcBigMap.bottom - rcBigMap.top) * MAP_SCALE) >> 1);

    int nCount = sizeof(stApply) / sizeof(stApply[0]);
    for (int i = 0; i < nCount; ++i) {
      if (rcBigMapArea.left < stApply[i].x &&
          stApply[i].x < rcBigMapArea.right &&
          rcBigMapArea.top < stApply[i].y &&
          stApply[i].y < rcBigMapArea.bottom) {
        CAreaInfo *pInfo = GetAreaInfo(stApply[i].nAreaID);
        if (!pInfo)
          continue;

        POINT ptRender; // 绘制 补给站的坐标（游戏坐标->屏幕坐标）
        ptRender.x = rcBigMap.left +
                     (LONG)((stApply[i].x - rcBigMapArea.left) / MAP_SCALE);
        ptRender.y = rcBigMap.top +
                     (LONG)((stApply[i].y - rcBigMapArea.top) / MAP_SCALE);
        ...
        g_pGameApp->GetFont()->DrawText(pInfo->szDataName, ptRender.x,
                                        ptRender.y);
      }
    }
  }
  ...
  if (rcBigMap.left < ptMouse.x && ptMouse.x < rcBigMap.right &&
    ...
    ptCurMouse.x =
        ptCenter.x +
        (int)((ptMouse.x - ptBigMapCenter.x - rcBigMap.left) * MAP_SCALE);
    ptCurMouse.y =
        ptCenter.y +
        (int)((ptMouse.y - ptBigMapCenter.y - rcBigMap.top) * MAP_SCALE);

    if (ptCurMouse.x < 0 || ptCurMouse.x > defMAP_GARNER_WIDTH ||
        ptCurMouse.y < 0 || ptCurMouse.y > defMAP_GARNER_WIDTH)
      return;
    ...
    for (;;) {
      ...
      if (pInfo->ptStart.x < ptCurMouse.x && ptCurMouse.x < pInfo->ptEnd.x &&
          ...
          0 == strcmp(CGameApp::GetCurScene()->GetTerrainName(),
                      pInfo->szArea)) {
        break;
      }
    }
    ...
    if (pInfo) {
      ...
      SetRect(&rcMonster, pInfo->ptStart.x, pInfo->ptStart.y, pInfo->ptEnd.x,
              pInfo->ptEnd.y);

      // 怪物试验（游戏坐标->屏幕坐标）
      rcMonster.left = rcBigMap.left + ptBigMapCenter.x +
                       (int)((rcMonster.left - ptCenter.x) / MAP_SCALE);
      rcMonster.top = rcBigMap.top + ptBigMapCenter.y +
                      (int)((rcMonster.top - ptCenter.y) / MAP_SCALE);
      rcMonster.right = rcBigMap.left + ptBigMapCenter.x +
                        (int)((rcMonster.right - ptCenter.x) / MAP_SCALE);
      rcMonster.bottom = rcBigMap.top + ptBigMapCenter.y +
                         (int)((rcMonster.bottom - ptCenter.y) / MAP_SCALE);
      ...
      for (int i = 0; i < MONSTER_LIST_MAX; ++i) {
        ...
        char szBuf[256] = {0};
        _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, " %d. %s   LV:%2d ",
                    nMonCount, pMonsterInfo[i]->szName, pMonsterInfo[i]->lLv);
        strMonsterName[i] = szBuf;

        SIZE size;
        g_pGameApp->GetFont()->GetTextSize(szBuf, &size);
        if (size.cx > sizeNameMax.cx)
          sizeNameMax.cx = size.cx;
        if (size.cy > sizeNameMax.cy)
          sizeNameMax.cy = size.cy;
      }

      if (nMonCount > 0) {
        //	int nLevelCha =
        //g_pGameApp->GetCurScene()->GetMainCha()->getGameAttr()->get(ATTR_LV);
        int nLevelCha =
            (int)g_pGameApp->GetCurScene()->GetMainCha()->getGameAttr()->get(
                ATTR_LV);
        ...
        GetRender().FillFrame(rcMonster.left, rcMonster.top, rcMonster.right,
                              rcMonster.bottom, dwAreaColor);

        // 怪物列表
        RECT rcRender = rcBigMap;
        GetRender().FillFrame(rcBigMap.left, rcRender.top,
                              rcRender.left + sizeNameMax.cx,
                              rcRender.top + (sizeNameMax.cy * nMonCount));
        for (int i = 0; i < nMonCount; ++i) {
          g_pGameApp->GetFont()->DrawText(
              const_cast<char *>(strMonsterName[i].c_str()), rcRender.left + 35,
              rcRender.top + 50);
    ...
    //        // 绘制鼠标指向的坐标
    char szBuf[256] = {0};
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, " Point: %d, %d ",
                ptCurMouse.x, ptCurMouse.y);

    SIZE size;
    g_pGameApp->GetFont()->GetTextSize(szBuf, &size);

    GetRender().FillFrame(ptMouse.x + 32, ptMouse.y, ptMouse.x + 32 + size.cx,
                          ptMouse.y + size.cy);
