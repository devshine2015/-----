...
CPageItem::CPageItem(const CPageItem &rhs)
    : CContainer(rhs), _pImage(new CImage(*rhs._pImage)),
      _pTitle(new CImage(*rhs._pTitle)) {
  _SetSelf();
}
...
void CPageItem::_OnTitleDown() { ((CPage *)(_pParent))->SetIndex(_nIndex); }
...
CPage::CPage(CForm &frmOwn)
    : CCompent(frmOwn), _nIndex(-1), _eButtonPos(bpLeftUp), _nButtonMargin(0),
      _nButtonWidth(100), _nButtonHeight(30), evtSelectPage(NULL),
      _nMaxPage(0) {
  _pImage = new CGuiPic(this);
}

CPage::CPage(const CPage &rhs) : CCompent(rhs), _nMaxPage(0) {
  _pImage = new CGuiPic(this);
}
