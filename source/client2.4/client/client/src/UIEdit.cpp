...
CEdit::CEdit(CForm &frmOwn)
    : CCompent(frmOwn), _bIsPassWord(false), _bIsMulti(false), _bIsDigit(false),
      _bIsWrap(false), _nOffset(0), _nMaxNum(64), _nMaxNumVisible(64),
      _nFontHeight(24), _nMaxLineNum(0), _nCursorCol(0), _nCursorFirstCol(0),
      _nCursorSecondCol(0), _nCursorRow(0), _bParseText(false),
      _color(COLOR_BLACK), evtKeyDown(NULL), evtKeyChar(NULL), _nLeftMargin(0),
      _nTopMargin(2), evtEnter(NULL), evtChange(NULL), _pEnterButton(NULL),
...
void CEdit::ClearText() { g_InputBox.ClearText(); }
...
bool CEdit::_IsCursorInHZ(long l, char *s) { return false; }

void CEdit::_Copy() { RefreshCursor(); }
  ...
  if (_bIsWrap) {
    ...
    if (_nOffset + GetWidth() - 5 < cursorPos) {
      ...
      _nOffset = cursorPos;
               ...
               _nOffset + GetWidth() - 5 >
                   CGuiFont::s_Font.GetWidth("S") * (int)(_str.length())) {
      _nOffset = CGuiFont::s_Font.GetWidth("S") * (int)(_str.length()) -
                 GetWidth() + 5;
      if (_nOffset < 0)
        _nOffset = 0;
    }
...
void CEdit::Render() {
  ...
  if (!GetIsPassWord()) {
    if (_bParseText) {
      ...
      if ((int)_str.size() > _nMaxNumVisible) {
        CGuiFont::s_Font.Render((char *)_strVisible.c_str(), GetX() - _nOffset,
                                GetY(), _color);
      } else {
        CGuiFont::s_Font.Render((char *)_str.c_str(), GetX() - _nOffset, GetY(),
                                _color);
      }
    }
  } else {
    ...
    CGuiFont::s_Font.Render((char *)cPassWord, GetX() - _nOffset, GetY(),
                            _color);
...
void CEdit::Init() {
  ...
  _nCursorHeight =
      CGuiFont::s_Font.GetHeight(RES_STRING(CL_MAIN_CPP_00001)) - 2;
  // End

  if (_pEnterButton && _pEnterButton->GetForm() != GetForm()) {
    _pEnterButton =
        dynamic_cast<CTextButton *>(GetForm()->Find(_pEnterButton->GetName()));
  }
}
...
void CEdit::_GetCursorPos(int nCurPos) {
  ...
  _nCursorX =
      (int)GetRender().DrawConvertX2((float)cx) + GetX(); // added by billy
...
void CEdit::OnLost() { RefreshCursor(); }
...
void CEdit::OnActive() {
  ...
  if (!GetIsMulti() && _nCursorCol >= 0 &&
      _nCursorCol <= (int)(strlen(_str.c_str()))) {
...
void CEdit::SetTextColor(DWORD color) { _color = color; }
