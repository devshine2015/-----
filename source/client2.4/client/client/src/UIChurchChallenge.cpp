...
CChurchChallengeMgr::CChurchChallengeMgr(void)
    : frmChurchSale(0), btnPriceUp(0), _pNumberBox(0), _nPrice(0),
      labInfoShow1(0), labInfoShow2(0), labInfoShow3(0) {}
...
void CChurchChallengeMgr::ShowForm(bool bShow) {
  if (frmChurchSale) {
    if (bShow) {
      ...
      if (labInfoShow2)
        labInfoShow2->SetCaption(
            g_pGameApp->GetCurScene()->GetMainCha()->getName());
      if (labInfoShow3)
...
void CChurchChallengeMgr::ChallengePriceAsk(void) {
  _pNumberBox = g_stUIBox.ShowNumberBox(_evtChallengeNumInput, -1,
                                        RES_STRING(CL_LANGUAGE_MATCH_936),
                                        true); // "请输入竟拍价格"
  ...
  if (_pNumberBox) {
    ...
    if (_nPrice >= _stInfo.nCurPrice + _stInfo.nMinbid) {
      ...
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_940),
                  _nPrice); // "确定竟拍价格 %d"
      g_stUIBox.ShowSelectBox(_evtChallengeNumCheck, szBuf, true);
    } else {
      g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_937),
                         _stInfo.nCurPrice +
                             _stInfo.nMinbid); //"不得低于最低价 %d"
    }
  }
}
...
void CChurchChallengeMgr::_evtChallengeButtonDown(CGuiData *pSender, int x,
                                                  int y, DWORD key) {
  g_stChurchChallenge.ChallengePriceAsk();
}

void CChurchChallengeMgr::_evtChallengeNumInput(CCompent *pSender, int nMsgType,
...
void CChurchChallengeMgr::_evtChallengeNumCheck(CCompent *pSender, int nMsgType,
                                                int x, int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
