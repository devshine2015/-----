...
CCompent::CCompent(CForm &frmOwn)
    : CGuiData(), _frmOwn(&frmOwn), _align(caNone), _isChild(false),
      _eMouseAction(enumMA_Gui), _IsFocus(false), evtLost(NULL),
      evtActive(NULL) {
  _pParent = _frmOwn;
}

CCompent::CCompent(const CCompent &rhs) : CGuiData(rhs) { _Copy(rhs); }
...
void CCompent::Refresh() {
  ...
  case caRightBottom: {
    SetPos(GetParent()->GetWidth() - GetWidth(),
           GetParent()->GetHeight() - GetHeight());
  } break;
  case caBottom: {
  ...
  case caHeightCenter:
    ...
    break;
  case caCenter:
    SetPos((GetParent()->GetWidth() - GetWidth()) / 2,
...
// class CContainer
//---------------------------------------------------------------------------
CContainer::CContainer(CForm &frmOwn) : CCompent(frmOwn) {}

CContainer::CContainer(const CContainer &rhs) : CCompent(rhs) { _SetSelf(rhs); }
...
void CContainer::_SetSelf(const CContainer &rhs) {
  CGuiData *p = NULL;
  CCompent *c = NULL;
  for (items::const_iterator it = rhs._items.begin(); it != rhs._items.end();
       it++) {
    p = (*it)->Clone();
