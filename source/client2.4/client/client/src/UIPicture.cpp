...
{
	if( _max==0 ) _max=1;
	_pImage = new LETexRect[_max];
}

CGuiPic::CGuiPic( const CGuiPic& rhs )
: _pParent( rhs._pParent ), _pImage(new LETexRect[rhs._max]), _max(rhs._max), _frame(rhs._frame), _bIsScale(rhs._bIsScale) 
{
...
: _pImage(new LETexRect[ppEnd]), _pOwn(pOwn), _bIsShowFrame(false), _bIsTitle(true)
{
...
: _pImage(new LETexRect[ppEnd]), _bIsShowFrame(rhs._bIsShowFrame), _bIsTitle(rhs._bIsTitle)
{
	// _pOwn = rhs._pOwn;
