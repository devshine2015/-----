...
CGuiPic::CGuiPic(const CGuiPic &rhs)
    : _pParent(rhs._pParent), _pImage(new LETexRect[rhs._max]), _max(rhs._max),
      _frame(rhs._frame), _bIsScale(rhs._bIsScale) {
...
CFramePic::CFramePic(CGuiData *pOwn)
    : _pImage(new LETexRect[ppEnd]), _pOwn(pOwn), _bIsShowFrame(false),
      _bIsTitle(true) {
...
CFramePic::CFramePic(const CFramePic &rhs)
    : _pImage(new LETexRect[ppEnd]), _bIsShowFrame(rhs._bIsShowFrame),
