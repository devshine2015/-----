...
struct __declspec(uuid("d27cdb6c-ae6d-11cf-96b8-444553540000"))
  ...
  __declspec(property(get = GetStacking, put = PutStacking)) _bstr_t Stacking;
  __declspec(property(get = GetWMode, put = PutWMode)) _bstr_t WMode;
  __declspec(property(get = GetMovie, put = PutMovie)) _bstr_t Movie;
  __declspec(property(get = GetSAlign, put = PutSAlign)) _bstr_t SAlign;
  __declspec(property(get = GetMenu, put = PutMenu)) VARIANT_BOOL Menu;
  __declspec(property(get = GetQuality, put = PutQuality)) int Quality;
  __declspec(property(get = GetLoop, put = PutLoop)) VARIANT_BOOL Loop;
  __declspec(property(get = GetFrameNum, put = PutFrameNum)) long FrameNum;
  __declspec(property(get = GetBase, put = PutBase)) _bstr_t Base;
  __declspec(property(get = Getscale, put = Putscale)) _bstr_t scale;
  __declspec(property(get = GetDeviceFont,
                      put = PutDeviceFont)) VARIANT_BOOL DeviceFont;
  __declspec(property(get = GetEmbedMovie,
                      put = PutEmbedMovie)) VARIANT_BOOL EmbedMovie;
  __declspec(property(get = GetBGColor, put = PutBGColor)) _bstr_t BGColor;
  __declspec(property(get = GetQuality2, put = PutQuality2)) _bstr_t Quality2;
  __declspec(property(get = GetReadyState)) long ReadyState;
  __declspec(property(get = GetScaleMode, put = PutScaleMode)) int ScaleMode;
  __declspec(property(get = GetAlignMode, put = PutAlignMode)) int AlignMode;
  __declspec(property(get = GetBackgroundColor,
                      put = PutBackgroundColor)) long BackgroundColor;
  __declspec(property(get = GetTotalFrames)) long TotalFrames;
  __declspec(property(get = GetPlaying, put = PutPlaying)) VARIANT_BOOL Playing;
  __declspec(property(get = GetSWRemote, put = PutSWRemote)) _bstr_t SWRemote;
...
struct __declspec(uuid("1171a62f-05d2-11d1-83fc-00a0c9089c5a")) FlashProp;
// [ default ] interface IUnknown

struct __declspec(uuid("d27cdb70-ae6d-11cf-96b8-444553540000")) IFlashFactory
    : IUnknown {};
...
struct __declspec(uuid("a6ef9860-c720-11d0-9337-00a0c90dcaa9")) IDispatchEx
  ...
  HRESULT GetDispID(_bstr_t bstrName, unsigned long grfdex, long *pid);
  HRESULT RemoteInvokeEx(long id, unsigned long lcid, unsigned long dwFlags,
                         struct DISPPARAMS *pdp, VARIANT *pvarRes,
                         struct EXCEPINFO *pei,
                         struct IServiceProvider *pspCaller,
                         unsigned int cvarRefArg, unsigned int *rgiRefArg,
                         VARIANT *rgvarRefArg);
  HRESULT DeleteMemberByName(_bstr_t bstrName, unsigned long grfdex);
  HRESULT DeleteMemberByDispID(long id);
  HRESULT GetMemberProperties(long id, unsigned long grfdexFetch,
                              unsigned long *pgrfdex);
  HRESULT GetMemberName(long id, BSTR *pbstrName);
  HRESULT GetNextDispID(unsigned long grfdex, long id, long *pid);
...
public:
  ...
  virtual ~FlashSink() { m_pFlashPlayer->m_nCOMCount--; }
  ...
  HRESULT Init(CFlashPlayer *pScene) {
    ...
    if ((m_pFlashPlayer->m_pShockwaveFlash->QueryInterface(
             IID_IConnectionPointContainer, (void **)&aConnectionPoint) ==
         S_OK) &&
        (aConnectionPoint->FindConnectionPoint(
             __uuidof(ShockwaveFlashObjects::_IShockwaveFlashEvents),
             &mConnectionPoint) == S_OK)) {
  ...
  virtual HRESULT STDMETHODCALLTYPE Invoke(DISPID dispIdMember, REFIID riid,
    ...
    case 0x96:
      if ((pDispParams->cArgs == 2) && (pDispParams->rgvarg[0].vt == VT_BSTR) &&
        ...
        FSCommand(pDispParams->rgvarg[1].bstrVal,
...
public:
  ...
  virtual HRESULT STDMETHODCALLTYPE ShowObject(void) { return E_NOTIMPL; }
  ...
  HRESULT STDMETHODCALLTYPE GetWindowContext(
      ...
      /* [out] */ LPRECT lprcPosRect, /* [out] */ LPRECT lprcClipRect,
  ...
  HRESULT STDMETHODCALLTYPE OnInPlaceActivateEx(
      /* [out] */ BOOL __RPC_FAR *pfNoRedraw, /* [in] */ DWORD dwFlags) {
    return S_OK;
  }
  ...
  HRESULT STDMETHODCALLTYPE SetFocus(/* [in] */ BOOL fFocus) { return S_OK; }

  HRESULT STDMETHODCALLTYPE GetDC(/* [in] */ LPCRECT pRect,
                                  /* [in] */ DWORD grfFlags,
  ...
  HRESULT STDMETHODCALLTYPE InvalidateRect(/* [in] */ LPCRECT pRect,
                                           /* [in] */ BOOL fErase) {
    if (pRect == NULL) {
      ...
      UnionRect(&m_pFlashPlayer->m_FlashDirtyRect,
                &m_pFlashPlayer->m_FlashDirtyRect, pRect);

    return S_OK;
  }

  HRESULT STDMETHODCALLTYPE InvalidateRgn(/* [in] */ HRGN hRGN,
                                          /* [in] */ BOOL fErase) {
    m_pFlashPlayer->GetRECT(&m_pFlashPlayer->m_FlashDirtyRect);
    m_pFlashPlayer->m_bFlashDirty = TRUE;
    return S_OK;
  ...
  HRESULT STDMETHODCALLTYPE ScrollRect(/* [in] */ INT dx, /* [in] */ INT dy,
...
typedef HRESULT(__stdcall *DllGetClassObjectFunc)(REFCLSID rclsid, REFIID riid,
                                                  LPVOID *ppv);
...
VOID CFlashPlayer::Init() {
  ...
  if (m_hFlashOCX) {
    ...
    DllGetClassObjectFunc pDllGetClassObjectFunc =
        (DllGetClassObjectFunc)GetProcAddress(m_hFlashOCX, "DllGetClassObject");
    pDllGetClassObjectFunc(CLSID_ShockwaveFlash, IID_IClassFactory,
                           (void **)&pClassFactory);
    pClassFactory->CreateInstance(NULL, IID_IOleObject, (void **)&m_pOleObject);
    pClassFactory->Release();
  } else
    CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER,
                     IID_IOleObject, (void **)&m_pOleObject);

  IOleClientSite *pClientSite = NULL;
  m_pControlSite->QueryInterface(__uuidof(IOleClientSite),
                                 (void **)&pClientSite);
  m_pOleObject->SetClientSite(pClientSite);

  m_pOleObject->QueryInterface(__uuidof(IShockwaveFlash),
                               (LPVOID *)&m_pShockwaveFlash);
  _bstr_t string = "Transparent";
  m_pShockwaveFlash->put_WMode(string);

  m_pOleObject->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pClientSite, 0, NULL,
                       NULL);
  pClientSite->Release();

  m_pOleObject->QueryInterface(__uuidof(IOleInPlaceObjectWindowless),
                               (LPVOID *)&m_pObjectWindowless);
  ...
  if (!m_lpDD) {
    ...
    Succeed =
        SUCCEEDED(m_lpDD->CreateSurface(&ddsd, &m_lpPrimarySurface, NULL));
    assert(Succeed);
    ...
    Succeed =
        SUCCEEDED(m_lpDD->CreateSurface(&ddsd, &m_lpSecondarySurface, NULL));
    assert(Succeed);
...
VOID CFlashPlayer::Paint() {
  ...
  if (m_bFlashDirty) {
    ...
    if (pViewObject != NULL) {
      ...
      HRGN aRgn = CreateRectRgn(
          m_FlashDirtyRect.left - m_FlashStartX,
          m_FlashDirtyRect.top - m_FlashStartY,
      ...
      pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, hDC, &rect,
                        NULL, NULL, TRUE);
...
HRESULT CFlashPlayer::CreateBackgroundSurface(const char *Filename) {
  ...
  _snprintf_s(szFilename, sizeof(szFilename), _TRUNCATE, "%s\\flash\\%s", dir,
              Filename);
  HBITMAP hBitmap =
      (HBITMAP)LoadImage(GetModuleHandle(NULL), szFilename, IMAGE_BITMAP, 800,
                         600, LR_CREATEDIBSECTION);
  if (!hBitmap) {
    hBitmap = (HBITMAP)LoadImage(NULL, szFilename, IMAGE_BITMAP, 800, 600,
  ...
  StretchBlt(hDirectDrawDC, 0, 0, ddsd.dwWidth, ddsd.dwHeight, hDCImage, 0, 0,
             800, 600, SRCCOPY);
...
VOID CFlashPlayer::Render() {
  ...
  if (m_lpFlashBuffer) {
    if (m_bFlashDirty) {
      ...
      IViewObject *pViewObject = NULL;
      m_pShockwaveFlash->QueryInterface(IID_IViewObject,
                                        (LPVOID *)&pViewObject);
      if (pViewObject != NULL) {
        RECTL rect = {0, 0, m_FlashWidth, m_FlashHeight};

        IntersectClipRect(hDC, m_FlashDirtyRect.left - m_FlashStartX,
                          m_FlashDirtyRect.top - m_FlashStartY,
    ...
    m_lpSecondarySurface->BltFast(m_FlashStartX, m_FlashStartY, m_lpFlashBuffer,
                                  NULL, 0);
  }
  ...
  while (TRUE) {
    HRESULT hr;
    if (m_bWindowed)
      hr = m_lpPrimarySurface->Blt(&m_WindowRect, m_lpSecondarySurface, NULL,
                                   DDBLT_WAIT, NULL);
    else
...
VOID CFlashPlayer::ResizeFlashZone(INT X, INT Y, INT Width, INT Height,
                                   BOOL CreateSurf) {
  ...
  BOOL Succeed =
      SUCCEEDED(m_lpDD->CreateSurface(&ddsd, &m_lpFlashBuffer, NULL));
  assert(Succeed);

  if (m_FlashStartX == X && m_FlashStartY == Y && m_FlashWidth == Width &&
  ...
  m_pOleObject->QueryInterface(__uuidof(IOleInPlaceObject),
...
DOUBLE CFlashPlayer::GetFlashVersion() {
  ...
  if (FAILED(CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER,
                              IID_IOleObject, (void **)&pOleObject)))
    return 0.0;

  IShockwaveFlash *pShockwaveFlash = NULL;
  if (FAILED(pOleObject->QueryInterface(__uuidof(IShockwaveFlash),
                                        (LPVOID *)&pShockwaveFlash)))
    return 0.0;
...
VOID CFlashPlayer::OnMouseMove(INT X, INT Y) {
  LRESULT result;
  m_pObjectWindowless->OnWindowMessage(WM_MOUSEMOVE, 0, MAKELPARAM(X, Y),
                                       &result);
}

VOID CFlashPlayer::OnLButtonDown(INT X, INT Y) {
  LRESULT result;
  m_pObjectWindowless->OnWindowMessage(WM_LBUTTONDOWN, MK_LBUTTON,
                                       MAKELPARAM(X, Y), &result);
}

VOID CFlashPlayer::OnLButtonUp(INT X, INT Y) {
  LRESULT result;
  m_pObjectWindowless->OnWindowMessage(WM_LBUTTONUP, 0, MAKELPARAM(X, Y),
                                       &result);
}
...
VOID CFlashPlayer::MessageProc(UINT message, WPARAM wParam, LPARAM lParam) {
  if (m_pObjectWindowless) {
    LRESULT result = S_OK;
    if (message == WM_SETCURSOR || message == WM_KEYDOWN ||
        message == WM_KEYUP || message == WM_CHAR || message == WM_MOUSEMOVE ||
...
VOID CFlashPlayer::UpdateBounds() {
  if (m_bWindowed) {
    ...
    SetRect(&m_WindowRect, 0, 0, GetSystemMetrics(SM_CXSCREEN),
