...
CGoodsGrid::CGoodsGrid(CForm &frmOwn)
    : CCommandCompent(frmOwn), _nLeftMargin(0), _nTopMargin(0),
      _nRightMargin(0), _nBottomMargin(0), _nUnitHeight(32), _nUnitWidth(32),
      _nSpaceX(2), _nSpaceY(2), _nRow(0), _nCol(0), _nMaxNum(0), _pItems(NULL),
      _nFirst(0), _nLast(0), _nTmpIndex(0), _eShowStyle(enumSmall),
      evtThrowItem(NULL), evtSwapItem(NULL), evtUseCommand(NULL),
      evtBeforeAccept(NULL), evtRMouseEvent(NULL), _nCurNum(0),
      _IsShowHint(true) {
...
CGoodsGrid::~CGoodsGrid() {
  ...
  SAFE_DELETE(_pScroll); // UI当机处理
  SAFE_DELETE(_pUnit);   // UI当机处理
}
...
void CGoodsGrid::Refresh() {
  ...
  _nPageShowNum =
      _nCol * (int)((GetHeight() - _nBottomMargin - _nTopMargin) / _nTotalH);
  _OnScrollChange();
}

//	2008-9-17	yangyinyu	add	begin!
extern bool
    _wait_select_lock_item_state_; //	用户已经使用锁道具，正在等待选择被锁定的道具。
...
bool CGoodsGrid::SetContent(int nRow, int nCol) {
  ...
  if (pOldItems) {
    ...
    SAFE_DELETE_ARRAY(pOldItems); //安全释放内存 by Waiting 2009-06-18
  }
  Init();
  return true;
}
...
void CGoodsGrid::DragRender() {
  if (_pDragItem) {
    if (_eShowStyle == enumSmall) {
      ...
      _pDragItem->SaleRender(_pDrag->GetX() - _nDragOffX,
                             _pDrag->GetY() - _nDragOffY, _nUnitWidth,
                             _nUnitHeight);
    } else if (_eShowStyle == enumOwnDef) {
      _pDragItem->OwnDefRender(_pDrag->GetX() - _nDragOffX,
                               _pDrag->GetY() - _nDragOffY, _nUnitWidth,
                               _nUnitHeight);
    }
  }
...
CCompent *CGoodsGrid::GetHintCompent(int x, int y) {
  if (GetIsShow() && InRect(x, y)) {
    ...
    if (_IsShowHint) {
      _GetHitItem(x, y);
      if (_nTmpIndex != -1 && _pItems[_nTmpIndex] &&
          SetHintItem(_pItems[_nTmpIndex])) {
...
void CGoodsGrid::Reset() { _pScroll->Reset(); }
