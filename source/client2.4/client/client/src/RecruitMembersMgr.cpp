...
bool CRecruitMembersMgr::DelRecruitMember(CRecruitMemberData *pRecruitMember) {
  ...
  for (Iter = m_pRecruitMembers.begin(); Iter != m_pRecruitMembers.end();
       Iter++) {
...
CRecruitMemberData *
CRecruitMembersMgr::FindRecruitMemberByName(string strName) {
  vector<CRecruitMemberData *>::iterator Iter;
  for (Iter = m_pRecruitMembers.begin(); Iter != m_pRecruitMembers.end();
       Iter++) {
    if ((*Iter)->GetName() == strName) {
...
CRecruitMemberData *
CRecruitMembersMgr::FindRecruitMemberByIndex(DWORD dwIndex) {
