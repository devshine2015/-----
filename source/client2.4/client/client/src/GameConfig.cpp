...
void CGameConfig::SetDefault() // 默认配置
{
  ...
  maxNumPerLine = 32; // rain added set a defualt value.
  m_nMaxChaType = 350;
  m_nMaxSceneObjType = 800;
  m_nMaxEffectType =
      14000; // 用于增加精炼道具的特效数	modify by Michael 2005-11-9
             // m_nMaxItemType      = 6000;
  m_nMaxItemType = 10000;
  ...
  strncpy_s(m_szMD5Pass, sizeof(m_szMD5Pass), "", _TRUNCATE); // 填写的MD5密码
  ...
  memset((void *)m_szFontName1, 0, sizeof(m_szFontName1));
  memset((void *)m_szFontName2, 0, sizeof(m_szFontName1));
  // Res
...
void CGameConfig::Load(char *pszFileName) //  读kop.cfg
{
  ...
  while (!in.eof()) //是否到达文件尾
  {
    ...
    //	Add by alfred.shi Dialog set	begin
    else if (strKey == "sMaxCom") {
      maxNumPerLine = atoi(strValue.c_str());
    }
    //	end.
    ...
    {
      strncpy_s(m_szVerErrorHTTP, sizeof(m_szVerErrorHTTP), strValue.c_str(),
                _TRUNCATE);
    ...
    // Add by lark.li 20080201
      ...
      strncpy_s(m_szFontName1, sizeof(m_szFontName1), strValue.c_str(),
                _TRUNCATE);
    } else if (strKey == "fontname2") {
      strncpy_s(m_szFontName2, sizeof(m_szFontName2), strValue.c_str(),
                _TRUNCATE);
    }
    // End
    // Add by lark.li 20080825 begin
    ...
    } else if (strKey == "enable_banner") {
      m_bGameBanner = Str2Int(strValue);
    }
    // End
  }
  in.close(); // 关闭文件
...
{
  m_IsMoveClient = v;
  // g_pGameApp->SysInfo( g_Config.m_IsMoveClient ?
  // RES_STRING(CL_LANGUAGE_MATCH_141) : RES_STRING(CL_LANGUAGE_MATCH_142) );
}
