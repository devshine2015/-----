...
//	2005.4.5	Arcol	add session node and handle the flash event in
//the QQ form
//------------------------------------------------------------------------
...
bool _Error(const char *strInfo, const char *strFormName,
...
CTalkSessionForm::CTalkSessionForm(DWORD sessionID, eTalkSessionStyle style)
    : m_dwSessioinID(sessionID), m_strWaitForSend(""), m_bActive(true),
      m_pForm(NULL), m_pEdit(NULL), m_strTalkMsg(""),
      /*m_pList(NULL),*/ m_pInformTextBtn(NULL), m_pInformTitle(NULL),
      m_nShowChannel(NULL), m_nShowSx(NULL), m_nShowSy(NULL),
      m_strWaitForAdded(""), m_bFlash(NULL), m_nFlashTimes(0), m_pTimer1(NULL),
      m_pTimer2(NULL) {
  //建立Form
  ...
  m_pInformTextBtn =
      dynamic_cast<CTextButton *>(m_pInformForm->Find("btnCheck"));
  if (!m_pInformTextBtn) {
    _Error(RES_STRING(CL_LANGUAGE_MATCH_411), m_pInformForm->GetName(),
           "btnCheck");
    return;
  }
  ...
  if (!btn) {
    _Error(RES_STRING(CL_LANGUAGE_MATCH_411), m_pNormalForm->GetName(),
           "btnMin");
    return;
  }
  ...
  // if( !btn )
  //{
  //	_Error("chat.clu界面<%s>上找不到控件<%s>", m_pNormalForm->GetName(),
  //"btnCloseD" ); 	return;
  //}
  // btn->evtMouseClick=_OnMouseQuitButton;
  // btn->pTag=this;
  ...
  if (!m_pFaceGrid) {
    Error(RES_STRING(CL_LANGUAGE_MATCH_411), m_pNormalForm->GetName(),
          "grdFace");
    return;
  }
  ...
  if (!btn) {
    _Error(RES_STRING(CL_LANGUAGE_MATCH_411), m_pSmallForm->GetName(),
           "btnMin");
    return;
  }
  btn->evtMouseClick = _OnMouseMinimizeButton;
  ...
  // if( !m_pSmallList )
  //{
  //	_Error("chat.clu界面<%s>上找不到控件<%s>", m_pSmallForm->GetName(),
  //"memCtrl" ); 	return;
  //}
  ...
  if (!btn) {
    _Error(RES_STRING(CL_LANGUAGE_MATCH_411), m_pMinimizeForm->GetName(),
           "btnMax");
    return;
  }
  btn->evtMouseClick = _OnMouseNormalButton;
...
int CTalkSessionForm::GetTotalMembers() { return (int)m_Members.size(); }

CForm *CTalkSessionForm::GetForm() { return m_pForm; }
...
void CTalkSessionForm::AddMembers(stNetSessCreate *pArrMembers[],
                                  int memberNum) {
...
bool CTalkSessionForm::ChangeStyle(eTalkSessionStyle style) {
  ...
  case enumTalkSessionInform: {
    ...
    m_nShowSx =
        INFORM_FORM_SHOW_SX + (m_nShowChannel - 1) * m_pInformForm->GetWidth();
...
void CTalkSessionForm::RefreshForm() {
  ...
  string str;
  if (total == 1) {
    str = string(RES_STRING(CL_LANGUAGE_MATCH_415)) +
          GetMemberByIndex(0)->GetName() + RES_STRING(CL_LANGUAGE_MATCH_416);
  } else {
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_417), total);
    str = buf;
  }
  ...
  if (total == 1) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_418), total);
    str = buf;
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_419),
              total);
  str = buf;
  ...
  for (int i = 0; i < GetTotalMembers(); i++) {
    ...
    CChatIconInfo *pIconInfo =
        GetChatIconInfo(GetMemberByIndex(i)->GetIconID());
    if (pIconInfo) {
      CGuiPic *pPic = pItem->GetImage();
      string strPath = "texture/ui/HEAD/";
      pPic->LoadImage((strPath + pIconInfo->szSmall).c_str(), 16, 16, 0,
                      pIconInfo->nSmallX, pIconInfo->nSmallY);
    }
  ...
  if (m_pMemo) {
    ...
    for (Iter = m_TalkMsg.begin(); Iter != m_TalkMsg.end(); Iter++) {
      ...
      m_pMemo->AddText(strName.c_str(), (*Iter)->text.c_str(), TEXT_COLOR_BLACK,
                       TEXT_FONT_9);
    }
...
void CTalkSessionForm::SetInformText(const char *name, const char *text) {
  char buf[100];
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "玩家 %s 说:", name);
  m_pInformTitle->SetCaption(buf);
  string str = StringLimit(text, 22);
  m_pInformTextBtn->SetCaption(str.c_str());
}

void CTalkSessionForm::_OnMouseInformButton(CGuiData *pSender, int x, int y,
                                            DWORD key) {
...
void CTalkSessionForm::_OnMouseSmallButton(CGuiData *pSender, int x, int y,
                                           DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseMinimizeButton(CGuiData *pSender, int x, int y,
                                              DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseNormalButton(CGuiData *pSender, int x, int y,
                                            DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseQuitButton(CGuiData *pSender, int x, int y,
                                          DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseCloseButton(CGuiData *pSender, int x, int y,
                                           DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseSendButton(CGuiData *pSender, int x, int y,
                                          DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
  ...
  _gmtime64_s(&ltime, &t);
  char ch[12];
  _snprintf_s(ch, _countof(ch), _TRUNCATE, "  %d:%d:%d", ltime.tm_hour + 8,
  ...
  for (DWORD i = 0; i < (DWORD)pThis->GetTotalMembers();
       i++) { // for循环处理多人聊天  ning.yan  2008-11-13  need modify
    if (CChatRecord::Save(pThis->GetMemberByIndex(i)->GetName(),
                          pThis->GetTotalMembers(), saveText)) {
      // 			// string
      // str="已成功保存到文件:\""+CChatRecord::GetLastSavePath()+"\"里"; 			string
      // str = RES_STRING(CL_LANGUAGE_MATCH_972) +
      // CChatRecord::GetLastSavePath() + RES_STRING(CL_LANGUAGE_MATCH_973);
      ...
      // RES_STRING(CL_LANGUAGE_MATCH_973) );
    }
  }
  // End
}
...
void CTalkSessionForm::_OnMouseFaceListButton(CGuiData *pSender, int x, int y,
                                              DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseCleanText(CGuiData *pSender, int x, int y,
                                         DWORD key) {
  CTalkSessionForm *pThis = ((CTalkSessionForm *)pSender->GetPointer());
  if (!pThis)
    return;
...
void CTalkSessionForm::_OnMouseSelectFace(CGuiData *pSender) {
  ...
  if (pGraph) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "#%.2d", index);
    str += buf;
...
void CTalkSessionForm::_OnMouseRightPlayerMenu(CGuiData *pSender, int x, int y,
                                               DWORD key) {
  if (key & Mouse_RDown) {
    ...
    CTextGraph *pSelectItem =
        dynamic_cast<CTextGraph *>(pList->GetSelectItem()->GetBegin());
    if (!pSelectItem)
      return;
...
void CTalkSessionForm::_OnPlayerSelect(CGuiData *pSender, int x, int y,
                                       DWORD key) {
  m_playerMouseRight->SetIsShow(false);
  ...
  if (str == "添加好友") {
    CSessionMember *pSessionMember =
        static_cast<CSessionMember *>(m_playerMouseRight->GetPointer());
    CS_Frnd_Invite(pSessionMember->GetName());
  }
}

void CTalkSessionForm::_OnMouseSaveButton(
    CGuiData *pSender, int x, int y,
    DWORD
        key) { // delete by ning.yan 2008-07-29
               // 去掉手动保存聊天记录功能，改为自动保存 Begin CTalkSessionForm
               // *pThis=((CTalkSessionForm*)pSender->GetPointer()); if (!pThis
               // || !pThis->GetMemberByIndex(0)) return; string saveText; vector
               // <sTalkMsg*>::iterator Iter; for
               // (Iter=pThis->m_TalkMsg.begin();Iter!=pThis->m_TalkMsg.end();Iter++)
               //{
               //	string strName=(*Iter)->name;
               //	if ((*Iter)->motto.length()==0)
               //	{
               ...
               //}
               // if
               // (CChatRecord::Save(pThis->GetMemberByIndex(0)->GetName(),pThis->GetTotalMembers(),saveText))
               //{
               //	string
               //str="已成功保存到文件:\""+CChatRecord::GetLastSavePath()+"\"里";
...
void CTalkSessionForm::GetMsg(DWORD chaID, string talkMsg) {
  ...
  if (pMsg->name != CGameScene::GetMainCha()->getName()) {
    ...
    for (DWORD i = 0; i < (DWORD)GetTotalMembers(); i++) {
      if (CChatRecord::Save(GetMemberByIndex(i)->GetName(), GetTotalMembers(),
                            saveText)) {
        // 				//string
        // str="已成功保存到文件:\""+CChatRecord::GetLastSavePath()+"\"里";
        // 				string str = RES_STRING(CL_LANGUAGE_MATCH_972) +
        // CChatRecord::GetLastSavePath() + RES_STRING(CL_LANGUAGE_MATCH_973);
        // 				CCozeForm::GetInstance()->OnSystemMsg(str.c_str());
        // 			}
        // 			else
        ...
        // CCozeForm::GetInstance()->OnSystemMsg("保存失败（内容空白，磁盘空间不足或文件目录没有可写权限）");
        // 				CCozeForm::GetInstance()->OnSystemMsg(
        // RES_STRING(CL_LANGUAGE_MATCH_974) );
      }
    }
  }
  // End
...
void CTalkSessionForm::SendBufferMsg() {
  if (!m_strWaitForSend.empty()) {
    ...
    char *token, *nexttoken; //修正好友聊天，对方下线再上限导致本机卡死 by
                             //Waiting 2009.07.06
    token = strtok_s(buf, "\n", &nexttoken);
    while (token != NULL) {
...
// CTalkSessionForm* CTalkSessionFormMgr::CreateSessionForm(CMember
// *pMember,eTalkSessionStyle style)
//{
//	vector <PS_Sessions>::iterator Iter;
//	for
//(Iter=m_SessionFormsLink.begin();Iter!=m_SessionFormsLink.end();Iter++)
//	{
...
//	CTalkSessionForm *pSessionForm=new
//CTalkSessionForm(enumTalkSessionNormal); 	if
//(pSessionForm->AddMember(pMember)==false)
//	{
//		delete pSessionForm;
//		return NULL;
//	}
...
// bool CTalkSessionFormMgr::AddToSessionForm(CMember *pMember,CTalkSessionForm
// *session)
//{
...
bool CTalkSessionFormMgr::AddToSession(DWORD sessionID,
                                       stNetSessCreate *pMember) {
...
void CTalkSessionFormMgr::CloseSessionForm(DWORD sessionID) {
  vector<CTalkSessionForm *>::iterator Iter;
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
       Iter++) {
...
void CTalkSessionFormMgr::LeaveSession(DWORD sessionID, DWORD chaID) {
  ...
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
       Iter++) {
    if ((*Iter)->GetSessionID() == sessionID) {
      if ((*Iter)->GetTotalMembers() == 1 &&
          chaID == (*Iter)->GetMemberByIndex(0)->GetID()) {
...
CTalkSessionForm *CTalkSessionFormMgr::GetSessionFormByName(string name,
                                                            int index) {
  vector<CTalkSessionForm *>::iterator Iter;
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
       Iter++) {
    ...
    if (pSessionForm) {
      if (pSessionForm->GetTotalMembers() == sApplyMember::numbers) {
        ...
        if (j == sApplyMember::numbers) {
          ...
          if (style == enumTalkSessionHidden ||
              style == enumTalkSessionInform ||
              style == enumTalkSessionMinimize) {
...
CTalkSessionForm *CTalkSessionFormMgr::CreateSessionForm(
    DWORD sessionID, stNetSessCreate *pArrMembers[], int memberNum) {
  CTalkSessionForm *pSessionForm = GetSessionFormByID(sessionID);
  if (pSessionForm)
    return pSessionForm;
  ...
  _itoa_s(memberNum, buf, sizeof(buf), 10);
  str += "(" + string(buf) + ")";
  ...
  if (pIconInfo) {
    ...
    pPic->LoadImage((strPath + pIconInfo->szSmall).c_str(), 16, 16, 0,
                    pIconInfo->nSmallX, pIconInfo->nSmallY);
    pPic->LoadImage("texture/ui/system/slide.tga", 16, 16, 1,
                    pIconInfo->nSmallX, pIconInfo->nSmallY);
    pPic->SetFrame(0);
  }
  g_stUIChat.GetTeamView()->Refresh();
...
CTalkSessionForm *CTalkSessionFormMgr::CreateHiddenSessionForm(
    DWORD sessionID, stNetSessCreate *pArrMembers[], int memberNum) {
  CTalkSessionForm *pSessionForm = GetSessionFormByID(sessionID);
  if (pSessionForm)
    return pSessionForm;
...
void CTalkSessionFormMgr::SessionReceiveMsg(DWORD sessionID, DWORD chaID,
                                            string talkMsg) {
  CTalkSessionForm *pSessionForm = GetSessionFormByID(sessionID);
  if (!pSessionForm)
    return;
...
POINT CTalkSessionFormMgr::GetMiniFormPosition(DWORD sessionID) {
  ...
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
    ...
    if ((*Iter)->GetSessionID() != sessionID) {
      ...
      SetRect(&rect, pForm->GetLeft(), pForm->GetTop(), pForm->GetRight(),
              pForm->GetBottom());
  ...
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
    ...
    if ((*Iter)->GetSessionID() != sessionID) {
      ...
      if (pt.x < 0 || pt.x > 5)
        continue;
      pt.y =
          (point.y - NORMAL_FORM_SHOW_SY - NORMAL_FORM_OFFSET_HEIGHT * pt.x) /
          (NORMAL_FORM_OFFSET_HEIGHT + NORMAL_FORM_OFFSET_HEIGHT / 2);
      // if ( pt.y >= 0 || pt.y<= 5)
      if (pt.y >= 0 && pt.y <= 5) // 静态检查修正
      {
        Flag[pt.y * 6 + pt.x]++;
      }
    }
...
void CTalkSessionFormMgr::OnFlashSession() {
  ...
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
    ...
    _itoa_s((*Iter)->GetTotalMembers(), buf, sizeof(buf), 10);
    str += "(" + string() + ")";
    pItem->SetHint(str.c_str());
    str = StringLimit(str, 14);
    pItem->SetString(str.c_str());
    ...
    if ((*Iter)->GetIsFlash()) {
      ...
      if ((*Iter)->GetStyle() == enumTalkSessionMinimize) {
        CLabelEx *pText =
            dynamic_cast<CLabelEx *>((*Iter)->m_pMinimizeForm->Find("labName"));
        if (pText) {
          pText->SetTextColor(((*Iter)->m_nFlashTimes & 1) ? 0xffa0a030
                                                           : COLOR_BLACK);
...
void CTalkSessionFormMgr::RefreshSessionMember(DWORD chaID,
                                               const char *szMotto) {
  if (szMotto == NULL)
    return;
  vector<CTalkSessionForm *>::iterator Iter;
...
bool CTalkSessionFormMgr::OnHotKeyShow(char &key, int &control) {
  ...
  for (pSessionForm = GetSessionFormByIndex(i); pSessionForm;
       pSessionForm = GetSessionFormByIndex(++i)) {
...
DWORD CTalkSessionFormMgr::GetSessionCount() { return m_SessionCounts; }
...
void CTalkSessionFormMgr::PrintTraceInfo() {
  ...
  for (Iter = m_SessionFormsLink.begin(); Iter != m_SessionFormsLink.end();
    ...
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
                "会话ID=%d 人数=%d 成员:", nMemberCount, dwSessionID);
    strInfo += szBuf;
