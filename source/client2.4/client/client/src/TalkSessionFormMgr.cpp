...
m_nShowSx(NULL), m_nShowSy(NULL), m_strWaitForAdded(""), m_bFlash(NULL), m_nFlashTimes(0), m_pTimer1(NULL), m_pTimer2(NULL)
{
	...
	if (!frmRemind)
	{
		if( !frmRemind ) Error(RES_STRING(CL_LANGUAGE_MATCH_410));
		return;
	}
	...
	if (!frmChat)
	{
		if( !frmChat ) Error(RES_STRING(CL_LANGUAGE_MATCH_412));
		 return;
	}
	...
	if (!frmChatMd)
	{
		if( !frmChatMd ) Error(RES_STRING(CL_LANGUAGE_MATCH_413));
		return;
	}
	...
	if (!frmChatMin)
	{
		if( !frmChatMin ) Error(RES_STRING(CL_LANGUAGE_MATCH_414));
		return;
	}
...
void CTalkSessionForm::RefreshForm()
{
	...
	{
		_snprintf_s( buf, _countof( buf ), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_417),total);
		str=buf;
	}
	...
	{
		_snprintf_s( buf, _countof( buf ), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_418),total);
		str=buf;
	...
	_snprintf_s( buf, _countof( buf ), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_419),total);
	str=buf;
...
void CTalkSessionForm::_OnMouseSendButton(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	_gmtime64_s( &ltime, &t);
	char ch[12];
	_snprintf_s(ch,  _countof( ch ), _TRUNCATE, "  %d:%d:%d",ltime.tm_hour+8,ltime.tm_min,ltime.tm_sec);
	string time = ch;
	if ( strMotto.size() == 0 )
	{
		strName +=":" + time + "\r\n  ";
	...
	{ // for循环处理多人聊天  ning.yan  2008-11-13  need modify
...
// 			string str = RES_STRING(CL_LANGUAGE_MATCH_972) + CChatRecord::GetLastSavePath() + RES_STRING(CL_LANGUAGE_MATCH_973);
...
void CTalkSessionForm::SendBufferMsg()
{
	if (!m_strWaitForSend.empty())
	{
		...
		char *token, *nexttoken; //修正好友聊天，对方下线再上限导致本机卡死 by Waiting 2009.07.06
		token = strtok_s( buf, "\n", &nexttoken);
		while( token != NULL )
		{
...
void CTalkSessionFormMgr::PrintTraceInfo()
{
	...
	for (Iter=m_SessionFormsLink.begin();Iter!=m_SessionFormsLink.end();Iter++)
	{
		...
		_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE, "会话ID=%d 人数=%d 成员:",nMemberCount,dwSessionID);
		strInfo+=szBuf;
