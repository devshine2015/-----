...
bool CStoreMgr::Init() {
  ...
  labLeftTimeTitle =
      dynamic_cast<CLabelEx *>(frmStore->Find("labLeftTimeTitle"));
  if (!labLeftTimeTitle) {
  ...
  for (int i = 0; i < STORE_PAGE_SIZE; ++i) {
    ...
    m_stStoreGui[i].labLeftTime =
        dynamic_cast<CLabelEx *>(frmStore->Find(szName));
    ...
    m_stStoreGui[i].labRightClickView =
        dynamic_cast<CLabelEx *>(frmStore->Find(szName));
    ...
    m_stStoreGui[i].cmdStore =
        dynamic_cast<COneCommand *>(frmStore->Find(szName));
    ...
    m_stStoreGui[i].btnBlue =
        dynamic_cast<CTextButton *>(frmStore->Find(szName));
  ...
  proStoreLoad =
      dynamic_cast<CProgressBar *>(frmStoreLoad->Find("proStoreLoad"));
  if (!proStoreLoad)
    return false;
...
#ifndef NEW_STORE
  CTextButton *btnClearAll =
      dynamic_cast<CTextButton *>(frmTryon->Find("btnClearAll"));
#else
  CTextButton *btnClearAll =
      dynamic_cast<CTextButton *>(frmStore->Find("btnClearAll"));
#endif
  if (btnClearAll) {
...
#ifndef NEW_STORE
  CTextButton *btnTurnleft3d =
      dynamic_cast<CTextButton *>(frmTryon->Find("btnTurnleft3d"));
#else
  CTextButton *btnTurnleft3d =
      dynamic_cast<CTextButton *>(frmStore->Find("btnTurnleft3d"));
#endif
  if (btnTurnleft3d) {
...
#ifndef NEW_STORE
  CTextButton *btnTurnright3d =
      dynamic_cast<CTextButton *>(frmTryon->Find("btnTurnright3d"));
#else
  CTextButton *btnTurnright3d =
      dynamic_cast<CTextButton *>(frmStore->Find("btnTurnright3d"));
#endif
  if (btnTurnright3d) {
...
void CStoreMgr::SwitchMap() { ShowStoreForm(false); }
...
void CStoreMgr::ShowTempKitbag(bool bShow) {
  if (bShow) {
    if (frmStore->GetIsShow()) {
      frmTempBag->SetPos(frmStore->GetX2(),
                         frmStore->GetHeight() - frmTempBag->GetHeight());
...
void CStoreMgr::OpenStoreAsk() {
  CBoxMgr::ShowSelectBox(_evtStoreOpenCheckEvent,
                         RES_STRING(CL_LANGUAGE_MATCH_858), true);
}
...
void CStoreMgr::ShowStoreLoad(bool bShow) {
  if (bShow) {
    ...
    frmStoreLoad->SetPos(
        (g_pGameApp->GetWindowWidth() - frmStoreLoad->GetWidth()) >> 1,
...
void CStoreMgr::ShowViewAllForm(bool bShow) {
  ...
  for (int i = 0; i < STORE_ITEM_COUNT; ++i) {
    ...
    if (m_stStoreInfo[m_nCurSel].comItemInfo[i].itemID > 0) {
      CItemRecord *pInfo =
          GetItemRecordInfo(m_stStoreInfo[m_nCurSel].comItemInfo[i].itemID);
      if (!pInfo)
        continue;
#ifndef NEW_STORE
      ...
      for (int j = 0; j < defITEM_INSTANCE_ATTR_NUM; ++j) {
        oItemGrid.sInstAttr[j][0] =
            m_stStoreInfo[m_nCurSel].comItemInfo[i].itemAttrID[j];
        oItemGrid.sInstAttr[j][1] =
            m_stStoreInfo[m_nCurSel].comItemInfo[i].itemAttrVal[j];
      }
  ...
  // 居中显示
  frmViewAll->SetPos(
      (g_pGameApp->GetWindowWidth() - frmViewAll->GetWidth()) >> 1,
...
void CStoreMgr::ShowStoreWebPage() {
  ...
  if (g_cooperate.code == COP_CGA) // 浩方特殊处理
  {
    ...
    ShellExecute(
        0, "open",
        RES_STRING(
            CL_LANGUAGE_MATCH_938), // "http://cache.moliyo.com/shop_77/",
        NULL, NULL, SW_SHOW);
  }
}
...
void CStoreMgr::ShowTryonForm(bool bShow) {
  if (bShow) {
    ...
    if (0 <= m_nCurSel && m_nCurSel < STORE_PAGE_SIZE) {
      for (int i = 5; i >= 0; --i) {
        // CItemCommand* pItemCmd =
        // dynamic_cast<CItemCommand*>(cmdStore[m_nCurSel][i]->GetCommand());
        ...
        if (1 <= sType && sType <= 10) // 武器
        {
          if (pItem->szAbleLink[0] < 10) {
            ...
            m_sCurLookInfo.SLook.SLink[pItem->szAbleLink[0]].sID =
                (short)pItem->lID;
...
void CStoreMgr::ShowAlphaMatteForm(bool bShow) {
  ...
  if (frmAlphaMatte) {
    frmAlphaMatte->SetSize(g_pGameApp->GetWindowWidth(),
...
// 从临时背包取出
bool CStoreMgr::PopFromTempKitbag(CGoodsGrid &rkDrag, CGoodsGrid &rkSelf,
  ...
  if (pkItemCmd->GetItemInfo()->GetIsPile() && pkItemCmd->GetTotalNum() > 1) {
    // 取出可以叠放的物品
    m_pkNumberBox =
        g_stUIBox.ShowNumberBox(_evtDragItemsEvent, pkItemCmd->GetTotalNum(),
    ...
    CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_KITBAGTMP_DRAG,
                   (void *)&(g_stUIStore.m_NetTempKitbag));
    return true;
  }
...
void CStoreMgr::_evtDragItemsEvent(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
...
// 添加商城树结点（父结点为零表示根结点）
void CStoreMgr::AddStoreTreeNode(long nParentID, long nID,
  ...
  static const int nWidth =
      trvStore->GetWidth() - trvStore->GetScroll()->GetWidth() - 28;
  ...
  if (0 == nParentID) {
    ...
    if (iterNode != m_mapNode.end()) {
      CTreeGridNode *pGridNode = dynamic_cast<GUI::CTreeGridNode *>(
          trvStore->GetRootNode()->FindNode(iterNode->second.c_str()));
      if (pGridNode) {
...
// 添加商城商品信息
void CStoreMgr::AddStoreItemInfo(long nSeq, long nID, const char *szName,
                                 long nPrice, const char *szRemark, bool isHot,
                                 long nTime, long nRemainNum,
                                 long nRemainTime) {
  ...
  char szTemp[128] = {0};
  //_snprintf_s( szTemp, _countof( szTemp ), _TRUNCATE,  "%s:%s",
  //RES_STRING(CL_LANGUAGE_MATCH_845), szName);
  m_stStoreGui[nSeq].labName->SetCaption(szName); // 商品名
  m_stStoreGui[nSeq].labName->SetIsShow(true);

  //_snprintf_s( szTemp, _countof( szTemp ), _TRUNCATE,  "%s:%s",
  //RES_STRING(CL_LANGUAGE_MATCH_846), StringSplitNum(nPrice, 3, ','));
  m_stStoreGui[nSeq].labPrice->SetCaption(
      StringSplitNum(nPrice, 3, ',')); // 商品价格
  m_stStoreGui[nSeq].labPrice->SetIsShow(true);

  _snprintf_s(szTemp, _countof(szTemp), _TRUNCATE, "%s:%s",
              RES_STRING(CL_LANGUAGE_MATCH_847),
              StringLimit(szRemark, 25).c_str());
  m_stStoreGui[nSeq].labRemark->SetCaption(
      strlen(szRemark) > 0 ? szTemp : szRemark); // 商品描述
  m_stStoreGui[nSeq].labRemark->SetIsShow(true);

  _snprintf_s(szTemp, _countof(szTemp), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_910), nRemainNum); // "%d个"
  m_stStoreGui[nSeq].labLeftNum->SetCaption(
      nRemainNum >= 0 ? szTemp : RES_STRING(CL_LANGUAGE_MATCH_912));
  m_stStoreGui[nSeq].labLeftNum->SetIsShow(true);

  _snprintf_s(szTemp, _countof(szTemp), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_911), nRemainTime); // "%d小时"
  m_stStoreGui[nSeq].labLeftTime->SetCaption(
      nRemainTime >= 0 ? szTemp : RES_STRING(CL_LANGUAGE_MATCH_912));
  ...
  // 129出界,数组0 Base最大只有128, strncpy_s用法也不对, 新的 strncpy_s 会自动加
  // '\0' -Waiting Fix 2009-03-13
  // 		//strncpy(m_stStoreInfo[nSeq].comName, szName, 20);	//
  // 商品名
  // 		strncpy_s(m_stStoreInfo[nSeq].comName,20, szName,_TRUNCATE );	//
  // 商品名 		m_stStoreInfo[nSeq].comName[21] = 0;
  //
  // 		//strncpy(m_stStoreInfo[nSeq].comRemark, szRemark, 128);	//
  // 商品备注 		strncpy_s(m_stStoreInfo[nSeq].comRemark,128, szRemark,_TRUNCATE );
  // // 商品备注 		m_stStoreInfo[nSeq].comRemark[129] = 0;
  strncpy_s(m_stStoreInfo[nSeq].comName, _countof(m_stStoreInfo[nSeq].comName),
            szName, _TRUNCATE); // 商品名
  strncpy_s(m_stStoreInfo[nSeq].comRemark,
            _countof(m_stStoreInfo[nSeq].comRemark), szRemark,
            _TRUNCATE); // 商品备注
}

// 添加商城商品细节信息
void CStoreMgr::AddStoreItemDetail(long nSeq, long nSubSeq, short sItemID,
                                   short sItemNum, short sFlute,
  ...
  memcpy(m_stStoreInfo[nSeq].comItemInfo[nSubSeq].itemAttrID, pItemAttrID,
         sizeof(short) * 5);
  memcpy(m_stStoreInfo[nSeq].comItemInfo[nSubSeq].itemAttrVal, pItemAttrVal,
         sizeof(short) * 5);
...
void CStoreMgr::SetStoreItemPage(long nCurPage, long nMaxPage) {
  ...
  if (m_nMaxPage > 0) {
    char szBuffer[32] = {0};
    _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE, "%d/%d", nCurPage,
                nMaxPage);
...
void CStoreMgr::SetStoreVip(long nVip) {
  // CTextButton* btnToVip =
  // dynamic_cast<CTextButton*>(frmStore->Find("btnToVIP"));
  if (nVip) {
...
// 设置商城按钮是否可用
void CStoreMgr::SetStoreBuyButtonEnable(bool b) { btnTrade->SetIsEnabled(b); }
...
bool CStoreMgr::ResetLastOperate(bool bSilent) {
  ...
  if (dwCurr - dwLast < 10000) {
    if (!bSilent) {
      g_pGameApp->MsgBox(
          RES_STRING(CL_LANGUAGE_MATCH_895)); // 商城操作过于频繁，请稍后再试！
    }
...
void CStoreMgr::_SetIsShowCozeForm(bool bShow) {
  ...
  CList *lstOnSaySystem =
      dynamic_cast<CList *>(frmMainChat->Find("lstOnSaySystem"));
  if (lstOnSaySystem)
    lstOnSaySystem->SetIsShow(bShow);

  CDragTitle *drpTitle =
      dynamic_cast<CDragTitle *>(frmMainChat->Find("drpTitle"));
  if (drpTitle)
    drpTitle->SetIsShow(bShow);

  CDragTitle *drpTitleSystem =
      dynamic_cast<CDragTitle *>(frmMainChat->Find("drpTitleSystem"));
  if (drpTitleSystem)
...
void CStoreMgr::_RefreshStoreListHighLight() {
  for (int i = 0; i < CStoreMgr::STORE_PAGE_SIZE; ++i) {
    g_stUIStore.m_stStoreGui[i].imgBlue->SetIsShow(
        i == g_stUIStore.m_nCurSel ? true : false);
  }
...
void CStoreMgr::_ShowTradeSelectBox() {
  if (0 <= m_nCurSel && m_nCurSel < STORE_PAGE_SIZE && GetCurSelItemID() > 0) {
    if (0 == m_nVip && _IsCurSelVipNode()) {
      g_pGameApp->MsgBox(
          RES_STRING(CL_LANGUAGE_MATCH_913)); // VIP专区仅限VIP高级用户购买！
      return;
    }

    char szTitle[256] = {0};
    _snprintf_s(
        szTitle, _countof(szTitle), _TRUNCATE, "%s%s: %s\n%s: %s",
        RES_STRING(CL_LANGUAGE_MATCH_857), RES_STRING(CL_LANGUAGE_MATCH_845),
        g_stUIStore.m_stStoreGui[g_stUIStore.m_nCurSel].labName->GetCaption(),
        RES_STRING(CL_LANGUAGE_MATCH_846),
  ...
  if (IsFirst) {
    ...
    if (fp) {
      if (fseek(fp, 0, SEEK_END) == 0) {
        ...
        fread(Info.szDesp,
              size > HELPINFO_DESPSIZE - 1 ? HELPINFO_DESPSIZE - 1 : size, 1,
              fp);
...
void CStoreMgr::_ChangeChaPart(stNetLookInfo &SLookInfo) {
  if (m_pCurrMainCha && SLookInfo.chSynType == enumSYN_LOOK_SWITCH) {
    if (SLookInfo.SLook.sTypeID != 0 &&
        m_pCurrMainCha->getTypeID() != SLookInfo.SLook.sTypeID) {
      if (SLookInfo.SLook.sTypeID != 0 &&
          SLookInfo.SLook.sTypeID != m_pCurrMainCha->getTypeID())
...
bool CStoreMgr::_IsCurSelVipNode(void) {
  ...
  if (it != m_mapNode.end()) {
    ...
    if (strName.size() > 3) {
      ...
      if (0 == _stricmp(strName.c_str(),
                        RES_STRING(CL_LANGUAGE_MATCH_902))) // 白金会员
      {
        return true;
      }
    }
...
// 商城树的鼠标点击事件
void CStoreMgr::_evtStoreTreeMouseClick(CGuiData *pSender, int x, int y,
                                        DWORD key) {
  CItemObj *pSelItem = g_stUIStore.trvStore->GetHitItem(x, y);
  if (pSelItem) {
    string strItem = pSelItem->GetString();
    for (MapNodeIter it = g_stUIStore.m_mapNode.begin();
         it != g_stUIStore.m_mapNode.end(); ++it) {
      if (it->second == strItem) {
        ...
        } else // 商品类型
        {
          ...
          CS_StoreListAsk(g_stUIStore.m_nCurClass,
                          (short)g_stUIStore.m_nCurPage, STORE_PAGE_SIZE);
        }
        break;
      }
    }
...
// 商城按钮点击事件
void CStoreMgr::_evtStoreFormMouseEvent(CCompent *pSender, int nMsgType, int x,
                                        int y, DWORD dwKey) {
  ...
  if (strName == "btnLeftPage") // 前一页
  {
    if (g_stUIStore.m_nCurPage > 1 && g_stUIStore.m_nCurClass > 0) {
      CS_StoreListAsk(g_stUIStore.m_nCurClass,
                      (short)g_stUIStore.m_nCurPage - 1, STORE_PAGE_SIZE);
    }
  } else if (strName == "btnRightPage") // 后一页
  {
    if (g_stUIStore.m_nCurPage < g_stUIStore.m_nMaxPage &&
        g_stUIStore.m_nCurClass > 0) {
      CS_StoreListAsk(g_stUIStore.m_nCurClass,
                      (short)g_stUIStore.m_nCurPage + 1, STORE_PAGE_SIZE);
    }
  ...
  } else if (strName == "btnReceiveMoney") // 兑换按钮
  {
    ...
    if (g_stUIStore.imgBackGround10->GetIsShow())
      return;
#endif
    g_stUIStore.m_pkExchangeNum = g_stUIBox.ShowNumberBox(
        _evtExchangeEvent, -1, RES_STRING(CL_LANGUAGE_MATCH_904),
        false); // 请输入兑换数量
  }
  // else if(strName == "btnToVIP")	// 成为VIP
  //{
  //	if(g_stUIStore.imgBackGround10->GetIsShow()) return;
  //	CBoxMgr::ShowSelectBox(_evtStoreToVipEvent,
  //RES_STRING(CL_LANGUAGE_MATCH_915), true); // 确定要成为VIP高级会员？
  //}
  else if (strName == "btnReceiveMoDou") // 兑换摩豆
  {
    ...
    ShellExecute(0, "open", strURL.c_str(), NULL, NULL, SW_SHOW);
             ...
             '0' <= strName[strName.size() - 1] &&
             strName[strName.size() - 1] <=
                 '0' + CStoreMgr::STORE_PAGE_SIZE) // 选中商品
  {
...
void CStoreMgr::_webFrmShow(CGuiData *pSender) {
  ...
  if (pBrowser) {
    pBrowser->setUrlHtml(
        "http://shop.moliyo.com/Iframe/Hdw/InnerShopBillboard.aspx");
    pBrowser->FrmActive();
  }
}
...
void CStoreMgr::_evtStoreListMouseRClick(CGuiData *pSender, int x, int y,
                                         DWORD key) {
  string strName = pSender->GetName();
...
void CStoreMgr::_evtStoreListMouseDBClick(CGuiData *pSender, int x, int y,
                                          DWORD key) {
  string strName = pSender->GetName();
  if (strName.size() > 0 && strName.substr(0, 8) == "btnBlue_" &&
      '0' <= strName[strName.size() - 1] &&
...
void CStoreMgr::_evtStoreOpenCheckEvent(CCompent *pSender, int nMsgType, int x,
                                        int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes) {
    return;
  }
...
// 成为VIP 确认
void CStoreMgr::_evtStoreToVipEvent(CCompent *pSender, int nMsgType, int x,
                                    int y, DWORD dwKey) {
  // if(nMsgType != CForm::mrYes)
  //{
  //	return;
...
// 代币兑换事件处理
void CStoreMgr::_evtExchangeEvent(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
  if (nMsgType != CForm::mrYes) {
    return;
  }
  ...
  if (num > 0) {
    ...
    _snprintf_s(szBuffer, _countof(szBuffer), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_905),
                g_stUIStore.m_nExchangeNum); // 确认兑换代币: %d
    CBoxMgr::ShowSelectBox(_evtExchangeCheckEvent, szBuffer, true);
  }
}

// 代币兑换确认
void CStoreMgr::_evtExchangeCheckEvent(CCompent *pSender, int nMsgType, int x,
                                       int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes) {
    return;
  }
...
// 确认购买处理
void CStoreMgr::_evtTradeCheckEvent(CCompent *pSender, int nMsgType, int x,
                                    int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes) {
    return;
  }

  if (0 <= g_stUIStore.m_nCurSel &&
      g_stUIStore.m_nCurSel < CStoreMgr::STORE_PAGE_SIZE &&
...
void CStoreMgr::_evtStoreViewAllMouseEvent(CCompent *pSender, int nMsgType,
...
void CStoreMgr::RenderChaTryon(int x, int y) {
  ...
  LEMatrix44 matCha = *m_pCurrMainCha->GetMatrix(); // 保存角色矩阵
...
void CStoreMgr::_evtChaEquipClearAll(CGuiData *sender, int x, int y,
                                     DWORD key) {
