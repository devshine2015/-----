...
bool CNpcTradeMgr::Init() {
  ...
  if (!pgeNPCtrade)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmNPCtrade->GetName(),
                 "pgeNPCtrade");

  // 交易的武器栏
  grdNPCtradeWeapon =
      dynamic_cast<CGoodsGrid *>(frmNPCtrade->Find("grdNPCtradeWeapon"));
  ...
  // 交易的装备栏
  grdNPCtradeEquip =
      dynamic_cast<CGoodsGrid *>(frmNPCtrade->Find("grdNPCtradeEquip"));
  if (!grdNPCtradeEquip)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmNPCtrade->GetName(),
                 "grdNPCtradeEquip");
  ...
  // 交易的药品栏
  grdNPCtradeOther =
      dynamic_cast<CGoodsGrid *>(frmNPCtrade->Find("grdNPCtradeOther"));
  if (!grdNPCtradeOther)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmNPCtrade->GetName(),
                 "grdNPCtradeOther");
...
void CNpcTradeMgr::ShowTradePage(const NET_TRADEINFO &TradeInfo, BYTE byCmd,
                                 DWORD dwNpcID) {
  _dwNpcID = dwNpcID;
  ...
  for (j = 0; j < TradeInfo.TradePage[0].byCount; j++) {
    ...
    if (!pItem) {
      LG("error", "msgCNpcTradeMgr::ShowTradePage item index[%d] out of range",
    ...
    CItemCommand *pObj =
        new CItemCommand(pItem, false); // modified guojiangang 20090108
  ...
  if (g_stUIEquip.GetItemForm()) {
    g_stUIEquip.GetItemForm()->SetPos(frmNPCtrade->GetX2(),
                                      frmNPCtrade->GetY());
...
void CNpcTradeMgr::SaleToNpc(BYTE byIndex, BYTE byCount, USHORT sItemID,
                             DWORD dwMoney) {}

void CNpcTradeMgr::BuyFromNpc(BYTE byIndex, BYTE byCount, USHORT sItemID,
                              DWORD dwMoney) {}
...
void CNpcTradeMgr::LocalBuyFromNpc(CGoodsGrid *pNpcGrid, CGoodsGrid *pSelfGrid,
  ...
  if (pBuy->GetPrice() > 0 && CGameScene::GetMainCha()) {
    ...
    // pBuy->GetPrice();
    nMax = (int)CGameScene::GetMainCha()->getGameAttr()->get(ATTR_GD) /
           pBuy->GetPrice();
  ...
  char buf[256] = {0};
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s[%s$]?", pBuy->GetName(),
              StringSplitNum(pBuy->GetPrice()));
  if (pBuy->GetIsPile() &&
      (_sBuy.pBox = g_stUIBox.ShowTradeBox(
           _BuyTradeEvent, (float)pBuy->GetPrice(), nMax, buf))) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_742), pBuy->GetName(),
                StringSplitNum(pBuy->GetPrice()));
...
void CNpcTradeMgr::LocalSaleToNpc(CGoodsGrid *pNpcGrid, CGoodsGrid *pSelfGrid,
                                  int nGridID, CCommandObj *pItem) {
  ...
  if (pSaleItem->GetIsPile() && pSaleItem->GetTotalNum() > 1) {
    char buf[256] = {0};
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s[%s$]",
                pSaleItem->GetItemInfo()->szName, StringSplitNum(nPrice));
    if (_sSale.pBox = g_stUIBox.ShowTradeBox(_SaleTradeEvent, (float)nPrice,
                                             pItem->GetTotalNum(), buf)) {
    ...
    bool OldItem = false;
    if (item.sEndure[0] != item.sEndure[1] ||
        item.sEnergy[0] != item.sEnergy[1])
      OldItem = true;
    if (OldItem)
    ...
    if (pSaleItem->GetItemInfo()->sType == 43) {
      CBoat *pBoat = g_stUIBoat.FindBoat(
          pSaleItem->GetData().GetDBParam(enumITEMDBP_INST_ID));
      if (pBoat) {
        if (OldItem)
          nPrice =
              (int)pBoat->GetCha()->getGameAttr()->get(ATTR_BOAT_PRICE) / 2;
        else
          nPrice =
              (int)pBoat->GetCha()->getGameAttr()->get(ATTR_BOAT_PRICE) / 8;
      }
    }

    if (OldItem)
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_992), pSaleItem->GetName(),
                  StringSplitNum(nPrice));
    else
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_743), pSaleItem->GetName(),
                  StringSplitNum(nPrice));
...
void CNpcTradeMgr::_BuyTradeEvent(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;

  stBuy &buy = g_stUINpcTrade._sBuy;
  CS_Buy(buy.dwNpcID, buy.nIndex, buy.nDragIndex, buy.nBuyGrid,
         buy.pBox->GetTradeNum());
}

void CNpcTradeMgr::_BuyEquipYesNoTradeEvent(CCompent *pSender, int nMsgType,
...
void CNpcTradeMgr::_SaleTradeEvent(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CNpcTradeMgr::_SaleEquipYesNoTradeEvent(CCompent *pSender, int nMsgType,
                                             int x, int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
