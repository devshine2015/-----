...
public:
	...
	CSceneObjInfo()
	{
	   strncpy_s(szName,sizeof(szName), RES_STRING(CL_LANGUAGE_MATCH_351),_TRUNCATE);
	   nPhotoTexID          = 0;
	   nAttachEffectID      = 0;
...
protected:
	...
	{   
        ...
        switch(pInfo->nType)
        {
        case SCENEOBJ_TYPE_POINTLIGHT: // (3)point light
...
//          LG("sceneobj", "Read Enviroment Sound [%s]\n", pInfo->szEnvSound);
            break;
        case SCENEOBJ_TYPE_NORMAL: // (0)普通物体
            {
	...
	virtual void _ProcessRawDataInfo(CRawDataInfo *pRawDataInfo)
	{
		CSceneObjInfo *pInfo = (CSceneObjInfo*)pRawDataInfo;
		char szPhoto[72]; 
		_snprintf_s(szPhoto,sizeof(szPhoto),_TRUNCATE, "texture/photo/sceneobj/%s.bmp", pInfo->szName);
