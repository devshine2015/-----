...
int CSystemProperties::ApplyVideo() {
  ...
  // ;//取消视野远近(Michael Chen 2005-04-22 nTexture-bGroundMark-nQuality
...
void CSystemMgr::LoadCustomProp() {
  ...
  if (!m_isLoad) {
    if (m_sysProp.Load("user\\system.ini")) {
      ...
      // m_sysProp.m_videoProp.bViewFar=true;      //取消视野远近(Michael Chen
      // 2005-04-22
...
bool CSystemMgr::Init() {
  ...
  if (!cbxTexture)
    return Error("msgui.clu界面<%s>上找不到控件<%s>", frmVideo->GetName(),
                 "cbxTexture");
  cbxTexture->SetActiveIndex(
      m_sysProp.m_videoProp
          .nTexture); //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  ...
  cbxMovie->SetActiveIndex(
      m_sysProp.m_videoProp.bAnimation
          ? 0
          : 1); //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  ...
  if (! cbxView )		return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  frmVideo->GetName(), "cbxView" );
  cbxView->SetActiveIndex(m_sysProp.m_videoProp.bViewFar ? 0 : 1);
  //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  */
  ...
  cbxColor->SetActiveIndex(
      m_sysProp.m_videoProp.bDepth32
          ? 1
          : 0); //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  ...
  cbxQuality->SetActiveIndex(
      m_sysProp.m_videoProp
          .nQuality); //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  ...
  proAudioMusic->SetPosition(static_cast<float>(
      m_sysProp.m_audioProp
          .nMusicSound)); //将配置文件的设置显示在控件上 Michael Chen 2005-04-22
  ...
  proAudioMidi->SetPosition(static_cast<float>(
      m_sysProp.m_audioProp.nMusicEffect)); //将配置文件的设置显示在控件上
...
void CSystemMgr::End() {
  ...
  // false;取消视野远近(Michael Chen 2005-04-22
  if (cbxTrail)
    m_sysProp.m_videoProp.bGroundMark =
...
void CSystemMgr::_evtVideoChangeChange(CGuiData *pSender) {
  ...
  if (g->GetActiveIndex() == 0) {
    ...
    // g_stUISystem.cbxView->SetActiveIndex(0);//取消视野远近(Michael Chen
    // 2005-04-22
    ...
    g_stUISystem.cbxCamera->SetActiveIndex(1);
    // g_stUISystem.cbxView->SetActiveIndex(1);//取消视野远近(Michael Chen
    // 2005-04-22
...
void CSystemMgr::_evtVideoFormMouseEvent(CCompent *pSender, int nMsgType, int x,
  ...
  if (name == "btnYes") //直接关闭表单
  {
    ...
    // bool bViewFar     =
    // g_stUISystem.cbxView->GetActiveIndex()==0?true:false;//取消视野远近(Michael
    // Chen 2005-04-22
    ...
    // g_stUISystem.cbxView->SetActiveIndex(g_nCbxView);//取消视野远近(Michael
    // Chen 2005-04-22
...
void CSystemMgr::_evtSystemFromMouseEvent(CCompent *pSender, int nMsgType,
  ...
  if (name == "btnChange") {
    ...
    // g_nCbxView    =
    // g_stUISystem.cbxView->GetActiveIndex();//取消视野远近(Michael Chen
    // 2005-04-22
