...
bool CUIGuildMgr::Init() {
  ...
  FORM_CONTROL_LOADING_CHECK(m_plabGuildName, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labName");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildMottoName, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labMaxim");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildType, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labState");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildMaster, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labPeople");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildMemberCount, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labNum");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildExperience, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labExp");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildMoney, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labMen");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildState, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labReason");
  FORM_CONTROL_LOADING_CHECK(m_plabGuildRemainTime, m_pGuildMgrForm, CLabelEx,
                             "manage.clu", "labRemain");
  FORM_CONTROL_LOADING_CHECK(m_pbtnGuildMottoEdit, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnMaxim");
  FORM_CONTROL_LOADING_CHECK(m_pbtnGuildDismiss, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnSan");
  FORM_CONTROL_LOADING_CHECK(m_pbtnMemberRecruit, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnYes");
  FORM_CONTROL_LOADING_CHECK(m_pbtnMemberRefuse, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnNo");
  FORM_CONTROL_LOADING_CHECK(m_pbtnMemberKick, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnkick");
  FORM_CONTROL_LOADING_CHECK(m_plstGuildMember, m_pGuildMgrForm, CListView,
                             "manage.clu", "lstNum");
  FORM_CONTROL_LOADING_CHECK(m_plstRecruitMember, m_pGuildMgrForm, CListView,
                             "manage.clu", "lstAsk");
  FORM_CONTROL_LOADING_CHECK(m_ppgeClass, m_pGuildMgrForm, CPage, "manage.clu",
                             "pgePublic");
  ...
  FORM_CONTROL_LOADING_CHECK(m_pedtGuildMottoName, m_pGuildMottoNameEditForm,
                             CEdit, "manage.clu", "edtMaxim");
  FORM_CONTROL_LOADING_CHECK(m_pbtnGuildMottoFormOK, m_pGuildMottoNameEditForm,
                             CTextButton, "manage.clu", "btnYes");
  m_pbtnGuildMottoFormOK->evtMouseClick = _OnClickMottoFormOK;
  //#ifdef GUILD
  //	Rain.
  FORM_CONTROL_LOADING_CHECK(btnyqrh, m_pGuildMgrForm, CTextButton,
                             "manage.clu", "btnyqrh");
  FORM_CONTROL_LOADING_CHECK(edtyqname, m_pGuildMgrForm, CEdit, "manage.clu",
                             "edtyqname");
...
void CUIGuildMgr::_OnClickEditMottoName(CGuiData *pSender, int x, int y,
                                        DWORD key) {
...
void CUIGuildMgr::_OnClickDismiss(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (pSelfData && pSelfData->IsManager()) {
    ...
    CBoxMgr::ShowSelectBox(_OnClickLeave, RES_STRING(CL_LANGUAGE_MATCH_596),
                           true);
  }
}
...
void CUIGuildMgr::_OnClickRecruit(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (pSelfData && pSelfData->IsManager()) {
    ...
    CRecruitMemberData *pMemberData =
        static_cast<CRecruitMemberData *>(pRow->GetPointer());
...
void CUIGuildMgr::_OnClickKick(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (pSelfData && pSelfData->IsManager()) {
    ...
    CGuildMemberData *pMemberData =
        static_cast<CGuildMemberData *>(pRow->GetPointer());
    string str = RES_STRING(CL_LANGUAGE_MATCH_601) + pMemberData->GetName() +
                 RES_STRING(CL_LANGUAGE_MATCH_602);
...
void CUIGuildMgr::_OnClickMottoFormOK(CGuiData *pSender, int x, int y,
                                      DWORD key) {
...
void CUIGuildMgr::RefreshAttribute() {
  ...
  m_plabGuildType->SetCaption((CGuildData::GetGuildType() == CGuildData::navy)
                                  ? RES_STRING(CL_LANGUAGE_MATCH_604)
                                  : RES_STRING(CL_LANGUAGE_MATCH_605));
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d",
              CGuildData::GetMemberCount(), CGuildData::GetMaxMembers());
  ...
  if (state == CGuildData::normal) {
    ...
    if (remain > 1440) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_611), remain / 1440);
    } else if (remain > 60) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_612), remain / 60);
    } else {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_613), remain);
    }
  ...
  //	Modify by alfred
  if (pSelfData && pSelfData->IsManager()) {
    m_pbtnGuildDismiss->GetImage()->LoadImage("texture/ui2009/guild4.tga", 43,
                                              22, 0, 92, 150);
    m_pbtnGuildDismiss->GetImage()->LoadImage("texture/ui2009/guild4.tga", 43,
                                              22, 1, 92, 194);
  } else {
...
void CUIGuildMgr::RefreshList() {
  ...
  for (DWORD i = 0; i < CGuildMembersMgr::GetTotalGuildMembers(); i++) {
    ...
    CItem *pMemberNameItem =
        new CItem(pMemberData->GetName().c_str(), COLOR_BLACK);
    CItem *pMemberJobItem =
        new CItem(pMemberData->GetJob().c_str(), COLOR_BLACK);
    // CItem*	pMemberLevelItem=new
    // CItem(_i64toa(pMemberData->GetLevel(),buf,10),COLOR_BLACK);
  ...
  for (DWORD i = 0; i < CRecruitMembersMgr::GetTotalRecruitMembers(); i++) {
    CRecruitMemberData *pMemberData =
        CRecruitMembersMgr::FindRecruitMemberByIndex(i);
    if (!pMemberData)
      continue;
...
void CUIGuildMgr::RemoveForm() { m_pGuildMgrForm->Close(); }

void CUIGuildMgr::_OnPassDismiss(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
...
void CUIGuildMgr::_OnClickLeave(CCompent *pSender, int nMsgType, int x, int y,
                                DWORD dwKey) {
  if (nMsgType == CForm::mrYes) {
    CM_GUILD_LEAVE();
  }
...
void CUIGuildMgr::_OnPassKick(CCompent *pSender, int nMsgType, int x, int y,
                              DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
  CItemRow *pRow = m_plstGuildMember->GetList()->GetSelectItem();
