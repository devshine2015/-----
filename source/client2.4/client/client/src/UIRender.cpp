...
CUIPanel::~CUIPanel() { SAFE_RELEASE(_lpVB); }
...
bool CUIPanel::Create(LPDIRECT3DDEVICE8 pDev) {
  ...
  UI_VERTEX Vertices[] = {
      {-(float)_w / 2, -(float)_h / 2, 0.0f, 1,
       D3DCOLOR_ARGB(255, 255, 255, 255), 0.0f, 1.0f},
      {-(float)_w / 2, (float)_h / 2, 0.0f, 1,
       D3DCOLOR_ARGB(255, 255, 255, 255), 0.0f, 0.0f},
      {(float)_w / 2, (float)_h / 2, 0.0f, 1, D3DCOLOR_ARGB(255, 255, 255, 255),
       1.0f, 0.0f},
      {(float)_w / 2, -(float)_h / 2, 0.0f, 1,
       D3DCOLOR_ARGB(255, 255, 255, 255), 1.0f, 1.0f},
  };

  if (FAILED(m_pDev->CreateVertexBuffer(4 * sizeof(UI_VERTEX),
                                        D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
                                        0, D3DPOOL_DEFAULT, &_lpVB)))
    return false;
  BYTE *lpVertex;
...
void CUIPanel::Draw(LPDIRECT3DTEXTURE8 pSrcTexture, CONST RECT *pSrcRect,
  ...
  lpVertex[1].x =
      (float)(_x + _w * pScaling->x); //_x -(float)_w/2;//-(float)_w/2;
  ...
  lpVertex[2].x =
      (float)(_x + _w * pScaling->x); //_x + _w - (float)_w/2;//(float)_w/2;
  lpVertex[2].y =
      (float)(_y + _h * pScaling->y); //_y + _h -(float)_h/2;//(float)_h/2;
  lpVertex[2].z = 0;
  ...
  lpVertex[3].y =
      (float)(_y + _h * pScaling->y); //_y -(float)_h/2;//-(float)_h/2;
  lpVertex[3].z = 0;
...
bool UIGetChooseColor(DWORD &c, DWORD *custcolor) {
  COLORREF cf =
      RGB((c & 0x00ff0000) >> 16, (c & 0x0000ff00) >> 8, c & 0x000000ff);
  CHOOSECOLOR cc;
  ...
  if (rt) {
    cf = cc.rgbResult;
    c = (c & 0xff000000) | (GetRValue(cf) << 16) | (GetGValue(cf) << 8) |
        GetBValue(cf);
  }
  return rt;
}
  ...
  if (InRect) {
    ...
    case stPress:
      ...
      if (key & Mouse_Move) {
        ...
        // 开始拖动
        if ((abs(_nStartX - x) >= (int)_nYareLen ||
             abs(_nStartY - y) >= (int)_nYareLen)) {
          _eState = stDrag;
          _pParent = gui;
          _pDrag = this;
          CGuiData::SetCursor(_crDrag);
...
bool CDrag::MouseRun(int x, int y, DWORD key) {
  ...
  if (key & Mouse_LUp) {
    if (_IsUseGrid) {
      _nDragX = (_pParent->GetX() + GetDragX() + _dwGridWidth / 2) /
                    _dwGridWidth * _dwGridWidth -
                _pParent->GetX();
      _nDragY = (_pParent->GetY() + GetDragY() + _dwGridHeight / 2) /
                    _dwGridHeight * _dwGridHeight -
                _pParent->GetY();
    }

    if (_IsMove) {
      _pParent->SetPos(_pParent->GetLeft() + GetDragX(),
                       _pParent->GetTop() + GetDragY());
      _pParent->Refresh();
    }
...
// class CGuiData
//---------------------------------------------------------------------------
HWND CGuiData::GetHWND() { return g_pGameApp->GetHWND(); }
...
UIRender::UIRender()
    : _nGuiWidth(1024), _nGuiHeight(768), _fScreenScaleX(1.0),
      _fScreenScaleY(1.0), _fDrawScaleX(1.0), _fDrawScaleY(1.0),
      _p2DSprite(NULL), _nTex(-1) {
  _rtFrame.left = 0;
  _rtFrame.top = 0;
  _rtFrame.right = 1;
  _rtFrame.bottom = 1;
...
void UIRender::Reset() { ui::UIClip::Instance()->Reset(); }
...
void UIRender::RegisterFunc() {
  LERegisterOutputLoseDeviceProc(UI_OnLostDevice);
  LERegisterOutputResetDeviceProc(UI_OnResetDevice);
}
...
void UIRender::RenderTextureRect(int nX, int nY, LETexRect *pRect) {
  ...
  //	VECTOR2 vecScale( DrawConvertX( pRect->fScaleX ), DrawConvertY(
  //pRect->fScaleY ) );

  RECT TexRect = {pRect->nTexSX, pRect->nTexSY, pRect->nTexSX + pRect->nTexW,
                  pRect->nTexSY + pRect->nTexH};

  RenderSprite(pTexture, &TexRect, &vecScale, &vecDest, pRect->dwColor);
}

void UIRender::RenderTextureAbsRect(int nX, int nY, LETexRect *pRect) {
  LPTEXTURE pTexture = GetTextureByID(pRect->nTextureNo);
  if (!pTexture)
    return;
...
void UIRender::LineThinFrame(int left, int top, int right, int bottom,
                             DWORD color) {
...
void UIRender::LineFrame(int left, int top, int right, int bottom,
                         DWORD color) {
  DrawConvert(left, top);
  DrawConvert(right, bottom);
...
void UIRender::SelectChaLineFrame(int left, int top, int right, int bottom,
                                  DWORD color) {
  DrawConvert(left, top);
  DrawConvert(right, bottom);
...
void UIRender::FillFrame(int left, int top, int right, int bottom,
                         DWORD color) {
  DrawConvert(left, top);
  DrawConvert(right, bottom);
...
void UIRender::SetScreen(int w, int h, bool isFull) {
  ...
  //     LG( "setscreen", "Screen:%f, %f Draw:%f, %f ScreenSize: %d, %d
  //     ClientSize: %d, %d  IsFull: %d\n"
  //         , _fScreenScaleX, _fScreenScaleY, _fDrawScaleX, _fDrawScaleY, w, h,
  //         sw, sh, isFull );
  ...
  CGuiData::SetScreen(_fDrawScaleX, _fDrawScaleY, _fScreenScaleX,
                      _fScreenScaleY);
...
// class CGuiPic
//---------------------------------------------------------------------------
bool CGuiPic::LoadImage(LETexRect *pTex, const char *file, int w, int h, int tx,
                        int ty, float scale_x, float scale_y) {
  if (!pTex)
    return false;
...
bool CGuiPic::LoadImage(const char *file, int w, int h, int frame, int tx,
...
bool CGuiPic::LoadImage(int frame, int nTextureID, int tx, int ty, int tw,
                        int th, float scale_x, float scale_y) {
  LETexRect *pTex = GetImage(frame);
  if (!pTex)
    return false;
  ...
  if (_bIsScale) {
    for (unsigned int i = 0; i < _max; i++) {
      if (_pImage[i].nTextureNo == -1)
        continue;
      _pImage[i].fScaleX =
          (float)_pParent->GetWidth() / (float)_pImage[i].nTexW;
      _pImage[i].fScaleY =
          (float)_pParent->GetHeight() / (float)_pImage[i].nTexH;
    }
  }
}
...
bool CFramePic::LoadImage(ePicPos p, int nTextureID, int tx, int ty, int tw,
                          int th) {
  if (p < 0 || p >= ppEnd)
    return false;

  LETexRect *pTex = &_pImage[p];
  pTex->nTexSX = tx;
  pTex->nTexSY = ty;
  pTex->nTexW = tw;
  pTex->nTexH = th;
...
bool CFramePic::LoadImage(const char *client, int cw, int ch, int tx, int ty,
...
// class CGuiFont
//---------------------------------------------------------------------------
CGuiFont::CGuiFont() : _pFont(NULL) {}

CGuiFont::~CGuiFont() { Clear(); }
...
void CGuiFont::SetScreen(int nScrWidth, int nScrHeight) {
  ...
  if (nScrWidth == 800) {
    for (stfont::iterator it = _stfonts.begin(); it != _stfonts.end(); ++it) {
      p = new CLEFont;
#ifdef USE_RENDER
      if (!p->CreateFont(&g_Render, (char *)it->strFont.c_str(), it->size800,
                         FONTLEVEN, it->dwStyle))
        LG("ERROR", "msgCreateFont");
#else
      p->CreateFont(g_Render.GetDevice(), (char *)it->strFont.c_str(),
                    it->size800, FONTLEVEN, it->dwStyle);
#endif
    ...
    for (stfont::iterator it = _stfonts.begin(); it != _stfonts.end(); ++it) {
      p = new CLEFont;
#ifdef USE_RENDER
      if (!p->CreateFont(&g_Render, (char *)it->strFont.c_str(), it->size1024,
                         FONTLEVEN, it->dwStyle))
        LG("ERROR", "msgCreateFont2");

#else
      p->CreateFont(g_Render.GetDevice(), (char *)it->strFont.c_str(),
                    it->size1024, FONTLEVEN, it->dwStyle);
#endif
...
// class CTextButton
//---------------------------------------------------------------------------
bool CTextButton::LoadImage(const char *file, int w, int h, int tx, int ty,
                            bool isHorizontal) {
  if (!_pImage)
    return false;
...
// class CScroll
//---------------------------------------------------------------------------
bool CScroll::LoadImage(const char *bkg, const char *up, const char *scroll,
                        const char *down, int w, int h) {
...
bool CEdit::InitCursor(const char *szFile) {
  LETexRect *pTex = &_CursorImage;

  int nTextureNo = GetTextureID(szFile);
  LETexInfo *pInfo = GetTextureInfo(nTextureNo);
  if (pInfo) {
...
void CEdit::ShowFocus() {
  ...
  // 显示闪烁光标 -Waiting Modify 2009-05-27
  ...
  if (_bCursorIsShow) {
    ...
    GetRender().FillFrame(_nCursorX, _nCursorY, _nCursorX + 2,
                          _nCursorY + _nCursorHeight, _nCursorColor);
  }
}
//---------------------------------------------------------------------------
// class CProgressBar
...
bool CProgressBar::LoadImage(const char *file, int w, int h,
                             bool isHorizontal) {
  if (!_pImage)
    return false;
...
void CProgressBar::Render() {
  ...
  if (_style == btHorizontal) {
    ...
    GetRender().RenderTextureRect(GetX(), GetY(), _pImage->GetImage());
    GetRender().RenderTextureRect(
        GetX(), int(GetY() + GetHeight() * (1 - _step.GetRate())), _pTex);
    // GetRender().RenderTextureRect( GetX(), (int)(GetY() + GetHeight() * (1 -
    // _step.GetRate())), _pTex );
  }
}
...
void CGrid::Init() {
  ...
  _snprintf_s(_strPage, sizeof(_strPage), _TRUNCATE, "0/0");
...
void CTreeNodeObj::ShowFocus() {
  if (IsShow() && _pOwn->InTree(_nX, _nY, _nX2, _nY2))
    GetRender().FillFrame(_nX - 1, _nY - 1, _nX2 + 1, _nY2 + 1,
...
void CTreeGridNode::ShowFocus() {
  if (_IsShowFocus) {
    if (_IsExpand && IsShow() &&
        _pOwn->InTree(_nFocusX1, _nFocusY1, _nFocusX2, _nFocusY2)) {
      _RenderTree();
    ...
    if (IsShow() && _pOwn->InTree(_nX, _nY, _nX + _pItem->GetWidth(),
                                  _nY + _pItem->GetHeight()))
      GetRender().FillFrame(_nX - 1, _nY - 1, _nX + _pItem->GetWidth() + 1,
                            _nY + _pItem->GetHeight() + 1,
                            _pOwn->GetSelectColor());
