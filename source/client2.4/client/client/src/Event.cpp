...
// class CEvent
//---------------------------------------------------------------------------
CEvent::CEvent(CGameScene *pScene) : _pScene(pScene) {
  // LG( "event", "new\n" );
  Init();
}
...
bool CEvent::DistanceTrigger(int x, int y) {
  if (GetIsValid() && GetIsEnabled() &&
      GetInfo()->sArouseType == enumEVENT_AROUSE_DISTANCE) {
    if (!_IsActive) {
      if (GetDistance(x, y, GetNode()->GetCurX(), GetNode()->GetCurY()) <=
          GetInfo()->sArouseRadius) {
        _IsActive = true;
...
// class CEventMgr
//---------------------------------------------------------------------------
CEventMgr::CEventMgr(CGameScene *pScene) : _pScene(pScene) {}

CEventMgr::~CEventMgr() { Clear(); }
...
void CEventMgr::DistanceTrigger(CCharacter *pCha) {
  for (events::iterator it = _events.begin(); it != _events.end(); it++) {
    if ((*it)->IsNormal() &&
        (*it)->DistanceTrigger(pCha->GetCurX(), pCha->GetCurY()))
      (*it)->ExecEvent(pCha);
  }
}
