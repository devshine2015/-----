...
CGrid::CGrid(CForm &frmOwn)
    : CCompent(frmOwn), _pImage(NULL), _nUnitHeight(32), _nUnitWidth(32),
      _nFirst(0), _nLast(0), _nColNum(0), _nMaxPage(0), _nPage(0), _nMargin(5),
      _nPageNum(0), _nSize(0), _pSelect(NULL), _nSpaceX(2), _nSpaceY(2),
      _pSelectImage(NULL), evtSelectChange(NULL), _clPageTextColor(0xff0000ff),
      _nSelectIndex(-1) {
  // _IsFocus = true;
...
CGrid::CGrid(const CGrid &rhs)
    : CCompent(rhs), _pImage(new CGuiPic(*rhs._pImage)),
      _pSelectImage(new CImage(*rhs._pSelectImage)),
      _pSizeImg(new CDragTitle(*rhs._pSizeImg)),
      _pNextPage(new CTextButton(*rhs._pNextPage)),
...
void CGrid::_Copy(const CGrid &rhs) {
  _Clear();
  for (memory::const_iterator it = rhs._memory.begin(); it != rhs._memory.end();
       it++)
  ...
  strncpy_s(_strPage, sizeof(_strPage), rhs._strPage, _TRUNCATE);
  _nStrX = rhs._nStrX;
  _nStrY = rhs._nStrY;
...
void CGrid::Refresh() {
  ...
  // _pSizeImg在右下角
  _pSizeImg->SetPos(GetWidth() - _pSizeImg->GetWidth(),
                    GetHeight() - _pSizeImg->GetHeight());
  _pSizeImg->Refresh();

  // 向前按钮，文字，向后前按钮依次在左下角
  _pPriorPage->SetPos(_nMargin,
                      GetHeight() - _pPriorPage->GetHeight() - _nMargin);
  _pPriorPage->Refresh();

  _nStrX = _pPriorPage->GetX2() + _nMargin;
  _nStrY = _pPriorPage->GetY() +
           (_pPriorPage->GetHeight() - CGuiFont::s_Font.GetHeight("S")) / 2;

  _pNextPage->SetPos(_pPriorPage->GetWidth() + _nMargin * 3 + _nStrWidth,
                     _pPriorPage->GetTop());
  _pNextPage->Refresh();
  ...
  _nRowNum =
      (GetHeight() - _nMargin - _nMargin - _pNextPage->GetHeight()) / _nTotalH;
...
bool CGrid::MouseRun(int x, int y, DWORD key) {
  ...
  if (IsNoDrag(x, y, key)) {
    ...
    if (_IsSelect) {
      ...
      _pSelect = _memory[x];
      _nSelectIndex = x;
      _pSelectImage->SetPos(_nTotalW * col + _nMargin,
                            _nTotalH * row + _nMargin);
...
void CGrid::_RefreshSize() {
  if ((_nDragWidth + _pSizeImg->GetDrag()->GetDragX()) > (3 * _nTotalW) &&
      (_nDragHeight + _pSizeImg->GetDrag()->GetDragY()) > (3 * _nTotalH)) {
    SetSize(_nDragWidth + _pSizeImg->GetDrag()->GetDragX(),
            _nDragHeight + _pSizeImg->GetDrag()->GetDragY());
    Refresh();
    if (_nPage > _nMaxPage) {
      _nPage = _nMaxPage;
      _PageChange();
...
void CGrid::SetIsDrag(bool v) { _pSizeImg->SetIsEnabled(v); }
