...
CSeatState::CSeatState(CActor *p)
    : CActionState(p), _nHeight(ERROR_POSE_HEIGHT), _nPose(0), _nAngle(0),
      _nPosX(0), _nPosY(0), _IsSeat(false) {
  _IsOver = true;
  _IsSend = true;
}
...
bool CSeatState::_Start() {
  ...
  if (_IsSend) {
    if (GetDistance(_pCha->GetCurX(), _pCha->GetCurY(), _nPosX, _nPosY) >
        _pCha->GetDefaultChaInfo()->sRadii + 101) {
      return false;
    }
