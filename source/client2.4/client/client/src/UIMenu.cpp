...
CMenuItem::CMenuItem(const char *str, DWORD v)
    : _IsCheck(false), _IsEnabled(true), _IsHide(false), _strText(str),
      _nTag(0), _dwIndex(v) {
  CGuiFont::s_Font.GetSize(str, _nWidth, _nHeight);
}

void CMenuItem::Render(int x, int y) {
  CGuiFont::s_Font.Render(_strText.c_str(), x, y,
                          _IsEnabled ? COLOR_BLACK : 0xffa0a0a0);
}
...
CMenu::CMenu(CForm *pForm)
    : CGuiData(), evtSelectChange(NULL), evtListMouseDown(NULL),
      _eStyle(enumMouseClick) {
...
CMenu::CMenu(const CMenu &rhs)
    : CGuiData(rhs), _pSelect(new CGuiPic(*rhs._pSelect)),
      _pImage(new CFramePic(*rhs._pImage)) {
  _Copy(rhs);
}
...
void CMenu::_Copy(const CMenu &rhs) {
  ...
  _MenusClear();
  for (menus::const_iterator it = rhs._menus.begin(); it != rhs._menus.end();
       it++)
  ...
  if (InRect(x, y)) {
    ...
    for (menus::iterator it = _menus.begin(); it != _menus.end(); it++) {
      ...
      if (y >= y0 && y <= y1) {
        if (_pSelectMenu != *it) {
          _pSelectMenu = *it;
          _pSelect->SetScale(_pSelectMenu->GetWidth(),
                             _pSelectMenu->GetHeight());
