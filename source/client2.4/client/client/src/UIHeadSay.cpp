...
DWORD CHeadSay::s_dwNamePartsColors[NAME_PART_NUM]
                                   [2] = //	前一列是前景色,后一列是阴影颜色
    {
        COLOR_WHITE,
        COLOR_BLACK, //	(
...
// Member Methods---------------------------------------------------------
bool CHeadSay::Init() {
  _pImgTeamLeaderFlag->LoadImage("texture/ui/flag.tga", 12, 12, 0, 0, 0, 1.0,
                                 1.0);

  _nShopFrameWidth = _ImgShop[0].GetWidth() - 1;
  _nShopFontYOff =
      (_ImgShop[0].GetHeight() -
       CGuiFont::s_Font.GetHeight(RES_STRING(CL_LANGUAGE_MATCH_489))) /
      2;
  return true;
}
...
CHeadSay::CHeadSay(CCharacter *p)
    : _pOwn(p), _nShowTime(0), _pObj(NULL), _fLifeW(1.0f), _dwFaceTime(0),
      _pCurFace(NULL), _IsShowLife(false), _IsShowName(false), _nChaNameOffX(0),
      _fScale(0.1f), _dwNameColor(COLOR_WHITE) {}

//安全释放内存 by Waiting 2009-06-18
CHeadSay::~CHeadSay() { SAFE_DELETE(_pObj); }
...
void CHeadSay::Render(D3DXVECTOR3 &pos) {
  static int x = 0, y = 0;
  static int nSayTotalWidth =
      32 * CGuiFont::s_Font.GetWidth("a"); // 32个字符的长度
  ...
  if (_IsShowLife) //血条
  {
    ...
    _pImgLife->RenderAll(x - nLifeWidth / 2 - nOffset, y1 + 20,
                         (DWORD)0xa0ffffff);
  }
  ...
  if (_nShowTime <= 0) {
    ...
    if (CCharacter::IsShowLogName) {
      char szLogName[128];
      _snprintf_s(szLogName, _countof(szLogName), _TRUNCATE, "%s (%u)",
                  _pOwn->getName(), _pOwn->getAttachID());
      int nNameLength = 0 - CGuiFont::s_Font.GetWidth(szLogName) / 2;
      CGuiFont::s_Font.BRender(szLogName, x + nNameLength, y - iNameHeightStep,
                               _dwNameColor, COLOR_BLACK);
    } else if (_IsShowName) {
      // 处理公会名
      if (*(_pOwn->getGuildName())) {
        ...
        //公会名
        if (*(_pOwn->getGuildName())) {
          // strncpy( s_szConsortiaNamePart[0], _pOwn->getGuildName(),
          // NAME_LENGTH );
          strncpy_s(s_szConsortiaNamePart[0], NAME_LENGTH,
                    _pOwn->getGuildName(), _TRUNCATE);
        } else {
        ...
        if (*(_pOwn->getGuildMotto())) {
          strncpy_s(s_szConsortiaNamePart[1],
                    _countof(s_szConsortiaNamePart[1]), "(", _TRUNCATE);
          strncpy_s(s_szConsortiaNamePart[2], NAME_LENGTH,
                    _pOwn->getGuildMotto(), _TRUNCATE);
          strncpy_s(s_szConsortiaNamePart[3],
                    _countof(s_szConsortiaNamePart[3]), ")", _TRUNCATE);
        } else {
        ...
        for (int i(0); i < 4; i++) {
          strncat_s(s_szConsortiaName, NAME_LENGTH, s_szConsortiaNamePart[i],
                    _TRUNCATE);
        }
        ...
        // render每一个部分
        for (int i(0); i < 4; i++) {
          CGuiFont::s_Font.BRender(s_szConsortiaNamePart[i], x + iStartPosX,
                                   y - iGuildNameHeightStep,
      ...
      //	strncpy(str1,str2,sizeof(str1)) Modified to
      //strncpy_s(str1,sizeof(str1),str2,_TRUNCATE).
      // 前缀名
      if (*(_pOwn->GetPreName())) {
        strncpy_s(s_sNamePart[PRENAME_SEP1_INDEX],
                  _countof(s_sNamePart[PRENAME_SEP1_INDEX]), "(", _TRUNCATE);
        strncpy_s(s_sNamePart[PRENAME_INDEX], NAME_LENGTH, _pOwn->GetPreName(),
                  _TRUNCATE);
        strncpy_s(s_sNamePart[PRENAME_SEP2_INDEX],
                  _countof(s_sNamePart[PRENAME_SEP2_INDEX]), ")", _TRUNCATE);
      ...
      if (*(_pOwn->getSecondName())) {
        strncpy_s(s_sNamePart[MOTTO_NAME_SEP1_INDEX],
                  _countof(s_sNamePart[MOTTO_NAME_SEP1_INDEX]), "(", _TRUNCATE);
        strncpy_s(s_sNamePart[MOTTO_NAME_INDEX], NAME_LENGTH,
                  _pOwn->getSecondName(), _TRUNCATE);
        strncpy_s(s_sNamePart[MOTTO_NAME_SEP2_INDEX],
                  _countof(s_sNamePart[MOTTO_NAME_SEP2_INDEX]), ")", _TRUNCATE);
      } else {
        strncpy_s(s_sNamePart[MOTTO_NAME_SEP1_INDEX], NAME_LENGTH, "",
                  _TRUNCATE);
        strncpy_s(s_sNamePart[MOTTO_NAME_INDEX], NAME_LENGTH, "", _TRUNCATE);
        strncpy_s(s_sNamePart[MOTTO_NAME_SEP2_INDEX], NAME_LENGTH, "",
                  _TRUNCATE);
      }
      ...
      if (_pOwn->IsBoat()) {
        strncpy_s(s_sNamePart[NAME_INDEX], NAME_LENGTH, _pOwn->getHumanName(),
                  _TRUNCATE);
        strncpy_s(s_sNamePart[BOAT_NAME_SEP1_INDEX],
                  _countof(s_sNamePart[BOAT_NAME_SEP1_INDEX]), "[", _TRUNCATE);
        strncpy_s(s_sNamePart[BOAT_NAME_INDEX], NAME_LENGTH, _pOwn->getName(),
                  _TRUNCATE);
        strncpy_s(s_sNamePart[BOAT_NAME_SEP2_INDEX],
                  _countof(s_sNamePart[BOAT_NAME_SEP2_INDEX]), "]", _TRUNCATE);
      } else {
        strncpy_s(s_sNamePart[BOAT_NAME_SEP1_INDEX], NAME_LENGTH, "",
                  _TRUNCATE);
        strncpy_s(s_sNamePart[BOAT_NAME_INDEX], NAME_LENGTH, "", _TRUNCATE);
        strncpy_s(s_sNamePart[BOAT_NAME_SEP2_INDEX], NAME_LENGTH, "",
                  _TRUNCATE);
        strncpy_s(s_sNamePart[NAME_INDEX], NAME_LENGTH, _pOwn->getName(),
                  _TRUNCATE);

        if (_pOwn->IsMonster()) { //如果是怪物，且等级高于自身10级的显示怪物等级
          ...
          if (g_stUIBoat.GetHuman()) {
            ...
            nMainLevel =
                (int)g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_LV);
          }
          ...
          if (nMonsterLevel - nMainLevel <= 10) { //显示等级
            _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "Lv%d %s",
                        nMonsterLevel, _pOwn->getName());
          } else {
      ...
      for (int i(0); i < NAME_PART_NUM; i++) {
        if (s_dwNamePartsColors[i][1]) {
          CGuiFont::s_Font.BRender(
              s_sNamePart[i], x + iStartPosX, y - iNameHeightStep,
              s_dwNamePartsColors[i][0], s_dwNamePartsColors[i][1]);
        } else {
          CGuiFont::s_Font.Render(s_sNamePart[i], x + iStartPosX,
                                  y - iNameHeightStep,
    ...
    if (_IsShowName) {
      if (_pOwn->IsBoat()) //显示角色名+船名
      {
        ...
        CGuiFont::s_Font.BRender(sNameBuf.c_str(), x + nNameLength,
                                 y - LINE_HEIGHT_STEP, _dwNameColor,
                                 COLOR_BLACK);
        if (_pOwn->IsShowSecondName()) {
          if (strlen(_pOwn->getSecondName()) > 0) {
            ...
            strSec += ")";
            CGuiFont::s_Font.Render(strSec.c_str(), x - nNameLength,
                                    y - LINE_HEIGHT_STEP, COLOR_BLACK);
          }
        }
      } else //显示角色名
      {
        int nNameLength = 0 - CGuiFont::s_Font.GetWidth(_pOwn->getName()) / 2;
        CGuiFont::s_Font.BRender(_pOwn->getName(), x + nNameLength,
                                 y - LINE_HEIGHT_STEP, _dwNameColor,
                                 COLOR_BLACK);
        if (_pOwn->IsShowSecondName()) {
          if (strlen(_pOwn->getSecondName()) > 0) {
  ...
  } else if (_nShowTime > 0) //聊天
  {
    ...
    if (_pObj) {
      ...
      if (str.find("#") != -1) {
        if (nLine == 1) {
          GetRender().FillFrame(left - 2, y - 20 - 32,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
          _pObj->RenderEx(left, y - 18 - 32, 32, _fScale);
        } else if (nLine == 2) {
          GetRender().FillFrame(left - 2, y - 20 - 44,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
          _pObj->RenderEx(left, y - 18 - 44, 24, _fScale);

        } else if (nLine == 3) {
          GetRender().FillFrame(left - 2, y - 20 - 76,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
        ...
        if (nLine == 1) {
          GetRender().FillFrame(left - 2, y - 20 - 18,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
          _pObj->RenderEx(left, y - 18 - 18, 12, _fScale);
        } else if (nLine == 2) {
          GetRender().FillFrame(left - 2, y - 20 - 36,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
          _pObj->RenderEx(left, y - 18 - 36, 12, _fScale);
        } else if (nLine == 3) {
          GetRender().FillFrame(left - 2, y - 20 - 54,
                                left + (int)(_fScale * _pObj->GetWidth()) + 4,
                                y - 14, _dwBkgColor);
...
#define FFQ 60
  if (_pCurFace && _dwFaceTime) //表情
  {
    ...
    if (dwElaped > _nMaxShowTime * FFQ / 2) {
      ...
      DWORD dwCurFrame =
          (dwElaped / (_nFaceFrequency * FFQ)) % _pCurFace->GetMax();
  ...
  if (CCharacter::IsShowLogName) {
    ...
    for (int i = 0; i < nCount; i++) {
      y -= 20;
      CGuiFont::s_Font.Render(pState->GetSkillState(i)->szName, x, y,
                              COLOR_WHITE);
    }
  }
#endif
...
void CHeadSay::_RenderShop(const char *szShopName, int x, int y) {
  ...
  if (nFontWidth < 40) {
    ...
    // CGuiFont::s_Font.BRender( szShopName, x + (nFontWidth - nFontTrueWidth) /
    // 2, y+_nShopFontYOff, COLOR_BLACK, COLOR_WHITE );
    CGuiFont::s_Font.Render(szShopName, x + (nFontWidth - nFontTrueWidth) / 2,
                            y + _nShopFontYOff, COLOR_BLACK);
  } else {
    x -= _nShopFrameWidth + nFontWidth / 2;
    ...
    _ImgShop[1].SetScaleW(nFontWidth);
    _ImgShop[1].Render(x, y, dwImgColor);
    // CGuiFont::s_Font.BRender( szShopName, x, y+_nShopFontYOff, COLOR_BLACK,
    // COLOR_WHITE );
...
void CHeadSay::RenderText(const char *szShopName, int x, int y) {
  ...
  if (nFontWidth < 40) {
    ...
    _ImgShop[1].SetScaleW(nFontWidth);
    _ImgShop[1].Render(x, y, dwImgColor);
    CGuiFont::s_Font.BRender(szShopName, x + (nFontWidth - nFontTrueWidth) / 2,
                             y + _nShopFontYOff, COLOR_BLACK, COLOR_WHITE);
  } else {
    x -= _nShopFrameWidth + nFontWidth / 2;
