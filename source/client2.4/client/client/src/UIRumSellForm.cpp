...
CRumSellMgr::CRumSellMgr()
    : frmRumSell(NULL), editCount(NULL), labelPrice(NULL), labelTotal(NULL),
...
bool CRumSellMgr::Init() {
  ...
  CTextButton *btnDeal03 =
      static_cast<CTextButton *>(frmRumSell->Find("btnDeal03"));
  assert(btnDeal03);
...
void CRumSellMgr::evtSell(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (g_stRumSellForm.BidPrice * g_stRumSellForm.BidCount +
