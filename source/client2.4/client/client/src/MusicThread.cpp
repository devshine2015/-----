...
bool CMusicThread::Begin() {
  if (!m_hThread) {
    ...
    m_hThread =
        CreateThread(NULL, 0, ThreadStartRoutine, this, 0, &m_dwThreadID);
    // if( m_hThread )
...
bool CMusicThread::Resume() { return -1 != ResumeThread(m_hThread); }

bool CMusicThread::Suspend() { return -1 != SuspendThread(m_hThread); }
...
void CMusicThread::SetThreadPriority(EThreadPriority NewPriority) {
  if (NewPriority != m_ThreadPriority) {
    ...
    ::SetThreadPriority(m_hThread, m_ThreadPriority == TPri_AboveNormal
...
UINT CMusicThread::Run() {
  ...
  /*	bool LoopEnd = false;
          while( !GThreadLoopEnd && !LoopEnd )
          {
                  ...
                  if( m_dwMusicID )
                          LoopEnd = AudioSDL::get_instance()->is_stopped(
     m_dwMusicID ) && ( !m_bLoop );
          }*/
