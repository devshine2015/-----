...
CRumDealMgr::CRumDealMgr()
    : CUIInterface(), frmRumDeal(NULL), labSellIndex(NULL),
      labMySellCount(NULL), labMySellPrice(NULL), btnSellCancel(NULL),
      editSellCount(NULL), editSellPrice(NULL), labSellTotal(NULL),
      labSellTax(NULL), btnSellConmmit(NULL), labBuyIndex(NULL),
      labMyBuyCount(NULL), labMyBuyPrice(NULL), btnBuyCancel(NULL),
      editBuyCount(NULL), editBuyPrice(NULL), labBuyTotal(NULL),
      labBuyTax(NULL), btnBuyConmmit(NULL), labCrystal(NULL), labGold(NULL),
      btnRefresh(NULL), btnGetMoney(NULL), GoldCoins(0), CrystalCoins(0),
      SellBidCount(0), SellBidPrice(0), BuyBidCount(0), BuyBidPrice(0),
      DealPrice(0), TotalSellSize(0), TotalBuySize(0), CancelType(-1) {
...
void CRumDealMgr::AddSellInfo(uChar SellListSize, LONG64 *SellPrices,
                              LONG64 *SellCounts) {
  char szText[32] = {0};
...
void CRumDealMgr::AddBuyInfo(uChar BuyListSize, LONG64 *BuyPrices,
                             LONG64 *BuyCounts) {
  char szText[32] = {0};
...
void CRumDealMgr::AddMyDealInfo(uChar MyDealSize, LONG64 *MyDealPrices,
                                LONG64 *MyDealCounts, short *MyDealTypes,
                                LONG64 *MyDealTotalPrices) {
...
void CRumDealMgr::evtShowMsgBox(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  g_stUIBox.ShowSelectBox(evtConfirmEvent, RES_STRING(CL_LANGUAGE_MATCH_996),
                          true);
}
...
void CRumDealMgr::evtConfirmEvent(CCompent *pSender, int nMsgType, int x, int y,
                                  DWORD dwKey) {
...
int CRumDealMgr::GetIndex(bool BuyTrade) {
  short DealSize =
      BuyTrade ? g_stRumDealForm.TotalBuySize : g_stRumDealForm.TotalSellSize;
