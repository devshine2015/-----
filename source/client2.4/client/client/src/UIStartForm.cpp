...
bool CStartMgr::Init() {
  ...
  // 玩家自己角色的HP，SP，EXP,名字,头像
  {
    ...
    if (frmDetail) {
      ...
      if (!proMainHP)
        return Error(RES_STRING(CL_LANGUAGE_MATCH_473), frmDetail->GetName(),
                     "proMainHP1");
      ...
      if (!proMainExp) {
        Error(RES_STRING(CL_LANGUAGE_MATCH_473), frmDetail->GetName(),
              "proMainEXP");
      } else {
      ...
      C3DCompent *d3dSelfDown =
          dynamic_cast<C3DCompent *>(frmDetail->Find("d3dSelfDown"));
      if (!d3dSelfDown)
        return Error(RES_STRING(CL_LANGUAGE_MATCH_473), frmDetail->GetName(),
                     "d3dSelfDown");
    ...
    if (!mnuSelf)
      return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMain800->GetName(),
                   "selfMouseRight");
  ...
  // frmMain800表单
  {
    ...
    // if( !grdHeart )	return Error( "msgui.clu界面<%s>上找不到控件<%s>",
    // frmMain800->GetName(), "grdHeart" ); grdHeart->evtSelectChange =
    // _evtChaHeartChange;

    // grdAction = dynamic_cast<CGrid*>(frmMain800->Find("grdAction"));
    // if( !grdAction ) return Error( "msgui.clu界面<%s>上找不到控件<%s>",
    // frmMain800->GetName(), "grdAction" ); grdAction->evtSelectChange =
    // _evtChaActionChange;
  }

  // 角色属性
  // proMainHP1 =  dynamic_cast<CProgressBar *> ( frmMain800->Find("proMainHP1")
  // ); if( !proMainHP1 ) return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  // frmMain800->GetName(), "proMainHP1" ); proMainHP1->SetPosition(10.0f );
  //
  // proMainHP2 =  dynamic_cast<CProgressBar *> ( frmMain800->Find("proMainHP2")
  // ); if( !proMainHP2 ) return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  // frmMain800->GetName(), "proMainHP2" ); proMainHP2->SetPosition(10.0f );

  // proMainHP3 =  dynamic_cast<CProgressBar *> ( frmMain800->Find("proMainHP3")
  // ); if( !proMainHP3 ) return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  // frmMain800->GetName(), "proMainHP3" ); proMainHP3->SetPosition(10.0f );

  // proMainSP =  dynamic_cast<CProgressBar *> ( frmMain800->Find("proMainSP")
  // ); if( !proMainSP ) return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  // frmMain800->GetName(), "proMainSP" );
  ...
  // frmMainFun表单
  {
    ...
    // 升级帮助，会闪烁
    btnLevelUpHelp =
        dynamic_cast<CTextButton *>(frmMainFun->Find("btnLevelUpHelp"));
    // FORM_CONTROL_LOADING_CHECK(btnLevelUpHelp, frmMainFun, CTextButton,
    // "msgui.clu", "btnLevelUpHelp");
    if (btnLevelUpHelp)
      btnLevelUpHelp->SetFlashCycle();

    //	Add by alfred.shi 20080904	begin
    btnLevelUpadd =
        dynamic_cast<CTextButton *>(frmMainFun->Find("btnLevelUpadd"));
    if (btnLevelUpadd)
    ...
    if (frmElfAfk) {
      ...
      if (!labelfexp)
        Error(RES_STRING(CL_LANGUAGE_MATCH_473), frmElfAfk->GetName(),
              "labelfexp");
    }
    btnelfafkYes = dynamic_cast<CTextButton *>(frmElfAfk->Find("btnelfafkYes"));
    frmElfAfk->evtEntrustMouseEvent = _evtFormMouseClick;

    FORM_CONTROL_LOADING_CHECK(cboexp, frmElfAfk, CCombo, "player.clu",
                               "cboexp");
    ...
    if (!cboexp)
      return Error(RES_STRING(CL_LANGUAGE_MATCH_719), frmElfAfk->GetName(),
                   "cboexp");
    ...
    if (!frmElfchat)
      return Error(RES_STRING(CL_LANGUAGE_MATCH_45), "frmElfchat",
                   "frmElfchat");

    ElfTxt02 = dynamic_cast<CLabel *>(frmElfchat->Find("ElfTxt02"));
    if (!ElfTxt02)
      Error(RES_STRING(CL_LANGUAGE_MATCH_473), frmElfchat->GetName(),
            "ElfTxt02");
  ...
  if (!labFollow)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmFollow->GetName(),
                 "labFollow");
  ...
  if (!labPetLv)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMainPet->GetName(),
                 "labPetLv");
  ...
  if (!frmHelpSystem)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), "frmHelpSystem",
                 "frmHelpSystem");

  lstHelpList = dynamic_cast<CList *>(frmHelpSystem->Find("lstHelpList"));
  if (!lstHelpList)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmHelpSystem->GetName(),
                 "lstHelpList");
  lstHelpList->evtSelectChange = _evtHelpListChange;

  CTextButton *btn = (CTextButton *)frmHelpSystem->Find(
      "btnShip1"); //	Modify by alfred.shi 20080822 按钮名称+1
  if (!btn)
    return false;
  ...
  if (!frmSociliaty)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), "frmSociliaty",
                 "frmSociliaty");
    ...
    for (int i = 0; i < nTotalIndex; ++i) {
      ...
      if (p && strcmp(p->szMapName, strMapName /*mapName*/) == 0) {
        ...
        std::string strAreaName =
            std::string("<") + std::string(p->szArea) + std::string(">");
        while (strAreaName.length() < 16) {
          strAreaName += " ";
        }
        _snprintf_s(buff, _countof(buff), _TRUNCATE, "%s%s<%d,%d>",
                    strName.c_str(), strAreaName.c_str(), p->dwxPos0,
                    p->dwyPos0);
...
void CStartMgr::ShowQueenButtonForm(bool bShow) {
  ...
  /*if(frmQueen)
  {*/
  /*frmQueen->SetPos(frmMainFun->GetX2() - frmQueen->GetWidth(),
  frmMainFun->GetY() - frmQueen->GetHeight()); frmQueen->Refresh();*/
...
void CStartMgr::UpdateShipSailForm() {
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", (int)pAttr->get(ATTR_HP),
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d/%d", (int)pAttr->get(ATTR_SP),
...
void CStartMgr::ShowQueryReliveForm(int nType, const char *str) {
  stSelectBox *pOriginRelive =
      g_stUIBox.ShowSelectBox(_evtOriginReliveFormMouseEvent, str, false);
...
void CStartMgr::_evtOriginReliveFormMouseEvent(CCompent *pSender, int nMsgType,
...
void CStartMgr::_evtReliveFormMouseEvent(CCompent *pSender, int nMsgType, int x,
                                         int y, DWORD dwKey) {
  // if( name=="btnReCity" )
  {
...
void CStartMgr::_evtStartFormMouseEvent(CCompent *pSender, int nMsgType, int x,
                                        int y, DWORD dwKey) {
  ...
  } else if (name == "btnOpenSociliaty") // 打开社交按钮界面
  {
    ...
    // g_stUIStart.frmSociliaty->SetIsShow(!
    // g_stUIStart.frmSociliaty->GetIsShow());
  ...
  } else if (name == "btnOpenTeam") // 志愿者
  {
    ...
    if (pMainCha) {
      ...
      if (g_stUIFindTeam.IsShowFom())
      ...
      else if (pMainCha && !pMainCha->IsBoat() &&
        ...
        if (pMainCha->IsBoat()) {
          ...
          g_pGameApp->MsgBox(RES_STRING(
              CL_LANGUAGE_MATCH_866)); //	Add by alfred.shi 20080905
        }
      }
    }
  }
...
void CStartMgr::MainChaDied() {
  if (frmMainChaRelive) {
    int nLeft =
        (g_pGameApp->GetWindowHeight() - frmMainChaRelive->GetWidth()) / 2;
    int nTop =
        (g_pGameApp->GetWindowHeight() - frmMainChaRelive->GetHeight()) / 2;
    nTop -= 80;
    frmMainChaRelive->SetPos(nLeft, nTop);
    frmMainChaRelive->Refresh();

    static CLabel *pInfo =
        dynamic_cast<CLabel *>(frmMainChaRelive->Find("labReCity"));
        ...
        if (CGameScene *pScene = CGameApp::GetCurScene()) {
          if (CMapInfo *pInfo = pScene->GetCurMapInfo()) {
            if (pInfo) {
              ...
              /*if( stricmp( pInfo->szDataName, "teampk" )==0 || stricmp(
                 pInfo->szDataName,"starena1") == 0
                      || stricmp( pInfo->szDataName,"starena2") == 0 || stricmp(
                 pInfo->szDataName,"starena3") == 0)*/
              if (_stricmp(pInfo->szDataName, "teampk") == 0 ||
                  _stricmp(pInfo->szDataName, "starena1") == 0 ||
                  _stricmp(pInfo->szDataName, "starena2") == 0 ||
                  _stricmp(pInfo->szDataName, "starena3") == 0 ||
                  _stricmp(pInfo->szDataName, "starena13") == 0 ||
                  _stricmp(pInfo->szDataName, "starena14") == 0 ||
                  _stricmp(pInfo->szDataName, "starena15") == 0 ||
                  _stricmp(pInfo->szDataName, "starena23") == 0 ||
                  _stricmp(pInfo->szDataName, "starena24") == 0 ||
                  _stricmp(pInfo->szDataName, "starena25") == 0 ||
                  _stricmp(pInfo->szDataName, "starena33") == 0 ||
                  _stricmp(pInfo->szDataName, "starena34") == 0 ||
...
void CStartMgr::_evtTaskMouseEvent(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
  string name = pSender->GetName();
  ...
  //	g_stUIStart.grdAction->SetIsShow( false );
  //	g_stUIStart.grdHeart->SetIsShow( false );
  //	g_stUICoze.GetFaceGrid()->SetIsShow(
  //!g_stUICoze.GetFaceGrid()->GetIsShow() ); 	return;
  //}
...
void CStartMgr::RefreshMainExperience(LONG64 num, LONG64 curlev,
                                      LONG64 nextlev) {
  //	LG("exp", RES_STRING(CL_LANGUAGE_MATCH_763), num, curlev, nextlev,
  ...
  // if (max!=0)
  //	_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE  , "%4.2f%%" ,
  //num*100.0f/max ); else 	_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE  ,
  //"0.00%"); szBuf[sizeof(szBuf)-1] = '\0';
...
void CStartMgr::_evtPopMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  if (str == RES_STRING(CL_LANGUAGE_MATCH_764)) // 邀请交易
  {
    ...
    CCharacter *pMain = CGameScene::GetMainCha();
    if (pCha && pMain && pCha != pMain && pCha->IsEnabled() &&
        pMain->IsEnabled() &&
      ...
      g_pGameApp->SysInfo(RES_STRING(
          CL_LANGUAGE_MATCH_765)); // 交易时必须双方都是玩家，或都是船
    }
  ...
  else if (str == RES_STRING(CL_UISTARTFORM_CPP_00001)) // 详细信息
  {
    ...
    if (pCha && pMain && !pCha->IsBoat() && !pMain->IsBoat()) {
      CMember *pMember =
          g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(pCha->getID());
      if (!pMember)
        pMember =
            g_stUIChat.GetTeamMgr()->Find(enumTeamGroup)->Find(pCha->getID());
      if (!pMember)
        pMember =
            g_stUIChat.GetTeamMgr()->Find(enumTeamGuild)->Find(pCha->getID());
      if (!pMember)
        pMember =
            g_stUIChat.GetTeamMgr()->Find(enumTeamMaster)->Find(pCha->getID());
      if (!pMember)
        pMember = g_stUIChat.GetTeamMgr()
                      ->Find(enumTeamPrentice)
                      ->Find(pCha->getID());
...
void CStartMgr::_evtAskTeamFightMouseEvent(CCompent *pSender, int nMsgType,
                                           int x, int y, DWORD dwKey) {
...
void CStartMgr::PopMenu(CCharacter *pCha) {
  ...
  if (g_stUIBank.GetBankGoodsGrid()
          ->GetForm()
          ->GetIsShow()) // 西门文档修改，开着银行不允许打开右键菜单
    return;

  if (g_stUIBooth.GetBoothItemsGrid()
          ->GetForm()
          ->GetIsShow()) // 西门文档修改，摆摊不允许打开右键菜单
    return;

  if (g_stUITrade.IsTrading()) // 西门文档修改，交易时不允许打开右键菜单
    return;

  if (mainMouseRight && pCha && pCha->IsValid() && !pCha->IsHide() &&
      pCha->IsPlayer()) {
    ...
    if (nMainGuildID > 0 && nChaGuildID > 0) {
      if (g_stUIMap.IsGuildWar() &&
          ((nMainGuildID <= 100 && nChaGuildID > 100) ||
           (nMainGuildID > 100 && nChaGuildID <= 100)))
        return; // 不同阵营
    }
    ...
    const char *MapName =
        g_pGameApp->GetCurScene()
            ->GetTerrainName(); // 取得人物当前所在地图 Add by ning.yan 20080715
    // Add by sunny.sun20080820
    // Begin
    //	if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2") ==
    //0 || stricmp( MapName,"starena3") == 0 )
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      //	if( stricmp( pItem->GetString(),
      //RES_STRING(CL_LANGUAGE_MATCH_764) )==0 )
      if (_stricmp(pItem->GetString(), RES_STRING(CL_LANGUAGE_MATCH_764)) ==
          0) {
      ...
      //	else if( stricmp( pItem->GetString(),
      //RES_STRING(CL_LANGUAGE_MATCH_482) )==0 )
      else if (_stricmp(pItem->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_482)) == 0) {
        pItem->SetIsEnabled(pMain != pCha);
      }
      //	else if( stricmp( pItem->GetString(),
      //RES_STRING(CL_LANGUAGE_MATCH_484) )==0 )
      else if (_stricmp(pItem->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_484)) ==
               0) { // 竞技场中取消组队功能 Add by ning.yan  20080715 Begin
        // if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2")
        // == 0 || stricmp( MapName,"starena3") == 0 ) 	pItem->SetIsEnabled( false
        //);
        //// End
        // else
        pItem->SetIsEnabled(
            g_stUIStart.IsCanTeam() && pMain != pCha &&
            (pMain->GetTeamLeaderID() == 0 ||
             (pMain->IsTeamLeader() &&
              pCha->GetTeamLeaderID() != pMain->GetTeamLeaderID())));
      }
      //	else if( stricmp( pItem->GetString(),
      //RES_STRING(CL_LANGUAGE_MATCH_483) )==0 )
      else if (_stricmp(pItem->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_483)) ==
               0) { // 竞技场中取消离开队伍功能  Add by ning.yan  20080715 Begin
        // if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2")
        // == 0 || stricmp( MapName,"starena3") == 0 ) 	pItem->SetIsEnabled( false
        //);
        //// End
        // else
        pItem->SetIsEnabled(
            g_stUIStart.IsCanTeam() && pMain->GetTeamLeaderID() != 0 &&
            pCha->GetTeamLeaderID() == pMain->GetTeamLeaderID());
      }
      //	else if( stricmp( pItem->GetString(),
      //RES_STRING(CL_LANGUAGE_MATCH_481) )==0 )
      else if (_stricmp(pItem->GetString(),
                        RES_STRING(CL_LANGUAGE_MATCH_481)) == 0) {
        pItem->SetIsEnabled(pMain != pCha);
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_766)) == 0) {
        pItem->SetIsEnabled(pMain != pCha && pMain->IsBoat() &&
                            pMain->IsEnabled() && pCha->IsBoat() &&
                            pCha->IsEnabled());
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_768)) == 0) {
        pItem->SetIsEnabled(pMain != pCha && pMain->IsEnabled() &&
                            !pMain->IsShop() && pCha->IsEnabled() &&
                            pCha->IsShop());
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_769)) == 0) {
        pItem->SetIsEnabled(g_stUIStart.IsCanTeam() && pMain != pCha &&
                            pMain->IsEnabled() && pMain->IsTeamLeader() &&
                            !pMain->IsShop() && pCha->IsEnabled() &&
                            pCha->IsTeamLeader() && !pCha->IsShop());
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_770)) == 0) {
        pItem->SetIsEnabled(g_stUIStart.IsCanTeam() && pMain != pCha &&
                            pCha->IsPlayer());
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_855)) == 0) // 请求拜师
      {
        pItem->SetIsEnabled(pMain != pCha && pCha->IsPlayer() &&
                            pMain->getGameAttr() &&
                            pMain->getGameAttr()->get(ATTR_LV) <= 40);
        //&& pCha->getGameAttr()  && pCha->getGameAttr()->get(ATTR_LV) > 40 );
      } else if (_stricmp(pItem->GetString(),
                          RES_STRING(CL_LANGUAGE_MATCH_859)) == 0) // 请求收徒
      {
        pItem->SetIsEnabled(pMain != pCha && pCha->IsPlayer() &&
                            pMain->getGameAttr() &&
                            pMain->getGameAttr()->get(ATTR_LV) > 40);
    ...
    int x = 0, y = 0;
    g_Render.WorldToScreen(pCha->GetPos().x, pCha->GetPos().y, pCha->GetPos().z,
                           &x, &y);
...
CTextButton *CStartMgr::GetShowQQButton() { return btnQQ; }

void CStartMgr::ShowBigText(const char *str) {
  int nType = 0;
  CCharacter *pMain = CGameScene::GetMainCha();
  if (pMain && CGameApp::GetCurScene() &&
      CGameApp::GetCurScene()->GetTerrain()) {
    int nArea = CGameApp::GetCurScene()
                    ->GetTerrain()
                    ->GetTile(pMain->GetCurX() / 100, pMain->GetCurY() / 100)
                    ->getIsland();
...
void CStartMgr::FrameMove(DWORD dwTime) {
  ...
  if (pet_time.IsTimeOut(dwTime)) {
    CItemCommand *pItem =
        dynamic_cast<CItemCommand *>(g_stUIEquip.GetGoodsGrid()->GetItem(1));
    if (pItem && pItem->GetItemInfo()->sType == enumItemTypePet &&
        pItem->GetData().IsValid() && CGameScene::GetMainCha()) {
...
void CStartMgr::_evtShowBoatAttr(CGuiData *pSender, int x, int y,
                                 DWORD key) // 显示船只属性
{
  xShipFactory *pkShip =
...
//~ 回调函数 =================================================================
void CStartMgr::_NewFrmMainMouseEvent(CCompent *pSender, int nMsgType, int x,
                                      int y, DWORD dwKey) {
  string name = pSender->GetName();
...
void CStartMgr::_HelpFrmMainMouseEvent(CCompent *pSender, int nMsgType, int x,
                                       int y, DWORD dwKey) {
  string name = pSender->GetName();
  if (name == "btnOpenHelp") {
    CForm *frm = CFormMgr::s_Mgr.Find("frmVHelp");
...
void CStartMgr::_CloseEvent(CCompent *pSender, int nMsgType, int x, int y,
                            DWORD dwKey) {
  CForm *frm = CFormMgr::s_Mgr.Find("frmVHelp");
  if (frm) {
    frm->Close();
...
void CStartMgr::_OnSelfMenu(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  // Begin
  const char *MapName =
      g_pGameApp->GetCurScene()->GetTerrainName(); // 取得人物当前所在地图
  //	if( stricmp( MapName,"starena1") == 0 || stricmp( MapName,"starena2") ==
  //0 || stricmp( MapName,"starena3") == 0 )
  if (_stricmp(MapName, "starena1") == 0 ||
      _stricmp(MapName, "starena2") == 0 ||
...
void CStartMgr::SetIsLeader(bool v) { imgLeader->SetIsShow(v); }

bool CStartMgr::GetIsLeader() { return imgLeader->GetIsShow(); }
...
void CStartMgr::ShowAfkButton(bool bShow) { btnafk->SetIsShow(bShow); }
void CStartMgr::ShowInfoCenterButton(bool bShow) {
  CTextButton *btnInfoCenter =
      dynamic_cast<CTextButton *>(frmMainFun->Find("btnInfoCenter"));
  if (btnInfoCenter) {
...
void CStartMgr::_evtHelpListChange(CGuiData *pSender) {
  ...
  // pSender->GetName());//g_stUIStart.lstHelpList->GetSelectItem()->
...
void CStartMgr::ShowBagButtonForm(bool bShow) {
  if (frmBag) {
    frmBag->SetPos(frmMainFun->GetX2() - frmBag->GetWidth() - 109,
                   frmMainFun->GetY() - frmBag->GetHeight() + 41);
    frmBag->Refresh();
...
void CStartMgr::_evtDragExpItem(CGuiData *pSender, CCommandObj *pItem,
                                bool &isAccept) {
  ...
  if (pItem->GetIsPile() && pItem->GetTotalNum() > 0) {
    g_stUIStart.m_NumBox = g_stUIBox.ShowNumberBox(
        _evtOOXXEvent, pItemCommand->GetTotalNum(), "设定数量", true);
  }
...
void CStartMgr::PopItem(int iIndex, bool bRetry) {
  ...
  CItemCommand *pItemCommand =
      dynamic_cast<CItemCommand *>(cmdexpItem[iIndex]->GetCommand());
  if (!pItemCommand)
    return;
...
void CStartMgr::PushItem(int iIndex, CItemCommand &rItem,
                         bool bRetry /* = false*/) {
  ...
  CItemCommand *pItemCommand =
      dynamic_cast<CItemCommand *>(cmdexpItem[0]->GetCommand());
  if (pItemCommand) {
    PopItem(iIndex);
  }
...
void CStartMgr::_evtFormMouseClick(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
  string name = pSender->GetName();
...
void CStartMgr::RefreshLeftExpName(short sSelectTime, string BaseExp,
                                   string AddExp, string TotalExp,
                                   string UpLevel, string addPercent,
                                   short sNum, string amplitude, short coin) {
  char sTemp[30] = "";
  if (ElfTxt02) {
  ...
  if (ElfTxt08) {
    _snprintf_s(sTemp, sizeof(sTemp), _TRUNCATE, "%s级%s%%经验",
                UpLevel.c_str(), addPercent.c_str());
  ...
  if (ElfTxt12) {
    _snprintf_s(sTemp, sizeof(sTemp), _TRUNCATE, "%d 个 %s倍增幅器", sNum,
                amplitude.c_str());
...
void CStartMgr::_evtOOXXEvent(CCompent *pSender, int nMsgType, int x, int y,
                              DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
  ...
  CItemCommand *pItemCommand =
      (CItemCommand *)g_stUIStart.frmElfAfk->GetPointer();
  if (num == 0) {
