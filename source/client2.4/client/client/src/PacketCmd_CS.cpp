...
// 协议C->S : 断开连接
//------------------------
void CS_Disconnect(int reason) { g_NetIF->m_pCProCir->Disconnect(reason); }

//------------------------
// 协议C->S : 登陆
//------------------------
void CS_Login(const char *accounts, const char *password,
              const char *passport) {
...
void CS_Logout() {
  ...
  // 因为加入了退出计时,且在玩家重新登录时又会重新生成密钥,因此退出进不用销毁密钥,
  // lh by 2006-02
  // 销毁密钥
...
// 协议C->S : 发送选择角色命令
//------------------------
void CS_BeginPlay(const char *cha) { g_NetIF->m_pCProCir->BeginPlay(cha); }

//------------------------
// 协议C->S : 发送重新选择角色命令
//------------------------
void CS_EndPlay() { g_NetIF->m_pCProCir->EndPlay(); }
...
// 协议C->S : 发送聊天消息
//------------------------
void CS_Say(const char *content) { g_NetIF->m_pCProCir->Say(content); }
...
// 协议C->S : 发送行动信息
void CS_BeginAction(CCharacter *pCha, DWORD type, void *param,
                    CActionState *pState) {
...
void CS_Buy(DWORD dwNpcID, BYTE byItemType, BYTE byIndex1, BYTE byIndex2,
            BYTE byCount) {
  WPacket packet = g_NetIF->GetWPacket();
  packet.WriteCmd(CMD_CM_REQUESTTRADE);
  packet.WriteLong(dwNpcID);
...
void CS_BuyGoods(DWORD dwNpcID, DWORD dwBoatID, BYTE byItemType, BYTE byIndex1,
                 BYTE byIndex2, BYTE byCount) {
  WPacket packet = g_NetIF->GetWPacket();
  packet.WriteCmd(CMD_CM_REQUESTTRADE);
  packet.WriteLong(dwNpcID);
  packet.WriteShort(CMD_CM_TRADEITEM);
...
void CS_BlackMarketExchangeReq(DWORD dwNpcID, short sSrcID, short sSrcNum,
                               short sTarID, short sTarNum, short sTimeNum,
                               BYTE byIndex) {
  WPacket packet = g_NetIF->GetWPacket();
  packet.WriteCmd(CMD_CM_REQUESTTRADE);
  packet.WriteLong(dwNpcID);
...
void CS_AddItem(BYTE byType, DWORD dwCharID, BYTE byOpType, BYTE byIndex,
                BYTE byItemIndex, BYTE byCount) {
  WPacket packet = g_NetIF->GetWPacket();
...
void CS_CreateBoat(const char szBoat[], char szHeader, char szEngine,
                   char szCannon, char szEquipment) {
  WPacket packet = g_NetIF->GetWPacket();
...
void CS_UpdateBoat(char szHeader, char szEngine, char szCannon,
                   char szEquipment) {
  WPacket packet = g_NetIF->GetWPacket();
...
// chPosID：对应位置的编号
void CS_ItemRepairAsk(long lRepairmanID, long lRepairmanHandle, char chPosType,
                      char chPosID) {
  WPacket packet = g_NetIF->GetWPacket();
...
void CS_Found(DWORD dwNPCID, int *iPos, int iCount, short big,
              bool asr /* = false */) {
  WPacket packet = g_NetIF->GetWPacket();
  if (asr) {
    packet.WriteCmd(CMD_CM_LIFESKILL_ASR);
...
void CS_Cooking(DWORD dwNPCID, int *iPos, int iCount, short percent,
                bool asr /* = false */) {
  WPacket packet = g_NetIF->GetWPacket();
  if (asr) {
    packet.WriteCmd(CMD_CM_LIFESKILL_ASR);
...
void CS_DropLock(dbc::Char lockPosType, //	锁道具的位置。
...
// 道具解锁 add by ning.yan 2008-11-11 begin
void CS_UnlockItem(const char szPassword[], // md5加密后的二次密码
