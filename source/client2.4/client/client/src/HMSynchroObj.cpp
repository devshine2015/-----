...
CStateSynchro::CStateSynchro()
    : _isExec(true), _pServerHarm(NULL),
      _dwExecTime(CGameApp::GetCurTick() + DELAY_TIME),
      _dwCreateTime(_dwExecTime) {
...
CSynchroManage::CSynchroManage() : _dwHead(0), _dwTail(0), _nSynchroNum(0) {
  memset(_All, 0, sizeof(_All));
}

CSynchroManage::~CSynchroManage() { Reset(); }
  ...
  for (i = _dwHead; i < _dwTail; i++) {
    ...
    if (p) {
      ...
      if (p->_isExec || dwTime >= p->_dwExecTime) {
        ...
        //				LG( "CSynchroManage", "Del (%s ID:%d,
        //Exec:%d), Num[%d], Head[%d], Tail[%d], Time[%d]\n", p->GetClassName(),
        //p->_nID, p->_isExec, _nSynchroNum, _dwHead, _dwTail, DELAY_TIME -
        //(p->_dwCreateTime - dwTime) );
        p->_Exec();
        delete p;
...
int CSynchroManage::_AddState(CStateSynchro *pState) {
  if (_nSynchroNum >= MAX_SIZE) {
    LG("CSynchroManage",
       "msgAddState() false, SynchroNum[%d], Head[%d], Tail[%d]\n",
  ...
  if (_All[_dwTail]) {
    LG("CSynchroManage",
       "msgAddState() false, Has Value SynchroNum[%d], Head[%d], Tail[%d], "
       "ID[%d]\n",
...
bool CSynchroManage::_DelState(CStateSynchro *pState) {
  ...
  if (nID == ERROR_ID) {
    LG("CSynchroManage",
       "DelState() false, SynchroNum[%d], Head[%d], Tail[%d]\n", _nSynchroNum,
       _dwHead, _dwTail);
    return false;
  }
  ...
  if (_All[nID] == pState) {
    ...
    LG("CSynchroManage",
       "DelState(), SynchroNum[%d], Head[%d], Tail[%d], ID[%d]\n", _nSynchroNum,
       _dwHead, _dwTail, nID);
    return true;
  }
  LG("CSynchroManage", RES_STRING(CL_LANGUAGE_MATCH_148), _nSynchroNum, _dwHead,
     _dwTail, nID);
  return false;
}
