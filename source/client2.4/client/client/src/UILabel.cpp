...
// class CLabelEx
//---------------------------------------------------------------------------
CLabel::CLabel(CForm &frmOwn) : CCompent(frmOwn), _color(COLOR_BLACK) {}
...
CLabelEx::CLabelEx(CForm &frmOwn)
    : CLabel(frmOwn), _IsShadow(true), _clShadow(COLOR_BLACK), _nFontIndex(0),
      _IsCenter(false), _IsFlash(false) {}

CLabelEx::CLabelEx(const CLabelEx &rhs) : CLabel(rhs) { _Copy(rhs); }
...
void CLabelEx::Render() {
  ...
  if (_IsShadow) {
    if (!_IsCenter) {
      // if(_caption == "白银城")
      //	CGuiFont::s_Font.BRender( _nFontIndex, "", GetX(), GetY(),
      //_color, _clShadow ); else
      CGuiFont::s_Font.BRender(_nFontIndex, (const char *)_caption.c_str(),
                               GetX(), GetY(), _color, _clShadow);
    } else {
      int posX = (GetWidth() - CGuiFont::s_Font.GetWidth(_caption.c_str())) / 2;
      CGuiFont::s_Font.BRender(_nFontIndex, (const char *)_caption.c_str(),
                               GetX() + posX, GetY(), _color, _clShadow);
    }
  } else {
    if (!_IsCenter) {
      CGuiFont::s_Font.Render(_nFontIndex, (char *)_caption.c_str(), GetX(),
                              GetY(), _color);
    } else {
      int posX = (GetWidth() - CGuiFont::s_Font.GetWidth(_caption.c_str())) / 2;
      CGuiFont::s_Font.Render(_nFontIndex, (char *)_caption.c_str(),
                              GetX() + posX, GetY(), _color);
    }
  }
