...
CAutoAttack::CAutoAttack(CMouseDown *pMouseDown) : _pMouseDown(pMouseDown) {
  Reset();
}
...
bool CAutoAttack::AttackStart(CCharacter *pMain, CSkillRecord *pSkill,
                              CCharacter *pCha) {
  ...
  g_stUIStart.SysLabel(RES_STRING(CL_LANGUAGE_MATCH_4),
                       g_stUIMap.IsPKSilver() ? "??????" : pCha->getName());
...
bool CAutoAttack::AttackStart(CCharacter *pMain, CSkillRecord *pSkill,
  ...
  g_stUIStart.SysLabel(RES_STRING(CL_LANGUAGE_MATCH_5), nScrX / 100,
                       nScrY / 100);
...
void CAutoAttack::FrameMove() {
  ...
  case eAttack: {
    ...
    if (_IsMove) {
      if (GetDistance(_nMoveX, _nMoveY, _pMain->GetCurX(), _pMain->GetCurY()) <
          50) {
        _IsMove = false;
      }
    ...
    if (_pTarget) {
      if (!_pTarget->IsValid() || !_pTarget->IsEnabled() ||
          _pTarget->IsHide()) {
        _eStyle = eNone;
      } else {
        _pMouseDown->ActAttackCha(_pMain, _pSkill, _pTarget, false, false,
                                  false);
      }
    } else {
      if (GetDistance(_pMain->GetCurX(), _pMain->GetCurY(), _nAttackX,
                      _nAttackY) > _nTotalDis) {
  ...
  case eFollow: {
    if (!_pMain->IsValid() || !_pMain->IsEnabled() || !_pTarget->IsValid() ||
        !_pTarget->IsPlayer()) {
      _eStyle = eNone;
    } else {
      static DWORD time = 0;
      if (CGameApp::GetCurTick() > time ||
          (_pMain->GetIsArrive() && _pMain->GetActor()->IsEmpty())) {
        time = CGameApp::GetCurTick() + 1000;

        const int dis = 200;
        if (GetDistance(_pMain->GetServerX(), _pMain->GetServerY(),
                        _pTarget->GetServerX(), _pTarget->GetServerY()) > dis) {
          int x, y;
          GetDistancePos(_pTarget->GetServerX(), _pTarget->GetServerY(),
                         _pMain->GetServerX(), _pMain->GetServerY(), dis - 100,
                         x, y);
...
bool CAutoAttack::AttackMoveTo(int nScrX, int nScrY) {
  ...
  // if( !_pTarget && GetDistance( _nAttackX, _nAttackY, nScrX, nScrY ) >
  // _nTotalDis )
  //{
  //	_eStyle = eNone;
  //	return false;
...
bool CAutoAttack::Follow(CCharacter *pMain, CCharacter *pTarget) {
  ...
  g_stUIStart.SysLabel(RES_STRING(CL_LANGUAGE_MATCH_6),
                       g_stUIMap.IsPKSilver() ? "??????" : pTarget->getName());
