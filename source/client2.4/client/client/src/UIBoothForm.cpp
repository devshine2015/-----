...
#include "Actor.h"
	...
	bool CReadBookMgr::IsCanReadBook(CCharacter* pCha)
	{
		...
		if(!pRHand || !pNeck)
		{
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_941));	// "请先装备学生证和书"
			return false;
		}
	...
	{
		CBoxMgr::ShowSelectBox(_evtSelectBox, RES_STRING(CL_LANGUAGE_MATCH_942), true);	// "确定是否要读书？"
		return true;
	}
	...
	void CReadBookMgr::_evtSelectBox(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
	{
		...
		stMsgBox* pMsgBox = CBoxMgr::ShowMsgBox(_evtMsgBox, RES_STRING(CL_LANGUAGE_MATCH_943), false);	// "按 \"确定\" 停止读书。"
