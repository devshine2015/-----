        ...
        if (infoex[i].GetType() == 0) // 场景物件
        {
          ...
          if (pObj) {
            ...
            pObj->setYaw(infoex[i].sYawAngle);
                         ...
                         .m_bEnableLGMsg) // 如果物件添加错误，则咨询将其删除
          {
            /*_stprintf(tcsPrint,
...
void CGameApp::HandleSuperKey() {
  ...
  try {
    ...
    if (IsKeyDown(DIK_C)) {
#ifdef __EDITOR__
      if (IsCtrlPress()) // Ctrl+C 切换 Pick 模式，
                         // 激活状态时，能显示当前鼠标指到的物体
      {
...
#ifdef _SKYBOX_
  // 通过bRot确定是否处于旋转(AD)状态，若处于旋转状态，则不允许前进与后退动作(WS)
