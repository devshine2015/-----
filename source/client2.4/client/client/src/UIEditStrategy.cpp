...
// class CEditRow
//---------------------------------------------------------------------------
CEditRow::CEditRow() : _dwWordCount(0), _dwHeight(0), _dwWidth(0) {}
...
// class CEditStrategy
//---------------------------------------------------------------------------
CEditStrategy::CEditStrategy(CEditArticle *pActicle) : _pActicle(pActicle) {}

CEditStrategy::~CEditStrategy() { Clear(); }
...
void CEditStrategy::ParseText(CEditTextObj *pText) {
  ...
  if (pRow->GetObjNum() > 0) {
    pSentence =
        dynamic_cast<CEditSentence *>(pRow->GetObj(pRow->GetObjNum() - 1));
