...
CMemberData::~CMemberData() { SAFE_DELETE(_pFace); }
...
// CMember::CMember( CTeam* pTeam, unsigned long id, const char* strName , const
// char* strMotto, DWORD icon_id) : _nID(id), _pData(NULL), _pTeam(pTeam),
//_strName(strName), _strMotto(strMotto), _pPointer(0), _nIcon_id(icon_id),
//_strShowName(""), _bFlash(false)
CMember::CMember(CTeam *pTeam, unsigned long id, const char *strName,
                 const char *strMotto, DWORD icon_id, const char *szGroupName)
    : _nID(id), _pData(NULL), _pTeam(pTeam), _strName(strName),
      _strMotto(strMotto), _pPointer(0), _nIcon_id(icon_id), _strShowName(""),
      _bFlash(false), _strGroupName(szGroupName), _bIsOnline(false)
// End
{
...
void CMember::SetShowStyle(eShowStyle style) { _nShowStyle = style; }

void CMember::ModifyAttr(string motto, string job, DWORD dwLv, DWORD nIcon_ID,
                         string guildName) {
  _strMotto = motto;
  _strJob = job;
  _dwLv = dwLv;
...
// Modify by lark.li 20080802 begin
// CMember* CTeam::Add( unsigned long nID, const char* szName,const char*
// szMotto, DWORD icon_id)
CMember *CTeam::Add(unsigned long nID, const char *szName, const char *szMotto,
                    DWORD icon_id, const char *szGroupName, bool onLine)
// End
{
  if (nID > 0) {
    _nCount++;
    ...
    tmp =
        new CMember(this, nID, szName, strMotto.c_str(), icon_id, szGroupName);
...
void CTeam::AddGroupName(const char *szGroupName) {
  ...
  for (vector<pair<string, int>>::iterator it = _groups.begin();
       it != _groups.end(); it++) {
...
void CTeam::ChangeGroupName(const char *szOldGroupName,
...
void CTeam::MoveGroup(unsigned long nID, const char *szOldGroupName,
                      const char *szNewGroupName) {
  ...
  for (members::iterator it = _member.begin(); it != _member.end(); it++) {
    if ((*it)->GetID() == nID) {
      pMember =
          new CMember(this, (*it)->GetID(), (*it)->GetName(), (*it)->GetMotto(),
                      (*it)->GetIconID(), (*it)->GetGroupName());
  ...
  if (pMember) {
    this->Add(pMember->GetID(), pMember->GetName(), pMember->GetMotto(),
              pMember->GetIconID(), szNewGroupName, isOnline);
  }
}
...
CTeamMgr::CTeamMgr() {
  ...
  _pPrenticeTeam =
      new CTeam(enumTeamPrentice, RES_STRING(CL_LANGUAGE_MATCH_851));
}
...
void CTeamMgr::ChangeStyle(eShowStyle style) {
  ...
  for (DWORD i = 0; i < _pFrndTeam->GetCount(); i++) {
    ...
    g_stUIChat.TeamSend(enumSTM_NODE_CHANGE, _pFrndTeam->GetMember(i),
                        enumTeamFrnd);
  }
  for (DWORD i = 0; i < _pGroupTeam->GetCount(); i++) {
    g_stUIChat.TeamSend(enumSTM_NODE_CHANGE, _pGroupTeam->GetMember(i),
                        enumTeamGroup);
  }
  for (DWORD i = 0; i < _pRoadTeam->GetCount(); i++) {
    g_stUIChat.TeamSend(enumSTM_NODE_CHANGE, _pRoadTeam->GetMember(i),
                        enumTeamRoad);
  }
  for (DWORD i = 0; i < _pMasterTeam->GetCount(); i++) {
    g_stUIChat.TeamSend(enumSTM_NODE_CHANGE, _pMasterTeam->GetMember(i),
                        enumTeamMaster);
  }
  for (DWORD i = 0; i < _pPrenticeTeam->GetCount(); i++) {
    g_stUIChat.TeamSend(enumSTM_NODE_CHANGE, _pPrenticeTeam->GetMember(i),
                        enumTeamPrentice);
  }
}
