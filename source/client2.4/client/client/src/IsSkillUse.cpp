...
// class CIsSkillUse
//---------------------------------------------------------------------------
CIsSkillUse::CIsSkillUse() { _eError = enumNone; }
...
bool CIsSkillUse::IsValid(CSkillRecord *pSkill, CCharacter *pSelf) {
  ...
  if (pSkill->GetSPExpend() >
      g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_SP)) {
    _eError = enumNotMP;
    return false;
  }
...
bool CIsSkillUse::IsUse(CSkillRecord *pSkill, CCharacter *pSelf,
                        CCharacter *pTarget) {
...
bool CIsSkillUse::IsAttack(CSkillRecord *pSkill, CCharacter *pSelf,
                           CCharacter *pTarget) {
  _pSkill = pSkill;

  if (enumSKILL_TYPE_PLAYER_DIE == pSkill->chApplyTarget) {
    if (pTarget->getChaCtrlType() == enumCHACTRL_PLAYER &&
        !pTarget->IsEnabled())
      return true;
  ...
  case enumSKILL_TYPE_TEAM:
    if (pSelf == pTarget ||
        (pSelf->GetTeamLeaderID() > 0 &&
         pSelf->GetTeamLeaderID() == pTarget->GetTeamLeaderID())) {
      return true;
    }
...
const char *CIsSkillUse::GetError() {
  ...
  case enumInValid:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_149), _pSkill->szName);
    break;
  case enumNotEnergy:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_150), _pSkill->szName);
    break;
  case enumNotAttack:
  ...
  case enumNotMP:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_153), _pSkill->szName);
    break;

  case enumSelf:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_154), _pSkill->szName);
    break;
  case enumFish:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_155), _pSkill->szName);
    break;
  case enumDieBoat:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_156), _pSkill->szName);
    break;
  case enumTree:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_157), _pSkill->szName);
    break;
  case enumMine:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_158), _pSkill->szName);
    break;
  case enumOnlyTeam:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_159), _pSkill->szName);
    break;
  case enumDie:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_160), _pSkill->szName);
    break;
  case enumTargetError:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_161), _pSkill->szName);
    break;

  case enumHelpMons:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_162), _pSkill->szName);
    break;
  case enumAttackMain:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_163), _pSkill->szName);
    break;
  case enumAttackTeam:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_164), _pSkill->szName);
    break;
  case enumAttackDie:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_165), _pSkill->szName);
    break;
  case enumAttackPlayer:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_166), _pSkill->szName);
    break;
  case enumRepair:
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_167), _pSkill->szName);
    break;
  }
  return buf;
}
