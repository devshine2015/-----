...
BOOL NetIF::HandlePacketMessage(DataSocket *datasock,LPRPACKET pk)
{
	...
	case CMD_MC_BEGIN_ITEM_LOTTERY: return SC_Lottery(pk);
		// End
	...
	case CMD_PC_FRND_MOVE_GROUP:return PC_FRND_MOVE_GROUP(pk);
		// End
	...
	case CMD_PC_QUERY_PERSONINFO:return PC_QUERY_PERSONINFO(pk);
		// End
	...
	case CMD_MC_ITEM_LOTTERY_ASK: return SC_ItemLotteryAsk(pk);
	case CMD_MC_ITEM_LOTTERY_ASR: return SC_ItemLotteryAnswer(pk);
	// End
	case CMD_MC_ITEM_AMPHITHEATER_ASR: return SC_ItemAmphitheaterAnswer(pk);//Add by sunny.sun 20080726
	...
	case CMD_MC_CAPTAIN_ASK: return SC_CaptainAsk(pk);
	// End

#ifdef	_XTRAP_
	// Add by Rock.wang 20090729 begin
	case XTRAP_CMD_STEP_ONE: return SC_XtrapStepOne(pk);
	// End
#endif
	}
