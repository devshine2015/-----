...
bool CNpcTalkMgr::Init() {
  ...
  if (!memCtrl)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmNPCchat->GetName(),
                 "memCtrl");
...
DWORD CNpcTalkMgr::GetNpcId() { return _npcID; }

void CNpcTalkMgr::ShowFuncPage(BYTE byFuncPage, BYTE byCount, BYTE byMisNum,
  ...
  if (byMisNum > 0) {
    ...
    for (int i = 0; i < byMisNum; i++) {
      if (FuncArray.MisItem[i].byState == ROLE_MIS_ACCEPT) {
        CGraph *p =
            new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 170, 119, 1);
        memCtrl->AddIcon(i, p);

      } else if (FuncArray.MisItem[i].byState == ROLE_MIS_DELIVERY) {
        CGraph *p =
            new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 177, 119, 1);
        memCtrl->AddIcon(i, p);
      } else if (FuncArray.MisItem[i].byState == ROLE_MIS_PENDING) {
        CGraph *p =
            new CGraph("texture/ui/SystemBotton3.tga", 7, 15, 184, 119, 1);
        memCtrl->AddIcon(i, p);
      }
...
void CNpcTalkMgr::_MainMouseNPCEvent(CCompent *pSender, int nMsgType, int x,
                                     int y, DWORD dwKey) {
...
void CNpcTalkMgr::_evtMemSelectChange(CGuiData *pSender) {
  ...
  case 0: {
    ...
    if (itemex.empty()) {
      ...
      ::ShellExecute(NULL, "open", itemex.c_str(), NULL, NULL, SW_SHOW);
    }
  } break;
  case 1:
  ...
  if (IsFirstWorldScene) {
    ...
    if (fp) {
      if (fseek(fp, 0, SEEK_END) == 0) {
        ...
        // fread(Info.szDesp, size > ROLE_MAXNUM_DESPSIZE - 1 ?
        // ROLE_MAXNUM_DESPSIZE - 1 : size, 1, fp);
        fread(Info.szDesp,
