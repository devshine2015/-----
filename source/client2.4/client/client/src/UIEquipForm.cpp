...
bool CEquipMgr::Init() {
  ...
  if (!lstFightSkill)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmSkill->GetName(),
                 "lstSkill");
  ...
  if (!imgItem4)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmItem->GetName(),
                 "imgItem4");
  ...
  // 上下换页按钮
  CTextButton *btnFastUp =
      dynamic_cast<CTextButton *>(frmFast->Find("btnFastUp"));
  if (!btnFastUp)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMain800->GetName(),
                 "btnFastUp");
  btnFastUp->evtMouseClick = _evtButtonClickEvent;

  CTextButton *btnFastDown =
      dynamic_cast<CTextButton *>(frmFast->Find("btnFastDown"));
  if (!btnFastUp)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmMain800->GetName(),
                 "btnFastDown");
  ...
  cnmEquip[enumEQUIP_HEAD] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdArmet"));
  cnmEquip[enumEQUIP_BODY] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdBody"));
  cnmEquip[enumEQUIP_GLOVE] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdGlove"));
  cnmEquip[enumEQUIP_SHOES] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdShoes"));
  cnmEquip[enumEQUIP_LHAND] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdLeftHand"));
  cnmEquip[enumEQUIP_RHAND] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdRightHand"));
  cnmEquip[enumEQUIP_NECK] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdNecklace"));
  cnmEquip[enumEQUIP_HAND1] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdCirclet1"));
  cnmEquip[enumEQUIP_HAND2] =
      dynamic_cast<COneCommand *>(frmItem->Find("cmdCirclet2"));
...
void CEquipMgr::SynSkillBag(DWORD dwCharID, stNetSkillBag *pSSkillBag) {
  ...
  case enumSYN_SKILLBAG_MODI: // 修改技能
  {
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      if (pSBag[i].chLv == 0 &&
          !GetSkillList(pInfo->chFightType)->DelSkill(pSBag[i].sID)) {
...
void CEquipMgr::_evtFastChange(CGuiData *pSender, CCommandObj *pItem,
                               bool &isAccept) {
...
void CEquipMgr::UpdataEquipData(const stNetChangeChaPart &SPart,
                                CCharacter *pCha) {
  // ID不等于零时,更新数值
  ...
  for (int i = 0; i < enumEQUIP_NUM; i++) {
    ...
    if (cnmEquip[i] == NULL || SPart.SLink[i].sID == 0)
      continue;
    // End
    ...
    if (pItem && pItem->GetItemInfo()->lID == SPart.SLink[i].sID) {
      ...
      LG("error", RES_STRING(CL_LANGUAGE_MATCH_554),
         (pItem ? pItem->GetItemInfo()->lID : 0), SPart.SLink[i].sID);
    }
  }
}
...
bool CEquipMgr::_UpdataEquip(SItemGrid &Item, int nLink) {
  ...
  if (nItemID > 0 && nItemID != enumEQUIP_BOTH_HAND) {
    CItemCommand *pItem =
        dynamic_cast<CItemCommand *>(cnmEquip[nLink]->GetCommand());
...
bool CEquipMgr::_GetCommandShortCutType(CCommandObj *pItem, char &chType,
                                        short &sGridID) {
  chType = 0;
  sGridID = 0;
  ...
  if (g_stUIEquip.GetGoodsGrid() &&
      pItem->GetParent() == g_stUIEquip.GetGoodsGrid()) {
    chType = defItemShortCutType;
    sGridID = pItem->GetIndex();
  } else if (g_stUIEquip.lstFightSkill &&
             g_stUIEquip.lstFightSkill == pItem->GetParent()) {
  ...
  } else if (g_stUIEquip.lstLifeSkill &&
             g_stUIEquip.lstLifeSkill == pItem->GetParent()) {
    CSkillCommand *pSkill = dynamic_cast<CSkillCommand *>(pItem);
    if (pSkill) {
      chType = defSkillLifeShortCutType;
      sGridID = pSkill->GetSkillID();
    }
  } else if (g_stUIEquip.lstSailSkill &&
             g_stUIEquip.lstSailSkill == pItem->GetParent()) {
    CSkillCommand *pSkill = dynamic_cast<CSkillCommand *>(pItem);
    if (pSkill) {
...
int CEquipMgr::RefreshServerShortCut() {
  ...
  for (int i = 0; i < SHORT_CUT_NUM; i++) {
    if (_pFastCommands[i]) {
      _GetCommandShortCutType(_pFastCommands[i]->GetCommand(), tmp.chType[i],
                              tmp.byGridID[i]);
    }
  }
  ...
  stNetShortCutChange param;
  for (int i = 0; i < SHORT_CUT_NUM; i++) {
    if ((tmp.chType[i] != _stShortCut.chType[i]) ||
        (tmp.byGridID[i] != _stShortCut.byGridID[i])) {
      param.chIndex = i;
      param.chType = tmp.chType[i];
      param.shyGrid = tmp.byGridID[i];
      CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_SHORTCUT,
                     (void *)&param);
      nCount++;

      // LG( "shortcut", "Index:%d, Type:%d, GridID:%d\n", param.chIndex,
      // param.chType, param.shyGrid );
    }
  }
  ...
  if (GetGoodsGrid()) {
    for (DWORD i = 0; i < SHORT_CUT_NUM; i++) {
      if (stShortCut.chType[i] == defItemShortCutType) {
        if (_pFastCommands[i]) {
          _pFastCommands[i]->AddCommand(
              GetGoodsGrid()->GetItem(stShortCut.byGridID[i]));
        }
      }
    }
  }
  ...
  if (lstFightSkill) {
    for (int i = 0; i < SHORT_CUT_NUM; i++) {
      if (stShortCut.chType[i] == defSkillFightShortCutType) {
        if (_pFastCommands[i]) {
          _pFastCommands[i]->AddCommand(
              lstFightSkill->FindSkill(stShortCut.byGridID[i]));
        }
      }
    }
  }
  ...
  if (lstLifeSkill) {
    for (int i = 0; i < SHORT_CUT_NUM; i++) {
      if (stShortCut.chType[i] == defSkillLifeShortCutType) {
        if (_pFastCommands[i]) {
          _pFastCommands[i]->AddCommand(
              lstLifeSkill->FindSkill(stShortCut.byGridID[i]));
        }
      }
    }
  }
  ...
  if (lstSailSkill) {
    for (int i = 0; i < SHORT_CUT_NUM; i++) {
      if (stShortCut.chType[i] == defSkillSailShortCutType) {
        if (_pFastCommands[i]) {
          _pFastCommands[i]->AddCommand(
              lstSailSkill->FindSkill(stShortCut.byGridID[i]));
        }
      }
    }
  }
...
void CEquipMgr::_evtButtonClickEvent(CGuiData *pSender, int x, int y,
                                     DWORD key) {
...
void CEquipMgr::_evtEquipEvent(CGuiData *pSender, CCommandObj *pItem,
                               bool &isAccept) {
  isAccept = false;
  ...
  if (pCom->nTag == 999) {
    ...
    g_stUIBox.ShowSelectBox(_evtLostYesNoEvent,
                            RES_STRING(CL_LANGUAGE_MATCH_556), true);
  } else {
    // Add by sunny.sun 20080902
    CItemRecord *_pItem = pItemCommand->GetItemInfo();
    if (_pItem->sType == 8 || _pItem->sType == 13 || _pItem->sType == 31 ||
        _pItem->sType == 36 || _pItem->sType == 41 || _pItem->sType == 42 ||
        _pItem->sType == 47 || _pItem->sType == 48 || _pItem->sType == 57 ||
        _pItem->sType == 58 || _pItem->sType == 65) {
...
void CEquipMgr::_evtLostYesNoEvent(CCompent *pSender, int nMsgType, int x,
                                   int y, DWORD dwKey) {
...
void CEquipMgr::_evtThrowEquipEvent(CGuiData *pSender, CCommandObj *pItem,
                                    bool &isThrow) {
  isThrow = false;
...
bool CEquipMgr::ExecFastKey(int key) {
  ...
  if (key == VK_F12 || (key == VK_OEM_6 && ::GetAsyncKeyState(VK_CONTROL))) {
    _ActiveFast(_nFastCur + 1);
    return true;
  }
...
void CEquipMgr::evtSwapItemEvent(CGuiData *pSender, int nFirst, int nSecond,
                                 bool &isSwap) {
  isSwap = false;
  CGoodsGrid *pGood = dynamic_cast<CGoodsGrid *>(CDrag::GetParent());
  ...
  if (g_pGameApp->IsShiftPress()) {
    if (pItem && pItem->GetTotalNum() > 1) {
      if (!pTarget ||
        ...
        SSplit.pSelf = pSelf;
        CBoxMgr::ShowNumberBox(SSplitItem::_evtSplitItemEvent,
                               pItem->GetTotalNum());
        return;
      }
    }
...
void CEquipMgr::SSplitItem::_evtSplitItemEvent(CCompent *pSender, int nMsgType,
...
void CEquipMgr::evtThrowItemEvent(CGuiData *pSender, int id, bool &isThrow) {
  ...
  if (pItem && pItem->GetItemInfo()->sType == 43) {
    stSelectBox *pBox = g_stUIBox.ShowSelectBox(
        _evtThrowBoatDialogEvent, RES_STRING(CL_LANGUAGE_MATCH_558), true);
    if (pBox) {
      pBox->pointer = &sthrow;
      return;
    }
  ...
  if (obj->GetIsPile() && obj->GetTotalNum() > 1) {
    stNumBox *pBox =
        g_stUIBox.ShowNumberBox(_evtThrowDialogEvent, obj->GetTotalNum());
    if (pBox) {
      pBox->pointer = &sthrow;
      return;
    }
...
void CEquipMgr::_evtThrowDialogEvent(CCompent *pSender, int nMsgType, int x,
                                     int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CEquipMgr::_evtThrowBoatDialogEvent(CCompent *pSender, int nMsgType, int x,
                                         int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CEquipMgr::_evtThrowDeleteConfirm(CCompent *pSender, int nMsgType, int x,
                                       int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes) {
    return;
  }
...
void CEquipMgr::_SendThrowData(const stThrow &sthrow, int nThrowNum) {
  ...
  if (CWorldScene::_IsThrowItemHint) {
    stSelectBox *pSelectBox = g_stUIBox.ShowSelectBox(
        _evtThrowDeleteConfirm, RES_STRING(CL_UIEQUIPFORM_CPP_00001), true);
...
void CEquipMgr::FrameMove(DWORD dwTime) {
  ...
  if (time.IsTimeOut(dwTime)) {
    ...
    if (frmSkill->GetIsShow()) {
      // 显示剩余技能点数
      _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%d",
                  pGameAttr->get(ATTR_TP));
      labPoint->SetCaption(szBuf);

      //_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE , "%d",
      //pGameAttr->get(ATTR_LIFETP) ); labPointLife->SetCaption( szBuf );
    ...
    if (pMainCha) {
      ...
      if (frmItem->GetIsShow()) {
        ...
        //StringSplitNum( pGameAttr->get(ATTR_GD) )  );
        _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%s",
                    StringSplitNum((long)pGameAttr->get(ATTR_GD)));
        ...
        CItemCommand *pItem = NULL;
        for (states::iterator it = _charges.begin(); it != _charges.end();
             it++) {
...
void CEquipMgr::_evtUnfixDialogEvent(CCompent *pSender, int nMsgType, int x,
                                     int y, DWORD dwKey) {
  if (nMsgType != CForm::mrYes)
    return;
...
void CEquipMgr::_StartUnfix(stUnfix &unfix) {
  ...
  if (pItem->GetIsPile() && pItem->GetTotalNum() > 1) {
    stNumBox *pBox =
        g_stUIBox.ShowNumberBox(_evtUnfixDialogEvent, pItem->GetTotalNum());
    if (pBox) {
      pBox->pointer = &unfix;
      return;
    }
...
void CEquipMgr::_evtRMouseGridEvent(CGuiData *pSender, CCommandObj *pItem,
                                    int nGridID) {
...
void CEquipMgr::_evtRepairEvent(CCompent *pSender, int nMsgType, int x, int y,
                                DWORD dwKey) {
...
void CEquipMgr::ShowRepairMsg(const char *pItemName, long lMoney) {
  ...
  _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE,
              RES_STRING(CL_LANGUAGE_MATCH_559), pItemName, lMoney);
...
// 锁定图片鼠标点击事件
void CEquipMgr::_evtItemFormMouseEvent(CCompent *pSender, int nMsgType, int x,
                                       int y, DWORD dwKey) {
      ...
      for (int i = 0; i < nCount; ++i) {
        ...
        if (pCmd) {
          ...
          if (pItemRecord && pItemRecord->lID == 2440) {
            CBoxMgr::ShowSelectBox(_CheckLockMouseEvent,
                                   RES_STRING(CL_LANGUAGE_MATCH_824),
                                   true); //确认锁定背包
...
bool CEquipMgr::GetIsLock() { return imgLock->GetIsShow(); }

// 加锁 MSGBOX 确认
void CEquipMgr::_CheckLockMouseEvent(CCompent *pSender, int nMsgType, int x,
                                     int y, DWORD dwKey) {
  if (nMsgType == CForm::mrYes) {
    CS_LockKitbag();
  }
