...
namespace GUI {
const int CGuildChallengeMgr::FIRST_CHARGE_MONEY[NUM] = {5000000, 3000000,
                                                         1000000};
...
bool CGuildChallengeMgr::Init() {
  ...
  char szBuf[32];
  for (int i(0); i < NUM; i++) {
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "labGuildName%d", i);
    labGuildName[i] = dynamic_cast<CLabel *>(frmGuildPK->Find(szBuf));
    if (!labGuildName[i])
      return Error(RES_STRING(CL_LANGUAGE_MATCH_561), frmGuildPK->GetName(),
                   szBuf);

    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "labChallenger%d", i);
    labChallenger[i] = dynamic_cast<CLabel *>(frmGuildPK->Find(szBuf));
    if (!labChallenger[i])
      return Error(RES_STRING(CL_LANGUAGE_MATCH_561), frmGuildPK->GetName(),
                   szBuf);

    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "labMoney%d", i);
    labMoney[i] = dynamic_cast<CLabel *>(frmGuildPK->Find(szBuf));
    if (!labMoney[i])
      return Error(RES_STRING(CL_LANGUAGE_MATCH_561), frmGuildPK->GetName(),
                   szBuf);

    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "btnCharge%d", i);
...
//-------------------------------------------------------------------------
void CGuildChallengeMgr::Show(bool bShow) { frmGuildPK->SetIsShow(bShow); }
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
void CGuildChallengeMgr::ChallegeSuccess(long lChaID) { return; }
//-------------------------------------------------------------------------
void CGuildChallengeMgr::ChallegeFailed(long lChaID) { return; }
//-------------------------------------------------------------------------
void CGuildChallengeMgr::ChallegeOther(long lChaID) { return; }
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_FirstChallengeButtonDown(CGuiData *pSender, int x,
                                                   int y, DWORD key) {
  g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::FIRST);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_SecondChallengeButtonDown(CGuiData *pSender, int x,
                                                    int y, DWORD key) {
  g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::SECOND);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_ThirdChallengeButtonDown(CGuiData *pSender, int x,
                                                   int y, DWORD key) {
  g_stGuildChallenge.ChargeMoney(CGuildChallengeMgr::THIRD);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_enterChargeMoney(CCompent *pSender, int nMsgType,
  ...
  if (!g_stGuildChallenge.IsValid(g_stGuildChallenge.m_iSelIndex,
                                  iChargeMoney)) {
    return;
  }
  ...
  char buf[256] = {0};
  _snprintf_s(buf, _countof(buf), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_583),
              StringSplitNum(iChargeMoney));
  g_stUIBox.ShowSelectBox(_ChargeEvent, buf, true);
}
//-------------------------------------------------------------------------
void CGuildChallengeMgr::_ChargeEvent(CCompent *pSender, int nMsgType, int x,
                                      int y, DWORD dwKey) {
  if (nMsgType !=
      CForm::mrYes) // 修正竞拍点取消也收钱的 BUG  add by Philip.Wu  2006-07-25
  {
    g_stGuildChallenge.m_iSelIndex = -1;
    return;
  }

  if (g_stGuildChallenge.m_iSelIndex == -1 &&
      g_stGuildChallenge.m_iChangeMoney == -1)
    return;
  g_stGuildChallenge.SendChallegeProtocol(g_stGuildChallenge.m_iSelIndex,
                                          g_stGuildChallenge.m_iChangeMoney);
...
void CGuildChallengeMgr::ChargeMoney(int iIndex) {
  ...
  if (strcmp(labChallenger[iIndex]->GetCaption(), pMainCha->getGuildName()) ==
      0) {
  ...
  char szBuf[64];
  stNumBox *numBox = CBoxMgr::ShowNumberBox(
      _enterChargeMoney, -1, RES_STRING(CL_LANGUAGE_MATCH_589), false);
  _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%d", iNextCharge);
