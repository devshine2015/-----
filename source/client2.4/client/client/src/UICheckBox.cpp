...
CCheckBox::CCheckBox(CForm &frmOwn)
    : CCompent(frmOwn), evtCheckChange(NULL), _pImage(NULL),
      _TextColor(0xffff0000), _isDown(false), _isChecked(false) {
  _pImage = new CGuiPic(this, stEnd);
  _SetSelf();
}

CCheckBox::CCheckBox(const CCheckBox &rhs)
    : CCompent(rhs), _pImage(rhs._pImage), _isChecked(rhs._isChecked),
      _TextColor(rhs._TextColor), _isDown(rhs._isDown) {
  _SetSelf();
}
...
void CCheckBox::Render() {
  ...
  if (!_strCaption.empty())
    CGuiFont::s_Font.Render(_strCaption.c_str(), GetX() + _nTextOffsetX,
                            GetY() + _nTextOffsetY, _TextColor);
}
...
void CCheckBox::Refresh() {
  ...
  _nTextOffsetY =
      (int)((_pImage->GetImage()->nTexH * _pImage->GetImage()->fScaleY -
             CGuiFont::s_Font.GetHeight("S")) /
            2.0);

  if (!_strCaption.empty()) {
    _nTextOffsetX =
        (int)(_pImage->GetImage()->nTexW * _pImage->GetImage()->fScaleX +
              s_nMarginLeft);
    _nX2 =
        _nX1 + _nTextOffsetX + CGuiFont::s_Font.GetWidth(_strCaption.c_str());
  } else {
    _nTextOffsetX =
        (int)(_pImage->GetImage()->nTexW * _pImage->GetImage()->fScaleX);
...
CCheckGroup::CCheckGroup(CForm &frmOwn)
    : CContainer(frmOwn), _nSize(0), _pActive(NULL), _nActive(-1),
      evtSelectChange(NULL) {}

CCheckGroup::CCheckGroup(const CCheckGroup &rhs) : CContainer(rhs) {
  _SetSelf();
}
