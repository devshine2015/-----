...
bool CStateMgr::Init() {
  ...
  if (!labStateName)
    return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmState->GetName(),
                 "labStateName");
  labStateName->SetIsCenter(true);

  FORM_CONTROL_LOADING_CHECK(labGuildName, frmState, CLabelEx, "preperty.clu",
                             "labStateGuid");
  ...
  // if( !btnLuk )			return Error( "msgui.clu界面<%s>上找不到控件<%s>",
  // frmState->GetName(), "btnLuk" ); btnLuk->evtMouseClick = MainMouseDown;
...
void CStateMgr::RefreshStateFrm() {
  ...
  // 血条
  if (labStateHP) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d/%d",
                (int)pCChaAttr->get(ATTR_HP), (int)pCChaAttr->get(ATTR_MXHP));
  ...
  if (labStateEXP) {
    if (max != 0)
      _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%4.2f%%",
                  num * 100.0f / max);
    else
  ...
  if (labStateSP) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d/%d", (int)num,
                (int)max);
  ...
  //玩家职业
  if (labStateJob) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%s",
  ...
  //玩家等级
  if (labStateLevel) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_LV)); //显示角色等级
  ...
  if (labStatePoint) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_AP)); //显示角色的属性点
    labStatePoint->SetCaption((const char *)pszCha);
  }

  if (labSkillPoint) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_TP)); //显示角色的技能点
  ...
  if (labStrshow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_STR));
    labStrshow->SetCaption((const char *)pszCha);
  }

  if (labDexshow) {
    //_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE , "%d" ,
    //pCChaAttr->get(ATTR_DEX)); labDexshow->SetCaption( (const char* ) pszCha);
  }

  if (labAgishow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_AGI));
    labAgishow->SetCaption((const char *)pszCha);
  }

  if (labConshow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_CON));
    labConshow->SetCaption((const char *)pszCha);
  }

  if (labStashow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_STA));
    labStashow->SetCaption((const char *)pszCha);
  }

  if (labSailLevel) {
    //_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE, "%d" ,
    //pCChaAttr->get(ATTR_SAILLV)); labSailLevel->SetCaption( (const char* )
    // pszCha);
  }

  if (labSailEXP) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_CSAILEXP));
    labSailEXP->SetCaption((const char *)pszCha);
  }

  //_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE , "%d" ,
  //pCChaAttr->get(ATTR_LUK));
  // if(labLukshow)		labLukshow->SetCaption( (const char* ) pszCha);

  // 8个附加属性
  if (labMinAtackShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_MNATK)); // 最小攻击力
    labMinAtackShow->SetCaption((const char *)pszCha);
  }

  if (labMaxAtackShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_MXATK)); // 最大攻击力
    labMaxAtackShow->SetCaption((const char *)pszCha);
  }

  if (labFleeShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_FLEE)); // 闪避率
  ...
  if (labHitShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_HIT)); // 命中率
    labHitShow->SetCaption((const char *)pszCha);
  }

  if (labDefenceShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_DEF)); // 防御力
  ...
  //{
  //	_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE , "%d" ,
  //pCChaAttr->get(ATTR_CRT)); // 爆击率 	labCriticalShow->SetCaption( (const
  //char* ) pszCha);
  //}
  ...
  //{
  //	_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE , "%d" ,
  //pCChaAttr->get(ATTR_MF));	 // 寻宝率 	labMfShow->SetCaption( (const char* )
  //pszCha);
  //}

  if (labPhysDefineShow) {
    //_snprintf_s( pszCha, _countof( pszCha ), _TRUNCATE , "%d" ,
    //pCChaAttr->get(ATTR_PDEF)); // 物理抵抗 labPhysDefineShow->SetCaption(
    // (const char* ) pszCha);
  }

  if (labFameShow) {
    _snprintf_s(pszCha, _countof(pszCha), _TRUNCATE, "%d",
                pCChaAttr->get(ATTR_FAME)); // 声望
