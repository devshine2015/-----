...
CSceneItem::CSceneItem()
    : CSceneNode(), _nCharacterID(-1), _fTerrainHeight(0.0f), _IsSystem(false),
      _IsShowName(false), _IsAlpha(false), _pItemInfo(NULL), _pArcTrack(NULL),
      _pEvent(NULL), _dwForgeValue(0), mParentCharacter(0), mEffect(0) {
...
BOOL CSceneItem::_Create(int nObjTypeID, int nType) {
  ...
  _snprintf_s(str, _countof(str), _TRUNCATE, "%s.lgo",
              _pItemInfo->chModule[nType - 1]);

  if (Load(str) == 0) {
    LG("error", RES_STRING(CL_LANGUAGE_MATCH_350), nObjTypeID,
       _pItemInfo->szName, str);
    return FALSE;
  }
  ...
  if (l) {
    ...
    drPrimitiveTexLit(p, l->str_buf[0], ".\\texture\\lit\\", l->color_op,
                      l->anim_type);
  }
  // end
...
HRESULT CSceneItem::LitResetTexture(DWORD item_id, DWORD lit_id) {
  ...
  if (_tcslen(lit_info->file) > 0) {
    drIPathInfo *path_info =
        res_mgr->GetSysGraphics()->GetSystem()->GetPathInfo();
    DR_FAILED_RET(pri->ResetTexture(
        1, 0, lit_info->file, path_info->GetPath(PATH_TYPE_TEXTURE_ITEM)));
  }
  if (lit_info->anim_type > 0) {
    ...
    res_mgr->CreateAnimCtrlObj((drIAnimCtrlObj **)&ctrlobj_tc,
                               ANIM_CTRL_TYPE_TEXCOORD);
...
void CSceneItem::Render() {
  if (GetScene()->GetTerrain()) {
    if (!GetScene()->GetTerrain()->IsPointVisible((float)_nCurX / 100.0f,
                                                  (float)_nCurY / 100.0f))
      return;
  }
...
void CSceneItem::PlayArcAni(D3DXVECTOR3 vStart, D3DXVECTOR3 vEnd, float fVel,
                            float fHei) {
  if (!_pArcTrack)
...
CEffectObj *CSceneItem::bindEffect(int dummyID, int effectID, bool isLoop,
                                   int angle) {
  if (effectID <= 0)
    return NULL;
