...
void CFoundMgr::CheckResult(short sRet, const char *num1, const char *num2,
                            const char *num3) {
  ...
  /*if(g_stUIEquip.GetGoodsGrid()->GetItem(iFoundPos[1]))
  {
          for(int i = 0; i < (FOUND_COUNT - 1); i++)
          {
                  ...
                  {
                          pItemCommand =
  dynamic_cast<CItemCommand*>(g_stUIEquip.GetGoodsGrid()->GetItem(iFoundPos[i]));
                          if(pItemCommand)
                          {
                                  if(3 == i)
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUIFound.cmdFound[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_STR);
  ...
  true);
                                  ...
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUIFound.cmdFound[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_CON);
                                          if(pItemCommand->GetItemInfo()->lID ==
  iItem)
                                          {
                                                  PushItem(i, *pItemCommand,
  true);
                                  ...
                                  {
                                          int iItem =
  (dynamic_cast<CItemCommand*>(g_stUIFound.cmdFound[1]->GetCommand()))->GetData().GetInstAttr(ITEMATTR_VAL_DEX);
                                          if(pItemCommand->GetItemInfo()->lID ==
  iItem)
                                          {
                                                  PushItem(i, *pItemCommand,
...
void CFoundMgr::DragToEquipGrid(int index) { PopItem(index); }
...
bool CFoundMgr::Init() {
  ...
  for (int i = 0; i < 6; i++) {
    _snprintf_s(buff, _countof(buff), _TRUNCATE, "imgShaiZi1_%i", (i + 1));
    Dice1[i] = dynamic_cast<CImage *>(frmFound->Find(buff));
    if (!Dice1[i]) {
      LG("gui", "frmFound::%s not found.\n", buff);
      return false;
    }
    _snprintf_s(buff, _countof(buff), _TRUNCATE, "imgShaiZi2_%i", (i + 1));
    Dice2[i] = dynamic_cast<CImage *>(frmFound->Find(buff));
    if (!Dice2[i]) {
      LG("gui", "frmFound::%s not found.\n", buff);
      return false;
    }
    _snprintf_s(buff, _countof(buff), _TRUNCATE, "imgShaiZi3_%i", (i + 1));
...
void CFoundMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
  ...
  CItemCommand *pItemCommand =
      dynamic_cast<CItemCommand *>(cmdFound[iIndex]->GetCommand());
  if (!pItemCommand)
    return;
...
void CFoundMgr::PushItem(int iIndex, CItemCommand &rItem,
                         bool bRetry /* = false*/) {
  if (iIndex < 0 || iIndex >= FOUND_COUNT)
    return;
...
void CFoundMgr::_evtbtnForgeYes(CCompent *pSender, int nMsgType, int x, int y,
                                DWORD dwKey) {
  ...
  if (szName == "btnForgeYes") {
    ...
    CS_Found(g_stUINpcTalk.GetNpcId(), g_stUIFound.iFoundPos, (FOUND_COUNT - 1),
             1, true);
    ...
    if (g_stUIFound.proFound->IsRuning()) {
      ...
      CS_Found(g_stUINpcTalk.GetNpcId(), g_stUIFound.iFoundPos,
               (FOUND_COUNT - 1), 1);
    }
...
void CFoundMgr::_evtDragItemBase1(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
...
void CFoundMgr::_evtDragItemBase2(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
  CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(pItem);
  if (!pItemCommand || !pItemCommand->GetIsValid())
    return;
...
void CFoundMgr::_evtDragItemBase3(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
  if (!g_stUIFound.cmdFound[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
    return;
...
void CFoundMgr::_evtDragItemBase4(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
  //  材料1
  if (!g_stUIFound.cmdFound[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
  ...
  if (!pItemCommand || !pItemCommand->GetIsValid())
    return;
  int iItem =
      (dynamic_cast<CItemCommand *>(g_stUIFound.cmdFound[1]->GetCommand()))
          ->GetData()
...
void CFoundMgr::_evtDragItemBase5(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
  //  材料2
  if (!g_stUIFound.cmdFound[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
...
void CFoundMgr::_evtDragItemBase6(CGuiData *pSender, CCommandObj *pItem,
                                  bool &isAccept) {
  //  材料3
  if (!g_stUIFound.cmdFound[1]->GetCommand()) {
    g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_896));
