...
CCharMsg::sChannelInfo CCharMsg::m_sChannelInfo[m_wTotalChannelsCount] = {
    ...
    {
        CHANNEL_ALL,
        /*"RES_STRING(CL_LANGUAGE_MATCH_493)",	*/ 0xFFFFFFFF,
        "",
        0,
    }, // CHANNEL_ALL
    {
        CHANNEL_SIGHT,
        /*"RES_STRING(CL_LANGUAGE_MATCH_494)",	*/ 0xFFFFFFFF,
        "",
        0,
    }, // CHANNEL_SIGHT
    {
        CHANNEL_PRIVATE,
        /*"RES_STRING(CL_LANGUAGE_MATCH_495)",	*/ 0xFFFFFFFF,
        "@",
        0,
    }, // CHANNEL_PRIVATE
    {
        CHANNEL_WORLD,
        /*"RES_STRING(CL_LANGUAGE_MATCH_496)",	*/ 0xFFFFFFFF,
        "*",
        0,
    }, // CHANNEL_WORLD
    {
        CHANNEL_TRADE,
        /*"RES_STRING(CL_LANGUAGE_MATCH_497)",	*/ 0xFFFFFFFF,
        "^",
        0,
    }, // CHANNEL_TRADE
    {
        CHANNEL_TEAM,
        /*"RES_STRING(CL_LANGUAGE_MATCH_299)",	*/ 0xFFFFFFFF,
        "!",
        0,
    }, // CHANNEL_TEAM
    {
        CHANNEL_GUILD,
        /*"RES_STRING(CL_LANGUAGE_MATCH_468)",	*/ 0xFFFFFFFF,
        "%",
        0,
    }, // CHANNEL_GUILD
    {
        CHANNEL_SYSTEM,
        /*"RES_STRING(CL_LANGUAGE_MATCH_498)",	*/ 0xFFFFFFFF,
        "",
        0,
    }, // CHANNEL_SYSTEM
    {
        CHANNEL_PUBLISH,
        /*"RES_STRING(CL_LANGUAGE_MATCH_499)",	*/ 0xFFFFFFFF,
        "",
        0,
    }, // CHANNEL_PUBLISH
    {
        CHANNEL_SIDE,
        /*"RES_STRING(CL_LANGUAGE_MATCH_932)",	*/ 0xFFFFFFFF,
        "|",
        0,
    }, // CHANNEL_SIDE
...
CCharMsg::CCharMsg() {
  m_ecboShowChannels = CHANNEL_ALL | CHANNEL_SIGHT | CHANNEL_PRIVATE |
                       CHANNEL_WORLD | CHANNEL_TRADE | CHANNEL_TEAM |
                       CHANNEL_GUILD | CHANNEL_SYSTEM | CHANNEL_PUBLISH |
                       CHANNEL_SIDE;
#ifdef MANAGE_VER
...
CCharMsg::~CCharMsg() { m_lstThisInstanceLink.remove(this); }
...
WORD CCharMsg::GetTotalChannelCount() { return m_wTotalChannelsCount; }

void CCharMsg::AddMsg(eChannel channel, string strWho, string strText,
                      bool bSendTo) {
  sTextInfo sText;
  if (channel == CHANNEL_PRIVATE) {
    if (bSendTo) {
      sText.strShowText =
          GetChannelShowName(channel) + "<To " + strWho +
          ">: "; // ning.yan 2008-11-07 英文语法中标点符号后要加空格
    } else {
  ...
  if (m_sChannelInfo[wChannelIndex].dwTotalMsg >= m_dwChannelBufferSize) {
    //缓冲区满
    lstTextInfoType::iterator iterRemoveObject =
    ...
    if (iterRemoveObject != m_lstMsgLink.end()) {
      for (lstInstanceType::iterator it = m_lstThisInstanceLink.begin();
           it != m_lstThisInstanceLink.end(); it++) {
...
bool CCharMsg::ModifyShowChannel(eChannel eShowChannel, bool bAddOrRemove,
                                 bool bShowTips) {
    ...
    if (bAddOrRemove) {
      ...
      if (bShowTips) {
        CCozeForm::GetInstance()->OnSystemMsg(
            GetChannelName(eShowChannel) + RES_STRING(CL_LANGUAGE_MATCH_500));
      }
    } else {
      SetShowChannels(m_ecboShowChannels & ~eShowChannel);
      if (bShowTips) {
        CCozeForm::GetInstance()->OnSystemMsg(
            GetChannelName(eShowChannel) + RES_STRING(CL_LANGUAGE_MATCH_501));
      }
    }
    return true;
  }
...
DWORD CCharMsg::GetShowChannels() { return m_ecboShowChannels; }
...
void CCardCase::AddCard(string str) {
  ...
  for (lstCardType::iterator it = m_lstCardData.begin();
       it != m_lstCardData.end(); it++) {
    if ((*it) == str) {
...
int CCardCase::GetTotalCount() { return (int)m_lstCardData.size(); }
...
void CChannelSwitchForm::SwitchCheck() {
  ...
  if (m_frmChannelSwitch->GetIsShow()) {
    ...
    CChannelSwitchForm::GetInstance()->m_chkPrivate->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_PRIVATE));
    CChannelSwitchForm::GetInstance()->m_chkSight->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_SIGHT));
    CChannelSwitchForm::GetInstance()->m_chkSystem->SetIsChecked(
        pCozeForm->m_lstSystemPage->GetIsShow());
    CChannelSwitchForm::GetInstance()->m_chkTeam->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_TEAM));
    CChannelSwitchForm::GetInstance()->m_chkGuild->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_GUILD));
    CChannelSwitchForm::GetInstance()->m_chkWorld->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_WORLD));
    CChannelSwitchForm::GetInstance()->m_chkTrade->SetIsChecked(
        pCozeForm->m_cMainMsg.IsShowChannel(CCharMsg::CHANNEL_TRADE));
...
bool CChannelSwitchForm::Init() {
  ...
  FORM_CONTROL_LOADING_CHECK(m_chkPrivate, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkPersonal");
  FORM_CONTROL_LOADING_CHECK(m_chkSight, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkNear");
  FORM_CONTROL_LOADING_CHECK(m_chkSystem, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkSystem");
  FORM_CONTROL_LOADING_CHECK(m_chkTeam, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkTeam");
  FORM_CONTROL_LOADING_CHECK(m_chkGuild, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkGuild");
  FORM_CONTROL_LOADING_CHECK(m_chkWorld, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkWorld");
  FORM_CONTROL_LOADING_CHECK(m_chkTrade, m_frmChannelSwitch, CCheckBox,
                             "main.clu", "chkTrade");
...
void CChannelSwitchForm::EventLostFocus(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_chkChannelSwitch->SetIsChecked(false);
  pCozeForm->m_chkChannelSwitch->MouseRun(
      pCozeForm->m_chkChannelSwitch->GetX() - 1,
      pCozeForm->m_chkChannelSwitch->GetY() - 1, 0);
...
void CChannelSwitchForm::EventPrivateCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_PRIVATE,
      CChannelSwitchForm::GetInstance()->m_chkPrivate->GetIsChecked());
...
void CChannelSwitchForm::EventSightCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_SIGHT,
      CChannelSwitchForm::GetInstance()->m_chkSight->GetIsChecked());
  pCozeForm->ResetPages();
}
...
void CChannelSwitchForm::EventTeamCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_TEAM,
      CChannelSwitchForm::GetInstance()->m_chkTeam->GetIsChecked());
  pCozeForm->ResetPages();
}
...
void CChannelSwitchForm::EventGuildCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_GUILD,
      CChannelSwitchForm::GetInstance()->m_chkGuild->GetIsChecked());
  pCozeForm->ResetPages();
}
...
void CChannelSwitchForm::EventWorldCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_WORLD,
      CChannelSwitchForm::GetInstance()->m_chkWorld->GetIsChecked());
  pCozeForm->ResetPages();
}
...
void CChannelSwitchForm::EventTradeCheckChange(CGuiData *pSender) {
  CCozeForm *pCozeForm = CCozeForm::GetInstance();
  pCozeForm->m_cMainMsg.ModifyShowChannel(
      CCharMsg::CHANNEL_TRADE,
      CChannelSwitchForm::GetInstance()->m_chkTrade->GetIsChecked());
  pCozeForm->ResetPages();
}
...
CCozeForm::CCozeForm()
    : m_cCallingCard(10), m_cSendMsgCard(10), m_bSendMsgCardSwitch(false) {
...
void CCozeForm::OnSightMsg(CCharacter *pChar, string strMsg) {
  ...
  string str = string(pChar->getHumanName()) + ": " +
               strMsg; // 需要修改 ning.yan 2008-11-07
  CItemEx *item = new CItemEx(
      str.c_str(), CCharMsg::GetChannelColor(CCharMsg::CHANNEL_SIGHT));
...
void CCozeForm::OnPrivateMsg(string strFromName, string strToName,
                             string strMsg) {
  ...
  if (strHumanName == strFromName) {
    ...
    if (strMsg == "{x}*") {
      OnSystemMsg(RES_STRING(CL_LANGUAGE_MATCH_505) + strFromName +
                  RES_STRING(CL_LANGUAGE_MATCH_506));
    } else {
...
void CCozeForm::OnPublishMsg(string strName, string strMsg) {
  ...
  g_pGameApp->ShowNotify(str.c_str(),
                         CCharMsg::GetChannelColor(CCharMsg::CHANNEL_PUBLISH));
}
// Add by sunny.sun20080804
void CCozeForm::OnPublishMsg1(string strMsg, int setnum) {
  string str;
  str = string(strMsg);
  g_pGameApp->ShowNotify1(str.c_str(), setnum,
                          CCharMsg::GetChannelColor(CCharMsg::CHANNEL_PUBLISH));
}
// End
void CCozeForm::OnPrivateNameSet(string strName) {
  ...
  string strCurCmd = CCharMsg::GetChannelCommand(
                         CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_PRIVATE)) +
                     strName + " ";
...
void CCozeForm::ReplaceSpecialFace(string &strMsg, const string &strReplace,
                                   const string &strFace) {
...
bool CCozeForm::Init() {
  ...
  FORM_CONTROL_LOADING_CHECK(m_edtPublishMsg, m_frmPublish, CEdit, "coze.clu",
                             "edtGMSay");
  m_edtPublishMsg->SetIsWrap(true);
  m_edtPublishMsg->evtEnter = EventPublishSendMsg;

  FORM_CONTROL_LOADING_CHECK(m_edtMsg, m_frmMainChat, CEdit, "main.clu",
                             "edtSay");
  ...
  FORM_CONTROL_LOADING_CHECK(m_edtPublishMsg1, m_frmPublish1, CEdit, "coze.clu",
                             "edtArenaSay");
  m_edtPublishMsg1->SetIsWrap(true);
  m_edtPublishMsg1->evtEnter = EventPublishSendMsg1;
  // End

  FORM_CONTROL_LOADING_CHECK(m_cmbChannel, m_frmMainChat, CCombo, "main.clu",
                             "cboChannel");
  m_cmbChannel->evtSelectChange = EventSendChannelChange;
  m_cmbChannel->evtMouseClick = EventSendChannelSwitchClick;
  m_cmbChannel->GetList()->GetItems()->Select(0);

  FORM_CONTROL_LOADING_CHECK(m_lstMainPage, m_frmMainChat, CList, "main.clu",
                             "lstOnSay");
  ...
  FORM_CONTROL_LOADING_CHECK(m_lstSystemPage, m_frmMainChat, CList, "main.clu",
                             "lstOnSaySystem");
  ...
  FORM_CONTROL_LOADING_CHECK(m_drgMainPage, m_frmMainChat, CDragTitle,
                             "main.clu", "drpTitle");
  ...
  FORM_CONTROL_LOADING_CHECK(m_drgSystemPage, m_frmMainChat, CDragTitle,
                             "main.clu", "drpTitleSystem");
  ...
  FORM_CONTROL_LOADING_CHECK(m_chkChannelSwitch, m_frmMainChat, CCheckBox,
                             "main.clu", "chkChatOn");
  m_chkChannelSwitch->SetIsChecked(false);
  m_chkChannelSwitch->evtCheckChange = EventChannelSwitchCheck;

  FORM_CONTROL_LOADING_CHECK(m_btnCallingCardSwitch, m_frmMainChat, CTextButton,
                             "main.clu", "btnCard");
  m_btnCallingCardSwitch->evtMouseClick = EventCallingCardSwitchClick; // switch

  FORM_CONTROL_LOADING_CHECK(m_lstCallingCard, m_frmMainChat, CList, "main.clu",
                             "lstCard");
  ...
  FORM_CONTROL_LOADING_CHECK(m_btnFaceSwitch, m_frmMainChat, CTextButton,
                             "main.clu", "btnChatFace");
  m_btnFaceSwitch->evtMouseClick = EventFacePanelSwitchClick; // switch

  FORM_CONTROL_LOADING_CHECK(m_grdFacePanel, m_frmMainChat, CGrid, "main.clu",
                             "grdFace");
  // m_grdFacePanel->evtLost=EventFacePanelLostFocus;
  m_grdFacePanel->evtSelectChange = EventFaceSelected;

  FORM_CONTROL_LOADING_CHECK(m_btnBrowSwitch, m_frmMainChat, CTextButton,
                             "main.clu", "btnBrow");
  m_btnBrowSwitch->evtMouseClick = EventBrowPanelSwitchClick; // switch

  FORM_CONTROL_LOADING_CHECK(m_grdBrowPanel, m_frmMainChat, CGrid, "main.clu",
                             "grdHeart");
  m_grdBrowPanel->evtSelectChange = EventBrowSelected;

  FORM_CONTROL_LOADING_CHECK(m_btnActionSwitch, m_frmMainChat, CTextButton,
                             "main.clu", "btnAction");
  m_btnActionSwitch->evtMouseClick = EventActionPanelSwitchClick; // switch

  FORM_CONTROL_LOADING_CHECK(m_grdActionPanel, m_frmMainChat, CGrid, "main.clu",
                             "grdAction");
...
void CCozeForm::SendMsg() {
  ...
  if (strMsg.empty() ||
      strMsg.length() == count(strMsg.begin(), strMsg.end(), ' ')) {
  ...
  if (g_stUIMap.IsPKSilver() && pChar->getGMLv() <= 0) {
    g_pGameApp->SysInfo(
        RES_STRING(CL_LANGUAGE_MATCH_901)); // 乱斗白银城中禁止发送消息
    return;
  }
    ...
    if (p != -1) {
      ...
      if (!strContent.empty()) {
        ...
        if (!strPersonName.empty() && strPersonName.length() <= 16) {
          ...
          strCurCmd = CCharMsg::GetChannelCommand(CCharMsg::GetChannelIndex(
                          CCharMsg::CHANNEL_PRIVATE)) +
                      strPersonName + " ";

          CCharacter *pMain = CGameScene::GetMainCha();
          if (pMain &&
              (pMain->IsBoat() || pMain->getGameAttr()->get(ATTR_LV) >= 9)) {
  ...
  strCurCmd =
      CCharMsg::GetChannelCommand(CCharMsg::GetChannelIndex(enumChannel));
  m_edtMsg->SetCaption(strCurCmd.c_str());
}
...
void CCozeForm::UpdatePages() {
  ...
  while (m_cMainMsg.MoveToNextMsg()) {
    ...
    while (!strMsg.empty()) {
      ...
      if (bIndentFlag) {
        try {
          strShowText = CutFaceText(
              strMsg,
              38); // 左下角聊天框，除第一行外，每行都加了6空格，所以要减少6个字符
        } catch (...) {
          strShowText = "";
          strMsg = "";
        }
      ...
      CItemEx *pItem = new CItemEx(strShowText.c_str(),
                                   CCharMsg::GetChannelColor(msg.eTextChannel));
  ...
  if (g_pGameApp->IsMouseButtonPress(0)) {
    ...
    if (m_lstMainPage->GetScroll()->IsNormal()) {
      ...
      if (step.GetPosition() > step.GetMin() &&
          step.GetPosition() + 5 < step.GetMax()) {
        IsPressScroll = true;
      }
    }
  }
...
void CCozeForm::ChangePrivatePlayerName(string strName) {
  ...
  string strPrivateCmd = CCharMsg::GetChannelCommand(
      CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_PRIVATE));
  int nLTrim = 0;
  if (strChat.find(CCharMsg::GetChannelCommand(
          CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_WORLD))) == 0 ||
      strChat.find(CCharMsg::GetChannelCommand(
          CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_TRADE))) == 0 ||
      strChat.find(CCharMsg::GetChannelCommand(
          CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_TEAM))) == 0 ||
      strChat.find(CCharMsg::GetChannelCommand(
          CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_GUILD))) == 0) {
    nLTrim = 1;
  ...
  if (key == VK_PRIOR || key == VK_NEXT) {
    ...
    if (!pThis->m_bSendMsgCardSwitch) {
      if (pThis->m_cSendMsgCard.MoveToLastCard()) {
        ...
        pThis->m_edtMsg->SetCaption(
            pThis->m_cSendMsgCard.GetCardInfo().c_str());
        return true;
      }
    } else {
      if (key == VK_UP) {
...
void CCozeForm::EventMainListKeyDown(CGuiData *pSender, int x, int y,
                                     DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
  if (key & Mouse_RDown) {
...
void CCozeForm::EventSendChannelSwitchClick(CGuiData *pSender, int x, int y,
                                            DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
void CCozeForm::EventSendChannelChange(CGuiData *pSender) {
  ...
  pThis->m_edtMsg->SetCaption(
      CCharMsg::GetChannelCommand(
          CCharMsg::GetChannelIndex(pThis->m_eCurSelChannel))
          .c_str());
}

void CCozeForm::EventMainPageDragBegin(CGuiData *pSender, int x, int y,
                                       DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
void CCozeForm::EventMainPageDragging(CGuiData *pSender, int x, int y,
                                      DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
  ...
  if (m_nMainPageMinHeight <= nHeight && nHeight <= m_nMainPageMaxHeight) {
    ...
    pList->Init();
    pList->Refresh();
    pList->GetItems()->SetFirstShowRow(pList->GetItems()->GetCount() -
                                       pList->GetItems()->GetShowCount());
    pList->Refresh();
    ...
    pList->SetPos(pList->GetLeft(),
                  pThis->m_nTopBeforeDrag + nDragTop - pList->GetHeight());
    pList->Init();
    pList->Refresh();
    pList->GetItems()->SetFirstShowRow(pList->GetItems()->GetCount() -
                                       pList->GetItems()->GetShowCount());
...
void CCozeForm::EventMainPageDragEnd(CGuiData *pSender, int x, int y,
                                     DWORD key) {
...
void CCozeForm::EventSystemPageDragBegin(CGuiData *pSender, int x, int y,
                                         DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
void CCozeForm::EventSystemPageDragging(CGuiData *pSender, int x, int y,
                                        DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
void CCozeForm::EventSystemPageDragEnd(CGuiData *pSender, int x, int y,
                                       DWORD key) {
...
void CCozeForm::EventCallingCardSwitchClick(CGuiData *pSender, int x, int y,
                                            DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
  ...
  CItemRow *pRow =
      pThis->m_lstCallingCard->Add(RES_STRING(CL_LANGUAGE_MATCH_515));
...
void CCozeForm::EventFacePanelSwitchClick(CGuiData *pSender, int x, int y,
                                          DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
//	CCozeForm *pThis=CCozeForm::GetInstance();
//	pThis->m_grdFacePanel->SetIsShow(false);
//	pThis->m_grdFacePanel->MouseRun(pThis->m_grdFacePanel->GetX()-1,
//pThis->m_grdFacePanel->GetY()-1, 0);
//}
...
void CCozeForm::EventFaceSelected(CGuiData *pSender) {
  ...
  if ((int)strlen(pThis->m_edtMsg->GetCaption()) >
      pThis->m_edtMsg->GetMaxNum() - 3) {
    return;
  }
  ...
  if (pThis->m_grdFacePanel->GetSelect()) {
    ...
    char lpszFace[10];
    _snprintf_s(lpszFace, sizeof(lpszFace), _TRUNCATE, "#%02d",
                pThis->m_grdFacePanel->GetSelectIndex());
    pThis->m_edtMsg->ReplaceSel(lpszFace);
  }
}

void CCozeForm::EventBrowPanelSwitchClick(CGuiData *pSender, int x, int y,
                                          DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
...
void CCozeForm::EventActionPanelSwitchClick(CGuiData *pSender, int x, int y,
                                            DWORD key) {
  CCozeForm *pThis = CCozeForm::GetInstance();
