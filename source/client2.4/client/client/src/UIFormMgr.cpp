...
bool CFormMgr::_IsDrawBackgroundInDebugMode =
    true; // 仅用于调试模式 -added by Arcol

CFormMgr::CFormMgr()
    : _bEnabled(false), _bInit(false), _nEnableHotKey(0xFFFFFFFF),
      _nTempleteNo(-1), _nMouseHover(0), _pHintGui(NULL) // 西门文档修改
{
...
CFormMgr::~CFormMgr() {
  ...
  for (frmtemplete::iterator it = _showforms.begin(); it != _showforms.end();
       ++it) {
    if (*it)
      delete *it;
  }
...
void CFormMgr::_DelForm(vfrm &list, CForm *frm) { list.remove(frm); }
...
void CFormMgr::FrameMove(int x, int y, DWORD dwMouseKey, DWORD dwTime) {
  ...
  if (dwTime >= dwNextTime) {
    ...
    for (vfrm::reverse_iterator rit = _show.rbegin(); rit != _show.rend();
         ++rit) {
  ...
  // else if( _eMouseAction!=enumMA_None && _modal.empty() ) 模态下也显示HINT
  else if (_eMouseAction != enumMA_None) {
    _nMouseHover++;
...
bool CFormMgr::_MouseRun(int x, int y, DWORD mouse) {
  ...
  if (!_OnMouseRun.empty()) {
    for (vmouses::iterator it = _OnMouseRun.begin(); it != _OnMouseRun.end();
         ++it)
      (*it)(x, y, mouse);
  }
...
CForm *CFormMgr::GetHitForm(int x, int y) {
  ...
  //	模态窗口计算	Rain Add.
  for (vfrm::reverse_iterator it_modal = _modal.rbegin();
       it_modal != _modal.rend(); ++it_modal) {
    vfrm &childs = (*it_modal)->_childs;
    if (!childs.empty()) {
      for (vfrm::iterator ch_modal = childs.begin(); ch_modal != childs.end();
           ch_modal++)
        if ((*ch_modal)->GetIsShow() && (*ch_modal)->InRect(x, y))
          return (*ch_modal);
    }

    if ((*it_modal)->GetIsShow() && (*it_modal)->InRect(x, y)) {
      return (*it_modal);
    }
  }
...
bool CFormMgr::MouseScroll(int nScroll) {
  ...
  if (!_OnMouseScroll.empty()) {
    for (vscrolls::iterator it = _OnMouseScroll.begin();
         it != _OnMouseScroll.end(); ++it)
      if ((*it)(nScroll))
        return true;
  }
  ...
  case WM_HOTKEY: {
    if (LOWORD(dwParam2) == MOD_ALT) {
      ...
      for (vfrm::const_iterator it = _forms->begin(); it != end; ++it) {
        if ((*it)->GetHotKey() > 0 &&
            (*it)->GetHotKey() == (UINT)HIWORD(dwParam2)) {
          if ((*it)->GetIsShow())
            (*it)->Hide();
          else
            (*it)->Show();
...
bool CFormMgr::AddMouseScrollEvent(MouseScrollEvent event) {
  vscrolls::iterator it =
      find(_OnMouseScroll.begin(), _OnMouseScroll.end(), event);
...
void CFormMgr::ShowDebugInfo() {
  ...
  int data0 = 0, data1 = 0, data2 = 0, data3 = 0, data4 = 0, data5 = 0,
      data6 = 0, data7 = 0, totalTick = 0;
  ...
  for (it = _forms->begin(); it != _forms->end(); it++, i++) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "(%4d,%4d)-(%4d,%4d)",
                pForm->GetLeft(), pForm->GetTop(), pForm->GetRight(),
                pForm->GetBottom());
    pInfo[i].coordinate = buf;
    pInfo[i].totalCompents = (int)pForm->_allCompents.size();
    _snprintf_s(temp, _countof(temp), _TRUNCATE, "%c", pForm->GetHotKey());
  ...
  for (it = _show.begin(); it != _show.end(); it++) {
    for (i = 0; i < (int)_forms->size(); i++) {
      if (pInfo[i].form == (*it)) {
        ...
        totalTick += tick;
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%d", tick);
    ...
    if (_IsDrawFrameInDebugMode) {
      GetRender().LineFrame((*it)->GetLeft(), (*it)->GetTop(),
                            (*it)->GetRight(), (*it)->GetBottom(), frameColor);
      CGuiFont::s_Font.Render((*it)->GetName(), (*it)->GetLeft(),
                              (*it)->GetTop(), ~frameColor);
    }
  }
  ...
  // CGuiFont::s_Font.Render("窗口ID       窗口名称      显示  禁止 坐标 父对象
  // 控件数    层",0,16,COLOR_WHITE);
  CGuiFont::s_Font.Render(RES_STRING(CL_LANGUAGE_MATCH_575), 0, 16,
                          COLOR_WHITE);
  for (i = 0; i < (int)_forms->size(); i++) {
    // pForm = *it;
    // pInfo[i].formName=pForm->GetName();
    ...
    //"(%4d,%4d)-(%4d,%4d)",pForm->GetLeft(),pForm->GetTop(),pForm->GetRight(),pForm->GetBottom());
    // pInfo[i].coordinate=buf;
    ...
    //_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%-6d %-20s %s     %s
    //(%4d,%4d)-(%4d,%4d) %-15s
    //%3d",i,pForm->GetName(),pForm->GetIsShow()?"Y":"N",pForm->GetIsEnabled()?"N":"Y",pForm->GetLeft(),pForm->GetTop(),pForm->GetRight(),pForm->GetBottom(),
    //	pForm->GetParent()?pForm->GetParent()->GetName():"没有父对象",pForm->_allCompents.size());

    DWORD color = COLOR_WHITE;
    //"窗口ID       窗口名称          坐标            显示  允许  菜单   层
    //控件数    热键     渲染时间"
    /*_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%-6d %-20s %s   %s     %s
       %s     %3s    %3d    %2s
       %4s",i,pInfo[i].formName.c_str(),pInfo[i].coordinate.c_str(),pInfo[i].isShow.c_str(),pInfo[i].isEnable.c_str(),pInfo[i].hasPopMenu.c_str(),(pInfo[i].layer>=0)?itoa(pInfo[i].layer,temp,10):"N/A",
            pInfo[i].totalCompents,pInfo[i].hotkey.c_str(),pInfo[i].renderTime.c_str());*/
    _itoa_s(pInfo[i].layer, temp, sizeof(temp), 10);
    _snprintf_s(
        buf, _countof(buf), _TRUNCATE,
        "%-6d %-20s %s   %s     %s     %s     %3s    %3d    %2s     %4s", i,
        pInfo[i].formName.c_str(), pInfo[i].coordinate.c_str(),
        pInfo[i].isShow.c_str(), pInfo[i].isEnable.c_str(),
        pInfo[i].hasPopMenu.c_str(), (pInfo[i].layer >= 0) ? temp : "N/A",
        pInfo[i].totalCompents, pInfo[i].hotkey.c_str(),
        pInfo[i].renderTime.c_str());
  ...
  _snprintf_s(buf, _countof(buf), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_576),
