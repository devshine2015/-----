...
CScript::CScript() {
  ...
  LG("error",
     "msgCScript::CScript Error, dwCount: %d, dwFreeCount: %d, dwLastFree: %d",
...
CScript::~CScript() {
  if (_dwScriptID > _dwCount)
    LG("error",
       "msgCScript::~CScript Error, dwCount: %d, dwFreeCount: %d, dwLastFree: "
       "%d",
       _dwCount, _dwFreeCount, _dwLastFree);
...
bool CScriptMgr::Init() {
  ...
  // char type[6] = "char*";
  // int ret = CLU_RegisterFunction("GetResString", type, "char*", CLU_CDECL,
  // CLU_CAST(Lua_GetResString));
  ...
  while (*szFormat) {
    ...
    case 'u':
      ...
      *va_arg(vl, unsigned int *) =
          (unsigned int)lua_tonumber(_pLuaState, nres);
      break;
    case 's':
