...
bool CForgeMoveMgr::Init() {
  ...
  cmdRequirement[1] =
      static_cast<COneCommand *>(frmforgeMove->Find("cmdStore1"));
  cmdRequirement[2] =
      static_cast<COneCommand *>(frmforgeMove->Find("cmdEquip2"));
  ...
  CTextButton *btnHelp =
      static_cast<CTextButton *>(frmforgeMove->Find("btnHelp"));
  if (!btnHelp)
    return false;
...
void CForgeMoveMgr::PushItem(int Index, CItemCommand &Item) {
  CItemCommand *pItemCommand =
      static_cast<CItemCommand *>(cmdRequirement[Index]->GetCommand());
  if (pItemCommand)
    PopItem(Index);
...
void CForgeMoveMgr::PopItem(int Index) {
  ...
  CCommandObj *pCommandObj =
      g_stUIEquip.GetGoodsGrid()->GetItem(RequirementGridIDs[Index]);
  if (pCommandObj)
...
void CForgeMoveMgr::ForgeMoveResponse(bool Success) {
  ...
  if (!Success) {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_984));
...
void CForgeMoveMgr::evtRequirementEvent(CGuiData *pSender, CCommandObj *pItem,
                                        bool &isAccept) {
  isAccept = false;
  ...
  if (pCmd->nTag != 999) {
    ...
    if (pCommandObj->GetTotalNum() > 1) {
      stNumBox *pNumBox = g_stUIBox.ShowNumberBox(evtThrowDialogEvent,
                                                  pCommandObj->GetTotalNum());
      if (pNumBox) {
    ...
    if (pCmd == cmdRequirement[0]) {
      if (pItemRecord->sType < 5 || pItemRecord->sType == 7 ||
          pItemRecord->sType == 9 || pItemRecord->sType == 11 ||
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_981));
        g_pGameApp->MsgBox(buf);
      }
      ...
      if (pItemRecord->lID == 6835) {
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_982));
        g_pGameApp->MsgBox(buf);
      }
      ...
      if (pItemRecord->sType < 5 || pItemRecord->sType == 7 ||
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_983));
        g_pGameApp->MsgBox(buf);
      }
    }
  }
...
void CForgeMoveMgr::evtForgeMove(CGuiData *pSender, int x, int y, DWORD key) {
  ...
  /*	if( !cmdRequirement[1]->GetCommand() )
     ...
     CL_LANGUAGE_MATCH_982 ) ); g_pGameApp->MsgBox( buf ); return;
          }*/
  ...
  for (int i = 0; i < REQUIRED_ITEM_COUNT; ++i) {
    kNetItemForgeAsk.SGroup[i].sCellNum =
        RequirementCounts[i] ? 1 : 0; // 始终是1
...
void CForgeMoveMgr::evtThrowDialogEvent(CCompent *pSender, int nMsgType, int x,
                                        int y, DWORD dwKey) {
...
void CForgeMoveMgr::PutRequirementEvent(ItemputStruct *pItemput,
                                        int ItemCount) {
  char buf[256] = {0};
...
void CForgeMoveMgr::evtShowHelpInfo(CGuiData *pSender, int x, int y,
                                    DWORD key) {
