...
CScroll::CScroll(CForm &frmOwn)
    : CCompent(frmOwn), _eStyle(btVertical), evtChange(NULL), _fPageNum(2.0),
      _IsAutoHide(true) {
...
CScroll::CScroll(const CScroll &rhs)
    : CCompent(rhs), _up(new CTextButton(*rhs._up)),
      _down(new CTextButton(*rhs._down)),
      _pScroll(new CDragTitle(*rhs._pScroll)),
...
void CScroll::Init() {
  ...
  case btVertical:
    ...
    _step.fLen = (float)(GetHeight() - _up->GetHeight() - _down->GetHeight() -
                         _pScroll->GetHeight());
  ...
  case btHorizontal:
    ...
    _step.fLen = (float)(GetWidth() - _up->GetWidth() - _down->GetWidth() -
                         _pScroll->GetWidth());
...
void CScroll::RenderHint(int x, int y) {
  char buf[80] = {0};
  if (_strHint.empty()) {
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%.02f/%.02f/%.02f",
                _step.GetMin(), _step.GetPosition(), _step.GetMax());
  } else {
    _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%.02f/%.02f/%.02f",
                _strHint.c_str(), _step.GetMin(), _step.GetPosition(),
                _step.GetMax());
  }
  _RenderHint(buf, x, y);
}
...
bool CScroll::StepMove(int val) { return true; }