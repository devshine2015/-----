...
CRCursor::CRCursor() { _pCursorEff = NULL; }

CRCursor::~CRCursor() { SAFE_DELETE(_pCursorEff); }
...
void CRCursor::MoveTo(D3DXVECTOR3 &vPos) { _vPos = vPos; }
...
void CRCursor::MouseDown(int nButton) {
  if (nButton == 0) {
    ...
    if (!g_pGameApp->GetMainCam()->m_bSelectMode)
      CGameApp::GetCurScene()->CreateEffect(536, _vPos.x * 100, _vPos.y * 100);
  }
}
