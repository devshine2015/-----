...
//
	...
	void CPurifyMgr::_evtDragPurifyOne(CGuiData *pSender,CCommandObj* pItem,bool& isAccept)
	{
		...
		if(g_stUIPurify.GetType() == CPurifyMgr::PURIFY_TYPE)
		{
			...
			//{
			//	g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_831)); // 主要装备不合法，请重新选择
			//	return;
			//}
		}
		// 贝壳
		else if(g_stUIPurify.GetType() == CPurifyMgr::ENERGY_TYPE)
		{
			...
			{
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_852)); // 您放入的不是贝壳，请重新选择
				return;
			}
		}
		// 宝石
		...
		else if(g_stUIPurify.GetType() == CPurifyMgr::REPAIR_OVEN_TYPE)
		{
			...
			{
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_891)); // 您放入的不是生活技能工具，请重新选择
				return;
			}
		}
		else	// 类型不合法
	...
	void CPurifyMgr::_evtDragPurifyTwo(CGuiData *pSender,CCommandObj* pItem,bool& isAccept)
	{
		...
		if(g_stUIPurify.GetType() == CPurifyMgr::PURIFY_TYPE)
		{
			...
			//{
			//	g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_829)); // 提纯装备和主要装备不是同一类型，请重新选择
			//	return;
			//}

			//if(pMainItem->GetData().GetItemLevel() > pItemCommand->GetData().GetItemLevel() + 5)
			//{
			//	g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_830)); // 提纯装备等级不能小于主要装备 5 级，请重新选择
			//	return;
			//}
		}
		// 贝壳
		...
		else if(g_stUIPurify.GetType() == CPurifyMgr::REPAIR_OVEN_TYPE)
		{
			if(NULL == pMainItem)
			{
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_892)); // 请先入放入生活技能工具
				return;
			}

			if(! g_stUIPurify.IsRepairLifeItem(*pItemCommand))
			{
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_893)); // 您放入的不是修补工具，请重新选择
				return;
			}
		}
		// 类型不合法
