...
public:
    ...
    drINodeObject* _model_lxo;
    drIModel* _model_lmo;
    drIItem* _arrow;

    DWORD _cha_num;
    DWORD _act_num;
    drINodePrimitive* _cha_obj[10];
    DWORD _act_obj[4];
    drVector3 _arrow_offset_pos[4];
	IgnoreStruct	ignoreStruct;

	drIItem*	_ArrowMarks[4];
	drVector3	_ArrowMarksOffset[4];
...
public:
    ...
    void Init(drIResourceMgr* res_mgr) { _res_mgr = res_mgr; }
    ...
    int LoadArrowItem(const char* file, const drVector3* offset_pos4);
    void OnMouseMove(int flag, int x, int y);
    int OnLButtonDown(int flag, int x, int y);

	bool	LoadArrowMark( const char* Filename, const drVector3* Offsets );

	int		SetInvisibleArray( bool bGirl, int StartIndex );
};
...
private:
	...
	static const char* GetDescripts(int index);	//四个人物的描述
	static const char* GetCityNames(int index);		//城市名
	static const char* GetCitiesDescripts(int index);	//城市描述
