...
CPackFile::~CPackFile(void) { Clear(); }

bool CPackFile::SaveToPack(char *pszDirName, char *pszOutFile,
                           char *pszFilter) {
  FILE *ptempfile;
  fopen_s(&ptempfile, "temp.tmp", "wb"); // 打开可读写文件
  if (!ptempfile)
    return false;
...
bool CPackFile::PackDirectory(FILE *ptf, DirectoryData *pParentDir,
                              char *pszDir) {
  DirectoryData *ptDir;
  if (!pParentDir) {
    ptDir = &_RootDir;
  ...
  _snprintf_s(t_Path, _countof(t_Path), _TRUNCATE, "%s\\%s", pszDir,
              strFilter.c_str());
  ...
  do {
    if (t_sfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      _snprintf_s(t_FilePath, _countof(t_FilePath), _TRUNCATE, "%s\\%s", pszDir,
                  t_sfd.cFileName);
...
long CPackFile::GetFileDataSize(FileData *pFileData) {
  long size = sizeof(DWORD) * 2 + (long)pFileData->strFileName.length() + 1;
...
long CPackFile::GetDirectorySize(DirectoryData *pDir) {
  ...
  size_t size = sizeof(DWORD) * 2 + ptDir->strDirName.length() + 1;
...
bool CMiniPack::Init(char *pszMapName) {
  ...
  _snprintf_s(pszPath, _countof(pszPath), _TRUNCATE,
              "texture\\minimap\\%s\\%s.pk", pszMapName, pszMapName);
  ...
  for (n = 0; n < _RootDir.dwFileNum; n++) {
    GetXY((char *)_RootDir.vecFile[n].strFileName.c_str(), vecIdx[n].x,
          vecIdx[n].y);
