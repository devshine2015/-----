...
// CItemCommand::CItemCommand( CItemRecord* pItem )//modified guojiangang
// 20090108
CItemCommand::CItemCommand(CItemRecord *pItem, bool isIconInStore)
    : _pItem(pItem), _dwColor(COLOR_WHITE), _pBoatHint(NULL) {
  if (!_pItem)
    LG("error",
       "msgCItemCommand::CItemCommand(CItemRecord* pItem) pItem is NULL");
...
void CItemCommand::SaleRender(int x, int y, int nWidth, int nHeight) {
  ...
  if (w > nWidth) {
    ...
    CGuiFont::s_Font.Render(szBuf1, x + (nWidth - w) / 2, nY - h - h - 2,
                            COLOR_BLACK);

    CGuiFont::s_Font.GetSize(szBuf2, w, h);
    CGuiFont::s_Font.Render(szBuf2, x + (nWidth - w) / 2, nY - h - 2,
                            COLOR_BLACK);
  } else {
    CGuiFont::s_Font.Render(_pItem->szName, x + (nWidth - w) / 2, nY - h - 2,
                            COLOR_BLACK);
  }

  _snprintf_s(buf, _countof(buf), _TRUNCATE, "$%s", StringSplitNum(_nPrice));
  CGuiFont::s_Font.GetSize(buf, w, h);
  CGuiFont::s_Font.Render(buf, x + (nWidth - w) / 2, nY + ITEM_HEIGHT + 2,
                          COLOR_BLACK);
}
...
void CItemCommand::Render(int x, int y) {
  ...
  if (_ItemData.dwDBID) {
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_961));
    static int w, h;
    CGuiFont::s_Font.GetSize(buf, w, h);
...
void CItemCommand::OwnDefRender(int x, int y, int nWidth, int nHeight) {
  ...
  CGuiFont::s_Font.Render(_OwnDefText.c_str(), x + (nWidth - w) / 2,
                          nY + ITEM_HEIGHT + 2, COLOR_BLACK);
}
...
void CItemCommand::AddHint(int x, int y, int index) {
  ...
  if (GetParent()) {
    ...
    if (name == "frmPlayertrade" || name == "frmItem" ||
        name == "frmNPCstorage" || name == "frmTempBag" || name == "frmBreak" ||
        name == "frmCooking" || name == "frmFound" || name == "frmBreak" ||
        name == "frmStore" || name == "frmViewAll" ||
        name == "frmSpiritMarry" || name == "frmSpiritErnie" ||
        name == "frmEquipPurify") {
      isMain = true;
    }
  }
  ...
  if (CItemRecord::IsVaildFusionID(_pItem) &&
      _ItemData.GetFusionItemID() > 0) // end
  {                                    //
  ...
  if (_pItem->sType >= 1 &&
      _pItem->sType <= 10) // 武器:剑,巨剑,弓,火绳枪,刀,拳套,匕首,钱袋,短棒,锤子
  {
    if (_pItem->sType == 2) {
      ...
      if (CItemRecord::IsVaildFusionID(_pItem) &&
          _ItemData.GetFusionItemID() > 0) // modify by ning.yan  20080821
      {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "Lv%d %s",
                    _ItemData.GetItemLevel(), GetName());
      } else {
    ...
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_625),
                  _GetValue(ITEMATTR_VAL_MNATK, item),
    ...
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_626), item.sEndure[0],
                  item.sEndure[1]);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
    }
    ...
    {
      _snprintf_s(
          buf, _countof(buf), _TRUNCATE, RES_STRING(CL_LANGUAGE_MATCH_627),
          _ItemData.GetItemLevel() * 2 + 80); // 发挥度定义 0级-80% 1级-82% ...
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
    }
        ...
        _ItemData.GetFusionItemID() > 0) // modify by ning.yan  20080821
    {
      if (_pItem->sNeedLv > pEquipItem->sNeedLv) {
        PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_628), _pItem->sNeedLv,
                  pAttr->get(ATTR_LV) >= _pItem->sNeedLv ? GENERIC_COLOR
                                                         : VALID_COLOR,
                  index);
      } else {
        PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_628), pEquipItem->sNeedLv,
                  pAttr->get(ATTR_LV) >= pEquipItem->sNeedLv ? GENERIC_COLOR
                                                             : VALID_COLOR,
                  index);
      }
    } else {
  ...
  } else if (_pItem->sType == 22 || _pItem->sType == 11 ||
             _pItem->sType == 27) // 衣服,盾,图腾
  {
  ...
  } else if (_pItem->sType == 25) // 项链
  {
    ...
    if (_pItem->lID == 6386 || _pItem->lID == 6387 || _pItem->lID == 6388 ||
        _pItem->lID == 6389 || _pItem->lID == 6390 || _pItem->lID == 6391 ||
        _pItem->lID == 6434 || _pItem->lID == 6428 || _pItem->lID == 6429 ||
        _pItem->lID == 6430 || _pItem->lID == 6431 || _pItem->lID == 6432) {
  ...
  } else if (_pItem->sType == 26) // 戒指
  {
    ...
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_862),
                  _ItemData.sEndure[0] * 10 - 1000,
                  _ItemData.sEndure[1] * 10 - 1000); // 具体计算方式询问策划
      PushHint(buf, COLOR_WHITE, 5, 1, index);
  ...
  } else if (_pItem->sType == 20) // 头发
  {
    ...
    // 已经熔合 =>
    // 取熔合之前两件装备的最高等级，两件装备的角色交集，两件装备的职业交集
    // 没有熔合 => 直接取当前装备的等级，角色，职业
    if (CItemRecord::IsVaildFusionID(_pItem) &&
        _ItemData.GetFusionItemID() > 0) // modify by ning.yan  20080821
    {
      short nFusionItemLevel = _pItem->sNeedLv > pEquipItem->sNeedLv
                                   ? _pItem->sNeedLv
                                   : pEquipItem->sNeedLv;
      PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_628), nFusionItemLevel,
                pAttr->get(ATTR_LV) >= nFusionItemLevel ? GENERIC_COLOR
                                                        : VALID_COLOR,
                index);
  ...
  } else if (_pItem->sType >= 31 &&
             _pItem->sType <= 33) // 使用品:恢复类,加成类,特殊效果类
  {
    ...
    if (5786 == _pItem->lID || 5787 == _pItem->lID || 5788 == _pItem->lID ||
        5789 == _pItem->lID || 6444 == _pItem->lID) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_644), item.sEndure[0],
                  item.sEndure[1]);
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    }
    // End
    ...
    if (_ItemData.sNum > 0) {
      AddHintHeight(6, index);
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_633), _ItemData.sNum);
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    }
  ...
  } else if (_pItem->sType == 18 || _pItem->sType == 19) // 斧头,铁镐
  {
    ...
    if (_pItem->nID == 3908 || _pItem->nID == 3108 || _pItem->nID == 6538 ||
        _pItem->nID == 6539 || _pItem->nID == 6585 || _pItem->nID == 6613 ||
        _pItem->nID == 6917 ||
        _pItem->nID == 6944) // 3108 add by Philip  2005-05-30
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_626), item.sEndure[0],
                  item.sEndure[1]);
  ...
  } else if (_pItem->sType == 43) // 船长证明
  {
    ...
    if (_pBoatHint) {
      ...
      if (pInfo) {
        ...
        if (nNeedLv > 0) {
          _snprintf_s(buf, _countof(buf), _TRUNCATE,
                      RES_STRING(CL_LANGUAGE_MATCH_628), nNeedLv);
          PushHint(buf,
                   g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_LV) >= nNeedLv
                       ? GENERIC_COLOR
                       : VALID_COLOR,
                   5, 1, index);
        }
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_634), _pBoatHint->sLevel);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00014), _pBoatHint->dwExp);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      AddHintHeight(6, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_636), _pBoatHint->dwHp,
                  (int)(_pBoatHint->dwMaxHp));
      PushHint(buf, COLOR_WHITE, 5, 1, index);
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_637), _pBoatHint->dwSp,
                  (int)(_pBoatHint->dwMaxSp));
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_638), _pBoatHint->dwMinAttack,
                  (int)(_pBoatHint->dwMaxAttack));
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_639), _pBoatHint->dwDef);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_640), _pBoatHint->dwSpeed);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_641), _pBoatHint->dwShootSpeed);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_642), _pBoatHint->byHasItem,
                  _pBoatHint->byCapacity);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_643),
                  StringSplitNum(_pBoatHint->dwPrice / 2));
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    } else {
      CBoat *pBoat =
          g_stUIBoat.FindBoat(_ItemData.GetDBParam(enumITEMDBP_INST_ID));
      if (pBoat) {
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_634), pAttr->get(ATTR_LV));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        char buff[32];
        _i64toa_s(pAttr->get(ATTR_CEXP), buff, sizeof(buff), 10);
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_635), buff);
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        AddHintHeight(6, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_636), (int)pAttr->get(ATTR_HP),
                    (int)pAttr->get(ATTR_MXHP));
        PushHint(buf, COLOR_WHITE, 5, 1, index);
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_637), (int)pAttr->get(ATTR_SP),
                    (int)pAttr->get(ATTR_MXSP));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_638),
                    (int)pAttr->get(ATTR_BMNATK), (int)pAttr->get(ATTR_BMXATK));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_639), pAttr->get(ATTR_BDEF));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_640), pAttr->get(ATTR_BMSPD));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_641), pAttr->get(ATTR_BASPD));
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        CGoodsGrid *pGoods = pBoat->GetGoodsGrid();
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_642), pGoods->GetCurNum(),
                    pGoods->GetMaxNum());
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_643),
                    StringSplitNum((long)pAttr->get(ATTR_BOAT_PRICE) / 2));
        PushHint(buf, COLOR_WHITE, 5, 1, index);
      }
    }
  ...
  } else if (_pItem->sType == 29) // 贝壳
  {
    ...
    // _snprintf_s( buf, _countof( buf ), _TRUNCATE,
    // RES_STRING(CL_LANGUAGE_MATCH_644), _ItemData.sEnergy[0],
    // _ItemData.sEnergy[1] );
    PushHint(buf, COLOR_WHITE, 5, 1, index);
  ...
  } else if (_pItem->sType == 45) // 贸易证明
  {
    ...
    _snprintf_s( buf, _countof( buf ), _TRUNCATE,
    RES_STRING(CL_LANGUAGE_MATCH_645), fRate ); PushHint( buf, COLOR_WHITE, 5,
    1, index );*/
  ...
  } else if (_pItem->sType == 34) // 技能书
  {
    ...
    if (pSkill) {
      if (pSkill->chJobSelect[0][0] >= 0) {
        ...
        PushHint(str.str(),
                 pSkill->IsJobAllow((int)pAttr->get(ATTR_JOB)) ? GENERIC_COLOR
                                                               : VALID_COLOR,
                 5, 1, index);
      ...
      if (pSkill->sLevelDemand != -1) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_648), pSkill->sLevelDemand);
        PushHint(buf,
                 pAttr->get(ATTR_LV) >= pSkill->sLevelDemand ? GENERIC_COLOR
                                                             : VALID_COLOR,
                 5, 1, index);
      }
      ...
      for (int i = 0; i < defSKILL_PRE_SKILL_NUM; i++) {
        ...
        if (p) {
          ...
          _snprintf_s(buf, _countof(buf), _TRUNCATE,
                      RES_STRING(CL_LANGUAGE_MATCH_649), p->szName,
                      pSkill->sPremissSkill[i][1]);
          if (pSelfSkill &&
              pSelfSkill->GetSkillGrid().chLv >= pSkill->sPremissSkill[i][1])
            PushHint(buf, COLOR_WHITE, 5, 1, index);
          else
  ...
  } else if (_pItem->sType == 46) // 勇者之证
  {
    ...
    // int show_attr[] = { ITEMATTR_VAL_STR, ITEMATTR_VAL_AGI, ITEMATTR_VAL_DEX,
    // ITEMATTR_VAL_CON, ITEMATTR_VAL_STA }; string show_text[] = {
    // RES_STRING(CL_LANGUAGE_MATCH_650), RES_STRING(CL_LANGUAGE_MATCH_651),
    // RES_STRING(CL_LANGUAGE_MATCH_652), RES_STRING(CL_LANGUAGE_MATCH_653),
    // RES_STRING(CL_LANGUAGE_MATCH_654) }; int value = 0; const int count =
    // sizeof(show_attr)/sizeof(show_attr[0]); for( int i=0; i<count; i++ )
    //{
    //	value = item.sInstAttr[show_attr[i]];
    //	item.sInstAttr[show_attr[i]] = 0;
    //	_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%d",
    //show_text[i].c_str(), value ); 	PushHint( buf, GENERIC_COLOR, 5, 1, index
    //);
    //}

    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
    //RES_STRING(CL_LANGUAGE_MATCH_655), _ItemData.sEndure[1] ); PushHint( buf,
    // GENERIC_COLOR, 5, 1, index );

    ////_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%d",
    ///RES_STRING(CL_LANGUAGE_MATCH_848), _ItemData.sEnergy[1] );	//
    ///乱斗点数
    // PushHint( buf, GENERIC_COLOR, 5, 1, index );
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_656),
                ConvertNumToChinese(item.sEnergy[1]).c_str(), _pItem->szName);
    PushHint(buf, COLOR_WHITE, 5, 1, index);
    if (index != -1)
      PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
    PushHint(GetStoneHint(1).c_str(), COLOR_WHITE, 5, 1,
             index); // 单个宝石都显示1级的属性
    _AddDescriptor(index);
    ...
    if (index != -1)
      PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
    _AddDescriptor(index);
  } else if (_pItem->sType == 59 ||
             _pItem->sType == 67) // 59 = 精灵，67 = 精灵蛋
  {
    ...
    PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_657),
              item.sInstAttr[ITEMATTR_VAL_STR], COLOR_WHITE, index);
    PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_658),
              item.sInstAttr[ITEMATTR_VAL_AGI], COLOR_WHITE, index);
    PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_659),
              item.sInstAttr[ITEMATTR_VAL_CON], COLOR_WHITE, index);
    //		PUSH_HINT( RES_STRING(CL_LANGUAGE_MATCH_660),
    //item.sInstAttr[ITEMATTR_VAL_DEX], COLOR_WHITE, index );
    PUSH_HINT(RES_STRING(CL_LANGUAGE_MATCH_661),
              item.sInstAttr[ITEMATTR_VAL_STA], COLOR_WHITE, index);
    ...
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_662), _ItemData.sEndure[0] / 50,
                  _ItemData.sEndure[1] / 50);
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
      //RES_STRING(CL_LANGUAGE_MATCH_663), _ItemData.sEnergy[0],
      //_ItemData.sEnergy[1] ); PushHint( buf, COLOR_WHITE, 5, 1, index );
    }
    ...
    int array[3][2] = {0};
    g_pGameApp->GetScriptMgr()->DoString(
        "GetElfSkill", "u-dddddd", _ItemData.lDBParam[0], &array[0][0],
        &array[0][1], &array[1][0], &array[1][1], &array[2][0], &array[2][1]);
  ...
  } else if (_pItem->sType == 65) // 黑龙祭坛  add by Philip.Wu  2006-06-19
  {
    ...
    if (5724 == _pItem->lID) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00001), item.sEnergy[0] / 60);
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    ...
    //	Add by alfred.shi 20081126	begin
      ...
      if (index != -1)
        PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%d",
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00002),
                  (item.sInstAttr[ITEMATTR_VAL_STR] * 32768 +
                   item.sInstAttr[ITEMATTR_VAL_CON]));
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%d",
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00003),
                  item.sInstAttr[ITEMATTR_VAL_AGI]);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
      //			_snprintf_s( buf, _countof( buf ), _TRUNCATE,
      //"%s:%d",RES_STRING(CL_UIITEMCOMMAND_CPP_00004),item.sInstAttr[ITEMATTR_VAL_DEX]);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
      return;
    }
    //	End
    //	Modify by alfred.shi 20080822
    if (3279 <= _pItem->lID && _pItem->lID <= 3282 || _pItem->lID == 6370 ||
        _pItem->lID == 6371 || _pItem->lID == 6376 || _pItem->lID == 6377 ||
        _pItem->lID == 6378 || (_pItem->lID >= 5882 && _pItem->lID <= 5893) ||
        _pItem->lID == 5895 || _pItem->lID == 5897 ||
        (_pItem->lID >= 6383 && _pItem->lID <= 6385) || _pItem->lID == 6378 ||
        (_pItem->lID >= 6440 &&
         _pItem->lID <= 6443)) // 读书系统特殊处理，暴恶心
    {
      PushHint(_pItem->szName, COLOR_WHITE, 5, 1, index);
      if (index != -1)
        PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
        ...
        _pItem->lID == 3066 || _pItem->lID == 3078) // 任务道具特殊处理
    {
      ...
      if (_pItem->lID == 2911 || _pItem->lID == 2952 || _pItem->lID == 3066 ||
          _pItem->lID == 3078) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                    RES_STRING(CL_LANGUAGE_MATCH_916), nMonth);
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                    RES_STRING(CL_LANGUAGE_MATCH_917), nDay);
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                    RES_STRING(CL_LANGUAGE_MATCH_918), nHour);
        PushHint(buf, COLOR_WHITE, 5, 1, index);

        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                    RES_STRING(CL_LANGUAGE_MATCH_919), nMinute);
        PushHint(buf, COLOR_WHITE, 5, 1, index);
      }

      if (_pItem->lID != 3066 && _pItem->lID != 3078) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                    RES_STRING(CL_LANGUAGE_MATCH_920), nSecond);
        PushHint(buf, COLOR_WHITE, 5, 1, index);
      }
    ...
    } else if (_pItem->lID == 2954) // 特殊道具处理（死亡证明）
    {
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s: %d",
                  RES_STRING(CL_LANGUAGE_MATCH_933), nCount); // "死亡次数"
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    ...
    // Begin
      ...
      if (_pItem->nID == 5803) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%d",
                    RES_STRING(CL_LANGUAGE_MATCH_651),
                    item.sInstAttr[ITEMATTR_VAL_STR]);
      }
      if (_pItem->nID == 6373) {
        int nCount = 0;
        for (int i = 0; i < 5; ++i) {
        ...
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_UIITEMCOMMAND_CPP_00010), nCount);
      }
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_664),
                5 - item.sInstAttr[ITEMATTR_VAL_AGI]);
    PushHint(buf, COLOR_WHITE, 5, 1, index);

    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_665),
                5 - item.sInstAttr[ITEMATTR_VAL_STR]);
    PushHint(buf, COLOR_WHITE, 5, 1, index);

    //		_snprintf_s( buf, _countof( buf ), _TRUNCATE,
    //RES_STRING(CL_LANGUAGE_MATCH_666), 5 - item.sInstAttr[ITEMATTR_VAL_DEX] );
    PushHint(buf, COLOR_WHITE, 5, 1, index);

    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_667),
                5 - item.sInstAttr[ITEMATTR_VAL_CON]);
    PushHint(buf, COLOR_WHITE, 5, 1, index);
    ...
    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
    //RES_STRING(CL_LANGUAGE_MATCH_672), _ItemData.sEnergy[0]); PushHint( buf,
    // COLOR_WHITE, 5, 1, index );
  ...
  } else if (_pItem->sType == 69) // XXX图纸
  {
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_869), _ItemData.sEndure[0]);
    PushHint(buf, GENERIC_COLOR, 5, 1, index);
    ...
    if (iItem) {
      ...
      if (pCItemRec) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_870), pCItemRec->szName);
        PushHint(buf, GENERIC_COLOR, 5, 1, index);
      }
    }

    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
    //RES_STRING(CL_LANGUAGE_MATCH_871), _ItemData.sEnergy[1] - 100);
    // PushHint(buf, GENERIC_COLOR, 5, 1, index );
    ...
    if (iItem) {
      ...
      if (pCItemRec) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_872), pCItemRec->szName,
                    (lForge / 10000000));
        PushHint(buf, GENERIC_COLOR, 5, 1, index);
      }
    }
    ...
    if (iItem) {
      ...
      if (pCItemRec) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_873), pCItemRec->szName,
                    (lForge / 10000));
        PushHint(buf, GENERIC_COLOR, 5, 1, index);
      }
    }
    ...
    if (iItem) {
      ...
      if (pCItemRec) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_874), pCItemRec->szName,
                    (lForge / 10));
        PushHint(buf, GENERIC_COLOR, 5, 1, index);
      }
    }
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_875),
                item.sInstAttr[ITEMATTR_VAL_STA]);
    PushHint(buf, GENERIC_COLOR, 5, 1, index);

    //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
    //RES_STRING(CL_LANGUAGE_MATCH_876), _ItemData.sEnergy[0] * 10);
    // PushHint(buf, GENERIC_COLOR, 5, 1, index );

    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_877), _ItemData.sEndure[1]);
    PushHint(buf, GENERIC_COLOR, 5, 1, index);
    ...
    if (_nPrice != 0) {
      AddHintHeight(6, index);
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_674),
                  StringSplitNum(isMain ? _nPrice / 2 : _nPrice));
      PushHint(buf, COLOR_WHITE, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s",
                  ConvertNumToChinese(isMain ? _nPrice / 2 : _nPrice).c_str());
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    }
    ...
    {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_909),
                  item.sInstAttr[ITEMATTR_VAL_STR]); // "情缘编码：%d"
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
    ...
    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_869),
                item.sInstAttr[ITEMATTR_VAL_STR]);
    PushHint(buf, GENERIC_COLOR, 5, 1, index);

    if (_pItem->lID != 2236) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_878), _ItemData.sEndure[0] / 50);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);

      //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
      //RES_STRING(CL_LANGUAGE_MATCH_897), _ItemData.sEnergy[0]); //
      //"工具经验：%i" PushHint(buf, GENERIC_COLOR, 5, 1, index );
    }
  ...
  } else if (_pItem->sType == 71 && _pItem->lID == 3289) //学生证处理
  {
    ...
    const char *arShowName[5] = {
        RES_STRING(CL_LANGUAGE_MATCH_944), RES_STRING(CL_LANGUAGE_MATCH_945),
        RES_STRING(CL_LANGUAGE_MATCH_946), RES_STRING(CL_LANGUAGE_MATCH_947),
        RES_STRING(CL_LANGUAGE_MATCH_948)}; // "幼儿园", "小学", "初中", "高中",
                                            // "大学" };
    if (0 <= nLevel && nLevel <= 4) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_949),
                  arShowName[nLevel]); // "学历:%s"
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    }

    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_950), item.sEndure[0],
                item.sEndure[1]); // "学分(%d/%d)"
    PushHint(buf, COLOR_WHITE, 5, 1, index);

    _snprintf_s(buf, _countof(buf), _TRUNCATE,
                RES_STRING(CL_LANGUAGE_MATCH_951), item.sEnergy[0] * 1000,
                item.sEnergy[1] * 1000); // "经验(%d/%d)"
    PushHint(buf, COLOR_WHITE, 5, 1, index);
  ...
  //	End.
    ...
    if (_pItem->lID == 58) // 疯女人要求特殊处理，蟹苗
    {
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_626), item.sEnergy[0],
                  item.sEnergy[1]);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
    ...
    } else if (_pItem->lID == 171) // bragi要求特殊处理
    {
      ...
      static const char *pszText[5] = {
          RES_STRING(CL_LANGUAGE_MATCH_952), RES_STRING(CL_LANGUAGE_MATCH_953),
          RES_STRING(CL_LANGUAGE_MATCH_954), RES_STRING(CL_LANGUAGE_MATCH_955),
          RES_STRING(CL_LANGUAGE_MATCH_956)};

      int nIndex = item.sEndure[0];

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_957),
    ...
    else if (_pItem->lID == 6535 ||
             _pItem->lID == 6536) // 李恒要求此两个道具显示耐久
    {
      PushHint(_pItem->szName, COLOR_WHITE, 5, 1, index);
      if (index != -1)
        PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
    ...
    // end
      ...
      if (index != -1)
        PushHint(RES_STRING(CL_LANGUAGE_MATCH_1041), COLOR_WHITE, 5, 1, index);
      _snprintf_s(buf, _countof(buf), RES_STRING(CL_UIITEMCOMMAND_CPP_00014),
                  item.sInstAttr[ITEMATTR_VAL_STR]);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
      ...
      _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s:%d",
                  RES_STRING(CL_LANGUAGE_MATCH_1028), value);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);
  ...
  } else if (_pItem->sType == 75) // 彩票
  {
    ...
    if (floor((float)item.sInstAttr[ITEMATTR_VAL_STR] / 1000) > 0) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00006),
                  item.sInstAttr[ITEMATTR_VAL_STR] % 1000);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);

      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_UIITEMCOMMAND_CPP_00007),
                  (short)floor((float)item.sInstAttr[ITEMATTR_VAL_AGI] / 100),
                  item.sInstAttr[ITEMATTR_VAL_AGI] % 100);
      PushHint(buf, GENERIC_COLOR, 5, 1, index);

      // short c1 = (short)floor( (float)item.sInstAttr[ITEMATTR_VAL_DEX] /
      // 100); short c2 = item.sInstAttr[ITEMATTR_VAL_DEX] % 100;

      // short c3 = (short)floor( (float)item.sInstAttr[ITEMATTR_VAL_CON] /
      // 100); short c4 = item.sInstAttr[ITEMATTR_VAL_CON] % 100;

      // short c5 = (short)floor( (float)item.sInstAttr[ITEMATTR_VAL_STA] /
      // 100); short c6 = item.sInstAttr[ITEMATTR_VAL_STA] % 100;

      ////_snprintf_s( buf, _countof( buf ), _TRUNCATE, "彩球号：%c %c %c %c %c
      ///%c", c1, c2, c3, c4, c5, c6);
      //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
      //RES_STRING(CL_UIITEMCOMMAND_CPP_00008), c1, c2, c3, c4, c5, c6);
      // PushHint( buf, GENERIC_COLOR, 5, 1, index );
    } else
      PushHint(RES_STRING(CL_UIITEMCOMMAND_CPP_00009), GENERIC_COLOR, 5, 1,
               index);
  ...
  } else // 一般道具
  {
    ...
    if (_pItem->sType == 77) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_980), item.sEndure[0],
                  item.sEndure[1]);
      PushHint(buf, COLOR_WHITE, 5, 1, index);
    }
  ...
  if (_pItem->sType == 80) {
    ...
    _TRUNCATE,"装备：%s",sNameInfo->szName); PushHint( buf, showcolor, 5, 1,
    index );
    }
    SetHintIsCenter(true);*/
  }
  ...
  if (_hints.GetCount() > 0 && _pItem->sType <= 27 && _pItem->sType != 18 &&
      _pItem->sType != 19 && _pItem->sType != 21) {
    // 能量最大值的百位代表颜色,十位及以下代表名字前缀
    char szBuf[16] = {0};
    //_snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE, "%09d",
    //_ItemData.sEnergy[1]/10 );		//
    //除十是因为服务器的数值为4位数,等待以后改为三位
    ...
    if (pInfo && strcmp(pInfo->szDataName, "0") != 0) {
      ...
      {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "Lv%d %s%s",
                    _ItemData.GetItemLevel(), pInfo->szDataName, GetName());
        ...
        if (index == -1) {
          if (_ItemData.dwDBID)
            _hints.GetHint(1)->hint =
                pInfo->szDataName + _hints.GetHint(1)->hint;
          else
            _hints.GetHint(0)->hint =
                pInfo->szDataName + _hints.GetHint(0)->hint;
        } else {
          if (_ItemData.dwDBID)
            _hints.GetHintRelated(index, 2)->hint =
                pInfo->szDataName + _hints.GetHintRelated(index, 2)->hint;
          else
            _hints.GetHintRelated(index, 0)->hint =
                pInfo->szDataName + _hints.GetHintRelated(index, 0)->hint;
        }
      }
    }
  ...
  if (_hints.GetCount() > 0 && Forge.IsForge) {
    if (Forge.nHoleNum > 0) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_673), Forge.nHoleNum);
    ...
    for (int i = 0; i < Forge.nStoneNum && i < Forge.nHoleNum; i++) {
      _snprintf_s(buf, _countof(buf), _TRUNCATE,
                  RES_STRING(CL_LANGUAGE_MATCH_656),
                  ConvertNumToChinese(Forge.nStoneLevel[i]).c_str(),
    ...
    if (Forge.nStoneNum > 0) {
      AddHintHeight(6, index);
      for (int i = 0; i < Forge.nStoneNum && i < Forge.nHoleNum; i++) {
        PushHint(Forge.szStoneHint[i], ADVANCED_COLOR, 5, 1, index);
      }
    }
    ...
    if (Forge.nLevel > 0) {
      if (index == -1) {
        if (_ItemData.dwDBID) {
          _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s +%d",
                      _hints.GetHint(1)->hint.c_str(), Forge.nLevel);
          _hints.GetHint(1)->hint = buf;
        } else {
          _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s +%d",
                      _hints.GetHint(0)->hint.c_str(), Forge.nLevel);
          _hints.GetHint(0)->hint = buf;
        }
      } else {
        if (_ItemData.dwDBID) {
          _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s +%d",
                      _hints.GetHintRelated(index, 2)->hint.c_str(),
                      Forge.nLevel);
          _hints.GetHintRelated(index, 2)->hint = buf;
        } else {
          _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s +%d",
                      _hints.GetHintRelated(index, 0)->hint.c_str(),
                      Forge.nLevel);
          _hints.GetHintRelated(index, 0)->hint = buf;
        }
      }
...
void CItemCommand::AddRelatedHint(int x, int y) {
  ...
  if (ParentName == "frmItem") {
    for (int i = 0; i < g_stUIEquip.GetGoodsGrid()->GetMaxNum(); ++i) {
      CItemCommand *pItemCommand =
          dynamic_cast<CItemCommand *>(g_stUIEquip.GetGoodsGrid()->GetItem(i));
      if (pItemCommand == this)
        track_related = true;
    }
  } else if (ParentName == "frmNPCtrade") {
    for (int i = 0; i < g_stUINpcTrade.GetNPCtradeWeaponGrid()->GetMaxNum();
         ++i) {
      CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
          g_stUINpcTrade.GetNPCtradeWeaponGrid()->GetItem(i));
      if (pItemCommand == this)
        track_related = true;
    }

    if (!track_related) {
      for (int i = 0; i < g_stUINpcTrade.GetNPCtradeEquipGrid()->GetMaxNum();
           ++i) {
        CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
            g_stUINpcTrade.GetNPCtradeEquipGrid()->GetItem(i));
        if (pItemCommand == this)
          track_related = true;
      }
    }
    ...
    if (!track_related) {
      for (int i = 0; i < g_stUINpcTrade.GetNPCtradeOtherGrid()->GetMaxNum();
           ++i) {
        CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
            g_stUINpcTrade.GetNPCtradeOtherGrid()->GetItem(i));
        if (pItemCommand == this)
          track_related = true;
      }
    }
    ...
    for (int i = 0; i < g_stUIBooth.GetBoothItemsGrid()->GetMaxNum(); ++i) {
      CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
          g_stUIBooth.GetBoothItemsGrid()->GetItem(i));
      if (pItemCommand == this)
        track_related = true;
    }
    ...
    for (int i = 0; i < g_stUIBank.GetBankGoodsGrid()->GetMaxNum(); ++i) {
      CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
          g_stUIBank.GetBankGoodsGrid()->GetItem(i));
      if (pItemCommand == this)
        track_related = true;
    }
  } else if (ParentName == "frmPlayertrade") {
    for (int i = 0; i < g_stUITrade.GetPlayertradeSaleGrid()->GetMaxNum();
         ++i) {
      CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
          g_stUITrade.GetPlayertradeSaleGrid()->GetItem(i));
      if (pItemCommand == this)
        track_related = true;
    }

    if (!track_related) {
      for (int i = 0; i < g_stUITrade.GetPlayertradeBuyGrid()->GetMaxNum();
           ++i) {
        CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
            g_stUITrade.GetPlayertradeBuyGrid()->GetItem(i));
        if (pItemCommand == this)
          track_related = true;
      }
    }
    ...
    if (!track_related) {
      for (int i = 0; i < g_stUITrade.GetRequestGrid()->GetMaxNum(); ++i) {
        CItemCommand *pItemCommand = dynamic_cast<CItemCommand *>(
            g_stUITrade.GetRequestGrid()->GetItem(i));
        if (pItemCommand == this)
          track_related = true;
      }
    }
  ...
  if (track_related) {
    ...
    for (int i = 0; i < enumEQUIP_NUM; ++i) {
      ...
      CItemCommand *pItem = dynamic_cast<CItemCommand *>(
          g_stUIEquip.cnmEquip[Link]->GetCommand());
      if (pItem) {
        ...
        if ( CItemRecord::IsVaildFusionID( pItemRecord ) &&
        pItem->_ItemData.GetFusionItemID() > 0 )
        {
                pRecord = GetItemRecordInfo( pItem->_ItemData.GetFusionItemID()
        ); if( pRecord ) item.Convert( pItem->_ItemData, pRecord );
        }
        else
                item.Convert( pItem->_ItemData, pItemRecord );

        if( CItemRecord::IsVaildFusionID( pItemRecord ) &&
        pItem->_ItemData.GetFusionItemID() > 0 ) _snprintf_s( buf, _countof( buf
        ), _TRUNCATE, "Lv%d %s", pItem->_ItemData.GetItemLevel(),
        pItem->GetName() ); else _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        "%s", pItem->GetName() );

        char szBuf[16] = { 0 };
        _snprintf_s( szBuf, _countof( szBuf ), _TRUNCATE, "%09d",
        pItem->_ItemData.sEnergy[1]/10 );		//
        除十是因为服务器的数值为4位数,等待以后改为三位 char szHundred[2] = {
        szBuf[6], 0 }; int nHundred = atoi( szHundred ); DWORD dwNameColor =
        COLOR_BLACK; switch( nHundred )
        {
        ...
        ); if( pItemRecord->sType == 2 ) PushHint( RES_STRING(
        CL_LANGUAGE_MATCH_624 ), COLOR_WHITE, 5, 1, index );
        ...
        if( pItemRecord->sType == 26 )
        {
                ...
                {
                        _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING( CL_LANGUAGE_MATCH_862 ), pItem->_ItemData.sEndure[0] * 10 -
        1000, pItem->_ItemData.sEndure[1] * 10 - 1000 ); PushHint( buf,
        COLOR_WHITE, 5, 1, index );
        ...
        if( pItemRecord->sType >= 1 && pItemRecord->sType <= 10 )
        {
                ...
                {
                        _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING( CL_LANGUAGE_MATCH_625 ), _GetValue( ITEMATTR_VAL_MNATK, item
        ), _GetValue( ITEMATTR_VAL_MXATK, item ) ); PushHint( buf, COLOR_WHITE,
        5, 1, index );
                }
        }
        ...
        if( item.sEndure[1] > 0 )
        {
                _snprintf_s( buf, _countof( buf ), _TRUNCATE, RES_STRING(
        CL_LANGUAGE_MATCH_626 ), item.sEndure[0], item.sEndure[1] ); PushHint(
        buf, COLOR_WHITE, 5, 1, index );
        }

        _PushValue( RES_STRING( CL_LANGUAGE_MATCH_630 ), ITEMATTR_VAL_PDEF,
        item, COLOR_WHITE, index ); _PushValue( RES_STRING(
        CL_LANGUAGE_MATCH_631 ), ITEMATTR_VAL_HIT, item, COLOR_WHITE, index );

        if( ( pItemRecord->sType >= 1 && pItemRecord->sType <= 11 ) ||
        pItemRecord->sType == 18 || pItemRecord->sType == 19 ||
        pItemRecord->sType == 20 || pItemRecord->sType == 22 ||
                pItemRecord->sType == 23 || pItemRecord->sType == 24 ||
        pItemRecord->sType == 27  )
        {
                if ( CItemRecord::IsVaildFusionID( pItemRecord ) &&
        pItem->_ItemData.GetFusionItemID() > 0 )
                {
                        _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING( CL_LANGUAGE_MATCH_627 ), pItem->_ItemData.GetItemLevel() * 2
        + 80 ); PushHint( buf, COLOR_WHITE, 5, 1, index );
                }
        }
        ...
        pItem->_ItemData.GetFusionItemID() > 0 )
        {
                short nFusionItemLevel = pItemRecord->sNeedLv > pRecord->sNeedLv
        ? pItemRecord->sNeedLv : pRecord->sNeedLv; PUSH_HINT( RES_STRING(
        CL_LANGUAGE_MATCH_628 ), nFusionItemLevel, COLOR_WHITE, index );
        }
        else
                PUSH_HINT( RES_STRING( CL_LANGUAGE_MATCH_628 ),
        pItemRecord->sNeedLv, COLOR_WHITE, index );
        ...
        pItem->_ItemData.GetFusionItemID() > 0 )
                {
                        ...
                        pItem->_ShowFusionWork( pItemRecord, pRecord, pAttr,
        index );
                }
                else
                {
        ...
        if( pItemRecord->lID != 6959 )
        {
                ...
                _PushValue( RES_STRING( CL_LANGUAGE_MATCH_632 ),
        ITEMATTR_VAL_FLEE, item, ADVANCED_COLOR, index );

                for( int i = 0; i < ITEMATTR_CLIENT_MAX; ++ i )
                {
                        if( item.sInstAttr[i] != 0 )
                                pItem->_PushItemAttr( i, item, ADVANCED_COLOR,
        index );
                }
        }
        ...
        if( pItem->_hints.GetCount() > 0 && Forge.IsForge )
        {
                ...
                {
                        _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING( CL_LANGUAGE_MATCH_673 ), Forge.nHoleNum ); PushHint( buf,
        ADVANCED_COLOR, 5, 0,index );
                }

                for( int i= 0; i < Forge.nStoneNum && i < Forge.nHoleNum; i++ )
                {
                        _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING(CL_LANGUAGE_MATCH_656 ), ConvertNumToChinese(
        Forge.nStoneLevel[i] ).c_str(), Forge.pStoneInfo[i]->szDataName );
                ...
                {
                        for( int i=0; i<Forge.nStoneNum && i<Forge.nHoleNum; i++
        ) PushHint( Forge.szStoneHint[i], ADVANCED_COLOR, 5, 0, index );
                }
        }
        ...
        if( pItem->_nPrice != 0 )
        {
                ...
                _snprintf_s( buf, _countof( buf ), _TRUNCATE,
        RES_STRING(CL_LANGUAGE_MATCH_674 ), StringSplitNum( pItem->_nPrice / 2 )
        ); PushHint( buf, COLOR_WHITE, 5, 0, index );

                _snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s",
        ConvertNumToChinese( pItem->_nPrice / 2 ).c_str() ); PushHint( buf,
        COLOR_WHITE, 5, 0, index );
        }

        if( pItemRecord->sType == 18 || pItemRecord->sType == 19 ||
                ( pItemRecord->sType == 25 &&
                ( pItemRecord->lID == 6386 || pItemRecord->lID == 6387 ||
        pItemRecord->lID == 6388 || pItemRecord->lID == 6389 || pItemRecord->lID
        == 6390 || pItemRecord->lID == 6391 || pItemRecord->lID == 6434 ||
        pItemRecord->lID == 6428 || pItemRecord->lID == 6429 || pItemRecord->lID
        == 6430 || pItemRecord->lID == 6431 || pItemRecord->lID == 6432 ) ) )
        {
                AddHintHeight( 6, index );
...
string CItemCommand::GetStoneHint(int nLevel) {
  ...
  if (_pItem->sType == 49) {
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      /*if( nLevel<0 )
              g_pGameApp->GetScriptMgr()->DoString( pInfo->szHintFunc, "u-s",
      _ItemData.sEnergy[1], &hint ); else
              g_pGameApp->GetScriptMgr()->DoString( pInfo->szHintFunc, "u-s", 1,
      &hint );*/
      return hint;
    }
  }
...
//					{
//						_snprintf_s( buf, _countof( buf ),
//_TRUNCATE, "%s:%+d%%", g_GetItemAttrExplain( item.sInstAttr[i][0]),
//item.sInstAttr[i][1] / 10 );
//					}
//					else
//					{
//						float f = (float)item.sInstAttr[i][1]
/// 10.0f; 						_snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%+.1f%%",
//g_GetItemAttrExplain( item.sInstAttr[i][0]), f );
//					}
//				}
//				else
//				{
//					_snprintf_s( buf, _countof( buf ), _TRUNCATE,
//"%s:%+d", g_GetItemAttrExplain( item.sInstAttr[i][0]), item.sInstAttr[i][1] );
//				}
//				PushHint( buf, color );
//
...
void CItemCommand::_PushValue(const char *szFormat, int attr, SItemHint &item,
                              DWORD color, int index) {
...
void CItemCommand::_PushItemAttr(int attr, SItemHint &item, DWORD color,
  ...
  /*if( attr <= ITEMATTR_COE_PDEF )
  {
          ...
          {
                  _snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%+d%%",
  g_GetItemAttrExplain( attr ), item.sInstAttr[attr] / 10 );
          }
          else
          {
                  float f = (float)item.sInstAttr[attr] / 10.0f;
                  _snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%+.1f%%",
  g_GetItemAttrExplain( attr ), f );
          }
  }
  else
  {
          _snprintf_s( buf, _countof( buf ), _TRUNCATE, "%s:%+d",
  g_GetItemAttrExplain( attr ), item.sInstAttr[attr] );
  }
  PushHint( buf, color, 5, 0, index );*/
...
bool CItemCommand::IsAllowThrow() { return _pItem->chIsThrow != 0; }

//	2008-9-17	yangyinyu	add	begin!
bool _wait_select_lock_item_state_ =
    false; //	用户已经使用锁道具，正在等待选择被锁定的道具。

// modify by ning.yan 2008-11-11
// 改为全局变量，使UIDoublePwd.cpp文件中定义的函数可以使用 begin
...
static void _evtSelectYesNoEvent(CCompent *pSender, int nMsgType, int x, int y,
                                 DWORD dwKey) {
  //	关闭选择框。
  ...
  if (!strcmp(name.c_str(), "btnYes")) {
    //	发送加锁信息。
    // CS_DropLock(	_lock_pos_,	_lock_item_id_,	_lock_grid_id_,
    // _lock_fusion_item_id_	);
    CS_DropLock(_lock_pos_, _lock_item_id_, (dbc::Char)_lock_grid_id_,
                _lock_fusion_item_id_);
    //	设置等待光标。
...
bool CItemCommand::MouseDown() {
  ...
  //	如果是等待被锁定状态。
    ...
    if (pOwn) {
      if (pOwn == g_stUIEquip.GetGoodsGrid()) {
        ...
        {
          //	弹出选择框。
          stSelectBox *pBox = g_stUIBox.ShowSelectBox(
              _evtSelectYesNoEvent, RES_STRING(CL_UIITEMCOMMAND_CPP_00011));
          //	pBox->dwTag = 0;
          //	pBox->dwParam = 0;
          ...
          // CS_DropLock(	_lock_pos_,	_lock_item_id_,	GridID,
          // _ItemData.GetFusionItemID()	);

          //	取消等待被锁定状态。
          // _wait_select_lock_item_state_	=	false; // delete by ning.yan
          // 2008-11-12
        }
  ...
  if (CRepairState *pState =
          dynamic_cast<CRepairState *>(pCha->GetActor()->GetCurState())) {
    // 如果处于修理道具状态
    if (_pItem->sType >= 31) {
    ...
    return false;
  } else if (CFeedState *pState =
                 dynamic_cast<CFeedState *>(pCha->GetActor()->GetCurState())) {
    // 喂食状态
    ...
    if (pOwn) {
      if (pOwn == g_stUIEquip.GetGoodsGrid()) {
        ...
        CS_BeginAction(g_stUIBoat.GetHuman(), enumACTION_ITEM_USE,
                       (void *)&param);
    ...
    return false;
  } else if (CFeteState *pState =
                 dynamic_cast<CFeteState *>(pCha->GetActor()->GetCurState())) {
...
void CItemCommand::RenderEnergy(int x, int y) {
  ...
  {
          float fLen = (float)_ItemData.sEnergy[0] / (float)_ItemData.sEnergy[1]
  * (float)ITEM_HEIGHT; int yb = y+ITEM_HEIGHT; GetRender().FillFrame( x, y,
  x+2, yb, COLOR_BLUE ); GetRender().FillFrame( x, yb-(int)fLen, x+2, yb,
  COLOR_RED );
  }*/
}
...
void CItemCommand::_AddDescriptor(int index) {
  ...
  StringNewLine(buf, sizeof(buf), 40, _pItem->szDescriptor,
                (unsigned int)strlen(_pItem->szDescriptor));
  PushHint(buf, COLOR_WHITE, 5, 0, index);
}
...
bool CItemCommand::UseCommand() {
  ...
  if (pOwn) {
    ...
    if (_pItem->sType == enumItemTypePetFodder ||
        _pItem->sType == enumItemTypePetSock) {
      ...
      return false;
    } else if (_pItem->sType == 66 ||
               (864 <= _pItem->lID &&
                _pItem->lID <=
                    866)) // 添加祭祀状态  add by Philip.Wu  2006-06-20
    {
      CActor *pActor = CGameScene::GetMainCha()->GetActor();
    ...
    } else if (GetItemInfo()->sType == 71) // 技能道具使用
    {
      ...
      if (nCoolDownTime > 0) // 有 cooldown 时间
      {
        ...
        _mapCoolDown[nSkillID] =
            g_pGameApp->GetCurTick(); // 保存下道具技能使用时间
      }
    ...
    if ((_pItem->sType < 31 || _ItemData.sID == 5939) &&
        pCha == CGameScene::GetMainCha() && g_stUIBoat.GetHuman() == pCha) {
      //	yyy	add	begin!
...
bool CItemCommand::GetIsPile() { return _pItem->GetIsPile(); }

int CItemCommand::GetPrice() { return _nPrice; }
...
int CItemCommand::GetTotalNum() { return _ItemData.sNum; }
...
const char *CItemCommand::GetName() {
  if (_ItemData.chForgeLv == 0) {
    ...
    _snprintf_s(szBuf, _countof(szBuf), _TRUNCATE, "%+d %s",
                _ItemData.chForgeLv, _pItem->szName);
    return szBuf;
  }
}
  ...
  for (int i = 0; i < MAX_JOB_TYPE; i++) {
    ...
    if (!isFind) {
      ...
      if (name.compare(g_GetJobName(pItem->szWork[i])) == 0) {
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s",
                    RES_STRING(CL_UIITEMCOMMAND_CPP_00013));
      } else {
        _snprintf_s(buf, _countof(buf), _TRUNCATE,
                    RES_STRING(CL_LANGUAGE_MATCH_679),
                    g_GetJobName(pItem->szWork[i]));
      }
...
void CItemCommand::_ShowFusionWork(CItemRecord *pAppearItem,
                                   CItemRecord *pEquipItem, SGameAttr *pAttr,
                                   int index) // 用于显示熔合后道具的职业限制
{
  bool isFind = false;
  int iAppearIndex = -1;
  int iEquipIndex = -1;
  ...
  for (int i = 0; i < MAX_JOB_TYPE; i++) {
    ...
    if (!isFind) {
      ...
      if (name.compare(g_GetJobName(pInfo->sPfLimit[i])) == 0) {
        //_snprintf_s( buf, _countof( buf ), _TRUNCATE,
        //"%s",RES_STRING(CL_UIITEMCOMMAND_CPP_00011));
        _snprintf_s(buf, _countof(buf), _TRUNCATE, "%s",
                    RES_STRING(CL_UIITEMCOMMAND_CPP_00013));
...
void CItemCommand::_ShowBody(int index) {
  ...
  PushHint(str.str(),
           _pItem->IsAllowEquip(nBodyType) ? GENERIC_COLOR : VALID_COLOR, 5, 0,
           index);
...
SItemForge &SItemForge::Convert(DWORD v, int nItemID) {
  ...
  for (int i = 0; i < 3; ++i) {
    nStoneData =
        (v / (int)(pow((double)1000, (double)(2 - i)))) % 1000; // 三位一取
    ...
    string strHint = "";
    if (g_pGameApp->GetScriptMgr()->DoString(pStoneInfo->szHintFunc, "u-s",
                                             forge.nStoneLevel[forge.nStoneNum],
                                             &strHint)) {
      // strncpy( forge.szStoneHint[forge.nStoneNum], strHint.c_str(),
      // sizeof(forge.szStoneHint[forge.nStoneNum]) );
      strncpy_s(forge.szStoneHint[forge.nStoneNum],
                sizeof(forge.szStoneHint[forge.nStoneNum]), strHint.c_str(),
                _TRUNCATE);
    }
  ...
  // int Num = 0;
  // if( g_pGameApp->GetScriptMgr()->DoString( "Get_HoleNum", "u-d",
  // dwForgeValue, &Num ) )
  //{
  //	if( Num>0 )
  //	{
  ...
  //	nStone = 0;
  //	if( !g_pGameApp->GetScriptMgr()->DoString( buf, "u-d", dwForgeValue,
  //&nStone ) ) 		continue;
  ...
  //		hint = "";
  //		if( g_pGameApp->GetScriptMgr()->DoString( pStone->szHintFunc, "u-s",
  //nStoneLv, &hint ) )
  //		{
  //			strncpy( forge.szStoneHint[StoneNum], hint.c_str(),
  //sizeof(forge.szStoneHint[StoneNum]) );
  //		}
  //	}
  //	StoneNum++;
  //}
...
void SItemForge::Refresh(int nItemID) {
  ...
  int nEffectID = 0;
  if (!g_pGameApp->GetScriptMgr()->DoString("Item_Stoneeffect", "ddd-d",
                                            nStoneType[0], nStoneType[1],
                                            nStoneType[2], &nEffectID))
    return;
...
float SItemForge::GetAlpha(int nTotalLevel) {
  ...
  static float fLevelBase[4] = {fLevelAlpha[1] - fLevelAlpha[0],
                                fLevelAlpha[2] - fLevelAlpha[1],
                                fLevelAlpha[3] - fLevelAlpha[2], 0.0f};
  ...
  return (fLevelAlpha[nLevel] +
          (float)(nTotalLevel % 4) / 4.0f * fLevelBase[nLevel]) /
         255.0f;
}
