...
CTitle::CTitle(CForm &frmOwn)
    : CCompent(frmOwn), _dwColor(0x00f01000), _nFontIndex(0), _dwShowTime(3000),
      _nFontH(5), _alpha(0), _endtime(0) {
  _pImage = new CGuiPic(NULL, enumMax);
}

CTitle::CTitle(const CTitle &rhs) : CCompent(rhs) { _Copy(rhs); }
...
CTitle::~CTitle(void) {
  SAFE_DELETE(_pImage); //安全释放内存 by Waiting 2009-06-18
}
...
void CTitle::Render() {
  if (_endtime >= CGameApp::GetCurTick()) {
    _alpha = (BYTE)(255.0f * (float)(_endtime - CGameApp::GetCurTick()) /
                    (float)_dwShowTime);
    ...
    CGuiFont::s_Font.Render(_nFontIndex, (char *)_strCaption.c_str(),
                            _posx[enumCenter], _posfonty,
                            (_alpha << 24) | (_dwColor & 0x00FFFFFF));
  }
}
