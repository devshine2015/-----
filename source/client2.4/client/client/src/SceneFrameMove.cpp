...
void CGameScene::_FrameMove(DWORD dwTimeParam) {
  ...
  if (_pTerrain) {
    ...
    //    g_Render.GetPickRayVector( g_pGameApp->GetMouseX(),
    //    g_pGameApp->GetMouseX(), &iorg, &iray ); if(
    //    HitTestSceneObjTerrain(&hit, &iorg, &iray ) > 0 )
    //    {
    ...
    // IP("Screen(%d,%d), Pick(%d,%d)\n", g_pGameApp->GetMouseX(),
    // g_pGameApp->GetMouseY(), _nMouseX, _nMouseY);
  ...
  for (i = 0; i < _nSceneItemCnt; i++) {
    ...
    if (pObj->IsValid()) {
      // drIPrimitive* pri = pObj->GetObject()->GetPrimitive();
  ...
  // D3DXVECTOR3 org, ray;
  // g_Render.GetPickRayVector( g_pGameApp->GetMouseX(),
  // g_pGameApp->GetMouseY(), &org, &ray ); if(
  // this->HitTestSceneObjTerrain(&vecHit, &org, &ray ) > 0 )
  //{
