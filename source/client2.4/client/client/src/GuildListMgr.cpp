...
void SwapData(vector<CGuildListData *>::reference swap1,
              vector<CGuildListData *>::reference swap2) {
...
CGuildListMgr::~CGuildListMgr(void) { ResetAll(); }
  ...
  for (int i = 0; i < int(m_pGuildList.size()) - 1; i++) {
    for (int j = i + 1; j < int(m_pGuildList.size()); j++) {
      if (m_eType == CGuildData::navy) {
        if ((m_pGuildList[i]->GetGuildID() > m_pGuildList[j]->GetGuildID() &&
             bInc) ||
            (m_pGuildList[i]->GetGuildID() < m_pGuildList[j]->GetGuildID() &&
             !bInc)) {
        ...
        if ((m_pGuildList[i]->GetGuildName() >
                 m_pGuildList[j]->GetGuildName() &&
             bInc) ||
            (m_pGuildList[i]->GetGuildName() <
                 m_pGuildList[j]->GetGuildName() &&
             !bInc)) {
          SwapData(m_pGuildList[i], m_pGuildList[j]);
        }
...
void CGuildListMgr::SortGuildsByMemberCount(bool bInc) {
  for (int i = 0; i < int(m_pGuildList.size()) - 1; i++) {
    for (int j = i + 1; j < int(m_pGuildList.size()); j++) {
      if (m_eType == CGuildData::navy) {
        if ((m_pGuildList[i]->GetMembers() > m_pGuildList[j]->GetMembers() &&
             bInc) ||
            (m_pGuildList[i]->GetMembers() < m_pGuildList[j]->GetMembers() &&
             !bInc)) {
          SwapData(m_pGuildList[i], m_pGuildList[j]);
        }
      }
...
void CGuildListMgr::SortGuildsByExperience(bool bInc) {
  for (int i = 0; i < int(m_pGuildList.size()) - 1; i++) {
    for (int j = i + 1; j < int(m_pGuildList.size()); j++) {
      if (m_eType == CGuildData::navy) {
        if ((m_pGuildList[i]->GetExperence() >
                 m_pGuildList[j]->GetExperence() &&
             bInc) ||
            (m_pGuildList[i]->GetExperence() <
                 m_pGuildList[j]->GetExperence() &&
             !bInc)) {
          SwapData(m_pGuildList[i], m_pGuildList[j]);
        }
...
void CGuildListMgr::SetType(CGuildData::eType type) { m_eType = type; }

CGuildData::eType CGuildListMgr::GetType() { return m_eType; }
