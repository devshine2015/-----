...
CMouseDown::~CMouseDown() { delete _pAutoAttack; }
...
bool CMouseDown::_AttackCha(CCharacter *pMain, CSkillRecord *pSkill,
                            CCharacter *pTarget) {
  if (!_IsEnabled)
    return false;
...
bool CMouseDown::_AttackArea(CCharacter *pMain, CSkillRecord *pSkill, int nScrX,
                             int nScrY) {
  if (!_IsEnabled)
    return false;
...
bool CMouseDown::PickItem(CCharacter *pMain) {
  ...
  if (nCount > 0) {
    ...
    //这个信息挺累赘的 by Waiting 2009-06-23
    return true;
  }
  return false;
}
...
void CMouseDown::MouseButtonDown() {
  ...
  CInsertState *seat =
      dynamic_cast<CInsertState *>(pMain->GetActor()->GetCurState());
  ...
  case eLastArea:
    ...
    break;
  case eLastDummy:
    ActDummyObj(pMain, _nDummyX, _nDummyY, _nDummyH, _nDummyAngle,
                _nDummyAction);
    break;
  case eLastFollow:
  ...
  default:
    ...
    _pScene->GetPickPos(g_pGameApp->GetMouseX(), g_pGameApp->GetMouseY() + 25,
                        vPos);
    ...
    if (CGameApp::GetCurTick() <= _dwAttackTime + 500) {
      ...
      if (_pAttackSkill == pSkill && _pAttackCha->IsValid()) {
        if (GetDistance(nAreaX, nAreaY, _pAttackCha->GetCurX(),
                        _pAttackCha->GetCurY()) <= 230 &&
            _AttackCha(pMain, _pAttackSkill, _pAttackCha)) {
          return;
        }
      }
    ...
    if (CGameApp::GetCurTick() <= _dwPickTime + 500) {
      if (!pMain->GetReadySkillInfo() && _pPickItem->IsValid())
        if (GetDistance(nAreaX, nAreaY, _pPickItem->GetCurX(),
                        _pPickItem->GetCurY()) <= 150)
...
void CMouseDown::CheckWalkContinue() {
  ...
  if (CGameApp::GetCurTick() >= dwAddTime) {
    ...
    if (_IsEnabled && pMain
      ...
      _pScene->GetPickPos(g_pGameApp->GetMouseX(), g_pGameApp->GetMouseY(),
                          vPos);
      nScrX = (int)(vPos.x * 100.0f);
      nScrY = (int)(vPos.y * 100.0f);
      if (pMain->getMoveSpeed() > 0) {
        int len = pMain->getMoveSpeed();
        len += GetDistance(pMain->GetCurX(), pMain->GetCurY(),
                           pMain->GetServerX(), pMain->GetServerY());
        if (GetDistance(pMain->GetCurX(), pMain->GetCurY(), nScrX, nScrY) < len)
          GetDistancePos(pMain->GetCurX(), pMain->GetCurY(), nScrX, nScrY, len,
                         nScrX, nScrY);
      }
...
bool CMouseDown::ActMove(CCharacter *pCha, int nScrX, int nScrY, bool isAdd,
                         bool isLine, bool IsCompart) {
  if (!_IsEnabled)
    return false;
  ...
  if (CGameApp::GetCurTick() <= dwLastTime && pLastCha == pCha &&
      nLastScrX == nScrX && nLastScrY == nScrY && isLastAdd == isAdd &&
      isLastLine == isLine) {
    return true;
  }
  ...
  if (!g_Config.m_IsMoveClient) {
    ...
    if (!isLine && MOVE_LENGTH > 0) {
      isOK =
          g_cFindPath
              .GetPathFindingState() /*g_cFindPath.Find( pCha->GetScene(), pCha,
                                        pCha->GetCurX(), pCha->GetCurY(), nScrX,
                                        nScrY, isLine )*/
          ;
    }
    if (isOK) {
      CMoveList cLocalList;
      COneMoveState::CompartMoveList(cLocalList, g_cFindPath.GetResultPath(),
                                     MOVE_LENGTH);

      if (cLocalList.GetCount() > 0) {
  ...
  if (IsCompart) {
    COneMoveState *pMoveState =
        dynamic_cast<COneMoveState *>(pCha->GetActor()->GetCurState());
    if (pMoveState) {
      if (isAdd) {
...
bool CMouseDown::ActAttackCha(CCharacter *pCha, CSkillRecord *pSkill,
                              CCharacter *pTarget, bool isTrace, bool IsForce,
                              bool IsCompart) {
  if (!_IsEnabled)
    return false;
  ...
  if (CGameApp::GetCurTick() <= dwLastTime && pLastCha == pCha &&
      pLastTarget == pTarget && pLastSkill == pSkill) {
    return true;
  }

  //	LG( "mousedown", "actattackcha: self:%s, target:%s, pos[%d, %d],
  //skill:%s\ttick:%u\n", pCha->getName(), pTarget->getName(),
  //pTarget->GetCurX(), pTarget->GetCurY(), pSkill->szName,
  //CGameApp::GetCurTick() );
  ...
  //	int nTotalDis = pCha->GetDefaultChaInfo()->sRadii +
  //pSkill->GetDistance() + pTarget->GetDefaultChaInfo()->sRadii + preMove +
  //ATTACT_DIS; 	bool isLine = false; 	if( g_cFindPath.Find( pCha->GetScene(),
  //pTarget, pTarget->GetServerX(), pTarget->GetServerY(), pCha->GetServerX(),
  //pCha->GetServerY(), isLine ) )
  //	{
  //		if( g_cFindPath.GetLength() > nTotalDis + ATTACT_DIS )
  //		{
  //			CMoveList cLocalList;
  //			COneMoveState::CompartMoveList( cLocalList,
  //g_cFindPath.GetResultPath(), nTotalDis ); 			int x = cLocalList.GetFront().x;
  //			int y = cLocalList.GetFront().y;

  //			pCha->GetActor()->ClearQueueState();
  //			COneMoveState* pMoveState = dynamic_cast<COneMoveState*>(
  //pCha->GetActor()->GetCurState() ); 			if( pMoveState )
  //			{
  //				pMoveState->ContinueMove( x, y, isLine );
  //			}
  //			else
  //			{
  //				COneMoveState * move = new
  //COneMoveState(pCha->GetActor()); 				move->SetWalkLine( isLine );
  //				move->SetMoveTo( x, y );
  //				pCha->GetActor()->SwitchState(move);
  //			}

  //			CAttackState *attack = isTrace ? new
  //CTraceAttackState(pCha->GetActor()) : new CAttackState(pCha->GetActor());
  ...
  CAttackState *attack = isTrace ? new CTraceAttackState(pCha->GetActor())
                                 : new CAttackState(pCha->GetActor());
  attack->SetIsForce(IsForce);
  attack->SetSkill(pSkill);
  attack->SetTarget(pTarget);
  attack->SetCommand(CCommandObj::GetReadyCommand());
...
bool CMouseDown::ActAttackArea(CCharacter *pCha, CSkillRecord *pSkill,
                               int nScrX, int nScrY, bool IsForce,
                               bool IsCompart) {
  if (!_IsEnabled)
    return false;
  ...
  if (CGameApp::GetCurTick() <= dwLastTime && pLastCha == pCha &&
      pLastSkill == pSkill && nLastScrX == nScrX && nLastScrY == nScrY) {
    return true;
  }

  //	LG( "mousedown", "actattackarea: self:%s, skill:%s, pos[%d,
  //%d]\ttick:%u\n", pCha->getName(), pSkill->szName, nScrX, nScrY,
  //CGameApp::GetCurTick() );
  ...
  //	if( preMove<ATTACT_DIS ) preMove = ATTACT_DIS;
  //	int nTotalDis = pCha->GetDefaultChaInfo()->sRadii +
  //pSkill->GetDistance() + preMove + ATTACT_DIS; 	bool isLine = false; 	if(
  //g_cFindPath.Find( pCha->GetScene(), pCha, nScrX, nScrY, pCha->GetServerX(),
  //pCha->GetServerY(), isLine ) )
  //	{
  //		if( g_cFindPath.GetLength() > nTotalDis + ATTACT_DIS )
  //		{
  ...
  //			CAttackState *attack = new
  //CAttackState(pCha->GetActor()); 			attack->SetIsForce( IsForce );
  //			attack->SetSkill( pSkill );
...
bool CMouseDown::ActTalkNpc(CCharacter *pCha, CCharacter *pNpc) {
  ...
  //pCha->getName(), pNpc->getName(), pNpc->GetCurX(), pNpc->GetCurY(),
  //CGameApp::GetCurTick() );
  ...
  if (pNpc->DistanceFrom(pCha) > 300) {
    int x = 0;
    int y = 0;
    GetDistancePos(pNpc->GetCurX(), pNpc->GetCurY(), pCha->GetCurX(),
                   pCha->GetCurY(), 200, x, y);
    ActMove(pCha, x, y);
  }
...
bool CMouseDown::ActPickItem(CCharacter *pCha, CSceneItem *pItem, bool IsMove) {
  ...
  //	LG( "mousedown", "actpicktime: self:%s, item:%s, pos[%d,
  //%d]\ttick:%u\n", pCha->getName(), pItem->GetItemInfo()->szName,
  //pItem->GetCurX(), pItem->GetCurY(), CGameApp::GetCurTick() );
  ...
  _pScene->GetSign()->Show(pItem->GetCurX(), pItem->GetCurY(),
                           (int)(h * 100.0f));
...
bool CMouseDown::ActEvent(CCharacter *pCha, CSceneNode *pNode, CEvent *pEvent) {
  ...
  if (pEvent->IsNormal() && pEvent->GetInfo()->IsValid(pCha->GetMainType()) &&
    ...
    if (CGameApp::GetCurTick() <= dwLastTime && pLastCha == pCha &&
        pLastNode == pNode && pLastEvent == pEvent) {
      return true;
    }

    //		LG( "mousedown", "actevent: self:%s, nodeid:%u, pos[%d, %d],
    //event:%s\ttick:%u\n", pCha->getName(), pNode->getAttachID(),
    //pNode->GetCurX(), pNode->GetCurY(), pEvent->GetInfo()->szName,
    //CGameApp::GetCurTick() );
...
bool CMouseDown::ActDummyObj(CCharacter *pCha, int x, int y, int h, int t_angle,
                             int action) {
  if (!_IsEnabled)
    return false;
  ...
  static CCharacter *pLastCha = NULL;
  static int nLastX = 0, nLastY = 0, nLastH = 0, nLastAngle = 0,
             nLastAction = 0;
  static DWORD dwLastTime = 0;

  if (CGameApp::GetCurTick() <= dwLastTime && pLastCha == pCha && nLastX == x &&
      nLastY == y && nLastH == h && nLastAngle == t_angle &&
      nLastAction == action) {
    return true;
  }

  //	LG( "mousedown", "actdummyobj: self:%s, x:%d, y:%d, h:%d, angle:%d,
  //action:%d\ttick:%u\n", pCha->getName(), x, y, h, t_angle, action,
  //CGameApp::GetCurTick() );
  ...
  CSeatState *seat = dynamic_cast<CSeatState *>(
      pCha->GetActor()->FindStateClass(typeid(CSeatState)));
  if (seat) {
    if (seat->GetAngle() == t_angle && seat->GetPosX() == x &&
        seat->GetPosY() == y && seat->GetHeight() == h &&
        seat->GetPose() == action) {
      return true;
    }
  }
...
bool CMouseDown::ActBackMove(CCharacter *pCha, int nScrX, int nScrY,
                             bool isAdd) {
...
void CMouseDown::FrameMove() {
  ...
  static DWORD dwTime =
      CGameApp::GetCurTick() + (rand() % 300) * 1000 + 30 * 1000;
...
BOOL CALLBACK CMouseDown::_EnumWindowsProc(HWND hWnd, LPARAM lParam) {
  ...
  if (strlen(szBuf) > 0 &&
      strstr(szBuf, RES_STRING(CL_LANGUAGE_MATCH_197)) > 0) {
...
bool CMouseDown::ActShop(CCharacter *pCha, CCharacter *pTarget) {
  ...
  if (pCha->DistanceFrom(pTarget) > 300) {
    ...
    GetAnglePos(pTarget->GetCurX(), pTarget->GetCurY(), 380, pTarget->getYaw(),
                x, y);
    ActMove(pCha, x, y);
