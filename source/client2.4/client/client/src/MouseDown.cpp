...
bool CMouseDown::PickItem( CCharacter* pMain )
{
	...
	{
		_pAutoAttack->Cancel();
//		g_pGameApp->SysInfo( RES_STRING(CL_LANGUAGE_MATCH_195), nCount ); 这个信息挺累赘的 by Waiting 2009-06-23
		return true;
	}
	return false;
}
...
BOOL CALLBACK CMouseDown::_EnumWindowsProc( HWND hWnd, LPARAM lParam )
{
	...
	if( strlen(szBuf)>0 && strstr( szBuf, RES_STRING(CL_LANGUAGE_MATCH_197))>0 )
	{
