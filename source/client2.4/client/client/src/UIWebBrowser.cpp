...
HRESULT CWebBrowser::EmbedBrowserObject() {
  ...
  m_hWndWeb =
      CreateWindow(TEXT("WebBrowser"), TEXT(""), WS_POPUP /* |WS_VISIBLE*/,
                   rect.left + this->GetX(), rect.top + this->GetY(),
                   this->GetWidth(), this->GetHeight(), g_pGameApp->GetHWND(),
                   NULL, g_pGameApp->GetAppInstance(), NULL);
  _ASSERT(m_hWndWeb);
  ...
  if (OleCreate(CLSID_WebBrowser, IID_IOleObject, OLERENDER_DRAW, 0,
                (IOleClientSite *)pClientSite, &Storage,
                (void **)&m_lpOleObject) == S_OK) {
    ...
    if (OleSetContainedObject((IUnknown *)m_lpOleObject, TRUE) == S_OK) {
      if (m_lpOleObject->DoVerb(OLEIVERB_SHOW, NULL,
                                (IOleClientSite *)pClientSite, -1, m_hWndWeb,
                                &rect) == S_OK) {
...
HRESULT CWebBrowser::DisplayHTMLPage(LPCTSTR pcsWebPage) {
  IWebBrowser2 *lpWebBrowser2;
  HRESULT hr =
      m_lpOleObject->QueryInterface(IID_IWebBrowser2, (VOID **)&lpWebBrowser2);
  if (FAILED(hr))
    return E_FAIL;
...
void CWebBrowser::Show() {
  ...
  RECT rect;
  GetWindowRect(g_pGameApp->GetHWND(), &rect);
  ::SetWindowPos(this->m_hWndWeb, NULL, rect.left + this->GetX(),
                 rect.top + this->GetY(), this->GetWidth(), this->GetHeight(),
...
void CWebBrowser::FrmActive() { this->Show(); }

void CWebBrowser::FrmLost() { this->Hide(); }

void CWebBrowser::WindowMove() {
  RECT rect;
  GetWindowRect(g_pGameApp->GetHWND(), &rect);
  for (vector<CWebBrowser *>::iterator it = CWebBrowser::WindowsHandleV.begin();
       it != CWebBrowser::WindowsHandleV.end(); it++) {
    SetWindowPos((*it)->m_hWndWeb, NULL, rect.left + (*it)->GetX(),
                 rect.top + (*it)->GetY(), (*it)->GetWidth(),
                 (*it)->GetHeight(), SWP_NOSIZE | SWP_NOZORDER);
  }
}