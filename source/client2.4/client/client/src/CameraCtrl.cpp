...
void CCamDither::_A() {
  ...
  if (_InitFlag) {
    ...
    if (_FrameNum >= N[_Step] * 4) {
      if (!drFloatEqual(_RunningMat._41, _InitMat._41, 1e-2f) ||
          !drFloatEqual(_RunningMat._42, _InitMat._42, 1e-2f) ||
          !drFloatEqual(_RunningMat._43, _InitMat._43, 1e-2f)) {
        int x = 0;
      }
      _FrameNum = 0;
      if (++_Step >= T) {
...
void CCamDither::Init(const LEMatrix44 *mat) {
  _Step = 0;
  _FrameNum = 0;
  if (mat) {
    _InitMat = *mat;
    _RunningMat = _InitMat;
    _Dir = *(LEVector3 *)&_RunningMat._11;
    _InitFlag = 1;
  } else {
    _InitFlag = 0;
  }
}
void CCamDither::FrameMove() { _A(); }
...
void CCameraCtrl::InitModel(int iType, D3DXVECTOR3 *pInitPos) {
  if (m_iType < 0 || m_fScale != 1.0f) {
    ...
    g_pGameApp->GetCurScene()->GetTerrain()->SetShowSize(m_iShowWidth,
                                                         m_iShowHeight);
...
void CCameraCtrl::ChangeModel() {
  if (m_pModel->m_nShowWidth > m_iShowWidth &&
      m_pModel->m_nShowHeight > m_iShowHeight) {
    m_iShowWidth = m_pModel->m_nShowWidth;
    m_iShowHeight = m_pModel->m_nShowHeight;
    g_pGameApp->GetCurScene()->GetTerrain()->SetShowSize(m_iShowWidth,
                                                         m_iShowHeight);
...
bool CCameraCtrl::IsDefaultView() {
  return ((m_fstep1 == 1.0f) && fabs(m_fAngle - m_InitAngle) <= 0) ? true
                                                                   : false;
}
...
#ifdef _SKYBOX_
  m_fCameraTileHeight = g_pGameApp->GetCurScene()->GetTerrain()->GetHeight(
      m_EyePos.x, m_EyePos.y);
...
void CCameraCtrl::SetFollowObj(D3DXVECTOR3 &vRefObj) { m_RefPos = vRefObj; }
...
void CCameraCtrl::Update() {
  ...
  _vCurNode.x =
      _vLastNode.x * (1.0f - _fProgressRate) + _vNextNode.x * _fProgressRate;
  _vCurNode.y =
      _vLastNode.y * (1.0f - _fProgressRate) + _vNextNode.y * _fProgressRate;
...
void CCameraCtrl::BeginMove() { m_iState = 1; }
...
void CCameraCtrl::SetViewTransform() {
  ...
  if (g_pGameApp->GetCurScene()) {
    if (g_pGameApp->GetCurScene()->GetMainCha()) {
      // CChaRecord* pInfo = GetChaRecordInfo(
      // g_pGameApp->GetCurScene()->GetMainCha()->GetTypeID() ); if( pInfo )
      {
  ...
  if (m_CamDither.GetState()) {
    LEMatrix44 mat = *m_CamDither.GetRunningMatrix();
...
Camera::Camera(Controller<D3DXVECTOR3> *_tgt_ctrl,
    ...
    : _my_targetController(_tgt_ctrl), _my_eyeController(_eye_ctrl),
      _mySC_End(45.f, 0.985398f, 0.0f), _myTheta_range(-FLT_MAX, FLT_MAX),
